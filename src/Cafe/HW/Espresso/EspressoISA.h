...
{
	// underscore at the end of the name means that this instruction always updates CR0 (as if RC
	// bit is set)
	ZERO = 0,
	VIRTUAL_HLE = 1,
...
inline void _decodeForm_D_branch(uint32 opcode, uint32& BD, BOField& BO, uint32& BI, bool& AA,
								 bool& LK)
{
	BD = opcode & 0xfffc;
	if (BD & 0x8000)
