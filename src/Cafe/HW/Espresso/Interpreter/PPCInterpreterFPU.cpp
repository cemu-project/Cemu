...
double frsqrte_espresso(double input)
{
	...
	uint32 idx = (x >> (ieee_double_m_bits - espresso_frsqrte_i_bits + 1ull)) &
				 ((1 << espresso_frsqrte_i_bits) - 1);
	// get step multiplier
	uint32 stepMul =
		(x >> (ieee_double_m_bits - espresso_frsqrte_i_bits + 1 - 11)) & ((1 << 11) - 1);
...
espresso_fres_entry_t fresLookupTable[32] = {
	...
	{0xfbe00, 0x143},  {0xd3800, 0x12d},  {0xade00, 0x12d},	 {0x88400, 0x11a},	{0x65000, 0x11a},
	{0x41c00, 0x108},  {0x20c00, 0x106}};

double fres_espresso(double input)
{
	// based on testing we know that fres uses only the first 15 bits of the mantissa
	// seee eeee eeee mmmm mmmm mmmm mmmx xxxx ....		(s = sign, e = exponent, m = mantissa, x =
	// not used) the mantissa bits are interpreted as following: 0000 0000 0000 iiii ifff ffff fff0
	// ... (i = table look up index , f = step multiplier)
	unsigned long long x = *(unsigned long long*)&input;

	// get index
	uint32 idx =
		(x >> (ieee_double_m_bits - espresso_fres_i_bits)) & ((1 << espresso_fres_i_bits) - 1);
	// get step multiplier
...
void PPCInterpreter_FMADDS(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	hCPU->fpr[frD].fpr =
		(float)(hCPU->fpr[frA].fpr * roundTo25BitAccuracy(hCPU->fpr[frC].fpr) + hCPU->fpr[frB].fpr);
	if (PPC_PSE)
		hCPU->fpr[frD].fp1 = hCPU->fpr[frD].fp0;
...
void PPCInterpreter_FMSUBS(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	hCPU->fpr[frD].fp0 =
		(float)(hCPU->fpr[frA].fp0 * roundTo25BitAccuracy(hCPU->fpr[frC].fp0) - hCPU->fpr[frB].fp0);
	if (PPC_PSE)
		hCPU->fpr[frD].fp1 = hCPU->fpr[frD].fp0;
...
void PPCInterpreter_FNMSUBS(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	hCPU->fpr[frD].fp0 = (float)-(hCPU->fpr[frA].fp0 * roundTo25BitAccuracy(hCPU->fpr[frC].fp0) -
								  hCPU->fpr[frB].fp0);
	if (PPC_PSE)
		hCPU->fpr[frD].fp1 = hCPU->fpr[frD].fp0;
...
void PPCInterpreter_FCMPO(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	if (IS_SNAN(hCPU->fpr[frA].guint) || IS_SNAN(hCPU->fpr[frB].guint))
		hCPU->fpscr |= FPSCR_VXSNAN;
	else if (!(hCPU->fpscr & FPSCR_VE) || IS_QNAN(hCPU->fpr[frA].guint) ||
			 IS_QNAN(hCPU->fpr[frB].guint))
