...
uint64 ppcMainThreadDECCycleStart =
	0; // at which cycle the dec register was set, if == 0 -> dec is 0
...
PPCInterpreter_t* PPCInterpreter_createInstance(unsigned int Entrypoint)
{
	...
	// create instance
	uint32 prefixAreaSize = 0x6000; // we need to allocate some bytes before the interpreter struct
									// because the recompiler will use it as stack area
									// (specifically when the exception handler is called)
	pData = (PPCInterpreter_t*)((uint8*)malloc(sizeof(PPCInterpreter_t) + prefixAreaSize) +
								prefixAreaSize);
...
{
	uint8* result =
		memory_getPointerFromVirtualOffset(ppcInterpreterCurrentInstance->gpr[1] - offset);
