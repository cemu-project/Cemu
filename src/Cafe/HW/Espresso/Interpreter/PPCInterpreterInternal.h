...
#define SPR_PVR                                                                                    \
	287 // processor version, for Wii U this must be 0x7001xxxx - this register is only readable
...
#define XER_BIT_CA                                                                                 \
	(29) // carry bit index. To accelerate frequent access, this bit is stored as a separate uint8
...
// assume fixed values for PSE/LSQE. This optimization is possible because Wii U applications run
// only in user mode (todo - handle this correctly in LLE mode)
...
static inline void ppc_update_cr0(PPCInterpreter_t* hCPU, uint32 r)
{
	...
	hCPU->cr[CR_BIT_GT] = hCPU->cr[CR_BIT_EQ] ^ hCPU->cr[CR_BIT_LT] ^
						  1; // this works because EQ and LT can never be set at the same time. So
							 // the only case where GT becomes 1 is when LT=0 and EQ=0
}
...
#define IS_NAN(X)                                                                                  \
	((((X)&0x000fffffffffffffULL) != 0) && (((X)&0x7ff0000000000000ULL) == 0x7ff0000000000000ULL))
#define IS_QNAN(X)                                                                                 \
	((((X)&0x000fffffffffffffULL) != 0) && (((X)&0x7ff8000000000000ULL) == 0x7ff8000000000000ULL))
#define IS_SNAN(X)                                                                                 \
	((((X)&0x000fffffffffffffULL) != 0) && (((X)&0x7ff8000000000000ULL) == 0x7ff0000000000000ULL))
