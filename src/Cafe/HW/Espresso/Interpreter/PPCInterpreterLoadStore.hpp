...
static void PPCInterpreter_STWCX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	// todo - this isnt accurate. STWCX can succeed even with a different EA if the reserved value
	// remained untouched
	if (hCPU->reservedMemAddr == ea)
	{
		uint32be reservedValue = hCPU->reservedMemValue; // this is the value we expect in memory
														 // (if it does not match, STWCX fails)
		std::atomic<uint32be>* wordPtr;
		if constexpr (ppcItpCtrl::allowSupervisorMode)
		{
			wordPtr = _rawPtrToAtomic(
				(uint32be*)(memory_base +
							ppcItpCtrl::ppcMem_translateVirtualDataToPhysicalAddr(hCPU, ea)));
			DSI_EXIT();
		}
		else
		{
...
static void PPCInterpreter_STWBRX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	int rA, rS, rB;
	PPC_OPC_TEMPL_X(Opcode, rS, rA, rB);
	ppcItpCtrl::ppcMem_writeDataU32(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
									_swapEndianU32(hCPU->gpr[rS]));
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_STHX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	int rA, rS, rB;
	PPC_OPC_TEMPL_X(Opcode, rS, rA, rB);
	ppcItpCtrl::ppcMem_writeDataU16(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
									(uint16)hCPU->gpr[rS]);
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_STHBRX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	PPC_OPC_TEMPL_X(Opcode, rS, rA, rB);
	ppcItpCtrl::ppcMem_writeDataU16(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
									_swapEndianU16((uint16)hCPU->gpr[rS]));
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_STBX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	int rA, rS, rB;
	PPC_OPC_TEMPL_X(Opcode, rS, rA, rB);
	ppcItpCtrl::ppcMem_writeDataU8(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
								   (uint8)hCPU->gpr[rS]);
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_LWBRX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	sint32 rA, rD, rB;
	PPC_OPC_TEMPL_X(Opcode, rD, rA, rB);
	hCPU->gpr[rD] = CPU_swapEndianU32(
		ppcItpCtrl::ppcMem_readDataU32(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB]));
...
static void PPCInterpreter_LHBRX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	sint32 rA, rD, rB;
	PPC_OPC_TEMPL_X(Opcode, rD, rA, rB);
	hCPU->gpr[rD] = CPU_swapEndianU16(
		ppcItpCtrl::ppcMem_readDataU16(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB]));
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_LFSUX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	if (PPC_LSQE)
		...
		hCPU->fpr[frD].fp0int =
			Val; // ppcItpCtrl::ppcMem_readDataFloat((rA?hCPU->gpr[rA]:0)+hCPU->gpr[rB]);
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_LFD(PPCInterpreter_t* hCPU, uint32 Opcode) // Copied
{
	...
	uint32 imm;
	PPC_OPC_TEMPL_D_SImm(Opcode, frD, rA, imm);
	hCPU->fpr[frD].fpr = ppcItpCtrl::ppcMem_readDataDouble(
		hCPU, (rA ? hCPU->gpr[rA] : 0) +
				  imm); // ppcItpCtrl::ppcMem_readDataQUAD((rA?hCPU->gpr[rA]:0)+imm);
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_LFDX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	PPC_OPC_TEMPL_X(Opcode, frD, rA, rB);
	hCPU->fpr[frD].fpr =
		ppcItpCtrl::ppcMem_readDataDouble(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB]);
	PPCInterpreter_nextInstruction(hCPU);
}
...
static void PPCInterpreter_STFS(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	PPC_OPC_TEMPL_D_SImm(Opcode, frD, rA, imm);
	if (PPC_LSQE)
		ppcItpCtrl::ppcMem_writeDataFloatEx(hCPU, (rA ? hCPU->gpr[rA] : 0) + imm,
											hCPU->fpr[frD].fp0int);
	else
		ppcItpCtrl::ppcMem_writeDataFloatEx(hCPU, (rA ? hCPU->gpr[rA] : 0) + imm,
											hCPU->fpr[frD].fp0int);
...
static void PPCInterpreter_STFSX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	if (PPC_LSQE)
		ppcItpCtrl::ppcMem_writeDataFloatEx(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
											hCPU->fpr[frS].fp0int);
	else
		ppcItpCtrl::ppcMem_writeDataFloatEx(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
											hCPU->fpr[frS].fp0int);
...
static void PPCInterpreter_STFDX(PPCInterpreter_t* hCPU, uint32 Opcode)
{
	...
	ppcItpCtrl::ppcMem_writeDataDouble(hCPU, (rA ? hCPU->gpr[rA] : 0) + hCPU->gpr[rB],
									   hCPU->fpr[frS].fpr);
...
static void PPCInterpreter_PSQ_ST(PPCInterpreter_t* hCPU, unsigned int opcode)
{
	...
	{
		if ((type == 4) || (type == 6))
			ppcItpCtrl::ppcMem_writeDataU8(hCPU, ea,
										   quantize((float)hCPU->fpr[frD].fp0, type, scale));
		else if ((type == 5) || (type == 7))
			ppcItpCtrl::ppcMem_writeDataU16(hCPU, ea,
											quantize((float)hCPU->fpr[frD].fp0, type, scale));
		else
			ppcItpCtrl::ppcMem_writeDataU32(hCPU, ea,
											quantize((float)hCPU->fpr[frD].fp0, type, scale));
	}
	else
	{
		...
		if ((type == 4) || (type == 6))
			ppcItpCtrl::ppcMem_writeDataU8(hCPU, ea + 1,
										   quantize((float)hCPU->fpr[frD].fp1, type, scale));
		else if ((type == 5) || (type == 7))
			ppcItpCtrl::ppcMem_writeDataU16(hCPU, ea + 2,
											quantize((float)hCPU->fpr[frD].fp1, type, scale));
		else
			ppcItpCtrl::ppcMem_writeDataU32(hCPU, ea + 4,
											quantize((float)hCPU->fpr[frD].fp1, type, scale));
	}
