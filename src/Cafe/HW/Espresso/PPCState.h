...
struct PPCInterpreter_t
{
	...
	FPR_t fpr[32];
	uint32 fpscr;
	uint8 cr[32]; // 0 -> bit not set, 1 -> bit set (upper 7 bits of each byte must always be zero)
				  // (cr0 starts at index 0, cr1 at index 4 ..)
	...
	/* Note: Everything above is potentially hardcoded into Cemuhook. Do not touch anything or it
	 * will risk breaking compatibility */
...
#define ppcGetCallParamU16(__index)                                                                \
	((uint16)(PPCInterpreter_getCallParamU32(hCPU, __index) & 0xFFFF))
#define ppcGetCallParamU8(__index) ((uint8)(PPCInterpreter_getCallParamU32(hCPU, __index) & 0xFF))
#define ppcGetCallParamStruct(__index, __type)                                                     \
	((__type*)memory_getPointerFromVirtualOffsetAllowNull(                                         \
		PPCInterpreter_getCallParamU32(hCPU, __index)))

// legacy way of accessing parameters
#define ppcDefineParamU32(__name, __index)                                                         \
	uint32 __name = PPCInterpreter_getCallParamU32(hCPU, __index)
#define ppcDefineParamU16(__name, __index)                                                         \
	uint16 __name = (uint16)PPCInterpreter_getCallParamU32(hCPU, __index)
#define ppcDefineParamU32BEPtr(__name, __index)                                                    \
	uint32be* __name = (uint32be*)((uint8*)memory_getPointerFromVirtualOffsetAllowNull(            \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamS32(__name, __index)                                                         \
	sint32 __name = (sint32)PPCInterpreter_getCallParamU32(hCPU, __index)
#define ppcDefineParamU64(__name, __index)                                                         \
	uint64 __name = PPCInterpreter_getCallParamU64(hCPU, __index)
#define ppcDefineParamMPTR(__name, __index)                                                        \
	MPTR __name = (MPTR)PPCInterpreter_getCallParamU32(hCPU, __index)
#define ppcDefineParamMEMPTR(__name, __type, __index)                                              \
	MEMPTR<__type> __name                                                                          \
	{                                                                                              \
		PPCInterpreter_getCallParamU32(hCPU, __index)                                              \
	}
#define ppcDefineParamU8(__name, __index)                                                          \
	uint8 __name = (PPCInterpreter_getCallParamU32(hCPU, __index) & 0xFF)
#define ppcDefineParamStructPtr(__name, __type, __index)                                           \
	__type* __name = ((__type*)memory_getPointerFromVirtualOffsetAllowNull(                        \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamTypePtr(__name, __type, __index)                                             \
	__type* __name = ((__type*)memory_getPointerFromVirtualOffsetAllowNull(                        \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamPtr(__name, __type, __index)                                                 \
	__type* __name = ((__type*)memory_getPointerFromVirtualOffsetAllowNull(                        \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamStr(__name, __index)                                                         \
	char* __name = ((char*)memory_getPointerFromVirtualOffsetAllowNull(                            \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamUStr(__name, __index)                                                        \
	uint8* __name = ((uint8*)memory_getPointerFromVirtualOffsetAllowNull(                          \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamWStr(__name, __index)                                                        \
	wchar_t* __name = ((wchar_t*)memory_getPointerFromVirtualOffsetAllowNull(                      \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
#define ppcDefineParamWStrBE(__name, __index)                                                      \
	uint16be* __name = ((uint16be*)memory_getPointerFromVirtualOffsetAllowNull(                    \
		PPCInterpreter_getCallParamU32(hCPU, __index)))
...
// FPU available check
//#define FPUCheckAvailable() if ((hCPU->msr & MSR_FP) == 0) { IPTException(hCPU,
// CPU_EXCEPTION_FPUUNAVAIL); return; }
#define FPUCheckAvailable() // since the emulated code always runs in usermode we can assume that
							// MSR_FP is always set
...
extern uint64 ppcCyclesSince2000TimerClock; // on init this is set to the cycles that passed
											// since 1.1.2000 / 20
