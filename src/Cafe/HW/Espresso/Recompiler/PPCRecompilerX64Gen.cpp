...
void x64Gen_writeU8(x64GenContext_t* x64GenContext, uint8 v)
{
	if (x64GenContext->codeBufferIndex + 1 > x64GenContext->codeBufferSize)
	{
		...
		x64GenContext->codeBuffer =
			(uint8*)realloc(x64GenContext->codeBuffer, x64GenContext->codeBufferSize);
	}
...
void _x64Gen_writeMODRMDeprecated(x64GenContext_t* x64GenContext, sint32 dataRegister,
								  sint32 memRegisterA64, sint32 memRegisterB64, sint32 memImmS32)
{
	bool forceUseOffset = false;
	if ((memRegisterA64 & 7) == 5)
	{
		// RBP and R13 have no memImmS32 == 0 encoding, therefore we need to use a 1 byte offset
		// with the value 0
		forceUseOffset = true;
	}
	...
	// note: Swapping mem register A and mem register B does not work because the instruction prefix
	// defines the register group which might not match (e.g. regA in r0-r8 range and regB in
	// RAX-RDI range)
...
void x64Emit_mov_reg32_mem32(x64GenContext_t* x64GenContext, sint32 destReg, sint32 memBaseReg64,
							 sint32 memOffset)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte<0x8B>>(x64GenContext, x64MODRM_opr_reg64(destReg),
											   x64MODRM_opr_memReg64(memBaseReg64, memOffset));
}

void x64Emit_mov_mem32_reg32(x64GenContext_t* x64GenContext, sint32 memBaseReg64, sint32 memOffset,
							 sint32 srcReg)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte_rev<0x89>>(
		x64GenContext, x64MODRM_opr_memReg64(memBaseReg64, memOffset), x64MODRM_opr_reg64(srcReg));
}

void x64Emit_mov_mem64_reg64(x64GenContext_t* x64GenContext, sint32 memBaseReg64, sint32 memOffset,
							 sint32 srcReg)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte_rev<0x89, true>>(
		x64GenContext, x64MODRM_opr_memReg64(memBaseReg64, memOffset), x64MODRM_opr_reg64(srcReg));
}
...
void x64Emit_mov_reg64_mem64(x64GenContext_t* x64GenContext, sint32 destReg, sint32 memBaseReg64,
							 sint32 memOffset)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte<0x8B, true>>(
		x64GenContext, x64MODRM_opr_reg64(destReg), x64MODRM_opr_memReg64(memBaseReg64, memOffset));
}
...
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte<0x8B, true>>(
		x64GenContext, x64MODRM_opr_reg64(destReg),
		x64MODRM_opr_memRegPlusReg(memBaseReg64, memIndexReg64, memOffset));
}

void x64Emit_mov_reg32_mem32(x64GenContext_t* x64GenContext, sint32 destReg, sint32 memBaseReg64,
							 sint32 memIndexReg64, sint32 memOffset)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte<0x8B>>(
		x64GenContext, x64MODRM_opr_reg64(destReg),
		x64MODRM_opr_memRegPlusReg(memBaseReg64, memIndexReg64, memOffset));
}
...
void x64Emit_mov_reg64b_mem8(x64GenContext_t* x64GenContext, sint32 destReg, sint32 memBaseReg64,
							 sint32 memIndexReg64, sint32 memOffset)
{
	x64Gen_writeMODRM_dyn<x64_opc_1byte<0x8A>>(
		x64GenContext, x64MODRM_opr_reg64(destReg),
		x64MODRM_opr_memRegPlusReg(memBaseReg64, memIndexReg64, memOffset));
}
...
void x64Emit_movZX_reg32_mem8(x64GenContext_t* x64GenContext, sint32 destReg, sint32 memBaseReg64,
							  sint32 memIndexReg64, sint32 memOffset)
{
	x64Gen_writeMODRM_dyn<x64_opc_2byte<0x0F, 0xB6>>(
		x64GenContext, x64MODRM_opr_reg64(destReg),
		x64MODRM_opr_memRegPlusReg(memBaseReg64, memIndexReg64, memOffset));
}
														...
														sint32 memRegisterB64, sint32 memImmS32)
{
	...
	_x64Gen_writeMODRMDeprecated(x64GenContext, dstRegister, memRegisterA64, memRegisterB64,
								 memImmS32);
}

void x64Gen_mov_mem64Reg64PlusReg64_reg64(x64GenContext_t* x64GenContext, sint32 srcRegister,
										  ...
										  sint32 memImmS32)
{
	...
	_x64Gen_writeMODRMDeprecated(x64GenContext, srcRegister, memRegisterA64, memRegisterB64,
								 memImmS32);
}
												  ...
												  sint32 memImmS32, sint32 srcRegister)
{
	...
	x64Gen_writeU8(x64GenContext, 0x66); // 16bit prefix
	x64Gen_movTruncate_mem32Reg64PlusReg64_reg64(x64GenContext, memRegisterA64, memRegisterB64,
												 memImmS32, srcRegister);
}
												 ...
												 sint32 memImmS32, sint32 srcRegister)
{
	...
	// todo: We don't need the REX byte when when the source register is AL,BL,CL or DL and neither
	// memRegister A or B are within r8 - r15
										 ...
										 sint32 destRegister, sint32 srcRegister)
{
	...
	if (conditionType == X86_CONDITION_CARRY || conditionType == X86_CONDITION_UNSIGNED_BELOW)
		x64Gen_writeU8(x64GenContext, 0x42);
			 ...
			 conditionType == X86_CONDITION_UNSIGNED_ABOVE_EQUAL)
		x64Gen_writeU8(x64GenContext, 0x43);
													...
													sint32 memRegisterB64)
{
	...
	_x64Gen_writeMODRMDeprecated(x64GenContext, dstRegister & 0x7, memRegisterA64 & 0x7,
								 memRegisterB64 & 0x7, 0);
}

void _x64_op_reg64Low_mem8Reg64(x64GenContext_t* x64GenContext, sint32 dstRegister,
								sint32 memRegister64, sint32 memImmS32, uint8 opByte)
{
												   ...
												   sint32 memImmS32, sint32 srcRegister)
{
	...
	if (srcRegister >= 8 || memRegisterA64 >= 8 || memRegisterB64 >= 8)
		x64Gen_writeU8(x64GenContext, 0x40 + ((srcRegister >= 8) ? 4 : 0) +
										  ((memRegisterA64 >= 8) ? 1 : 0) +
										  ((memRegisterB64 >= 8) ? 2 : 0));
										  ...
										  sint32 memImmS32, sint32 srcRegister)
{
	...
	if (srcRegister >= 8 || memRegister64 >= 8)
		x64Gen_writeU8(x64GenContext,
					   0x40 + ((srcRegister >= 8) ? 4 : 0) + ((memRegister64 >= 8) ? 1 : 0));
...
{
	assert_dbg(); // do not use this instruction, it's result is always undefined. Instead use ROL
				  // <reg16>, 8
	...
	else
	{
		...
		else if (conditionType == X86_CONDITION_UNSIGNED_ABOVE_EQUAL)
		{
			...
			x64Gen_writeU32(x64GenContext, (uint32)memoryImmU32);
				 ...
				 conditionType == X86_CONDITION_CARRY)
		{
			x64Gen_writeU8(x64GenContext, 0x0F);
			x64Gen_writeU8(x64GenContext, 0x92);
			x64Gen_writeU8(x64GenContext, 0x84);
