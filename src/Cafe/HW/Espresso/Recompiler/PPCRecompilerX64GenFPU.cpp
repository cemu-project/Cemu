...
void x64Gen_genSSEVEXPrefix2(x64GenContext_t* x64GenContext, sint32 xmmRegister1,
							 sint32 xmmRegister2, bool use64BitMode)
{
								 ...
								 sint32 xmmRegisterSource)
{
	...
	x64Gen_writeU8(
		x64GenContext,
		0x28); // alternative encoding: 0x29, source and destination register are exchanged
...
{
	// SSE2
	// move lower double from xmm register into lower half of a different xmm register, leave other
	// half untouched
	x64Gen_writeU8(x64GenContext, 0xF2);
	x64Gen_genSSEVEXPrefix2(x64GenContext, xmmRegisterSrc, xmmRegisterDest, false);
	x64Gen_writeU8(x64GenContext, 0x0F);
									 ...
									 uint32 memReg, uint32 memImmS32)
{
	...
	if (memReg == REG_R15)
	{
		x64Gen_genSSEVEXPrefix1(x64GenContext, xmmRegisterDest,
								true); // todo: should be x64Gen_genSSEVEXPrefix2() with memReg?
		x64Gen_writeU8(x64GenContext, 0x0F);
...
{
	// SSE2
	// convert two doubles into two 32-bit integers in bottom part of xmm register, reset upper 64
	// bits of destination register
	x64Gen_writeU8(x64GenContext, 0x66);
	x64Gen_genSSEVEXPrefix2(x64GenContext, xmmRegisterSrc, xmmRegisterDest, false);
	x64Gen_writeU8(x64GenContext, 0x0F);
...
{
	// SSE2
	// converts bottom 64bit double to 32bit signed integer in general purpose register, round based
	// on float-point control
	x64Gen_writeU8(x64GenContext, 0xF2);
	x64Gen_genSSEVEXPrefix2(x64GenContext, xmmRegisterSrc, registerDest, false);
	x64Gen_writeU8(x64GenContext, 0x0F);
...
{
	// SSE2
	// converts bottom 64bit double to 32bit signed integer in general purpose register, always
	// truncate
	x64Gen_writeU8(x64GenContext, 0xF2);
	x64Gen_genSSEVEXPrefix2(x64GenContext, xmmRegisterSrc, registerDest, false);
	x64Gen_writeU8(x64GenContext, 0x0F);
								   ...
								   sint32 registerSrc)
{
	...
	x64Gen_writeU8(
		x64GenContext,
		0x6E); // alternative encoding: 0x29, source and destination register are exchanged
								   ...
								   sint32 xmmRegisterSrc)
{
	...
	x64Gen_writeU8(
		x64GenContext,
		0x7E); // alternative encoding: 0x29, source and destination register are exchanged
