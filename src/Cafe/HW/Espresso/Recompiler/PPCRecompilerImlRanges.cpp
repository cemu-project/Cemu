...
void PPCRecRARange_addLink_perVirtualGPR(raLivenessSubrange_t** root,
										 raLivenessSubrange_t* subrange)
{
...
void PPCRecRARange_addLink_allSubrangesGPR(raLivenessSubrange_t** root,
										   raLivenessSubrange_t* subrange)
{
...
void PPCRecRARange_removeLink_perVirtualGPR(raLivenessSubrange_t** root,
											raLivenessSubrange_t* subrange)
{
	raLivenessSubrange_t* tempPrev = subrange->link_sameVirtualRegisterGPR.prev;
	if (subrange->link_sameVirtualRegisterGPR.prev)
		subrange->link_sameVirtualRegisterGPR.prev->link_sameVirtualRegisterGPR.next =
			subrange->link_sameVirtualRegisterGPR.next;
	else
...
void PPCRecRARange_removeLink_allSubrangesGPR(raLivenessSubrange_t** root,
											  raLivenessSubrange_t* subrange)
{
	raLivenessSubrange_t* tempPrev = subrange->link_segmentSubrangesGPR.prev;
	if (subrange->link_segmentSubrangesGPR.prev)
		subrange->link_segmentSubrangesGPR.prev->link_segmentSubrangesGPR.next =
			subrange->link_segmentSubrangesGPR.next;
	else
...
raLivenessRange_t* PPCRecRA_createRangeBase(ppcImlGenContext_t* ppcImlGenContext,
...
raLivenessSubrange_t* PPCRecRA_createSubrange(ppcImlGenContext_t* ppcImlGenContext,
											  ...
											  sint32 endIndex)
{
	...
	// add to segment
	PPCRecRARange_addLink_perVirtualGPR(
		&(imlSegment->raInfo.linkedList_perVirtualGPR[range->virtualRegister]), livenessSubrange);
	PPCRecRARange_addLink_allSubrangesGPR(&imlSegment->raInfo.linkedList_allSubranges,
										  livenessSubrange);
...
void _unlinkSubrange(raLivenessSubrange_t* subrange)
{
	...
	PPCRecRARange_removeLink_perVirtualGPR(
		&imlSegment->raInfo.linkedList_perVirtualGPR[subrange->range->virtualRegister], subrange);
...
void PPCRecRA_deleteSubrange(ppcImlGenContext_t* ppcImlGenContext, raLivenessSubrange_t* subrange)
{
	...
	subrange->range->list_subranges.erase(std::find(
		subrange->range->list_subranges.begin(), subrange->range->list_subranges.end(), subrange));
...
void _PPCRecRA_deleteSubrangeNoUnlinkFromRange(ppcImlGenContext_t* ppcImlGenContext,
											   raLivenessSubrange_t* subrange)
{
	_unlinkSubrange(subrange);
	PPCRecompilerIml_removeSegmentPoint(&subrange->start);
...
void PPCRecRA_deleteRange(ppcImlGenContext_t* ppcImlGenContext, raLivenessRange_t* range)
{
	...
	ppcImlGenContext->raInfo.list_ranges.erase(
		std::find(ppcImlGenContext->raInfo.list_ranges.begin(),
				  ppcImlGenContext->raInfo.list_ranges.end(), range));
...
void PPCRecRA_mergeRanges(ppcImlGenContext_t* ppcImlGenContext, raLivenessRange_t* range,
						  raLivenessRange_t* absorbedRange)
{
...
void PPCRecRA_mergeSubranges(ppcImlGenContext_t* ppcImlGenContext, raLivenessSubrange_t* subrange,
							 raLivenessSubrange_t* absorbedSubrange)
{
#ifndef PUBLIC_RELEASE
...
// remove all inter-segment connections from the range and split it into local ranges (also removes
// empty ranges)
void PPCRecRA_explodeRange(ppcImlGenContext_t* ppcImlGenContext, raLivenessRange_t* range)
{
	...
	for (auto& subrange : range->list_subranges)
	{
		if (subrange->list_locations.empty())
			continue;
		raLivenessRange_t* newRange =
			PPCRecRA_createRangeBase(ppcImlGenContext, range->virtualRegister, range->name);
		raLivenessSubrange_t* newSubrange = PPCRecRA_createSubrange(
			ppcImlGenContext, newRange, subrange->imlSegment,
			subrange->list_locations.data()[0].index,
			subrange->list_locations.data()[subrange->list_locations.size() - 1].index + 1);
		// copy locations
...
void PPCRecRA_debugValidateSubrange(raLivenessSubrange_t* subrange)
{
	...
	if (subrange->subrangeBranchTaken &&
		subrange->subrangeBranchTaken->imlSegment != subrange->imlSegment->nextSegmentBranchTaken)
		assert_dbg();
	if (subrange->subrangeBranchNotTaken && subrange->subrangeBranchNotTaken->imlSegment !=
												subrange->imlSegment->nextSegmentBranchNotTaken)
		assert_dbg();
}
#else
void PPCRecRA_debugValidateSubrange(raLivenessSubrange_t* subrange) {}
...
// If trimToHole is true, the end of the head subrange and the start of the tail subrange will be
// moved to fit the locations Ranges that begin at RA_INTER_RANGE_START are allowed and can be split
raLivenessSubrange_t* PPCRecRA_splitLocalSubrange(ppcImlGenContext_t* ppcImlGenContext,
												  raLivenessSubrange_t* subrange, sint32 splitIndex,
												  bool trimToHole)
{
	// validation
#ifndef PUBLIC_RELEASE
...
#endif
	// create tail
	raLivenessRange_t* tailRange = PPCRecRA_createRangeBase(
		ppcImlGenContext, subrange->range->virtualRegister, subrange->range->name);
	raLivenessSubrange_t* tailSubrange = PPCRecRA_createSubrange(
		ppcImlGenContext, tailRange, subrange->imlSegment, splitIndex, subrange->end.index);
	// copy locations
	for (auto& location : subrange->list_locations)
	{
	...
	if (trimToHole)
	{
		...
		{
			assert_dbg(); // should not happen? (In this case we can just avoid generating a tail at
						  // all)
		}
		else
		{
...
void PPCRecRA_updateOrAddSubrangeLocation(raLivenessSubrange_t* subrange, sint32 index, bool isRead,
										  bool isWrite)
{
	if (subrange->list_locations.empty())
	{
	...
	raLivenessLocation_t* lastLocation =
		subrange->list_locations.data() + (subrange->list_locations.size() - 1);
...
sint32 PPCRecRARange_estimateCost(raLivenessRange_t* range)
{
	...
	// todo - this algorithm isn't accurate. If we have 10 parallel branches with a load each then
	// the actual cost is still only that of one branch (plus minimal extra cost for generating more
	// code).
	...
	for (auto& subrange : range->list_subranges)
	{
		if (subrange->start.index != RA_INTER_RANGE_START)
		{
			...
			mostExpensiveRead =
				std::max(mostExpensiveRead, PPCRecRARange_getReadWriteCost(subrange->imlSegment));
			readCount++;
		}
		if (subrange->end.index != RA_INTER_RANGE_END)
		{
			// cost += PPCRecRARange_getReadWriteCost(subrange->imlSegment);
			mostExpensiveWrite =
				std::max(mostExpensiveWrite, PPCRecRARange_getReadWriteCost(subrange->imlSegment));
			writeCount++;
		}
	}
...
sint32 PPCRecRARange_estimateAdditionalCostAfterRangeExplode(raLivenessRange_t* range)
{
	...
	for (auto& subrange : range->list_subranges)
	{
		if (subrange->list_locations.empty())
			continue;
		cost += PPCRecRARange_getReadWriteCost(subrange->imlSegment) *
				2; // we assume a read and a store
	}
	return cost;
}

sint32 PPCRecRARange_estimateAdditionalCostAfterSplit(raLivenessSubrange_t* subrange,
													  sint32 splitIndex)
{
	// validation
#ifndef PUBLIC_RELEASE
	...
	cost += PPCRecRARange_getReadWriteCost(subrange->imlSegment) *
			2; // currently we assume that the additional region will require a read and a store
