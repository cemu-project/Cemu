...
PPCRecImlSegment_t* PPCRecompiler_getSegmentByPPCJumpAddress(ppcImlGenContext_t* ppcImlGenContext,
															 uint32 ppcOffset)
{
	...
	debug_printf(
		"PPCRecompiler_getSegmentByPPCJumpAddress(): Unable to find segment (ppcOffset 0x%08x)\n",
		ppcOffset);
	return NULL;
}
...
void PPCRecompilerIML_linkSegments(ppcImlGenContext_t* ppcImlGenContext)
{
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		PPCRecImlSegment_t* nextSegment =
			isLastSegment ? NULL : ppcImlGenContext->segmentList[s + 1];
		// handle empty segment
		if (imlSegment->imlListCount == 0)
		{
			if (isLastSegment == false)
				PPCRecompilerIml_setLinkBranchNotTaken(
					imlSegment,
					ppcImlGenContext->segmentList[s + 1]); // continue execution to next segment
			else
			...
			imlInstruction->type == PPCREC_IML_TYPE_CJUMP_CYCLE_CHECK)
		{
			...
			PPCRecImlSegment_t* jumpDestSegment = PPCRecompiler_getSegmentByPPCJumpAddress(
				ppcImlGenContext, imlInstruction->op_conditionalJump.jumpmarkAddress);
			if (jumpDestSegment)
			{
...
void PPCRecompilerIML_isolateEnterableSegments(ppcImlGenContext_t* ppcImlGenContext)
{
	...
	for (sint32 i = 0; i < ppcImlGenContext->segmentListCount; i++)
	{
		...
		if (imlSegment->list_prevSegments.empty() == false && imlSegment->isEnterable)
		{
			...
			PPCRecompilerIml_insertSegments(ppcImlGenContext, ppcImlGenContext->segmentListCount,
											1);
			PPCRecImlSegment_t* entrySegment =
