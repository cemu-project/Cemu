...
#define PPCREC_CR_REG_TEMP                                                                         \
	8 // there are only 8 cr registers (0-7) we use the 8th as temporary cr register that is never
	  // stored (BDNZ instruction for example)
...
enum
{
	...
	PPCREC_IML_OP_MULTIPLY_HIGH_UNSIGNED, // unsigned 64bit multiply, store only high 32bit-word of
										  // result
	PPCREC_IML_OP_MULTIPLY_HIGH_SIGNED,	  // signed 64bit multiply, store only high 32bit-word of
										  // result
	...
	PPCREC_IML_OP_ADD_CARRY_ME, // complex operation, result = operand + carry bit + (-1), updates
								// carry bit
	...
	PPCREC_IML_OP_FPR_EXPAND_BOTTOM32_TO_BOTTOM64_AND_TOP64, // expand bottom f32 to f64 in bottom
															 // and top half
	PPCREC_IML_OP_FPR_BOTTOM_FRES_TO_BOTTOM_AND_TOP, // calculate reciprocal with Espresso accuracy
													 // of source bottom half and write result to
													 // destination bottom and top half
	...
	PPCREC_IML_OP_FPR_ROUND_TO_SINGLE_PRECISION_BOTTOM, // round 64bit double to 64bit double with
														// 32bit float precision (in bottom half of
														// xmm register)
	PPCREC_IML_OP_FPR_ROUND_TO_SINGLE_PRECISION_PAIR,	// round two 64bit doubles to 64bit double
														// with 32bit float precision
	PPCREC_IML_OP_FPR_BOTTOM_RECIPROCAL_SQRT,
	PPCREC_IML_OP_FPR_BOTTOM_FCTIWZ,
	PPCREC_IML_OP_FPR_SELECT_BOTTOM, // selectively copy bottom value from operand B or C based on
									 // value in operand A
	PPCREC_IML_OP_FPR_SELECT_PAIR, // selectively copy top/bottom from operand B or C based on value
								   // in top/bottom of operand A
	// PS
...
enum
{
	...
	PPCREC_CR_MODE_ARITHMETIC, // arithmetic use (for use with add/sub instructions without
...
enum
{
	...
	PPCREC_IML_TYPE_PPC_ENTER,		   // used to mark locations that should be written to
									   // recompilerCallTable
...
enum
{
	...
	// PPCREC_NAME_CR0 = 3000, // value mapped condition register (usually it isn't needed and can
	// be optimized away)
};

// special cases for LOAD/STORE
#define PPC_REC_LOAD_LWARX_MARKER                                                                  \
	(100) // lwarx instruction (similar to LWZX but sets reserved address/value)
#define PPC_REC_STORE_STWCX_MARKER                                                                 \
	(100) // stwcx instruction (similar to STWX but writes only if reservation from LWARX is valid)
...
bool PPCRecompiler_generateIntermediateCode(ppcImlGenContext_t& ppcImlGenContext,
											PPCRecFunction_t* PPCRecFunction,
...
// IML instruction generation (new style, can generate new instructions but also overwrite existing
// ones)
...
void PPCRecompilerIml_setLinkBranchNotTaken(PPCRecImlSegment_t* imlSegmentSrc,
											PPCRecImlSegment_t* imlSegmentDst);
void PPCRecompilerIml_setLinkBranchTaken(PPCRecImlSegment_t* imlSegmentSrc,
										 PPCRecImlSegment_t* imlSegmentDst);
void PPCRecompilerIML_relinkInputSegment(PPCRecImlSegment_t* imlSegmentOrig,
										 PPCRecImlSegment_t* imlSegmentNew);
void PPCRecompilerIML_removeLink(PPCRecImlSegment_t* imlSegmentSrc,
								 PPCRecImlSegment_t* imlSegmentDst);
...
void PPCRecompiler_dumpIMLSegment(PPCRecImlSegment_t* imlSegment, sint32 segmentIndex,
								  bool printLivenessRangeInfo = false);
...
typedef struct
{
	...
	union
	{
		struct
		{
			// note: If destination operand is not fully written, it will be added as a read FPR as
			// well
			sint16 readFPR1;
			sint16 readFPR2;
			sint16 readFPR3;
