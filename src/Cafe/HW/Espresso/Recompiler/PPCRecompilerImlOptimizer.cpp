									  ...
									  PPCImlOptimizerUsedRegisters_t* registersUsed)
{
	...
	else if (imlInstruction->type == PPCREC_IML_TYPE_R_R)
	{
		if (imlInstruction->operation == PPCREC_IML_OP_COMPARE_SIGNED ||
			...
			imlInstruction->operation == PPCREC_IML_OP_DCBZ)
		{
	...
	{
		if (imlInstruction->operation == PPCREC_IML_MACRO_BL ||
			imlInstruction->operation == PPCREC_IML_MACRO_B_FAR ||
			imlInstruction->operation == PPCREC_IML_MACRO_BLR ||
			imlInstruction->operation == PPCREC_IML_MACRO_BLRL ||
			imlInstruction->operation == PPCREC_IML_MACRO_BCTR ||
			imlInstruction->operation == PPCREC_IML_MACRO_BCTRL ||
			imlInstruction->operation == PPCREC_IML_MACRO_LEAVE ||
			imlInstruction->operation == PPCREC_IML_MACRO_DEBUGBREAK ||
			imlInstruction->operation == PPCREC_IML_MACRO_COUNT_CYCLES ||
			imlInstruction->operation == PPCREC_IML_MACRO_HLE ||
			imlInstruction->operation == PPCREC_IML_MACRO_MFTB)
		{
			// no effect on registers
		}
		else
...
void PPCRecompiler_replaceGPRRegisterUsageMultiple(ppcImlGenContext_t* ppcImlGenContext,
												   PPCRecImlInstruction_t* imlInstruction,
												   sint32 gprRegisterSearched[4],
												   sint32 gprRegisterReplaced[4])
{
	if (imlInstruction->type == PPCREC_IML_TYPE_R_NAME)
	{
		imlInstruction->op_r_name.registerIndex = replaceRegisterMultiple(
			imlInstruction->op_r_name.registerIndex, gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_NAME_R)
	{
		imlInstruction->op_r_name.registerIndex = replaceRegisterMultiple(
	...
	{
		imlInstruction->op_r_r.registerResult = replaceRegisterMultiple(
			imlInstruction->op_r_r.registerResult, gprRegisterSearched, gprRegisterReplaced);
		imlInstruction->op_r_r.registerA = replaceRegisterMultiple(
			imlInstruction->op_r_r.registerA, gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_R_S32)
	{
		imlInstruction->op_r_immS32.registerIndex = replaceRegisterMultiple(
			imlInstruction->op_r_immS32.registerIndex, gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_CONDITIONAL_R_S32)
	{
		imlInstruction->op_conditional_r_s32.registerIndex =
			replaceRegisterMultiple(imlInstruction->op_conditional_r_s32.registerIndex,
									gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_R_R_S32)
	{
		// in all cases result is written and other operand is read only
		imlInstruction->op_r_r_s32.registerResult = replaceRegisterMultiple(
			imlInstruction->op_r_r_s32.registerResult, gprRegisterSearched, gprRegisterReplaced);
		imlInstruction->op_r_r_s32.registerA = replaceRegisterMultiple(
			imlInstruction->op_r_r_s32.registerA, gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_R_R_R)
	{
		// in all cases result is written and other operands are read only
		imlInstruction->op_r_r_r.registerResult = replaceRegisterMultiple(
			imlInstruction->op_r_r_r.registerResult, gprRegisterSearched, gprRegisterReplaced);
		imlInstruction->op_r_r_r.registerA = replaceRegisterMultiple(
			imlInstruction->op_r_r_r.registerA, gprRegisterSearched, gprRegisterReplaced);
		imlInstruction->op_r_r_r.registerB = replaceRegisterMultiple(
			imlInstruction->op_r_r_r.registerB, gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_CJUMP ||
			 imlInstruction->type == PPCREC_IML_TYPE_CJUMP_CYCLE_CHECK)
	{
	...
	{
		imlInstruction->op_storeLoad.registerData = replaceRegisterMultiple(
			imlInstruction->op_storeLoad.registerData, gprRegisterSearched, gprRegisterReplaced);
		if (imlInstruction->op_storeLoad.registerMem != PPC_REC_INVALID_REGISTER)
		{
			imlInstruction->op_storeLoad.registerMem = replaceRegisterMultiple(
				imlInstruction->op_storeLoad.registerMem, gprRegisterSearched, gprRegisterReplaced);
		}
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_LOAD_INDEXED)
	{
		...
		if (imlInstruction->op_storeLoad.registerMem != PPC_REC_INVALID_REGISTER)
			imlInstruction->op_storeLoad.registerMem = replaceRegisterMultiple(
				imlInstruction->op_storeLoad.registerMem, gprRegisterSearched, gprRegisterReplaced);
		if (imlInstruction->op_storeLoad.registerMem2 != PPC_REC_INVALID_REGISTER)
			imlInstruction->op_storeLoad.registerMem2 =
				replaceRegisterMultiple(imlInstruction->op_storeLoad.registerMem2,
										gprRegisterSearched, gprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_STORE)
	{
	...
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD)
	{
		...
		{
			imlInstruction->op_storeLoad.registerGQR = replaceRegisterMultiple(
				imlInstruction->op_storeLoad.registerGQR, gprRegisterSearched, gprRegisterReplaced);
		}
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD_INDEXED)
	{
...
void PPCRecompiler_replaceFPRRegisterUsageMultiple(ppcImlGenContext_t* ppcImlGenContext,
												   PPCRecImlInstruction_t* imlInstruction,
												   sint32 fprRegisterSearched[4],
												   sint32 fprRegisterReplaced[4])
{
	if (imlInstruction->type == PPCREC_IML_TYPE_R_NAME)
	{
		// not affected
	...
	{
		imlInstruction->op_r_name.registerIndex = replaceRegisterMultiple(
			imlInstruction->op_r_name.registerIndex, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_NAME_R)
	{
		imlInstruction->op_r_name.registerIndex = replaceRegisterMultiple(
	...
	{
		imlInstruction->op_storeLoad.registerData = replaceRegisterMultiple(
			imlInstruction->op_storeLoad.registerData, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD_INDEXED)
	{
		imlInstruction->op_storeLoad.registerData = replaceRegisterMultiple(
	...
	{
		imlInstruction->op_fpr_r_r.registerResult = replaceRegisterMultiple(
			imlInstruction->op_fpr_r_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r.registerOperand = replaceRegisterMultiple(
			imlInstruction->op_fpr_r_r.registerOperand, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R_R_R)
	{
		imlInstruction->op_fpr_r_r_r.registerResult = replaceRegisterMultiple(
			imlInstruction->op_fpr_r_r_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r.registerOperandA =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r.registerOperandA,
									fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r.registerOperandB =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r.registerOperandB,
									fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R_R_R_R)
	{
		imlInstruction->op_fpr_r_r_r_r.registerResult =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r_r.registerResult,
									fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandA =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r_r.registerOperandA,
									fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandB =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r_r.registerOperandB,
									fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandC =
			replaceRegisterMultiple(imlInstruction->op_fpr_r_r_r_r.registerOperandC,
									fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R)
	{
		imlInstruction->op_fpr_r.registerResult = replaceRegisterMultiple(
			imlInstruction->op_fpr_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
	}
	else
	{
		cemu_assert_unimplemented();
...
void PPCRecompiler_replaceFPRRegisterUsage(ppcImlGenContext_t* ppcImlGenContext,
										   PPCRecImlInstruction_t* imlInstruction,
										   sint32 fprRegisterSearched, sint32 fprRegisterReplaced)
{
	if (imlInstruction->type == PPCREC_IML_TYPE_R_NAME)
	{
		// not affected
	...
	{
		imlInstruction->op_r_name.registerIndex = replaceRegister(
			imlInstruction->op_r_name.registerIndex, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_NAME_R)
	{
	...
	{
		imlInstruction->op_storeLoad.registerData = replaceRegister(
			imlInstruction->op_storeLoad.registerData, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD_INDEXED)
	{
	...
	{
		imlInstruction->op_fpr_r_r.registerResult = replaceRegister(
			imlInstruction->op_fpr_r_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r.registerOperand = replaceRegister(
			imlInstruction->op_fpr_r_r.registerOperand, fprRegisterSearched, fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R_R_R)
	{
		imlInstruction->op_fpr_r_r_r.registerResult = replaceRegister(
			imlInstruction->op_fpr_r_r_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r.registerOperandA =
			replaceRegister(imlInstruction->op_fpr_r_r_r.registerOperandA, fprRegisterSearched,
							fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r.registerOperandB =
			replaceRegister(imlInstruction->op_fpr_r_r_r.registerOperandB, fprRegisterSearched,
							fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R_R_R_R)
	{
		imlInstruction->op_fpr_r_r_r_r.registerResult =
			replaceRegister(imlInstruction->op_fpr_r_r_r_r.registerResult, fprRegisterSearched,
							fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandA =
			replaceRegister(imlInstruction->op_fpr_r_r_r_r.registerOperandA, fprRegisterSearched,
							fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandB =
			replaceRegister(imlInstruction->op_fpr_r_r_r_r.registerOperandB, fprRegisterSearched,
							fprRegisterReplaced);
		imlInstruction->op_fpr_r_r_r_r.registerOperandC =
			replaceRegister(imlInstruction->op_fpr_r_r_r_r.registerOperandC, fprRegisterSearched,
							fprRegisterReplaced);
	}
	else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R)
	{
		imlInstruction->op_fpr_r.registerResult = replaceRegister(
			imlInstruction->op_fpr_r.registerResult, fprRegisterSearched, fprRegisterReplaced);
	}
	else
	{
...
typedef struct
{
	struct
	{
		...
		bool nameMustBeMaintained; // must be stored before replacement and loaded after replacement
								   // ends
	} replacedRegisterEntry[PPC_X64_GPR_USABLE_REGISTERS];
	sint32 count;
} replacedRegisterTracker_t;

bool PPCRecompiler_checkIfGPRRegisterIsAccessed(PPCImlOptimizerUsedRegisters_t* registersUsed,
												sint32 gprRegister)
{
...
bool PPCRecompiler_findAvailableRegisterDepr(ppcImlGenContext_t* ppcImlGenContext,
											 PPCRecImlSegment_t* imlSegment, sint32 imlIndexStart,
											 replacedRegisterTracker_t* replacedRegisterTracker,
											 sint32* registerIndex, sint32* registerName,
											 bool* isUsed)
{
	PPCImlOptimizerUsedRegisters_t registersUsed;
	PPCRecompiler_checkRegisterUsage(ppcImlGenContext, imlSegment->imlList + imlIndexStart,
									 &registersUsed);
	...
	{
		replacementReservedRegisterMask |=
			(1 << replacedRegisterTracker->replacedRegisterEntry[i].index);
	}

	// potential improvement: Scan ahead a few instructions and look for registers that are the
	// least used (or ideally never used)

	// pick available register
	const uint32 allRegisterMask =
		(1 << (PPC_X64_GPR_USABLE_REGISTERS + 1)) - 1; // mask with set bit for every register
	uint32 reservedRegisterMask = instructionReservedRegisterMask | replacementReservedRegisterMask;
	cemu_assert(instructionReservedRegisterMask !=
				allRegisterMask); // no usable register! (Need to store a register from the
								  // replacedRegisterTracker)
	sint32 usedRegisterIndex = -1;
	for (sint32 i = 0; i < PPC_X64_GPR_USABLE_REGISTERS; i++)
	{
		if ((reservedRegisterMask & (1 << i)) == 0)
		{
			if ((instructionReservedRegisterMask & (1 << i)) == 0 &&
				ppcImlGenContext->mappedRegister[i] != -1)
			{
...
bool PPCRecompiler_hasSuffixInstruction(PPCRecImlSegment_t* imlSegment)
{
	...
	if (imlInstruction->type == PPCREC_IML_TYPE_MACRO &&
		...
		imlInstruction->type == PPCREC_IML_TYPE_MACRO &&
			imlInstruction->operation == PPCREC_IML_MACRO_BL ||
		imlInstruction->type == PPCREC_IML_TYPE_MACRO &&
			imlInstruction->operation == PPCREC_IML_MACRO_B_FAR ||
		imlInstruction->type == PPCREC_IML_TYPE_MACRO &&
			imlInstruction->operation == PPCREC_IML_MACRO_BLRL ||
...
void PPCRecompiler_storeReplacedRegister(ppcImlGenContext_t* ppcImlGenContext,
										 ...
										 replacedRegisterTracker_t* replacedRegisterTracker,
										 sint32 registerTrackerIndex, sint32* imlIndex)
{
	// store register
	...
	imlInstructionItr->op_r_name.registerIndex =
		replacedRegisterTracker->replacedRegisterEntry[registerTrackerIndex].index;
	imlInstructionItr->op_r_name.name =
		replacedRegisterTracker->replacedRegisterEntry[registerTrackerIndex].registerNewName;
	...
	if (replacedRegisterTracker->replacedRegisterEntry[registerTrackerIndex].nameMustBeMaintained)
	{
		...
		imlInstructionItr->op_r_name.registerIndex =
			replacedRegisterTracker->replacedRegisterEntry[registerTrackerIndex].index;
		imlInstructionItr->op_r_name.name =
			replacedRegisterTracker->replacedRegisterEntry[registerTrackerIndex]
				.registerPreviousName; // ppcImlGenContext->mappedRegister[replacedRegisterTracker.replacedRegisterEntry[i].index];
		imlInstructionItr->op_r_name.copyWidth = 32;
		imlInstructionItr->op_r_name.flags = 0;
		imlIndexEdit += 1;
	}
	// move last entry to current one
	memcpy(replacedRegisterTracker->replacedRegisterEntry + registerTrackerIndex,
		   replacedRegisterTracker->replacedRegisterEntry + replacedRegisterTracker->count - 1,
		   sizeof(replacedRegisterTracker->replacedRegisterEntry[0]));
...
bool PPCRecompiler_reduceNumberOfFPRRegisters(ppcImlGenContext_t* ppcImlGenContext)
{
	...
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		while (imlIndex < imlSegment->imlListCount)
		{
			...
			if (imlInstructionItr->type == PPCREC_IML_TYPE_FPR_R_NAME ||
	...
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		while (imlIndex < imlSegment->imlListCount)
		{
			...
			while (true)
			{
				PPCRecompiler_checkRegisterUsage(ppcImlGenContext, imlSegment->imlList + imlIndex,
												 &registersUsed);
				if (registersUsed.readFPR1 >= PPC_X64_FPR_USABLE_REGISTERS ||
					registersUsed.readFPR2 >= PPC_X64_FPR_USABLE_REGISTERS ||
					registersUsed.readFPR3 >= PPC_X64_FPR_USABLE_REGISTERS ||
					registersUsed.readFPR4 >= PPC_X64_FPR_USABLE_REGISTERS ||
					registersUsed.writtenFPR1 >= PPC_X64_FPR_USABLE_REGISTERS)
				{
					...
					if (unusedRegisterIndex == -1)
						assert_dbg();
					// determine if the placeholder register is actually used (if not we must not
					// load/store it)
					uint32 unusedRegisterName =
						ppcImlGenContext->mappedFPRRegister[unusedRegisterIndex];
					bool replacedRegisterIsUsed = true;
					if (unusedRegisterName >= PPCREC_NAME_FPR0 &&
						unusedRegisterName < (PPCREC_NAME_FPR0 + 32))
					{
						replacedRegisterIsUsed =
							imlSegment->ppcFPRUsed[unusedRegisterName - PPCREC_NAME_FPR0];
					}
					// replace registers that are out of range
					PPCRecompiler_replaceFPRRegisterUsage(ppcImlGenContext,
														  imlSegment->imlList + imlIndex,
														  fprToReplace, unusedRegisterIndex);
					...
					// name_unusedRegister = unusedRegister
					PPCRecImlInstruction_t* imlInstructionItr =
						imlSegment->imlList + (imlIndex + 0);
					memset(imlInstructionItr, 0x00, sizeof(PPCRecImlInstruction_t));
					...
					imlInstructionItr->op_r_name.registerIndex = unusedRegisterIndex;
					imlInstructionItr->op_r_name.name =
						ppcImlGenContext->mappedFPRRegister[fprToReplace];
					imlInstructionItr->op_r_name.copyWidth = 32;
					imlInstructionItr->op_r_name.flags = 0;
...
ppcRecRegisterMapping_t*
PPCRecompiler_findAvailableRegisterDepr(ppcRecManageRegisters_t* rCtx,
										PPCImlOptimizerUsedRegisters_t* instructionUsedRegisters)
{
	// find free register
...
ppcRecRegisterMapping_t*
PPCRecompiler_findUnloadableRegister(ppcRecManageRegisters_t* rCtx,
									 PPCImlOptimizerUsedRegisters_t* instructionUsedRegisters,
									 uint32 unloadLockedMask)
{
...
bool PPCRecompiler_manageFPRRegistersForSegment(ppcImlGenContext_t* ppcImlGenContext,
												sint32 segmentIndex)
{
	...
	while (idx < imlSegment->imlListCount)
	{
		if (PPCRecompiler_isSuffixInstruction(imlSegment->imlList + idx))
			break;
		PPCRecompiler_checkRegisterUsage(ppcImlGenContext, imlSegment->imlList + idx,
										 &registersUsed);
		sint32 fprMatch[4];
		sint32 fprReplace[4];
		fprMatch[0] = -1;
			...
			if (rCtx.ppcRegToMapping[virtualFpr] == -1)
			{
				...
				while (true)
				{
					...
					if (regMapping == NULL)
					{
						...
						ppcRecRegisterMapping_t* unloadRegMapping =
							PPCRecompiler_findUnloadableRegister(&rCtx, &registersUsed,
																 unloadLockedMask);
						// mark as locked
						...
						imlInstructionTemp->op_r_name.registerIndex =
							(uint8)(unloadRegMapping - rCtx.currentMapping);
						imlInstructionTemp->op_r_name.name =
							ppcImlGenContext->mappedFPRRegister[unloadRegMapping->virtualReg];
				...
				imlInstructionTemp->operation = PPCREC_IML_OP_ASSIGN;
				imlInstructionTemp->op_r_name.registerIndex =
					(uint8)(regMapping - rCtx.currentMapping);
				imlInstructionTemp->op_r_name.name =
					ppcImlGenContext->mappedFPRRegister[virtualFpr];
				imlInstructionTemp->op_r_name.copyWidth = 32;
				imlInstructionTemp->op_r_name.flags = 0;
				idx++;
				// update mapping
		...
		{
			PPCRecompiler_replaceFPRRegisterUsageMultiple(
				ppcImlGenContext, imlSegment->imlList + idx, fprMatch, fprReplace);
		}
		// next
		idx++;
	}
	...
	if (numLoadedRegisters > 0)
	{
		...
		for (sint32 i = 0; i < PPC_X64_FPR_USABLE_REGISTERS; i++)
		{
			...
			imlInstructionTemp->op_r_name.name =
				ppcImlGenContext->mappedFPRRegister[rCtx.currentMapping[i].virtualReg];
			imlInstructionTemp->op_r_name.copyWidth = 32;
			imlInstructionTemp->op_r_name.flags = 0;
			idx++;
		}
 ...
 */
bool PPCRecompiler_trackRedundantNameLoadInstruction(ppcImlGenContext_t* ppcImlGenContext,
													 PPCRecImlSegment_t* imlSegment,
													 sint32 startIndex,
													 PPCRecImlInstruction_t* nameStoreInstruction,
													 sint32 scanDepth)
{
	...
	for (sint32 i = startIndex; i < imlSegment->imlListCount; i++)
	{
		...
		if (registersUsed.readNamedReg1 == registerIndex ||
			registersUsed.readNamedReg2 == registerIndex ||
			registersUsed.readNamedReg3 == registerIndex)
			return false;
 ...
 * Returns true if the loaded value is guaranteed to be overwritten
 */
bool PPCRecompiler_trackRedundantFPRNameLoadInstruction(
	ppcImlGenContext_t* ppcImlGenContext, PPCRecImlSegment_t* imlSegment, sint32 startIndex,
	PPCRecImlInstruction_t* nameStoreInstruction, sint32 scanDepth)
{
	...
	for (sint32 i = startIndex; i < imlSegment->imlListCount; i++)
	{
		...
		PPCImlOptimizerUsedRegisters_t registersUsed;
		PPCRecompiler_checkRegisterUsage(ppcImlGenContext, imlSegment->imlList + i, &registersUsed);
		if (registersUsed.readFPR1 == registerIndex || registersUsed.readFPR2 == registerIndex ||
			registersUsed.readFPR3 == registerIndex || registersUsed.readFPR4 == registerIndex)
			return false;
 ...
 */
bool PPCRecompiler_trackOverwrittenNameStoreInstruction(
	ppcImlGenContext_t* ppcImlGenContext, PPCRecImlSegment_t* imlSegment, sint32 startIndex,
	PPCRecImlInstruction_t* nameStoreInstruction, sint32 scanDepth)
{
	// sint16 registerIndex = nameStoreInstruction->op_r_name.registerIndex;
	...
	if (imlSegment->nextSegmentBranchTaken &&
		PPCRecompiler_trackOverwrittenNameStoreInstruction(
			ppcImlGenContext, imlSegment->nextSegmentBranchTaken, 0, nameStoreInstruction,
			scanDepth + 1) == false)
		return false;
	if (imlSegment->nextSegmentBranchNotTaken &&
		PPCRecompiler_trackOverwrittenNameStoreInstruction(
			ppcImlGenContext, imlSegment->nextSegmentBranchNotTaken, 0, nameStoreInstruction,
			scanDepth + 1) == false)
		return false;
 ...
 */
bool PPCRecompiler_trackRedundantFPRNameStoreInstruction(
	ppcImlGenContext_t* ppcImlGenContext, PPCRecImlSegment_t* imlSegment, sint32 startIndex,
	PPCRecImlInstruction_t* nameStoreInstruction, sint32 scanDepth)
{
	sint16 registerIndex = nameStoreInstruction->op_r_name.registerIndex;
...
uint32 _PPCRecompiler_getCROverwriteMask(ppcImlGenContext_t* ppcImlGenContext,
										 PPCRecImlSegment_t* imlSegment,
										 uint32 currentOverwriteMask, uint32 currentReadMask,
										 uint32 scanDepth)
{
	...
	if (imlSegment->nextSegmentIsUncertain == false && scanDepth < 3)
	{
		...
		{
			uint32 mask0 = _PPCRecompiler_getCROverwriteMask(
				ppcImlGenContext, imlSegment->nextSegmentBranchTaken, 0, 0, scanDepth + 1);
			uint32 mask1 = _PPCRecompiler_getCROverwriteMask(
				ppcImlGenContext, imlSegment->nextSegmentBranchNotTaken, 0, 0, scanDepth + 1);
			nextSegmentOverwriteMask = mask0 & mask1;
		}
		else if (imlSegment->nextSegmentBranchNotTaken)
		{
			nextSegmentOverwriteMask = _PPCRecompiler_getCROverwriteMask(
				ppcImlGenContext, imlSegment->nextSegmentBranchNotTaken, 0, 0, scanDepth + 1);
		}
...
/*
 * Returns a mask of all CR bits that are overwritten (written but not read) in the segment and all
 * it's following segments If the write state of a CR bit cannot be determined, it is returned as 0
 * (not overwritten)
 */
uint32 PPCRecompiler_getCROverwriteMask(ppcImlGenContext_t* ppcImlGenContext,
										PPCRecImlSegment_t* imlSegment)
{
	if (imlSegment->nextSegmentIsUncertain)
	{
	...
	{
		uint32 mask0 = _PPCRecompiler_getCROverwriteMask(
			ppcImlGenContext, imlSegment->nextSegmentBranchTaken, 0, 0, 0);
		uint32 mask1 = _PPCRecompiler_getCROverwriteMask(
			ppcImlGenContext, imlSegment->nextSegmentBranchNotTaken, 0, 0, 0);
		return mask0 & mask1; // only return bits that are overwritten in both branches
	}
	else if (imlSegment->nextSegmentBranchNotTaken)
	{
		uint32 mask = _PPCRecompiler_getCROverwriteMask(
			ppcImlGenContext, imlSegment->nextSegmentBranchNotTaken, 0, 0, 0);
		return mask;
	}
	else
...
void PPCRecompiler_removeRedundantCRUpdates(ppcImlGenContext_t* ppcImlGenContext)
{
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		for (sint32 i = 0; i < imlSegment->imlListCount; i++)
		{
			...
			if (imlInstruction->type == PPCREC_IML_TYPE_CJUMP)
			{
				...
				{
					uint32 crBitFlag = 1
									   << (imlInstruction->op_conditionalJump.crRegisterIndex * 4 +
										   imlInstruction->op_conditionalJump.crBitIndex);
					imlSegment->crBitsInput |=
						(crBitFlag &
						 ~imlSegment
							  ->crBitsWritten); // flag bits that have not already been written
			...
			else if (imlInstruction->type == PPCREC_IML_TYPE_CONDITIONAL_R_S32)
			{
				...
				imlSegment->crBitsInput |=
					(crBitFlag &
					 ~imlSegment->crBitsWritten); // flag bits that have not already been written
				imlSegment->crBitsRead |= (crBitFlag);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_R_S32 &&
	...
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		{
			PPCRecImlInstruction_t* imlInstruction = imlSegment->imlList + i;
			if (PPCRecompilerImlAnalyzer_canTypeWriteCR(imlInstruction) &&
				imlInstruction->crRegister >= 0 && imlInstruction->crRegister <= 7)
			{
				uint32 crBitFlags = 0xF << ((uint32)imlInstruction->crRegister * 4);
				uint32 crOverwriteMask =
					PPCRecompiler_getCROverwriteMask(ppcImlGenContext, imlSegment);
...
sint32 PPCRecompiler_scanBackwardsForReusableRegister(ppcImlGenContext_t* ppcImlGenContext,
													  PPCRecImlSegment_t* startSegment,
													  sint32 startIndex, sint32 name)
{
	// current segment
	...
	while (true)
	{
		...
		{
			if (currentSegment->imlList[currentIndex].type == PPCREC_IML_TYPE_NAME_R &&
				currentSegment->imlList[currentIndex].op_r_name.name == name)
			{
	...
	while (true)
	{
		while (currentIndex >= 0)
		{
			...
			PPCRecompiler_checkRegisterUsage(
				ppcImlGenContext, currentSegment->imlList + currentIndex, &registersUsed);
...
void PPCRecompiler_optimizeDirectFloatCopiesScanForward(ppcImlGenContext_t* ppcImlGenContext,
														PPCRecImlSegment_t* imlSegment,
														sint32 imlIndexLoad, sint32 fprIndex)
{
	...
	PPCImlOptimizerUsedRegisters_t registersUsed;
	sint32 scanRangeEnd = std::min(
		imlIndexLoad + 25,
		imlSegment->imlListCount); // don't scan too far (saves performance and also the chances we
								   // can merge the load+store become low at high distances)
	...
	for (sint32 i = imlIndexLoad + 1; i < scanRangeEnd; i++)
	{
		...
		if ((imlInstruction->type == PPCREC_IML_TYPE_FPR_STORE &&
			 imlInstruction->op_storeLoad.mode == PPCREC_FPR_ST_MODE_SINGLE_FROM_PS0) ||
	...
	if (foundMatch)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_fpr_r(
			ppcImlGenContext, newExpand, PPCREC_IML_OP_FPR_EXPAND_BOTTOM32_TO_BOTTOM64_AND_TOP64,
			fprIndex);
	}
}
 ...
 * For these patterns the store and load is modified to work with un-extended values (float remains
 * as float, no double conversion) The float->double extension is then executed later Advantages:
...
void PPCRecompiler_optimizeDirectFloatCopies(ppcImlGenContext_t* ppcImlGenContext)
{
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		{
			PPCRecImlInstruction_t* imlInstruction = imlSegment->imlList + i;
			if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD &&
				imlInstruction->op_storeLoad.mode == PPCREC_FPR_LD_MODE_SINGLE_INTO_PS0_PS1)
			{
				PPCRecompiler_optimizeDirectFloatCopiesScanForward(
					ppcImlGenContext, imlSegment, i, imlInstruction->op_storeLoad.registerData);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD_INDEXED &&
					 imlInstruction->op_storeLoad.mode == PPCREC_FPR_LD_MODE_SINGLE_INTO_PS0_PS1)
			{
...
void PPCRecompiler_optimizeDirectIntegerCopiesScanForward(ppcImlGenContext_t* ppcImlGenContext,
														  PPCRecImlSegment_t* imlSegment,
														  sint32 imlIndexLoad, sint32 gprIndex)
{
	PPCRecImlInstruction_t* imlInstructionLoad = imlSegment->imlList + imlIndexLoad;
	...
	for (; i < scanRangeEnd; i++)
	{
		...
		if ((imlInstruction->type == PPCREC_IML_TYPE_STORE &&
			 imlInstruction->op_storeLoad.copyWidth == 32))
		{
			...
			if (imlInstruction->op_storeLoad.registerData == gprIndex)
			{
				...
				if (foundMatch == false)
				{
					...
					imlInstructionLoad->op_storeLoad.flags2.swapEndian =
						!imlInstructionLoad->op_storeLoad.flags2.swapEndian;
					foundMatch = true;
				}
				// switch the endian swap flag for the store instruction
				imlInstructionStore->op_storeLoad.flags2.swapEndian =
					!imlInstructionStore->op_storeLoad.flags2.swapEndian;
				// keep scanning
				continue;
			}
		...
		PPCRecompiler_checkRegisterUsage(ppcImlGenContext, imlInstruction, &registersUsed);
		if (registersUsed.readNamedReg1 == gprIndex || registersUsed.readNamedReg2 == gprIndex ||
	...
	if (foundMatch)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, newExpand, PPCREC_IML_OP_ENDIAN_SWAP, gprIndex, gprIndex);
	}
}
...
void PPCRecompiler_optimizeDirectIntegerCopies(ppcImlGenContext_t* ppcImlGenContext)
{
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		{
			PPCRecImlInstruction_t* imlInstruction = imlSegment->imlList + i;
			if (imlInstruction->type == PPCREC_IML_TYPE_LOAD &&
				imlInstruction->op_storeLoad.copyWidth == 32 &&
				imlInstruction->op_storeLoad.flags2.swapEndian)
			{
				PPCRecompiler_optimizeDirectIntegerCopiesScanForward(
					ppcImlGenContext, imlSegment, i, imlInstruction->op_storeLoad.registerData);
			}
		}
	}
...
bool PPCRecompiler_isUGQRValueKnown(ppcImlGenContext_t* ppcImlGenContext, sint32 gqrIndex,
									uint32& gqrValue)
{
	// UGQR 2 to 7 are initialized by the OS and we assume that games won't ever permanently touch
	// those todo - hack - replace with more accurate solution
	if (gqrIndex == 2)
		gqrValue = 0x00040004;
	else if (gqrIndex == 3)
		gqrValue = 0x00050005;
...
/*
 * If value of GQR can be predicted for a given PSQ load or store instruction then replace it with
		...
		for (sint32 i = 0; i < imlSegment->imlListCount; i++)
		{
				...
				imlInstruction->type == PPCREC_IML_TYPE_FPR_LOAD_INDEXED)
			{
				if (imlInstruction->op_storeLoad.mode != PPCREC_FPR_LD_MODE_PSQ_GENERIC_PS0 &&
					...
					continue;
				// get GQR value
				cemu_assert_debug(imlInstruction->op_storeLoad.registerGQR !=
								  PPC_REC_INVALID_REGISTER);
				sint32 gqrIndex = _getGQRIndexFromRegister(
					ppcImlGenContext, imlInstruction->op_storeLoad.registerGQR);
				...
				else if (imlInstruction->op_storeLoad.mode ==
 ...
 */
bool PPCRecompilerAnalyzer_checkForGPROverwrite(PPCImlOptimizerUsedRegisters_t* registerRead,
												PPCImlOptimizerUsedRegisters_t* registerWrite)
{
...
void _reorderConditionModifyInstructions(PPCRecImlSegment_t* imlSegment)
{
	...
	for (sint32 i = imlSegment->imlListCount - 2; i >= 0; i--)
	{
		...
		if ((imlInstruction->type == PPCREC_IML_TYPE_NAME_R ||
			 imlInstruction->type == PPCREC_IML_TYPE_R_NAME ||
			 imlInstruction->type == PPCREC_IML_TYPE_NO_OP) ||
			(imlInstruction->type == PPCREC_IML_TYPE_FPR_NAME_R ||
	...
	PPCRecompiler_checkRegisterUsage(NULL, imlSegment->imlList + crSetterInstructionIndex,
									 &regTrackingCRSetter);
	if (regTrackingCRSetter.writtenFPR1 >= 0 || regTrackingCRSetter.readFPR1 >= 0 ||
		regTrackingCRSetter.readFPR2 >= 0 || regTrackingCRSetter.readFPR3 >= 0 ||
		regTrackingCRSetter.readFPR4 >= 0)
	...
	if ((unsafeInstructionIndex + 1) <= crSetterInstructionIndex)
		assert_dbg();
#endif
	PPCRecImlInstruction_t* newCRSetterInstruction =
		PPCRecompiler_insertInstruction(imlSegment, unsafeInstructionIndex + 1);
	memcpy(newCRSetterInstruction, imlSegment->imlList + crSetterInstructionIndex,
		   sizeof(PPCRecImlInstruction_t));
	PPCRecompilerImlGen_generateNewInstruction_noOp(NULL,
													imlSegment->imlList + crSetterInstructionIndex);
}
