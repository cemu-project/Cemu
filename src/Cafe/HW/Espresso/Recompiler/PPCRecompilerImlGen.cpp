...
PPCRecompilerImlGen_generateNewEmptyInstruction(ppcImlGenContext_t* ppcImlGenContext)
{
	if (ppcImlGenContext->imlListCount + 1 > ppcImlGenContext->imlListSize)
	{
		sint32 newSize = ppcImlGenContext->imlListCount * 2 + 2;
		ppcImlGenContext->imlList = (PPCRecImlInstruction_t*)realloc(
			ppcImlGenContext->imlList, sizeof(PPCRecImlInstruction_t) * newSize);
		ppcImlGenContext->imlListSize = newSize;
	}
	PPCRecImlInstruction_t* imlInstruction =
		ppcImlGenContext->imlList + ppcImlGenContext->imlListCount;
...
void PPCRecompilerImlGen_generateNewInstruction_jumpmark(ppcImlGenContext_t* ppcImlGenContext,
														 uint32 address)
{
	// no-op that indicates possible destination of a jump
	PPCRecImlInstruction_t* imlInstruction =
		PPCRecompilerImlGen_generateNewEmptyInstruction(ppcImlGenContext);
	imlInstruction->type = PPCREC_IML_TYPE_JUMPMARK;
	imlInstruction->op_jumpmark.address = address;
}

void PPCRecompilerImlGen_generateNewInstruction_macro(ppcImlGenContext_t* ppcImlGenContext,
													  uint32 macroId, uint32 param, uint32 param2,
													  uint16 paramU16)
{
	// no-op that indicates possible destination of a jump
	PPCRecImlInstruction_t* imlInstruction =
 ...
 */
void PPCRecompilerImlGen_generateNewInstruction_ppcEnter(ppcImlGenContext_t* ppcImlGenContext,
														 uint32 ppcAddress)
{
	// no-op that indicates possible destination of a jump
	PPCRecImlInstruction_t* imlInstruction =
...
void PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext_t* ppcImlGenContext,
													PPCRecImlInstruction_t* imlInstruction,
													uint32 operation, uint8 registerResult,
													uint8 registerA, uint8 crRegister, uint8 crMode)
{
...
void PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext_t* ppcImlGenContext,
													  uint32 operation, uint8 registerResult,
													  uint8 registerA, uint8 registerB,
													  uint8 crRegister = PPC_REC_INVALID_REGISTER,
													  uint8 crMode = 0)
{
...
void PPCRecompilerImlGen_generateNewInstruction_name_r(ppcImlGenContext_t* ppcImlGenContext,
													   uint32 operation, uint8 registerIndex,
													   uint32 name, uint32 copyWidth,
													   bool signExtend, bool bigEndian)
{
	...
	imlInstruction->op_r_name.flags = (signExtend ? PPCREC_IML_OP_FLAG_SIGNEXTEND : 0) |
									  (bigEndian ? PPCREC_IML_OP_FLAG_SWITCHENDIAN : 0);
}

void PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext_t* ppcImlGenContext,
													  uint32 operation, uint8 registerIndex,
													  sint32 immS32, uint32 copyWidth,
													  bool signExtend, bool bigEndian,
													  uint8 crRegister, uint32 crMode)
{
	// two variations:
	// operation without store (e.g. "'r3' < 123" which has no effect other than updating a
	// condition flags register) operation with store (e.g. "'r3' = 123")
	PPCRecImlInstruction_t* imlInstruction =
		PPCRecompilerImlGen_generateNewEmptyInstruction(ppcImlGenContext);
...
void PPCRecompilerImlGen_generateNewInstruction_conditional_r_s32(
	ppcImlGenContext_t* ppcImlGenContext, PPCRecImlInstruction_t* imlInstruction, uint32 operation,
	uint8 registerIndex, sint32 immS32, uint32 crRegisterIndex, uint32 crBitIndex,
	bool bitMustBeSet)
{
	if (imlInstruction == NULL)
		imlInstruction = PPCRecompilerImlGen_generateNewEmptyInstruction(ppcImlGenContext);
...
void PPCRecompilerImlGen_generateNewInstruction_jump(ppcImlGenContext_t* ppcImlGenContext,
													 PPCRecImlInstruction_t* imlInstruction,
...
void PPCRecompilerImlGen_generateNewInstruction_jumpSegment(ppcImlGenContext_t* ppcImlGenContext,
															PPCRecImlInstruction_t* imlInstruction)
{
	// jump
	if (imlInstruction == NULL)
...
void PPCRecompilerImlGen_generateNewInstruction_noOp(ppcImlGenContext_t* ppcImlGenContext,
													 PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction == NULL)
		imlInstruction = PPCRecompilerImlGen_generateNewEmptyInstruction(ppcImlGenContext);
...
void PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext_t* ppcImlGenContext,
												   uint32 operation, uint8 crD, uint8 crA,
												   uint8 crB)
{
	// multiple variations:
...
void PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
	ppcImlGenContext_t* ppcImlGenContext, uint32 jumpmarkAddress, uint32 jumpCondition,
...
void PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext_t* ppcImlGenContext,
														 uint8 registerDestination,
														 uint8 registerMemory, sint32 immS32,
														 uint32 copyWidth, bool signExtend,
														 bool switchEndian)
{
	// load from memory
	PPCRecImlInstruction_t* imlInstruction =
	...
	// imlInstruction->op_storeLoad.flags = (signExtend ? PPCREC_IML_OP_FLAG_SIGNEXTEND : 0) |
	// (switchEndian ? PPCREC_IML_OP_FLAG_SWITCHENDIAN : 0);
	imlInstruction->op_storeLoad.flags2.swapEndian = switchEndian;
	imlInstruction->op_storeLoad.flags2.signExtend = signExtend;
}

void PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
	ppcImlGenContext_t* ppcImlGenContext, uint8 registerDestination, uint8 registerMemory1,
	uint8 registerMemory2, uint32 copyWidth, bool signExtend, bool switchEndian)
{
	...
	imlInstruction->op_storeLoad.copyWidth = copyWidth;
	// imlInstruction->op_storeLoad.flags =
	// (signExtend?PPCREC_IML_OP_FLAG_SIGNEXTEND:0)|(switchEndian?PPCREC_IML_OP_FLAG_SWITCHENDIAN:0);
	imlInstruction->op_storeLoad.flags2.swapEndian = switchEndian;
	imlInstruction->op_storeLoad.flags2.signExtend = signExtend;
}

void PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext_t* ppcImlGenContext,
														 uint8 registerSource, uint8 registerMemory,
														 sint32 immS32, uint32 copyWidth,
														 bool switchEndian)
{
	// load from memory
...
void PPCRecompilerImlGen_generateNewInstruction_memory_memory(
	ppcImlGenContext_t* ppcImlGenContext, PPCRecImlInstruction_t* imlInstruction, uint8 srcMemReg,
	sint32 srcImmS32, uint8 dstMemReg, sint32 dstImmS32, uint8 copyWidth)
{
...
uint32 PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(ppcImlGenContext_t* ppcImlGenContext,
													  uint32 mappedName)
{
	if (mappedName == PPCREC_NAME_NONE)
	{
		debug_printf(
			"PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(): Invalid mappedName parameter\n");
													  ...
													  uint32 mappedName)
{
	...
	{
		debug_printf(
			"PPCRecompilerImlGen_getAndLockFreeTemporaryFPR(): Invalid mappedName parameter\n");
		return PPC_REC_INVALID_REGISTER;
	}
										...
										bool loadNew)
{
	if (loadNew == false)
	{
		uint32 loadedRegisterIndex =
			PPCRecompilerImlGen_findRegisterByMappedName(ppcImlGenContext, mappedName);
		if (loadedRegisterIndex != PPC_REC_INVALID_REGISTER)
			return loadedRegisterIndex;
	}
	uint32 registerIndex =
		PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(ppcImlGenContext, mappedName);
 ...
 */
uint32 PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext_t* ppcImlGenContext,
												 uint32 mappedName)
{
	uint32 loadedRegisterIndex =
		PPCRecompilerImlGen_findRegisterByMappedName(ppcImlGenContext, mappedName);
 ...
 */
uint32 PPCRecompilerImlGen_loadFPRRegister(ppcImlGenContext_t* ppcImlGenContext, uint32 mappedName,
										   bool loadNew)
{
	if (loadNew == false)
	{
		uint32 loadedRegisterIndex =
			PPCRecompilerImlGen_findFPRRegisterByMappedName(ppcImlGenContext, mappedName);
		if (loadedRegisterIndex != PPC_REC_INVALID_REGISTER)
			return loadedRegisterIndex;
	}
	uint32 registerIndex =
		PPCRecompilerImlGen_getAndLockFreeTemporaryFPR(ppcImlGenContext, mappedName);
	return registerIndex;
}

/*
 * Checks if a PPC fpr register is already loaded into any IML register
 * If no, it will create a new undefined temporary IML FPU register and map the name (effectively
 * overwriting the old ppc register)
 */
uint32 PPCRecompilerImlGen_loadOverwriteFPRRegister(ppcImlGenContext_t* ppcImlGenContext,
													uint32 mappedName)
{
	uint32 loadedRegisterIndex =
		PPCRecompilerImlGen_findFPRRegisterByMappedName(ppcImlGenContext, mappedName);
...
void PPCRecompilerImlGen_TW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	//	PPCRecompilerImlGen_generateNewInstruction_macro(ppcImlGenContext,
	// PPCREC_IML_MACRO_DEBUGBREAK, ppcImlGenContext->ppcAddressOfCurrentInstruction, 0, 0); #endif
	PPCRecompilerImlGen_generateNewInstruction_macro(
		ppcImlGenContext, PPCREC_IML_MACRO_LEAVE, ppcImlGenContext->ppcAddressOfCurrentInstruction,
		0, 0);
}
...
bool PPCRecompilerImlGen_MTSPR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		uint32 gprReg =
			PPCRecompilerImlGen_findRegisterByMappedName(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		if (gprReg == PPC_REC_INVALID_REGISTER)
			gprReg = PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		uint32 sprReg =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_SPR0 + spr);
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   sprReg, gprReg);
	}
...
bool PPCRecompilerImlGen_MFSPR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (spr == SPR_LR || spr == SPR_CTR)
	{
		...
		uint32 gprReg =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   gprReg, sprReg);
	}
	else if (spr >= SPR_UGQR0 && spr <= SPR_UGQR7)
...
bool PPCRecompilerImlGen_MFTB(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (spr == 268 || spr == 269)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_macro(
			ppcImlGenContext, PPCREC_IML_MACRO_MFTB,
			ppcImlGenContext->ppcAddressOfCurrentInstruction, param2, 0);
		return true;
	}
	return false;
}
...
bool PPCRecompilerImlGen_MFCR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 gprReg =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_MFCR, gprReg,
													 0, 0, false, false, PPC_REC_INVALID_REGISTER,
													 0);
	return true;
}
...
bool PPCRecompilerImlGen_MTCRF(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 gprReg =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rS);
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_MTCRF, gprReg,
													 crMask, 0, false, false,
													 PPC_REC_INVALID_REGISTER, 0);
	return true;
}
...
void PPCRecompilerImlGen_CMP(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	cr >>= 2;
	uint32 gprRegisterA =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false);
	uint32 gprRegisterB =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rB, false);
	PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
												   PPCREC_IML_OP_COMPARE_SIGNED, gprRegisterA,
												   gprRegisterB, cr, PPCREC_CR_MODE_COMPARE_SIGNED);
}
...
void PPCRecompilerImlGen_CMPL(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_r(
		ppcImlGenContext, NULL, PPCREC_IML_OP_COMPARE_UNSIGNED, gprRegisterA, gprRegisterB, cr,
		PPCREC_CR_MODE_COMPARE_UNSIGNED);
}
...
void PPCRecompilerImlGen_CMPI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 gprRegister =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false);
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_COMPARE_SIGNED,
													 gprRegister, b, 0, false, false, cr,
...
void PPCRecompilerImlGen_CMPLI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_s32(
		ppcImlGenContext, PPCREC_IML_OP_COMPARE_UNSIGNED, gprRegister, (sint32)b, 0, false, false,
		cr, PPCREC_CR_MODE_COMPARE_UNSIGNED);
}
...
void PPCRecompiler_generateInlinedCode(ppcImlGenContext_t* ppcImlGenContext, uint32 startAddress,
									   sint32 instructionCount)
{
...
bool PPCRecompilerImlGen_B(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_LK)
	{
		...
		{
			// generate NOP iml instead of BL macro (this assures that segment PPC range remains
			// intact)
			PPCRecompilerImlGen_generateNewInstruction_noOp(ppcImlGenContext, NULL);
			// forceLog_printf("Inline func 0x%08x at %08x", jumpAddressDest,
			// ppcImlGenContext->ppcAddressOfCurrentInstruction);
			uint32* prevInstructionPtr = ppcImlGenContext->currentInstruction;
			ppcImlGenContext->currentInstruction =
				(uint32*)memory_getPointerFromVirtualOffset(jumpAddressDest);
			PPCRecompiler_generateInlinedCode(ppcImlGenContext, jumpAddressDest,
		...
		PPCRecompilerImlGen_generateNewInstruction_macro(
			ppcImlGenContext, PPCREC_IML_MACRO_BL, ppcImlGenContext->ppcAddressOfCurrentInstruction,
			jumpAddressDest, ppcImlGenContext->cyclesSinceLastBranch);
		PPCRecompilerImlGen_generateNewInstruction_ppcEnter(
			ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4);
		return true;
	}
	// is jump destination within recompiled function?
	if (jumpAddressDest >= ppcImlGenContext->functionRef->ppcAddress &&
		jumpAddressDest <
			(ppcImlGenContext->functionRef->ppcAddress + ppcImlGenContext->functionRef->ppcSize))
	{
	...
	{
		// todo: Inline this jump destination if possible (in many cases it's a bunch of GPR/FPR
		// store instructions + BLR)
		PPCRecompilerImlGen_generateNewInstruction_macro(
			ppcImlGenContext, PPCREC_IML_MACRO_B_FAR,
			ppcImlGenContext->ppcAddressOfCurrentInstruction, jumpAddressDest,
			ppcImlGenContext->cyclesSinceLastBranch);
	}
	return true;
}
...
bool PPCRecompilerImlGen_BC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	bool decrementerMustBeZero =
		(BO & 2) != 0; // bit set -> branch if CTR = 0, bit not set -> branch if CTR != 0
	...
	if (opcode & PPC_OPC_LK)
	{
		...
		if (ignoreCondition == false)
		{
			...
			// generate instruction
			// PPCRecompilerImlGen_generateNewInstruction_macro(ppcImlGenContext,
			// PPCREC_IML_MACRO_DEBUGBREAK, ppcImlGenContext->ppcAddressOfCurrentInstruction, 0, 0);
			PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
				ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4,
				jumpCondition, crRegister, crBit, !conditionMustBeTrue);
			PPCRecompilerImlGen_generateNewInstruction_macro(
				ppcImlGenContext, PPCREC_IML_MACRO_BL,
				ppcImlGenContext->ppcAddressOfCurrentInstruction, jumpAddressDest,
				ppcImlGenContext->cyclesSinceLastBranch);
			PPCRecompilerImlGen_generateNewInstruction_ppcEnter(
				ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4);
	...
	if (useDecrementer)
	{
		...
		uint32 ctrRegister =
			PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_SPR0 + SPR_CTR, false);
		PPCRecompilerImlGen_generateNewInstruction_r_s32(
			ppcImlGenContext, PPCREC_IML_OP_SUB, ctrRegister, 1, 0, false, false,
			PPCREC_CR_REG_TEMP, PPCREC_CR_MODE_ARITHMETIC);
		if (decrementerMustBeZero)
			PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
				ppcImlGenContext, jumpAddressDest, PPCREC_JUMP_CONDITION_E, PPCREC_CR_REG_TEMP, 0,
				false);
		else
			PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
				ppcImlGenContext, jumpAddressDest, PPCREC_JUMP_CONDITION_NE, PPCREC_CR_REG_TEMP, 0,
				false);
		return true;
	}
	else
	{
		if (ignoreCondition)
		{
			...
			debugBreakpoint();
			crRegister = PPC_REC_INVALID_REGISTER; // not necessary but lets optimizer know we dont
												   // care for cr register on this instruction
		}
		else
		{
			// generate jump condition
			...
			if (jumpAddressDest >= ppcImlGenContext->functionRef->ppcAddress &&
				jumpAddressDest < (ppcImlGenContext->functionRef->ppcAddress +
								   ppcImlGenContext->functionRef->ppcSize))
			{
				// near jump
				PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
					ppcImlGenContext, jumpAddressDest, jumpCondition, crRegister, crBit,
					conditionMustBeTrue);
			}
			else
			{
...
bool PPCRecompilerImlGen_BCLR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	else
	{
		if (ignoreCondition)
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_macro(
					ppcImlGenContext, PPCREC_IML_MACRO_BLRL,
					ppcImlGenContext->ppcAddressOfCurrentInstruction, 0,
					ppcImlGenContext->cyclesSinceLastBranch);
				PPCRecompilerImlGen_generateNewInstruction_ppcEnter(
					ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4);
			}
			else
			{
				// branch always, no condition and no decrementer
				PPCRecompilerImlGen_generateNewInstruction_macro(
					ppcImlGenContext, PPCREC_IML_MACRO_BLR,
					ppcImlGenContext->ppcAddressOfCurrentInstruction, 0,
					ppcImlGenContext->cyclesSinceLastBranch);
			}
		}
		else
		{
			...
			if (saveLR)
			{
				uint32 registerLR = PPCRecompilerImlGen_loadOverwriteRegister(
					ppcImlGenContext, PPCREC_NAME_SPR0 + SPR_LR);
				PPCRecompilerImlGen_generateNewInstruction_r_s32(
					ppcImlGenContext, PPCREC_IML_OP_ASSIGN, registerLR,
					(ppcImlGenContext->ppcAddressOfCurrentInstruction + 4) & 0x7FFFFFFF, 0, false,
					false, PPC_REC_INVALID_REGISTER, 0);
			}
			// generate jump condition
			...
			// jump if BCLR condition NOT met (jump to jumpmark of next instruction, essentially
			// skipping current instruction)
			PPCRecompilerImlGen_generateNewInstruction_conditionalJump(
				ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4,
				jumpCondition, crRegister, crBit, invertedConditionMustBeTrue);
			PPCRecompilerImlGen_generateNewInstruction_macro(
				ppcImlGenContext, PPCREC_IML_MACRO_BLR,
				ppcImlGenContext->ppcAddressOfCurrentInstruction, 0,
				ppcImlGenContext->cyclesSinceLastBranch);
		...
		if (ignoreCondition)
		{
			...
			if (saveLR)
			{
				...
				PPCRecompilerImlGen_generateNewInstruction_r_s32(
					...
					false, PPC_REC_INVALID_REGISTER, 0);
				PPCRecompilerImlGen_generateNewInstruction_macro(
					ppcImlGenContext, PPCREC_IML_MACRO_BCTRL,
					ppcImlGenContext->ppcAddressOfCurrentInstruction, 0,
					ppcImlGenContext->cyclesSinceLastBranch);
				PPCRecompilerImlGen_generateNewInstruction_ppcEnter(
					ppcImlGenContext, ppcImlGenContext->ppcAddressOfCurrentInstruction + 4);
			...
			else
			{
				// branch always, no condition and no decrementer
				PPCRecompilerImlGen_generateNewInstruction_macro(
					ppcImlGenContext, PPCREC_IML_MACRO_BCTR,
					ppcImlGenContext->ppcAddressOfCurrentInstruction, 0,
					ppcImlGenContext->cyclesSinceLastBranch);
			}
		}
...
bool PPCRecompilerImlGen_ADD(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_ADD,
														 registerRD, registerRA, registerRB, 0,
														 PPCREC_CR_MODE_LOGICAL);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_ADD,
														 registerRD, registerRA, registerRB);
	}
	return true;
}
...
bool PPCRecompilerImlGen_ADDC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_ADD_UPDATE_CARRY, registerRD, registerRA, registerRB, 0,
			PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_ADD_UPDATE_CARRY, registerRD, registerRA, registerRB);
	return true;
}
...
bool PPCRecompilerImlGen_ADDE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_ADD_CARRY_UPDATE_CARRY, registerRD, registerRB,
			registerRA, 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext,
														 PPCREC_IML_OP_ADD_CARRY_UPDATE_CARRY,
														 registerRD, registerRB, registerRA);
	return true;
}
...
bool PPCRecompilerImlGen_ADDZE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   registerRD, registerRA);
	}
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
													   PPCREC_IML_OP_ADD_CARRY, registerRD,
													   registerRD, 0, PPCREC_CR_MODE_LOGICAL);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_ADD_CARRY, registerRD, registerRD);
	}
	return true;
}
...
bool PPCRecompilerImlGen_ADDME(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
													   PPCREC_IML_OP_ADD_CARRY_ME, registerRD,
													   registerRD, 0, PPCREC_CR_MODE_LOGICAL);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_ADD_CARRY_ME, registerRD, registerRD);
	}
	return true;
}
...
bool PPCRecompilerImlGen_ADDI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA != 0)
	{
		...
		uint32 registerRD =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
														   registerRD, registerRA, imm);
	}
	else
	{
		...
		uint32 registerRD =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ASSIGN,
														 registerRD, imm, 0, false, false,
														 PPC_REC_INVALID_REGISTER, 0);
	}
...
bool PPCRecompilerImlGen_ADDIS(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA != 0)
	{
		...
		uint32 registerRD =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
														   registerRD, registerRA, (sint32)imm);
	}
	else
	{
		...
		uint32 registerRD =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ASSIGN,
														 registerRD, (sint32)imm, 0, false, false,
														 PPC_REC_INVALID_REGISTER, 0);
	}
	// never updates any cr
	return true;
...
bool PPCRecompilerImlGen_ADDIC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
		ppcImlGenContext, PPCREC_IML_OP_ADD_UPDATE_CARRY, registerRD, registerRA, imm);
	// never updates any cr
	return true;
}
...
bool PPCRecompilerImlGen_ADDIC_(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext,
													   PPCREC_IML_OP_ADD_UPDATE_CARRY, registerRD,
													   registerRA, imm, 0, PPCREC_CR_MODE_LOGICAL);
	return true;
}
...
bool PPCRecompilerImlGen_SUBF(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SUB,
														 registerRD, registerRB, registerRA, 0,
														 PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SUB,
														 registerRD, registerRB, registerRA);
...
bool PPCRecompilerImlGen_SUBFE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_SUB_CARRY_UPDATE_CARRY, registerRD, registerRB,
			registerRA, 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext,
														 PPCREC_IML_OP_SUB_CARRY_UPDATE_CARRY,
														 registerRD, registerRB, registerRA);
	return true;
}
...
bool PPCRecompilerImlGen_SUBFZE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_SUB_CARRY_UPDATE_CARRY, registerRD, registerRA, 0,
			PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_SUB_CARRY_UPDATE_CARRY, registerRD, registerRA);
	return true;
}
...
bool PPCRecompilerImlGen_SUBFC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SUBFC,
													 registerRD, registerRA, registerRB);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_AND,
													   registerRD, registerRD, 0,
													   PPCREC_CR_MODE_LOGICAL);
	return true;
}
...
bool PPCRecompilerImlGen_SUBFIC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRD =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD);
	PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_SUBFC,
													   registerRD, registerRA, imm);
	return true;
}
...
bool PPCRecompilerImlGen_MULLI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerResult =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD, false);
	uint32 registerOperand =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false);
	PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
		ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_SIGNED, registerResult, registerOperand,
		(sint32)imm);
	return true;
}
...
bool PPCRecompilerImlGen_MULLW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerResult =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rD, false);
	uint32 registerOperand1 =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false);
	uint32 registerOperand2 =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rB, false);
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_SIGNED, registerResult, registerOperand1,
			registerOperand2, 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_SIGNED, registerResult, registerOperand1,
			registerOperand2);
...
bool PPCRecompilerImlGen_MULHW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_HIGH_SIGNED, registerResult, registerOperand1,
			registerOperand2, 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_HIGH_SIGNED, registerResult, registerOperand1,
...
bool PPCRecompilerImlGen_MULHWU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_HIGH_UNSIGNED, registerResult,
			registerOperand1, registerOperand2, 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_MULTIPLY_HIGH_UNSIGNED, registerResult,
...
bool PPCRecompilerImlGen_DIVW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_DIVIDE_SIGNED, registerResult, registerOperand1,
			registerOperand2, 0, PPCREC_CR_MODE_ARITHMETIC);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
...
bool PPCRecompilerImlGen_DIVWU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(
			ppcImlGenContext, PPCREC_IML_OP_DIVIDE_UNSIGNED, registerResult, registerOperand1,
			registerOperand2, 0, PPCREC_CR_MODE_ARITHMETIC);
	}
	else
	{
...
bool PPCRecompilerImlGen_RLWINM(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRS =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rS, false);
	uint32 registerRA =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
	...
	else if (ME == (31 - SH) && MB == 0)
	{
		...
		if (opcode & PPC_OPC_RC)
			PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
				ppcImlGenContext, PPCREC_IML_OP_LEFT_SHIFT, registerRA, registerRS, SH, 0,
				PPCREC_CR_MODE_LOGICAL);
		else
			PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
				ppcImlGenContext, PPCREC_IML_OP_LEFT_SHIFT, registerRA, registerRS, SH);
		return true;
	}
	else if (SH == (32 - MB) && ME == 31)
	{
		...
		if (opcode & PPC_OPC_RC)
			PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
				ppcImlGenContext, PPCREC_IML_OP_RIGHT_SHIFT, registerRA, registerRS, MB, 0,
				PPCREC_CR_MODE_LOGICAL);
		else
			PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
				ppcImlGenContext, PPCREC_IML_OP_RIGHT_SHIFT, registerRA, registerRS, MB);
		return true;
	}
	// general handler
	if (registerRA != registerRS)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   registerRA, registerRS);
	if (SH != 0)
		PPCRecompilerImlGen_generateNewInstruction_r_s32(
			ppcImlGenContext, PPCREC_IML_OP_LEFT_ROTATE, registerRA, SH, 0, false, false,
			PPC_REC_INVALID_REGISTER, 0);
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_AND,
														 registerRA, (sint32)mask, 0, false, false,
														 0, PPCREC_CR_MODE_LOGICAL);
	}
	else
	{
		if (mask != 0xFFFFFFFF)
			PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_AND,
															 registerRA, (sint32)mask, 0, false,
															 false, PPC_REC_INVALID_REGISTER, 0);
	}
	return true;
}
...
bool PPCRecompilerImlGen_RLWIMI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_RLWIMI,
													   registerRA, registerRS, (sint32)vImm,
													   PPC_REC_INVALID_REGISTER, 0);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_AND,
													   registerRA, registerRA, 0,
													   PPCREC_CR_MODE_LOGICAL);
	return true;
}
...
bool PPCRecompilerImlGen_RLWNM(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_LEFT_ROTATE,
													 registerRA, registerRS, registerRB);
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_AND,
														 registerRA, (sint32)mask, 32, false, false,
														 0, PPCREC_CR_MODE_LOGICAL);
	}
	else
	{
		if (mask != 0xFFFFFFFF)
			PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_AND,
															 registerRA, (sint32)mask, 32, false,
															 false, PPC_REC_INVALID_REGISTER, 0);
	}
	return true;
}
...
bool PPCRecompilerImlGen_SRAW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if ((opcode & PPC_OPC_RC) != 0)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SRAW,
														 registerRA, registerRS, registerRB, 0,
														 PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SRAW,
														 registerRA, registerRS, registerRB);
...
bool PPCRecompilerImlGen_SRAWI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRA =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_SRAW,
														   registerRA, registerRS, (sint32)SH, 0,
														   PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_s32(ppcImlGenContext, PPCREC_IML_OP_SRAW,
														   registerRA, registerRS, (sint32)SH);
	return true;
}
...
bool PPCRecompilerImlGen_SLW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SLW,
														 registerRA, registerRS, registerRB, 0,
														 PPCREC_CR_MODE_LOGICAL);
	}
	else
...
bool PPCRecompilerImlGen_SRW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_SRW,
														 registerRA, registerRS, registerRB, 0,
														 PPCREC_CR_MODE_LOGICAL);
	}
	else
...
bool PPCRecompilerImlGen_EXTSH(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
													   PPCREC_IML_OP_ASSIGN_S16_TO_S32, registerRA,
													   registerRS, 0, PPCREC_CR_MODE_ARITHMETIC);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN_S16_TO_S32, registerRA, registerRS);
	}
	return true;
}
...
bool PPCRecompilerImlGen_EXTSB(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
													   PPCREC_IML_OP_ASSIGN_S8_TO_S32, registerRA,
													   registerRS, 0, PPCREC_CR_MODE_ARITHMETIC);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(
			ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN_S8_TO_S32, registerRA, registerRS);
	}
	return true;
}
...
bool PPCRecompilerImlGen_CNTLZW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	uint32 registerRA =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
	PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_CNTLZW,
												   registerRA, registerRS);
...
bool PPCRecompilerImlGen_NEG(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (opcode & PPC_OPC_RC)
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_NEG,
													   registerRD, registerRA, 0,
													   PPCREC_CR_MODE_ARITHMETIC);
	}
	else
	{
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_NEG,
...
void PPCRecompilerImlGen_LWZ(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA == 0)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_macro(
			ppcImlGenContext, PPCREC_IML_MACRO_DEBUGBREAK,
			ppcImlGenContext->ppcAddressOfCurrentInstruction,
			ppcImlGenContext->ppcAddressOfCurrentInstruction,
			ppcImlGenContext->cyclesSinceLastBranch);
		return;
	...
	if (destinationRegister == PPC_REC_INVALID_REGISTER)
		destinationRegister = PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(
			ppcImlGenContext, PPCREC_NAME_R0 + rD); // else just create new register
	// load half
	PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
														gprRegister, imm, 32, false, true);
}
...
void PPCRecompilerImlGen_LWZU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
													 gprRegister, (sint32)imm, 0, false, false,
													 PPC_REC_INVALID_REGISTER, 0);
	// check if destination register is already loaded
	uint32 destinationRegister =
		PPCRecompilerImlGen_findRegisterByMappedName(ppcImlGenContext, PPCREC_NAME_R0 + rD);
...
void PPCRecompilerImlGen_LHA(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (destinationRegister == PPC_REC_INVALID_REGISTER)
		destinationRegister = PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(
			ppcImlGenContext, PPCREC_NAME_R0 + rD); // else just create new temporary register
	// load half
	PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
														gprRegister, imm, 16, true, true);
}
...
bool PPCRecompilerImlGen_LWZX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
		ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 32, false, true);
	return true;
}
...
bool PPCRecompilerImlGen_LWZUX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (destinationRegister == PPC_REC_INVALID_REGISTER)
		destinationRegister = PPCRecompilerImlGen_getAndLockFreeTemporaryGPR(
			ppcImlGenContext, PPCREC_NAME_R0 + rD); // else just create new register
	// add rB to rA
	PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ADD,
												   gprRegisterA, gprRegisterB);
	// load word
	PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
														gprRegisterA, 0, 32, false, true);
	return true;
}
...
bool PPCRecompilerImlGen_LWBRX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA)
		PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
			ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 32, false, false);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
															gprRegisterB, 0, 32, false, false);
	return true;
}
...
bool PPCRecompilerImlGen_LHAX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// load half word
	PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
		ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 16, true, true);
	return true;
}
...
bool PPCRecompilerImlGen_LHZX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// load half word
	PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
		ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 16, false, true);
	return true;
}
...
void PPCRecompilerImlGen_LHBRX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// load memory rA and rB into register
	uint32 gprRegisterA =
		rA != 0 ? PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false)
				: 0;
	uint32 gprRegisterB =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rB, false);
	// check if destination register is already loaded
	...
	if (rA == 0)
		PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
															gprRegisterB, 0, 16, false, false);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
			ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 16, false, false);
}
...
bool PPCRecompilerImlGen_LBZX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// load byte
	PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
		ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, 8, false, true);
	return true;
}
...
bool PPCRecompilerImlGen_LWARX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA != 0)
		PPCRecompilerImlGen_generateNewInstruction_r_memory_indexed(
			ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB,
			PPC_REC_LOAD_LWARX_MARKER, false, true);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_memory(ppcImlGenContext, destinationRegister,
															gprRegisterB, 0,
															PPC_REC_LOAD_LWARX_MARKER, false, true);
...
void PPCRecompilerImlGen_LMW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	while (rD <= 31)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_r_memory(
			ppcImlGenContext, destinationRegister, gprRegister, imm + index * 4, 32, false, true);
		// next
		rD++;
		index++;
	}
...
void PPCRecompilerImlGen_STW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA == 0)
	{
		...
		// note: Darksiders 2 has this instruction form but it is never executed.
		// PPCRecompilerImlGen_generateNewInstruction_macro(ppcImlGenContext,
		// PPCREC_IML_MACRO_DEBUGBREAK, ppcImlGenContext->ppcAddressOfCurrentInstruction,
		// ppcImlGenContext->ppcAddressOfCurrentInstruction,
		// ppcImlGenContext->cyclesSinceLastBranch);
		return;
	}
	...
	uint32 sourceRegister = PPCRecompilerImlGen_loadRegister(
		ppcImlGenContext, PPCREC_NAME_R0 + rD, false); // can be the same as gprRegister
	// store word
	PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, sourceRegister,
														gprRegister, imm, 32, true);
}
...
void PPCRecompilerImlGen_STWU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// store&update instructions where rD==rA store the register contents without added imm,
	// therefore we need to handle it differently get memory gpr register
	uint32 gprRegister =
		PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA, false);
	// get source register
	uint32 sourceRegister = PPCRecompilerImlGen_loadRegister(
	...
	if (rD != rA)
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
														 ...
														 PPC_REC_INVALID_REGISTER, 0);
	// store word
	PPCRecompilerImlGen_generateNewInstruction_memory_r(
		ppcImlGenContext, sourceRegister, gprRegister, (rD == rA) ? imm : 0, 32, true);
...
void PPCRecompilerImlGen_STHU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// store word
	PPCRecompilerImlGen_generateNewInstruction_memory_r(
		ppcImlGenContext, sourceRegister, gprRegister, (rD == rA) ? imm : 0, 16, true);
	// add imm to memory register late if we couldn't do it early
	if (rD == rA)
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
														 gprRegister, (sint32)imm, 0, false, false,
...
void PPCRecompilerImlGen_STB(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// load source register
	uint32 sourceRegister = PPCRecompilerImlGen_loadRegister(
		ppcImlGenContext, PPCREC_NAME_R0 + rS, false); // can be the same as gprRegister
	// store byte
	PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, sourceRegister,
														gprRegister, imm, 8, true);
}
...
void PPCRecompilerImlGen_STBU(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// store byte
	PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, sourceRegister,
														gprRegister, (rD == rA) ? imm : 0, 8, true);
	// add imm to memory register late if we couldn't do it early
	if (rD == rA)
		PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_ADD,
														 gprRegister, (sint32)imm, 0, false, false,
...
bool PPCRecompilerImlGen_STORE_INDEXED(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode,
									   uint32 storeBitWidth, bool byteReversed = false)
{
	sint32 rA, rS, rB;
	PPC_OPC_TEMPL_X(opcode, rS, rA, rB);
	// prepare registers
	...
	{
		PPCRecompilerImlGen_generateNewInstruction_memory_r(
			ppcImlGenContext, destinationRegister, gprRegisterB, 0, storeBitWidth, !byteReversed);
	}
	else
		PPCRecompilerImlGen_generateNewInstruction_memory_r_indexed(
			ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB, storeBitWidth, false,
			!byteReversed);
	return true;
}

bool PPCRecompilerImlGen_STORE_INDEXED_UPDATE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode,
											  uint32 storeBitWidth)
{
	sint32 rA, rS, rB;
	PPC_OPC_TEMPL_X(opcode, rS, rA, rB);
	if (rA == 0)
	...
	// store word
	PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, sourceRegister,
														gprRegisterA, 0, storeBitWidth, true);
	return true;
}
...
bool PPCRecompilerImlGen_STWCX(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rA != 0)
		PPCRecompilerImlGen_generateNewInstruction_memory_r_indexed(
			ppcImlGenContext, destinationRegister, gprRegisterA, gprRegisterB,
			PPC_REC_STORE_STWCX_MARKER, false, true);
	else
		PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, destinationRegister,
															gprRegisterB, 0,
															PPC_REC_STORE_STWCX_MARKER, true);
	return true;
}
...
void PPCRecompilerImlGen_STMW(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	while (rS <= 31)
	{
		...
		// store word
		PPCRecompilerImlGen_generateNewInstruction_memory_r(ppcImlGenContext, sourceRegister,
															gprRegister, imm + index * 4, 32, true);
		// next
		rS++;
		index++;
	}
...
bool PPCRecompilerImlGen_LSWI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	else if (nb == 2)
	{
		...
		// shift
		PPCRecompilerImlGen_generateNewInstruction_r_r_s32(
			ppcImlGenContext, PPCREC_IML_OP_LEFT_SHIFT, destinationRegister, destinationRegister,
			16);
		return true;
	}
	else if (nb == 3)
	{
		// if nb == 3 this instruction loads a 3-byte big-endian and the result is shifted left by 8
		// bits
		if (rA == 0)
		{
#ifndef PUBLIC_RELEASE
...
bool PPCRecompilerImlGen_STSWI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	else if (nb == 2)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_memory_r(
			ppcImlGenContext, sourceRegister, gprRegister, 0, PPC_REC_STORE_STSWI_2, false);
		return true;
	}
	else if (nb == 3)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_memory_r(
			ppcImlGenContext, sourceRegister, gprRegister, 0, PPC_REC_STORE_STSWI_3, false);
		return true;
	}
...
bool PPCRecompilerImlGen_DCBZ(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	// store
	if (rA != 0)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_DCBZ,
													   gprRegisterA, gprRegisterB);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_DCBZ,
													   gprRegisterB, gprRegisterB);
	return true;
}
...
bool PPCRecompilerImlGen_OR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rS == rB)
	{
		...
		{
			sint32 gprSourceReg =
				PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rS);
			sint32 gprDestReg =
				PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
			if (opcode & PPC_OPC_RC)
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN, gprDestReg, gprSourceReg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
			else
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN, gprDestReg, gprSourceReg);
			}
		}
		else
		{
			if (opcode & PPC_OPC_RC)
			{
				...
				sint32 gprSourceReg =
					PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rS);
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN, gprSourceReg, gprSourceReg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
			else
			{
	...
	{
		// rA = rS | rA
		sint32 gprSource1Reg =
			PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rS);
		sint32 gprSource2Reg =
			PPCRecompilerImlGen_loadRegister(ppcImlGenContext, PPCREC_NAME_R0 + rB);
		sint32 gprDestReg =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
		if (gprSource1Reg == gprDestReg || gprSource2Reg == gprDestReg)
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_OR, gprDestReg, gprSource1Reg);
			}
			if (opcode & PPC_OPC_RC)
			{
				// fixme: merge CR update into OR instruction above
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_AND, gprDestReg, gprDestReg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
		}
		else
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN, gprDestReg, gprSource1Reg);
			}
			// rA |= rB
			if (opcode & PPC_OPC_RC)
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_OR, gprDestReg, gprSource2Reg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
			else
			{
...
bool PPCRecompilerImlGen_ORC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	sint32 gprDestReg =
		PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
	if (opcode & PPC_OPC_RC)
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_ORC,
														 gprDestReg, gprSource1Reg, gprSource2Reg,
														 0, PPCREC_CR_MODE_LOGICAL);
	else
		PPCRecompilerImlGen_generateNewInstruction_r_r_r(ppcImlGenContext, PPCREC_IML_OP_ORC,
														 gprDestReg, gprSource1Reg, gprSource2Reg);
...
bool PPCRecompilerImlGen_NOR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rS == rB)
	{
		...
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_NOT, gprDestReg, gprDestReg, 0,
				PPCREC_CR_MODE_ARITHMETIC);
		}
		else
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_NOT, gprDestReg, gprDestReg);
		}
	}
	else
	{
		...
		else
		{
			...
			// rA |= rB
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_OR,
														   gprDestReg, gprSource2Reg);
			if (opcode & PPC_OPC_RC)
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_NOT, gprDestReg, gprDestReg, 0,
...
bool PPCRecompilerImlGen_AND(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	else
	{
		...
		if (gprSource1Reg == gprDestReg || gprSource2Reg == gprDestReg)
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_AND, gprDestReg, gprSource1Reg);
			}
			if (opcode & PPC_OPC_RC)
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
		...
		else
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_AND, gprDestReg, gprSource2Reg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
			else
			{
...
bool PPCRecompilerImlGen_ANDC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rS == rB)
	{
		...
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
														   PPCREC_IML_OP_XOR, gprDestReg,
														   gprDestReg, 0, PPCREC_CR_MODE_LOGICAL);
		}
		else
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_XOR, gprDestReg, gprDestReg);
		}
	}
	else if (rA == rB)
	{
		...
		// rA = ~rA
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_NOT,
													   gprRA, gprRA);
		// rA &= rS
		if (opcode & PPC_OPC_RC)
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_AND, gprRA, gprRS, 0, PPCREC_CR_MODE_LOGICAL);
		}
		else
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
														   PPCREC_IML_OP_AND, gprRA, gprRS);
		}
	}
	else
	{
		...
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
														   PPCREC_IML_OP_ASSIGN, gprRA, gprRS);
		}
		...
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_NOT, gprRA, gprRA, 0, PPCREC_CR_MODE_LOGICAL);
		}
		else
		{
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
														   PPCREC_IML_OP_NOT, gprRA, gprRA);
		}
	}
	return true;
}
...
void PPCRecompilerImlGen_ANDI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (gprDestReg != gprSourceReg)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   gprDestReg, gprSourceReg);
	// rA &= imm32
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_AND,
													 gprDestReg, (sint32)imm, 0, false, false, 0,
													 PPCREC_CR_MODE_LOGICAL);
}
...
bool PPCRecompilerImlGen_XOR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	else
	{
		...
		if (gprSource1Reg == gprDestReg || gprSource2Reg == gprDestReg)
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_XOR, gprDestReg, gprSource2Reg);
			}
			else
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_XOR, gprDestReg, gprSource1Reg);
			}
			if (opcode & PPC_OPC_RC)
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
		...
		else
		{
			...
			{
				PPCRecompilerImlGen_generateNewInstruction_r_r(
					ppcImlGenContext, NULL, PPCREC_IML_OP_XOR, gprDestReg, gprSource2Reg, 0,
					PPCREC_CR_MODE_LOGICAL);
			}
			else
			{
...
bool PPCRecompilerImlGen_EQV(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (rS == rB)
	{
		...
		sint32 gprDestReg =
			PPCRecompilerImlGen_loadOverwriteRegister(ppcImlGenContext, PPCREC_NAME_R0 + rA);
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_XOR,
													   gprDestReg, gprDestReg);
		if (opcode & PPC_OPC_RC)
			PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL,
														   PPCREC_IML_OP_NOT, gprDestReg,
														   gprDestReg, 0, PPCREC_CR_MODE_LOGICAL);
		else
			PPCRecompilerImlGen_generateNewInstruction_r_r(
				ppcImlGenContext, NULL, PPCREC_IML_OP_NOT, gprDestReg, gprDestReg);
...
void PPCRecompilerImlGen_ORI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (gprDestReg != gprSourceReg)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   gprDestReg, gprSourceReg);
	// rA |= imm32
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_OR, gprDestReg,
													 (sint32)imm, 0, false, false,
													 PPC_REC_INVALID_REGISTER, 0);
}
...
void PPCRecompilerImlGen_XORI(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (gprDestReg != gprSourceReg)
		PPCRecompilerImlGen_generateNewInstruction_r_r(ppcImlGenContext, NULL, PPCREC_IML_OP_ASSIGN,
													   gprDestReg, gprSourceReg);
	// rA |= imm32
	PPCRecompilerImlGen_generateNewInstruction_r_s32(ppcImlGenContext, PPCREC_IML_OP_XOR,
													 gprDestReg, (sint32)imm, 0, false, false,
													 PPC_REC_INVALID_REGISTER, 0);
}
...
bool PPCRecompilerImlGen_CROR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_OR, crD, crA,
												  crB);
	return true;
}
...
bool PPCRecompilerImlGen_CRORC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	int crD, crA, crB;
	PPC_OPC_TEMPL_X(opcode, crD, crA, crB);
	PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_ORC, crD, crA,
												  crB);
	return true;
}
...
bool PPCRecompilerImlGen_CRAND(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	int crD, crA, crB;
	PPC_OPC_TEMPL_X(opcode, crD, crA, crB);
	PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_AND, crD, crA,
												  crB);
	return true;
}
...
bool PPCRecompilerImlGen_CRANDC(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	int crD, crA, crB;
	PPC_OPC_TEMPL_X(opcode, crD, crA, crB);
	PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_ANDC, crD, crA,
												  crB);
	return true;
}
...
bool PPCRecompilerImlGen_CRXOR(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (crA == crB)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_CLEAR, crD,
													  0, 0);
		return true;
	}
	else
...
bool PPCRecompilerImlGen_CREQV(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	if (crA == crB)
	{
		...
		PPCRecompilerImlGen_generateNewInstruction_cr(ppcImlGenContext, PPCREC_IML_OP_CR_SET, crD,
													  0, 0);
		return true;
	}
	else
...
bool PPCRecompilerImlGen_HLE(ppcImlGenContext_t* ppcImlGenContext, uint32 opcode)
{
	...
	PPCRecompilerImlGen_generateNewInstruction_macro(
		ppcImlGenContext, PPCREC_IML_MACRO_HLE, ppcImlGenContext->ppcAddressOfCurrentInstruction,
		hleFuncId, 0);
	return true;
}
...
void PPCRecompilerDebug_printLivenessRangeInfo(StringBuf& currentLineText,
											   PPCRecImlSegment_t* imlSegment, sint32 offset)
{
...
void PPCRecompiler_dumpIMLSegment(PPCRecImlSegment_t* imlSegment, sint32 segmentIndex,
								  bool printLivenessRangeInfo)
{
	StringBuf strOutput(1024);

	strOutput.addFmt("SEGMENT 0x{:04x} 0x{:08x} PPC 0x{:08x} - 0x{:08x} Loop-depth {}",
					 segmentIndex, imlSegment->ppcAddress, imlSegment->ppcAddrMin,
					 imlSegment->ppcAddrMax, imlSegment->loopDepth);
	...
	for (sint32 i = 0; i < imlSegment->imlListCount; i++)
	{
		...
		if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_NO_OP &&
			imlSegment->imlList[i].associatedPPCAddress == MPTR_NULL)
			continue;
		strOutput.reset();
		strOutput.addFmt("{:08x} ", imlSegment->imlList[i].associatedPPCAddress);
		if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_R_NAME ||
			imlSegment->imlList[i].type == PPCREC_IML_TYPE_NAME_R)
		{
			if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_R_NAME)
			...
			strOutput.addFmt("name_{} (", imlSegment->imlList[i].op_r_name.registerIndex,
							 imlSegment->imlList[i].op_r_name.name);
			if (imlSegment->imlList[i].op_r_name.name >= PPCREC_NAME_R0 &&
				imlSegment->imlList[i].op_r_name.name < (PPCREC_NAME_R0 + 999))
			{
				strOutput.addFmt("r{}", imlSegment->imlList[i].op_r_name.name - PPCREC_NAME_R0);
			}
			else if (imlSegment->imlList[i].op_r_name.name >= PPCREC_NAME_SPR0 &&
					 imlSegment->imlList[i].op_r_name.name < (PPCREC_NAME_SPR0 + 999))
			{
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_R_R_R)
		{
			...
			PPCRecDebug_addRegisterParam(strOutput, imlSegment->imlList[i].op_r_r_r.registerB,
										 true);
			if (imlSegment->imlList[i].crRegister != PPC_REC_INVALID_REGISTER)
			{
				strOutput.addFmt(" -> CR{}", imlSegment->imlList[i].crRegister);
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_R_R_S32)
		{
			...
			PPCRecDebug_addRegisterParam(strOutput,
										 imlSegment->imlList[i].op_r_r_s32.registerResult);
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_R_S32)
		{
			...
			PPCRecDebug_addRegisterParam(strOutput,
										 imlSegment->imlList[i].op_r_immS32.registerIndex);
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_LOAD ||
				 imlSegment->imlList[i].type == PPCREC_IML_TYPE_STORE ||
				 imlSegment->imlList[i].type == PPCREC_IML_TYPE_LOAD_INDEXED ||
				 imlSegment->imlList[i].type == PPCREC_IML_TYPE_STORE_INDEXED)
		{
			if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_LOAD ||
				imlSegment->imlList[i].type == PPCREC_IML_TYPE_LOAD_INDEXED)
				strOutput.add("LD_");
			else
				strOutput.add("ST_");
			...
			PPCRecDebug_addRegisterParam(strOutput,
										 imlSegment->imlList[i].op_storeLoad.registerData);

			if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_LOAD_INDEXED ||
				imlSegment->imlList[i].type == PPCREC_IML_TYPE_STORE_INDEXED)
				strOutput.addFmt("[t{}+t{}]", imlSegment->imlList[i].op_storeLoad.registerMem,
								 imlSegment->imlList[i].op_storeLoad.registerMem2);
			else
				strOutput.addFmt("[t{}+{}]", imlSegment->imlList[i].op_storeLoad.registerMem,
								 imlSegment->imlList[i].op_storeLoad.immS32);
		}
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_MEM2MEM)
		{
			strOutput.addFmt("{} [t{}+{}] = [t{}+{}]", imlSegment->imlList[i].op_mem2mem.copyWidth,
							 imlSegment->imlList[i].op_mem2mem.dst.registerMem,
							 imlSegment->imlList[i].op_mem2mem.dst.immS32,
							 imlSegment->imlList[i].op_mem2mem.src.registerMem,
							 imlSegment->imlList[i].op_mem2mem.src.immS32);
		}
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_CJUMP)
		{
			...
			strOutput.addFmt(" jm_{:08x} (cr{})",
							 imlSegment->imlList[i].op_conditionalJump.jumpmarkAddress,
							 imlSegment->imlList[i].crRegister);
		}
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_MACRO)
		{
			...
			{
				strOutput.addFmt("MACRO BLR 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_BLRL)
			{
				strOutput.addFmt("MACRO BLRL 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_BCTR)
			{
				strOutput.addFmt("MACRO BCTR 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_BCTRL)
			{
				strOutput.addFmt("MACRO BCTRL 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_BL)
			{
				strOutput.addFmt("MACRO BL 0x{:08x} -> 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 imlSegment->imlList[i].op_macro.param2,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_B_FAR)
			{
				strOutput.addFmt("MACRO B_FAR 0x{:08x} -> 0x{:08x} cycles (depr): {}",
								 imlSegment->imlList[i].op_macro.param,
								 imlSegment->imlList[i].op_macro.param2,
								 (sint32)imlSegment->imlList[i].op_macro.paramU16);
			}
			...
			{
				strOutput.addFmt("MACRO LEAVE ppc: 0x{:08x}",
								 imlSegment->imlList[i].op_macro.param);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_HLE)
			{
				strOutput.addFmt("MACRO HLE ppcAddr: 0x{:08x} funcId: 0x{:08x}",
								 imlSegment->imlList[i].op_macro.param,
								 imlSegment->imlList[i].op_macro.param2);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_MFTB)
			{
				strOutput.addFmt("MACRO MFTB ppcAddr: 0x{:08x} sprId: 0x{:08x}",
								 imlSegment->imlList[i].op_macro.param,
								 imlSegment->imlList[i].op_macro.param2);
			}
			else if (imlSegment->imlList[i].operation == PPCREC_IML_MACRO_COUNT_CYCLES)
			{
				strOutput.addFmt("MACRO COUNT_CYCLES cycles: {}",
								 imlSegment->imlList[i].op_macro.param);
			}
			else
			{
		...
		{
			strOutput.addFmt("fpr_t{} = name_{} (", imlSegment->imlList[i].op_r_name.registerIndex,
							 imlSegment->imlList[i].op_r_name.name);
			if (imlSegment->imlList[i].op_r_name.name >= PPCREC_NAME_FPR0 &&
				imlSegment->imlList[i].op_r_name.name < (PPCREC_NAME_FPR0 + 999))
			{
				strOutput.addFmt("fpr{}", imlSegment->imlList[i].op_r_name.name - PPCREC_NAME_FPR0);
			}
			else if (imlSegment->imlList[i].op_r_name.name >= PPCREC_NAME_TEMPORARY_FPR0 &&
					 imlSegment->imlList[i].op_r_name.name < (PPCREC_NAME_TEMPORARY_FPR0 + 999))
			{
				strOutput.addFmt("tempFpr{}", imlSegment->imlList[i].op_r_name.name -
												  PPCREC_NAME_TEMPORARY_FPR0);
			}
			else
				strOutput.add("ukn");
			strOutput.add(")");
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_FPR_LOAD)
		{
			...
			if (imlSegment->imlList[i].op_storeLoad.flags2.signExtend)
				...
				strOutput.add("U");
			strOutput.addFmt("{} [t{}+{}] mode {}",
							 imlSegment->imlList[i].op_storeLoad.copyWidth / 8,
							 imlSegment->imlList[i].op_storeLoad.registerMem,
							 imlSegment->imlList[i].op_storeLoad.immS32,
							 imlSegment->imlList[i].op_storeLoad.mode);
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_FPR_STORE)
		{
			if (imlSegment->imlList[i].op_storeLoad.flags2.signExtend)
				...
				strOutput.add("U");
			strOutput.addFmt("{} [t{}+{}]", imlSegment->imlList[i].op_storeLoad.copyWidth / 8,
							 imlSegment->imlList[i].op_storeLoad.registerMem,
							 imlSegment->imlList[i].op_storeLoad.immS32);
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_FPR_R_R)
		{
			...
			strOutput.addFmt("fpr{:02d}, fpr{:02d}",
							 imlSegment->imlList[i].op_fpr_r_r.registerResult,
							 imlSegment->imlList[i].op_fpr_r_r.registerOperand);
		}
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_FPR_R_R_R_R)
		{
			strOutput.addFmt("{:-6} ", PPCRecompiler_getOpcodeDebugName(&imlSegment->imlList[i]));
			strOutput.addFmt("fpr{:02d}, fpr{:02d}, fpr{:02d}, fpr{:02d}",
							 imlSegment->imlList[i].op_fpr_r_r_r_r.registerResult,
							 imlSegment->imlList[i].op_fpr_r_r_r_r.registerOperandA,
							 imlSegment->imlList[i].op_fpr_r_r_r_r.registerOperandB,
							 imlSegment->imlList[i].op_fpr_r_r_r_r.registerOperandC);
		}
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_FPR_R_R_R)
		{
			strOutput.addFmt("{:-6} ", PPCRecompiler_getOpcodeDebugName(&imlSegment->imlList[i]));
			strOutput.addFmt("fpr{:02d}, fpr{:02d}, fpr{:02d}",
							 imlSegment->imlList[i].op_fpr_r_r_r.registerResult,
							 imlSegment->imlList[i].op_fpr_r_r_r.registerOperandA,
							 imlSegment->imlList[i].op_fpr_r_r_r.registerOperandB);
		}
		...
		else if (imlSegment->imlList[i].type == PPCREC_IML_TYPE_CONDITIONAL_R_S32)
		{
			...
			if (imlSegment->imlList[i].operation == PPCREC_IML_OP_ASSIGN)
			{
				...
				strOutput.addFmt("(unknown operation CONDITIONAL_R_S32 {})",
								 imlSegment->imlList[i].operation);
			if (displayAsHex)
...
void PPCRecompilerIml_setSegmentPoint(ppcRecompilerSegmentPoint_t* segmentPoint,
									  PPCRecImlSegment_t* imlSegment, sint32 index)
{
 ...
 */
void PPCRecompiler_pushBackIMLInstructions(PPCRecImlSegment_t* imlSegment, sint32 index,
										   sint32 shiftBackCount)
{
	...
	{
		sint32 newSize =
			imlSegment->imlListCount + shiftBackCount + std::max(2, imlSegment->imlListSize / 2);
		imlSegment->imlList = (PPCRecImlInstruction_t*)realloc(
			imlSegment->imlList, sizeof(PPCRecImlInstruction_t) * newSize);
		imlSegment->imlListSize = newSize;
	}
	for (sint32 i = (sint32)imlSegment->imlListCount - 1; i >= index; i--)
	{
		memcpy(imlSegment->imlList + (i + shiftBackCount), imlSegment->imlList + i,
			   sizeof(PPCRecImlInstruction_t));
	}
	...
	if (imlSegment->segmentPointList)
	{
		...
		while (segmentPoint)
		{
			if (segmentPoint->index != RA_INTER_RANGE_START &&
				segmentPoint->index != RA_INTER_RANGE_END)
			{
 ...
 * Warning: Can invalidate any previous instruction structs from the same segment
 */
PPCRecImlInstruction_t* PPCRecompiler_insertInstruction(PPCRecImlSegment_t* imlSegment,
														sint32 index)
{
...
void PPCRecompilerIml_insertSegments(ppcImlGenContext_t* ppcImlGenContext, sint32 index,
									 sint32 count)
{
	if ((ppcImlGenContext->segmentListCount + count) > ppcImlGenContext->segmentListSize)
	{
		...
		ppcImlGenContext->segmentList = (PPCRecImlSegment_t**)realloc(
			ppcImlGenContext->segmentList,
			ppcImlGenContext->segmentListSize * sizeof(PPCRecImlSegment_t*));
	}
	for (sint32 i = (sint32)ppcImlGenContext->segmentListCount - 1; i >= index; i--)
	{
		memcpy(ppcImlGenContext->segmentList + (i + count), ppcImlGenContext->segmentList + i,
			   sizeof(PPCRecImlSegment_t*));
	}
	ppcImlGenContext->segmentListCount += count;
	for (sint32 i = 0; i < count; i++)
	{
		// memset(ppcImlGenContext->segmentList+index+i, 0x00, sizeof(PPCRecImlSegment_t*));
		ppcImlGenContext->segmentList[index + i] =
			(PPCRecImlSegment_t*)malloc(sizeof(PPCRecImlSegment_t));
		memset(ppcImlGenContext->segmentList[index + i], 0x00, sizeof(PPCRecImlSegment_t));
		ppcImlGenContext->segmentList[index + i]->list_prevSegments =
			std::vector<PPCRecImlSegment_t*>();
	}
}
...
{
	if (iml->type == PPCREC_IML_TYPE_MACRO &&
			(iml->operation == PPCREC_IML_MACRO_BLR || iml->operation == PPCREC_IML_MACRO_BCTR) ||
		...
		iml->type == PPCREC_IML_TYPE_PPC_ENTER || iml->type == PPCREC_IML_TYPE_CJUMP ||
...
bool PPCRecompiler_generateIntermediateCode(ppcImlGenContext_t& ppcImlGenContext,
											...
											std::set<uint32>& entryAddresses)
{
	...
	ppcImlGenContext.currentInstruction =
		(uint32*)memory_getPointerFromVirtualOffset(ppcRecFunc->ppcAddress);
	bool unsupportedInstructionFound = false;
	...
	{
		uint32 addressOfCurrentInstruction =
			(uint32)((uint8*)ppcImlGenContext.currentInstruction - memory_base);
		ppcImlGenContext.ppcAddressOfCurrentInstruction = addressOfCurrentInstruction;
		ppcImlGenContext.cyclesSinceLastBranch++;
		PPCRecompilerImlGen_generateNewInstruction_jumpmark(&ppcImlGenContext,
		...
		if (entryAddresses.find(addressOfCurrentInstruction) != entryAddresses.end())
		{
			...
			PPCRecompilerImlGen_generateNewInstruction_ppcEnter(&ppcImlGenContext,
																addressOfCurrentInstruction);
		}
		else if (ppcImlGenContext.currentInstruction != firstCurrentInstruction)
		{
			// add PPCEnter mark if code is seemingly unreachable (for example if between two
			// unconditional jump instructions without jump goal)
			...
			if (((opcodePrevious >> 26) == 18) && ((opcodeCurrent >> 26) == 18))
			{
				...
				if ((opcodePrevious & PPC_OPC_LK) && (opcodePrevious & PPC_OPC_AA) == 0)
				{
					...
					sint32 inlineSize = 0;
					if (PPCRecompiler_canInlineFunction(li + addressOfCurrentInstruction - 4,
														&inlineSize))
						canInlineFunction = true;
				}
				if (canInlineFunction == false && (opcodePrevious & PPC_OPC_LK) == false)
					PPCRecompilerImlGen_generateNewInstruction_ppcEnter(
						&ppcImlGenContext, addressOfCurrentInstruction);
			}
	...
	ppcImlGenContext.ppcAddressOfCurrentInstruction =
		0; // reset current instruction offset (any future generated IML instruction will be
		   // assigned to ppc address 0)
...
#ifndef PUBLIC_RELEASE
			if (map_jumpMarks.find(ppcImlGenContext.imlList[i].op_jumpmark.address) !=
				map_jumpMarks.end())
				assert_dbg();
#endif
			map_jumpMarks.emplace(ppcImlGenContext.imlList[i].op_jumpmark.address,
								  ppcImlGenContext.imlList + i);
		}
	}
	...
	for (sint32 i = 0; i < ppcImlGenContext.imlListCount; i++)
	{
		...
		{
			memcpy(ppcImlGenContext.imlList + currentImlIndex, ppcImlGenContext.imlList + i,
				   sizeof(PPCRecImlInstruction_t));
		}
		currentImlIndex++;
	}
	...
	// each segment is defined by one or more instructions with no branches or jump destinations in
	// between a branch instruction may only be the very last instruction of a segment
	ppcImlGenContext.segmentListCount = 0;
	ppcImlGenContext.segmentListSize = 2;
	ppcImlGenContext.segmentList = (PPCRecImlSegment_t**)malloc(ppcImlGenContext.segmentListSize *
																sizeof(PPCRecImlSegment_t*));
	...
	while (segmentImlIndex < ppcImlGenContext.imlListCount)
	{
		...
		if (ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_CJUMP ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_BLR ||
			  ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_BLRL ||
			  ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_BCTR ||
			  ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_BCTRL)) ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_BL)) ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_B_FAR)) ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_LEAVE)) ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_HLE)) ||
			(ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_MACRO &&
			 (ppcImlGenContext.imlList[segmentImlIndex].operation == PPCREC_IML_MACRO_MFTB)))
		{
				 ...
				 ppcImlGenContext.imlList[segmentImlIndex].type == PPCREC_IML_TYPE_PPC_ENTER)
		{
			...
			{
				PPCRecImlSegment_t* ppcRecSegment =
					PPCRecompiler_generateImlSegment(&ppcImlGenContext);
				ppcRecSegment->startOffset = segmentStart;
	...
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		...
		if (imlCount > 0)
		{
			...
			ppcImlGenContext.segmentList[s]->imlList = (PPCRecImlInstruction_t*)malloc(
				sizeof(PPCRecImlInstruction_t) * ppcImlGenContext.segmentList[s]->imlListSize);
			ppcImlGenContext.segmentList[s]->imlListCount = imlCount;
			memcpy(ppcImlGenContext.segmentList[s]->imlList,
				   ppcImlGenContext.imlList + imlStartIndex,
				   sizeof(PPCRecImlInstruction_t) * imlCount);
		}
		else
		{
			// empty segments are allowed so we can handle multiple PPC entry addresses pointing to
			// the same code
	...
	ppcImlGenContext.imlListCount = 999999; // set to high number to force crash in case old code
											// still uses ppcImlGenContext.imlList
	// calculate PPC address of each segment based on iml instructions inside that segment (we need
	// this info to calculate how many cpu cycles each segment takes)
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		...
		for (sint32 i = 0; i < ppcImlGenContext.segmentList[s]->imlListCount; i++)
		{
			if (ppcImlGenContext.segmentList[s]->imlList[i].associatedPPCAddress == 0)
				continue;
			// if( ppcImlGenContext.segmentList[s]->imlList[i].type == PPCREC_IML_TYPE_JUMPMARK ||
			// ppcImlGenContext.segmentList[s]->imlList[i].type == PPCREC_IML_TYPE_NO_OP ) continue;
			//// jumpmarks and no-op instructions must not affect segment ppc address range
			segmentPPCAddrMin =
				std::min(ppcImlGenContext.segmentList[s]->imlList[i].associatedPPCAddress,
						 segmentPPCAddrMin);
			segmentPPCAddrMax =
				std::max(ppcImlGenContext.segmentList[s]->imlList[i].associatedPPCAddress,
						 segmentPPCAddrMax);
		}
	...
	// ppcEnter instruction marks a segment as enterable (BL, BCTR, etc. instructions can enter at
	// this location from outside) jumpmarks mark the segment as a jump destination (within the same
	// function)
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		while (ppcImlGenContext.segmentList[s]->imlListCount > 0)
		{
			if (ppcImlGenContext.segmentList[s]->imlList[0].type == PPCREC_IML_TYPE_PPC_ENTER)
			{
				...
				ppcImlGenContext.segmentList[s]->enterPPCAddress =
					ppcImlGenContext.segmentList[s]->imlList[0].op_ppcEnter.ppcAddress;
			...
			else if (ppcImlGenContext.segmentList[s]->imlList[0].type == PPCREC_IML_TYPE_JUMPMARK)
			{
				...
				ppcImlGenContext.segmentList[s]->jumpDestinationPPCAddress =
					ppcImlGenContext.segmentList[s]->imlList[0].op_jumpmark.address;
	...
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		...
		if (imlSegment->nextSegmentBranchNotTaken == NULL ||
			imlSegment->nextSegmentBranchTaken == NULL)
			continue; // not a branching segment
		PPCRecImlInstruction_t* lastInstruction = PPCRecompilerIML_getLastInstruction(imlSegment);
		if (lastInstruction->type != PPCREC_IML_TYPE_CJUMP ||
			lastInstruction->op_conditionalJump.crRegisterIndex != 0)
			continue;
		...
		if (imlSegment->nextSegmentBranchTaken !=
			imlSegment->nextSegmentBranchNotTaken->nextSegmentBranchNotTaken)
			continue;
		...
		if (conditionalSegment->isEnterable)
			continue;
		// check if the segment contains only iml instructions that can be turned into conditional
		// moves (Value assignment, register assignment)
		...
		for (sint32 f = 0; f < conditionalSegment->imlListCount; f++)
		{
			...
			if (imlInstruction->type == PPCREC_IML_TYPE_R_S32 &&
				...
				PPCRecompilerImlGen_generateNewInstruction_conditional_r_s32(
					&ppcImlGenContext, PPCRecompiler_appendInstruction(imlSegment),
					PPCREC_IML_OP_ASSIGN, imlInstruction->op_r_immS32.registerIndex,
					imlInstruction->op_r_immS32.immS32, branchCond_crRegisterIndex,
					branchCond_crBitIndex, !branchCond_bitMustBeSet);
			else
				assert_dbg();
		}
		// update segment links
		// source segment: imlSegment, conditional/removed segment: conditionalSegment, final
		...
		if (finalSegment->isEnterable == false && finalSegment->list_prevSegments.size() == 1)
		{
			...
			{
				memcpy(PPCRecompiler_appendInstruction(imlSegment), finalSegment->imlList + f,
					   sizeof(PPCRecImlInstruction_t));
			}
		...
		// todo: If possible, merge with the segment following conditionalSegment (merging is only
		// possible if the segment is not an entry point or has no other jump sources)
	}
	...
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		...
		// note: This algorithm correctly counts inlined functions but it doesn't count NO-OP
		// instructions like ISYNC
		...
		// uint32 ppcCount = imlSegment->ppcAddrMax-imlSegment->ppcAddrMin+4; -> No longer works
	...
	// find segments that have a (conditional) jump instruction that points in reverse direction of
	// code flow for these segments there is a risk that the recompiler could get trapped in an
	// infinite busy loop. todo: We should do a loop-detection prepass where we flag segments that
	// are actually in a loop. We can then use this information below to avoid generating the
	// scheduler-exit code for segments that aren't actually in a loop despite them referencing an
	// earlier segment (which could be an exit segment for example)
	uint32 currentLoopEscapeJumpMarker =
		0xFF000000; // start in an area where no valid code can be located
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		// todo: This currently uses segment->ppcAddrMin which isn't really reliable. (We already
		// had a problem where function inlining would generate falsified segment ranges by omitting
		// the branch instruction). Find a better solution (use jumpmark/enterable offsets?)
		PPCRecImlSegment_t* imlSegment = ppcImlGenContext.segmentList[s];
		if (imlSegment->imlListCount == 0)
			continue;
		if (imlSegment->imlList[imlSegment->imlListCount - 1].type != PPCREC_IML_TYPE_CJUMP ||
			imlSegment->imlList[imlSegment->imlListCount - 1].op_conditionalJump.jumpmarkAddress >
				imlSegment->ppcAddrMin)
			continue;
		if (imlSegment->imlList[imlSegment->imlListCount - 1].type != PPCREC_IML_TYPE_CJUMP ||
			imlSegment->imlList[imlSegment->imlListCount - 1]
				.op_conditionalJump.jumpAccordingToSegment)
			continue;
		...
		// P0: This segment checks if the remaining cycles counter is still above zero. If yes, it
		// jumps to segment P2 (it's also the jump destination for other segments) P1: This segment
		// consists only of a single ppc_leave instruction and is usually skipped. Register unload
		// instructions are later inserted here. P2: This segment contains the iml instructions of
		// the original segment PEntry: This segment is used to enter the function, it jumps to P0
		// All segments are considered to be part of the same PPC instruction range
		// The first segment also retains the jump destination and enterable properties from the
		// original segment.
		...
		PPCRecImlSegment_t* imlSegmentPEntry =
			ppcImlGenContext.segmentList[ppcImlGenContext.segmentListCount - 1];
		// relink segments
		...
		imlSegmentP0->imlList[0].op_conditionalJump.jumpmarkAddress =
			imlSegmentP2->jumpDestinationPPCAddress;
		...
		PPCRecompilerImlGen_generateNewInstruction_jumpSegment(&ppcImlGenContext,
															   imlSegmentPEntry->imlList + 0);

		// skip the newly created segments
		s += 2;
	}

	// isolate entry points from function flow (enterable segments must not be the target of any
	// other segment) this simplifies logic during register allocation
	...
	for (sint32 s = 0; s < ppcImlGenContext.segmentListCount; s++)
	{
		...
		for (sint32 i = 0; i < imlSegment->imlListCount; i++)
		{
			PPCRecompiler_checkRegisterUsage(&ppcImlGenContext, imlSegment->imlList + i,
											 &registersUsed);
			// PPCRecompilerImlGen_findRegisterByMappedName(ppcImlGenContext,
			// registersUsed.readGPR1);
