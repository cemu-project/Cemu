...
sint32 x64Gen_registerMap[12] = // virtual GPR to x64 register mapping
	{REG_RAX, REG_RDX, REG_RBX, REG_RBP, REG_RSI, REG_RDI,
	 REG_R8,  REG_R9,  REG_R10, REG_R11, REG_R12, REG_RCX};
 ...
 */
void PPCRecompilerX64Gen_rememberRelocatableOffset(x64GenContext_t* x64GenContext, uint8 type,
												   void* extraInfo = nullptr)
{
	if (x64GenContext->relocateOffsetTableCount >= x64GenContext->relocateOffsetTableSize)
	{
		x64GenContext->relocateOffsetTableSize =
			std::max(4, x64GenContext->relocateOffsetTableSize * 2);
		x64GenContext->relocateOffsetTable = (x64RelocEntry_t*)realloc(
			x64GenContext->relocateOffsetTable,
			sizeof(x64RelocEntry_t) * x64GenContext->relocateOffsetTableSize);
	}
	x64GenContext->relocateOffsetTable[x64GenContext->relocateOffsetTableCount].offset =
		x64GenContext->codeBufferIndex;
	x64GenContext->relocateOffsetTable[x64GenContext->relocateOffsetTableCount].type = type;
	x64GenContext->relocateOffsetTable[x64GenContext->relocateOffsetTableCount].extraInfo =
		extraInfo;
 ...
 * Should be called before each x64 instruction which overwrites the current status flags (with
 * mappedCRRegister set to PPCREC_CR_TEMPORARY unless explicitly set by PPC instruction)
 */
void PPCRecompilerX64Gen_crConditionFlags_set(PPCRecFunction_t* PPCRecFunction,
											  ppcImlGenContext_t* ppcImlGenContext,
											  x64GenContext_t* x64GenContext,
											  sint32 mappedCRRegister, sint32 crState)
{
 ...
 */
void PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction_t* PPCRecFunction,
												 ppcImlGenContext_t* ppcImlGenContext,
												 x64GenContext_t* x64GenContext)
{
...
void PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext_t* x64GenContext,
											  sint32 jumpInstructionOffset,
											  sint32 destinationOffset)
{
...
void PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction_t* PPCRecFunction,
										 ppcImlGenContext_t* ppcImlGenContext,
										 x64GenContext_t* x64GenContext,
										 PPCRecImlInstruction_t* imlInstruction)
{
	...
	if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_LT))) == 0)
		x64Gen_setcc_mem8(
			x64GenContext, X86_CONDITION_SIGN, REG_RSP,
			offsetof(PPCInterpreter_t, cr) +
				sizeof(uint8) *
					(crRegister * 4 + PPCREC_CR_BIT_LT)); // check for sign instead of _BELOW (CF)
														  // which is not set by TEST
	if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_GT))) == 0)
		x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_SIGNED_GREATER, REG_RSP,
						  offsetof(PPCInterpreter_t, cr) +
							  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_GT));
	if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_EQ))) == 0)
		x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_EQUAL, REG_RSP,
						  offsetof(PPCInterpreter_t, cr) +
							  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_EQ));
	// todo: Set CR SO if XER SO bit is set
	PPCRecompilerX64Gen_crConditionFlags_set(PPCRecFunction, ppcImlGenContext, x64GenContext,
											 crRegister, PPCREC_CR_STATE_TYPE_LOGICAL);
}
...
bool PPCRecompilerX64Gen_imlInstruction_macro(PPCRecFunction_t* PPCRecFunction,
											  ppcImlGenContext_t* ppcImlGenContext,
											  x64GenContext_t* x64GenContext,
											  PPCRecImlInstruction_t* imlInstruction)
{
	PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext, x64GenContext);
	if (imlInstruction->operation == PPCREC_IML_MACRO_BLR ||
		imlInstruction->operation == PPCREC_IML_MACRO_BLRL)
	{
		uint32 currentInstructionAddress = imlInstruction->op_macro.param;
		// MOV EDX, [SPR_LR]
		x64Emit_mov_reg64_mem32(x64GenContext, REG_RDX, REG_RSP,
								offsetof(PPCInterpreter_t, spr.LR));
		// if BLRL, then update SPR LR
		if (imlInstruction->operation == PPCREC_IML_MACRO_BLRL)
			x64Gen_mov_mem32Reg64_imm32(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, spr.LR),
		...
		x64Gen_writeU32(x64GenContext, (uint32)offsetof(PPCRecompilerInstanceData_t,
														ppcRecompilerDirectJumpTable));
		return true;
	}
	else if (imlInstruction->operation == PPCREC_IML_MACRO_BCTR ||
			 imlInstruction->operation == PPCREC_IML_MACRO_BCTRL)
	{
		uint32 currentInstructionAddress = imlInstruction->op_macro.param;
		// MOV EDX, [SPR_CTR]
		x64Emit_mov_reg64_mem32(x64GenContext, REG_RDX, REG_RSP,
								offsetof(PPCInterpreter_t, spr.CTR));
	...
	else if (imlInstruction->operation == PPCREC_IML_MACRO_BL)
	{
		...
		// since RDX is constant we can use JMP [R15+const_offset] if jumpTableOffset+RDX*2 does not
		// exceed the 2GB boundary
		uint64 lookupOffset =
			(uint64)offsetof(PPCRecompilerInstanceData_t, ppcRecompilerDirectJumpTable) +
			(uint64)newIP * 2ULL;
	...
	else if (imlInstruction->operation == PPCREC_IML_MACRO_LEAVE)
	{
		...
		uint64 lookupOffset =
			(uint64) & (((PPCRecompilerInstanceData_t*)NULL)->ppcRecompilerDirectJumpTable) +
						   (uint64)newIP * 2ULL;
		// JMP [R15+offset]
		x64Gen_writeU8(x64GenContext, 0x41);
		x64Gen_writeU8(x64GenContext, 0xFF);
	...
	else if (imlInstruction->operation == PPCREC_IML_MACRO_COUNT_CYCLES)
	{
		...
		x64Gen_sub_mem32reg64_imm32(x64GenContext, REG_RSP,
									offsetof(PPCInterpreter_t, remainingCycles), cycleCount);
		return true;
	}
	else if (imlInstruction->operation == PPCREC_IML_MACRO_HLE)
	{
		...
		x64Gen_mov_mem32Reg64_imm32(x64GenContext, REG_RSP,
									offsetof(PPCInterpreter_t, instructionPointer), ppcAddress);
		//// save hCPU (RSP)
		...
		x64Emit_mov_reg64_mem64(x64GenContext, REG_RSP, REG_RESV_HCPU,
								offsetof(PPCInterpreter_t, rspTemp));
		// x64Emit_mov_reg64_mem64(x64GenContext, REG_RSP, REG_R14, 0);
		// x64Gen_int3(x64GenContext);
		//  reserve space on stack for call parameters
		x64Gen_sub_reg64_imm32(
			x64GenContext, REG_RSP,
			8 * 11); // must be uneven number in order to retain stack 0x10 alignment
		...
		x64Gen_bt_mem8(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, remainingCycles),
					   31); // check if negative
		...
		x64Emit_mov_reg64_mem32(x64GenContext, REG_RDX, REG_RSP,
								offsetof(PPCInterpreter_t, instructionPointer));
		// set EAX to 0 (we assume that ppcRecompilerDirectJumpTable[0] will be a recompiler escape
		// function)
		...
		// x64Gen_int3(x64GenContext);
		x64Gen_jmp_memReg64(
			x64GenContext, REG_RAX,
			(uint32)offsetof(PPCRecompilerInstanceData_t, ppcRecompilerDirectJumpTable));
		PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionOffset1,
												 x64GenContext->codeBufferIndex);
		// check if instruction pointer was changed
		// assign new instruction pointer to EAX
		x64Emit_mov_reg64_mem32(x64GenContext, REG_RAX, REG_RSP,
								offsetof(PPCInterpreter_t, instructionPointer));
 ...
 * Load from memory
 */
bool PPCRecompilerX64Gen_imlInstruction_load(PPCRecFunction_t* PPCRecFunction,
											 ppcImlGenContext_t* ppcImlGenContext,
											 x64GenContext_t* x64GenContext,
											 PPCRecImlInstruction_t* imlInstruction, bool indexed)
{
	...
	else
	{
		...
		{
			// for indexed memory access realRegisterData must not be the same register as the
			// second memory register, this can easily be fixed by swapping the logic of
			// realRegisterMem and realRegisterMem2
		...
		if (imlInstruction->op_storeLoad.copyWidth == 32)
		{
			...
			if (indexed)
			{
				x64Gen_lea_reg64Low32_reg64Low32PlusReg64Low32(x64GenContext, REG_RESV_TEMP,
															   realRegisterMem, realRegisterMem2);
			}
			if (hasMOVBESupport && switchEndian)
			{
				if (indexed)
				{
					x64Gen_movBEZeroExtend_reg64_mem32Reg64PlusReg64(
						x64GenContext, realRegisterData, REG_R13, REG_RESV_TEMP,
						imlInstruction->op_storeLoad.immS32);
					// if (indexed && realRegisterMem != realRegisterData)
					//	x64Gen_sub_reg64Low32_reg64Low32(x64GenContext, realRegisterMem,
					// realRegisterMem2);
				}
				else
				{
			...
			else
			{
				if (indexed)
				{
					x64Emit_mov_reg32_mem32(x64GenContext, realRegisterData, REG_R13, REG_RESV_TEMP,
											imlInstruction->op_storeLoad.immS32);
				...
				{
					x64Emit_mov_reg32_mem32(x64GenContext, realRegisterData, REG_R13,
		...
		else if (imlInstruction->op_storeLoad.copyWidth == 16)
		{
			PPCRecompilerX64Gen_crConditionFlags_forget(
				...
				x64GenContext); // todo: We can avoid this if MOVBE is available
			if (indexed)
			{
			...
			if (signExtend)
				x64Gen_movSignExtend_reg64Low32_reg64Low16(x64GenContext, realRegisterData,
														   realRegisterData);
			else
				x64Gen_movZeroExtend_reg64Low32_reg64Low16(x64GenContext, realRegisterData,
														   realRegisterData);
		}
		else if (imlInstruction->op_storeLoad.copyWidth == 8)
		{
			...
			if (signExtend)
				x64Gen_movSignExtend_reg64Low32_mem8Reg64PlusReg64(
					x64GenContext, realRegisterData, REG_R13, realRegisterMem,
					imlInstruction->op_storeLoad.immS32);
			else
				x64Emit_movZX_reg32_mem8(x64GenContext, realRegisterData, REG_R13, realRegisterMem,
										 imlInstruction->op_storeLoad.immS32);
			if (indexed && realRegisterMem != realRegisterData)
		...
		else if (imlInstruction->op_storeLoad.copyWidth == PPC_REC_LOAD_LWARX_MARKER)
		{
			...
			if (indexed)
				x64Gen_add_reg64Low32_reg64Low32(x64GenContext, realRegisterMem, realRegisterMem2);
			x64Emit_mov_mem32_reg32(x64GenContext, REG_RSP,
									(uint32)offsetof(PPCInterpreter_t, reservedMemAddr),
									realRegisterMem); // remember EA for reservation
			...
			if (switchEndian)
				x64Gen_bswap_reg64Lower32bit(x64GenContext, realRegisterData);
			x64Emit_mov_mem32_reg32(x64GenContext, REG_RSP,
									(uint32)offsetof(PPCInterpreter_t, reservedMemValue),
									realRegisterData); // remember value for reservation
			// LWARX instruction costs extra cycles (this speeds up busy loops)
			x64Gen_sub_mem32reg64_imm32(x64GenContext, REG_RSP,
										offsetof(PPCInterpreter_t, remainingCycles), 20);
		}
		else if (imlInstruction->op_storeLoad.copyWidth == PPC_REC_STORE_LSWI_3)
		{
			...
			if (indexed)
				x64Gen_add_reg64Low32_reg64Low32(
					x64GenContext, realRegisterMem,
					realRegisterMem2); // can be replaced with LEA temp, [memReg1+memReg2] (this way
									   // we can avoid the SUB instruction after the move)
			if (hasMOVBESupport)
			{
				x64Gen_movBEZeroExtend_reg64_mem32Reg64PlusReg64(
					x64GenContext, realRegisterData, REG_R13, realRegisterMem,
 ...
 * Write to memory
 */
bool PPCRecompilerX64Gen_imlInstruction_store(PPCRecFunction_t* PPCRecFunction,
											  ppcImlGenContext_t* ppcImlGenContext,
											  x64GenContext_t* x64GenContext,
											  PPCRecImlInstruction_t* imlInstruction, bool indexed)
{
	...
	else
	{
		...
		if (imlInstruction->op_storeLoad.copyWidth == 32)
		{
			...
			if (hasMOVBESupport && swapEndian)
				x64Gen_movBETruncate_mem32Reg64PlusReg64_reg64(
					x64GenContext, REG_R13, realRegisterMem, imlInstruction->op_storeLoad.immS32,
					valueRegister);
			else
		...
		else if (imlInstruction->op_storeLoad.copyWidth == 8)
		{
			...
			if (indexed)
				x64Gen_add_reg64Low32_reg64Low32(x64GenContext, realRegisterMem, realRegisterMem2);
			x64Gen_movTruncate_mem8Reg64PlusReg64_reg64(
				x64GenContext, REG_RESV_MEMBASE, realRegisterMem,
				imlInstruction->op_storeLoad.immS32, realRegisterData);
			if (indexed)
				x64Gen_sub_reg64Low32_reg64Low32(x64GenContext, realRegisterMem, realRegisterMem2);
		}
		else if (imlInstruction->op_storeLoad.copyWidth == PPC_REC_STORE_STWCX_MARKER)
		{
			...
			sint32 crRegister = 0;
			x64Gen_mov_mem8Reg64_imm8(x64GenContext, REG_RSP,
									  offsetof(PPCInterpreter_t, cr) +
										  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_LT),
									  0);
			x64Gen_mov_mem8Reg64_imm8(x64GenContext, REG_RSP,
									  offsetof(PPCInterpreter_t, cr) +
										  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_GT),
			...
			x64Gen_cmp_reg64Low32_mem32reg64(x64GenContext, realRegisterMem, REG_RESV_HCPU,
											 offsetof(PPCInterpreter_t, reservedMemAddr));
			...
			x64Emit_mov_mem32_reg32(x64GenContext, REG_RSP,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[0]), REG_EAX);
			// backup REG_RESV_MEMBASE
			x64Emit_mov_mem64_reg64(x64GenContext, REG_RESV_HCPU,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[2]),
									REG_RESV_MEMBASE);
			...
			x64Emit_mov_reg64_mem32(x64GenContext, REG_EAX, REG_RESV_HCPU,
									offsetof(PPCInterpreter_t, reservedMemValue));
			// bswap EAX
			...
			// realRegisterMem, 0, REG_RESV_TEMP);
			...
			// reset reservation
			x64Gen_mov_mem32Reg64_imm32(x64GenContext, REG_RESV_HCPU,
										(uint32)offsetof(PPCInterpreter_t, reservedMemAddr), 0);
			x64Gen_mov_mem32Reg64_imm32(x64GenContext, REG_RESV_HCPU,
										(uint32)offsetof(PPCInterpreter_t, reservedMemValue), 0);

			// restore EAX
			x64Emit_mov_reg64_mem32(x64GenContext, REG_EAX, REG_RESV_HCPU,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[0]));
			// restore REG_RESV_MEMBASE
			x64Emit_mov_reg64_mem64(x64GenContext, REG_RESV_MEMBASE, REG_RESV_HCPU,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[2]));

			// copy XER SO to CR0 SO
			x64Gen_bt_mem8(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, spr.XER), 31);
			x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_CARRY, REG_RESV_HCPU,
							  offsetof(PPCInterpreter_t, cr) +
								  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_SO));
			// end
			PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionOffsetJumpToEnd,
													 x64GenContext->codeBufferIndex);
		}
		...
		else if (imlInstruction->op_storeLoad.copyWidth == PPC_REC_STORE_STSWI_3)
		{
			...
			x64Gen_movTruncate_mem8Reg64PlusReg64_reg64(x64GenContext, REG_R13, realRegisterMem,
														imlInstruction->op_storeLoad.immS32 + 2,
														REG_RESV_TEMP);
			x64Gen_shr_reg64Low32_imm8(x64GenContext, REG_RESV_TEMP, 8);
			x64Gen_movTruncate_mem8Reg64PlusReg64_reg64(x64GenContext, REG_R13, realRegisterMem,
														imlInstruction->op_storeLoad.immS32 + 1,
														REG_RESV_TEMP);
			x64Gen_shr_reg64Low32_imm8(x64GenContext, REG_RESV_TEMP, 8);
			x64Gen_movTruncate_mem8Reg64PlusReg64_reg64(x64GenContext, REG_R13, realRegisterMem,
 ...
 */
void PPCRecompilerX64Gen_imlInstruction_mem2mem(PPCRecFunction_t* PPCRecFunction,
												ppcImlGenContext_t* ppcImlGenContext,
												x64GenContext_t* x64GenContext,
												PPCRecImlInstruction_t* imlInstruction)
{
	...
	// PPCRecompilerX64Gen_crConditionFlags_forget() is not needed here, since MOVs don't affect
	// eflags
	if (imlInstruction->op_mem2mem.copyWidth == 32)
	{
		x64Emit_mov_reg32_mem32(x64GenContext, REG_RESV_TEMP, REG_R13, realSrcMemReg,
								realSrcMemImm);
		x64Gen_movTruncate_mem32Reg64PlusReg64_reg64(x64GenContext, REG_R13, realDstMemReg,
													 realDstMemImm, REG_RESV_TEMP);
	}
	else
	{
		assert_dbg();
...
bool PPCRecompilerX64Gen_imlInstruction_r_r(PPCRecFunction_t* PPCRecFunction,
											ppcImlGenContext_t* ppcImlGenContext,
											x64GenContext_t* x64GenContext,
											PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction->operation == PPCREC_IML_OP_ASSIGN)
	{
		...
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			x64Gen_mov_reg64_reg64(x64GenContext,
								   tempToRealRegister(imlInstruction->op_r_r.registerResult),
								   tempToRealRegister(imlInstruction->op_r_r.registerA));
			if (imlInstruction->crMode == PPCREC_CR_MODE_LOGICAL)
			{
				// since MOV doesn't set eflags we need another test instruction
				x64Gen_test_reg64Low32_reg64Low32(
					x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
					tempToRealRegister(imlInstruction->op_r_r.registerResult));
				// set cr bits
				PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction, ppcImlGenContext, x64GenContext,
													imlInstruction);
			}
			else
			{
		...
		{
			x64Gen_mov_reg64Low32_reg64Low32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
				tempToRealRegister(imlInstruction->op_r_r.registerA));
		}
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_ENDIAN_SWAP)
	{
		...
		if (imlInstruction->op_r_r.registerA != imlInstruction->op_r_r.registerResult)
			assert_dbg();
		x64Gen_bswap_reg64Lower32bit(x64GenContext,
									 tempToRealRegister(imlInstruction->op_r_r.registerResult));
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_ADD)
	{
		...
		x64Gen_add_reg64Low32_reg64Low32(x64GenContext,
										 tempToRealRegister(imlInstruction->op_r_r.registerResult),
										 tempToRealRegister(imlInstruction->op_r_r.registerA));
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_ASSIGN_S8_TO_S32)
	{
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		x64Gen_movSignExtend_reg64Low32_reg64Low8(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
			tempToRealRegister(imlInstruction->op_r_r.registerA));
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			 ...
			 imlInstruction->operation == PPCREC_IML_OP_XOR)
	{
		...
		{
			// registerResult |= registerA
			x64Gen_or_reg64Low32_reg64Low32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
				tempToRealRegister(imlInstruction->op_r_r.registerA));
		}
		...
		{
			// registerResult &= registerA
			x64Gen_and_reg64Low32_reg64Low32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
				tempToRealRegister(imlInstruction->op_r_r.registerA));
		}
		else
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_NOT)
	{
		...
		x64Gen_not_reg64Low32(x64GenContext,
							  tempToRealRegister(imlInstruction->op_r_r.registerResult));
		// update cr bits
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			// NOT instruction does not update flags, so we have to generate an additional TEST
			// instruction
			x64Gen_test_reg64Low32_reg64Low32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
				tempToRealRegister(imlInstruction->op_r_r.registerResult));
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_CNTLZW)
	{
		...
		{
			x64Gen_test_reg64Low32_reg64Low32(x64GenContext,
											  tempToRealRegister(imlInstruction->op_r_r.registerA),
											  tempToRealRegister(imlInstruction->op_r_r.registerA));
			sint32 jumpInstructionOffset1 = x64GenContext->codeBufferIndex;
			x64Gen_jmpc_near(x64GenContext, X86_CONDITION_EQUAL, 0);
			x64Gen_bsr_reg64Low32_reg64Low32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
				tempToRealRegister(imlInstruction->op_r_r.registerA));
			x64Gen_neg_reg64Low32(x64GenContext,
								  tempToRealRegister(imlInstruction->op_r_r.registerResult));
			x64Gen_add_reg64Low32_imm32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult), 32 - 1);
			sint32 jumpInstructionOffset2 = x64GenContext->codeBufferIndex;
			x64Gen_jmpc_near(x64GenContext, X86_CONDITION_NONE, 0);
			PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionOffset1,
													 x64GenContext->codeBufferIndex);
			x64Gen_mov_reg64Low32_imm32(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult), 32);
			PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionOffset2,
													 x64GenContext->codeBufferIndex);
		}
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_COMPARE_SIGNED ||
			 imlInstruction->operation == PPCREC_IML_OP_COMPARE_UNSIGNED)
	{
		...
		if (imlInstruction->operation == PPCREC_IML_OP_COMPARE_SIGNED)
			PPCRecompilerX64Gen_crConditionFlags_set(PPCRecFunction, ppcImlGenContext,
													 x64GenContext, imlInstruction->crRegister,
													 PPCREC_CR_STATE_TYPE_SIGNED_ARITHMETIC);
		else
			PPCRecompilerX64Gen_crConditionFlags_set(PPCRecFunction, ppcImlGenContext,
													 x64GenContext, imlInstruction->crRegister,
													 PPCREC_CR_STATE_TYPE_UNSIGNED_ARITHMETIC);
		...
		{
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_LT))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_SIGNED_LESS, REG_ESP,
								  offsetof(PPCInterpreter_t, cr) +
									  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_LT));
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_GT))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_SIGNED_GREATER, REG_ESP,
								  offsetof(PPCInterpreter_t, cr) +
									  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_GT));
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_EQ))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_EQUAL, REG_ESP,
								  offsetof(PPCInterpreter_t, cr) +
									  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_EQ));
		...
		{
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_LT))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_UNSIGNED_BELOW, REG_ESP,
								  offsetof(PPCInterpreter_t, cr) +
									  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_LT));
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_GT))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_UNSIGNED_ABOVE, REG_ESP,
								  offsetof(PPCInterpreter_t, cr) +
									  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_GT));
			if ((imlInstruction->crIgnoreMask & (1 << (crRegister * 4 + PPCREC_CR_BIT_EQ))) == 0)
				x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_EQUAL, REG_ESP,
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_ADD_CARRY)
	{
		...
		// add carry bit
		x64Gen_adc_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_r.registerResult), 0);
		// update xer carry
		x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_CARRY, REG_RSP,
						  offsetof(PPCInterpreter_t, xer_ca));
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			// set cr bits
			sint32 crRegister = imlInstruction->crRegister;
			x64Gen_setcc_mem8(
				...
				offsetof(PPCInterpreter_t, cr) +
					sizeof(uint8) *
						(crRegister * 4 + PPCREC_CR_BIT_LT)); // check for sign instead of _BELOW
															  // (CF) which is not set by AND/OR
			x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_UNSIGNED_ABOVE, REG_RSP,
							  offsetof(PPCInterpreter_t, cr) +
								  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_GT));
			x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_EQUAL, REG_RSP,
							  offsetof(PPCInterpreter_t, cr) +
								  sizeof(uint8) * (crRegister * 4 + PPCREC_CR_BIT_EQ));
			// todo: Use different version of PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction,
			// ppcImlGenContext, x64GenContext, imlInstruction) todo: Also set summary overflow if
			// xer bit is set
		}
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_ADD_CARRY_ME)
	{
		...
		x64Gen_bt_mem8(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, xer_ca), 0);
		// add carry bit
		x64Gen_adc_reg64Low32_imm32(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult), (uint32)-1);
		// update xer carry
		x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_CARRY, REG_RSP,
						  offsetof(PPCInterpreter_t, xer_ca));
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_ASSIGN_S16_TO_S32)
	{
		...
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		x64Gen_movSignExtend_reg64Low32_reg64Low16(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r.registerResult),
			reg32ToReg16(tempToRealRegister(imlInstruction->op_r_r.registerA)));
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			if (imlInstruction->crMode == PPCREC_CR_MODE_ARITHMETIC)
			{
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_DCBZ)
	{
		...
		if (imlInstruction->op_r_r.registerResult != imlInstruction->op_r_r.registerA)
		{
			x64Gen_mov_reg64_reg64(x64GenContext, REG_RESV_TEMP,
								   tempToRealRegister(imlInstruction->op_r_r.registerA));
			x64Gen_add_reg64Low32_reg64Low32(
				x64GenContext, REG_RESV_TEMP,
				tempToRealRegister(imlInstruction->op_r_r.registerResult));
...
bool PPCRecompilerX64Gen_imlInstruction_r_s32(PPCRecFunction_t* PPCRecFunction,
											  ppcImlGenContext_t* ppcImlGenContext,
											  x64GenContext_t* x64GenContext,
											  PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction->operation == PPCREC_IML_OP_ASSIGN)
	{
		...
		cemu_assert_debug(imlInstruction->crRegister == PPC_REC_INVALID_REGISTER);
		x64Gen_mov_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									(uint32)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_ADD)
	{
		// registerResult += immS32
		...
		x64Gen_add_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									(uint32)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_SUB)
	{
		...
		x64Gen_sub_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									(uint32)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_AND)
	{
		...
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		x64Gen_and_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									(uint32)imlInstruction->op_r_immS32.immS32);
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_OR)
	{
		...
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		cemu_assert_debug(imlInstruction->crRegister == PPC_REC_INVALID_REGISTER);
		x64Gen_or_reg64Low32_imm32(x64GenContext,
								   tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
								   (uint32)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_XOR)
	{
		...
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		cemu_assert_debug(imlInstruction->crRegister == PPC_REC_INVALID_REGISTER);
		x64Gen_xor_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									(uint32)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_LEFT_ROTATE)
	{
		...
		x64Gen_rol_reg64Low32_imm8(x64GenContext,
								   tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
								   (uint8)imlInstruction->op_r_immS32.immS32);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_COMPARE_SIGNED ||
			 imlInstruction->operation == PPCREC_IML_OP_COMPARE_UNSIGNED)
	{
		...
		if (imlInstruction->crRegister >= 8)
		{
			debug_printf("PPCRecompilerX64Gen_imlInstruction_r_s32(): Unsupported CMP with "
						 "crRegister = 8\n");
			return false;
		}
		// update state of cr register
		if (imlInstruction->operation == PPCREC_IML_OP_COMPARE_SIGNED)
			...
			PPCRecompilerX64Gen_crConditionFlags_set(PPCRecFunction, ppcImlGenContext,
													 ...
													 PPCREC_CR_STATE_TYPE_UNSIGNED_ARITHMETIC);
		// create compare instruction
		x64Gen_cmp_reg64Low32_imm32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_immS32.registerIndex),
									imlInstruction->op_r_immS32.immS32);
		// set cr bits
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_MTCRF)
	{
		...
		for (sint32 f = 0; f < 32; f++)
		{
			if (((crBitMask >> f) & 1) == 0)
				continue;
			x64Gen_mov_mem8Reg64_imm8(x64GenContext, REG_ESP,
									  offsetof(PPCInterpreter_t, cr) + sizeof(uint8) * (f), 0);
			x64Gen_test_reg64Low32_imm32(x64GenContext, srcRegister, 0x80000000 >> f);
			x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_NOT_EQUAL, REG_ESP,
							  offsetof(PPCInterpreter_t, cr) + sizeof(uint8) * (f));
		}
	}
	else
	{
...
bool PPCRecompilerX64Gen_imlInstruction_conditional_r_s32(PPCRecFunction_t* PPCRecFunction,
														  ppcImlGenContext_t* ppcImlGenContext,
														  x64GenContext_t* x64GenContext,
														  PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction->operation == PPCREC_IML_OP_ASSIGN)
	{
		...
		x64Gen_mov_reg64Low32_imm32(x64GenContext, REG_RESV_TEMP,
									(uint32)imlInstruction->op_conditional_r_s32.immS32);

		uint8 crBitIndex = imlInstruction->op_conditional_r_s32.crRegisterIndex * 4 +
		...
		if (imlInstruction->op_conditional_r_s32.crRegisterIndex == x64GenContext->activeCRRegister)
		{
			if (x64GenContext->activeCRState == PPCREC_CR_STATE_TYPE_UNSIGNED_ARITHMETIC)
			{
				...
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet ? X86_CONDITION_CARRY
																		  : X86_CONDITION_NOT_CARRY,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
				else if (imlInstruction->op_conditional_r_s32.crBitIndex == CR_BIT_EQ)
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet ? X86_CONDITION_EQUAL
																		  : X86_CONDITION_NOT_EQUAL,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
				else if (imlInstruction->op_conditional_r_s32.crBitIndex == CR_BIT_GT)
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet
							? X86_CONDITION_UNSIGNED_ABOVE
							: X86_CONDITION_UNSIGNED_BELOW_EQUAL,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
			...
			else if (x64GenContext->activeCRState == PPCREC_CR_STATE_TYPE_SIGNED_ARITHMETIC)
			{
				if (imlInstruction->op_conditional_r_s32.crBitIndex == CR_BIT_LT)
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet
							? X86_CONDITION_SIGNED_LESS
							: X86_CONDITION_SIGNED_GREATER_EQUAL,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
				...
				else if (imlInstruction->op_conditional_r_s32.crBitIndex == CR_BIT_GT)
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet
							? X86_CONDITION_SIGNED_GREATER
							: X86_CONDITION_SIGNED_LESS_EQUAL,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
			...
			else if (x64GenContext->activeCRState == PPCREC_CR_STATE_TYPE_LOGICAL)
			{
				if (imlInstruction->op_conditional_r_s32.crBitIndex == CR_BIT_LT)
				{
					x64Gen_cmovcc_reg64Low32_reg64Low32(
						x64GenContext,
						imlInstruction->op_conditional_r_s32.bitMustBeSet ? X86_CONDITION_SIGN
																		  : X86_CONDITION_NOT_SIGN,
						tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
						REG_RESV_TEMP);
					return true;
				}
		...
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		x64Gen_bt_mem8(x64GenContext, REG_RSP,
					   offsetof(PPCInterpreter_t, cr) + crBitIndex * sizeof(uint8), 0);
		if (imlInstruction->op_conditional_r_s32.bitMustBeSet)
			x64Gen_cmovcc_reg64Low32_reg64Low32(
				x64GenContext, X86_CONDITION_CARRY,
				tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
				REG_RESV_TEMP);
		else
			x64Gen_cmovcc_reg64Low32_reg64Low32(
				x64GenContext, X86_CONDITION_NOT_CARRY,
				tempToRealRegister(imlInstruction->op_conditional_r_s32.registerIndex),
				REG_RESV_TEMP);
		return true;
	}
...
bool PPCRecompilerX64Gen_imlInstruction_r_r_r(PPCRecFunction_t* PPCRecFunction,
											  ppcImlGenContext_t* ppcImlGenContext,
											  x64GenContext_t* x64GenContext,
											  PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction->operation == PPCREC_IML_OP_ADD ||
		imlInstruction->operation == PPCREC_IML_OP_ADD_UPDATE_CARRY ||
		imlInstruction->operation == PPCREC_IML_OP_ADD_CARRY_UPDATE_CARRY)
	{
		...
		// update carry
		if (imlInstruction->operation == PPCREC_IML_OP_ADD_UPDATE_CARRY ||
			imlInstruction->operation == PPCREC_IML_OP_ADD_CARRY_UPDATE_CARRY)
		{
			x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_CARRY, REG_RSP,
							  offsetof(PPCInterpreter_t, xer_ca));
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_MULTIPLY_SIGNED)
	{
		...
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			...
			// since IMUL instruction leaves relevant flags undefined, we have to use another TEST
			// instruction to get the correct results
			x64Gen_test_reg64Low32_reg64Low32(x64GenContext, rRegResult, rRegResult);
			PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction, ppcImlGenContext, x64GenContext,
												imlInstruction);
		}
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_SUBFC)
	{
		...
		else
		{
			...
			x64Gen_sub_reg64Low32_reg64Low32(x64GenContext, rRegResult, rRegOperandA);
			 ...
			 imlInstruction->operation == PPCREC_IML_OP_SRW)
	{
		...
		else if (hasBMI2Support && imlInstruction->operation == PPCREC_IML_OP_SLW)
		{
			...
			x64Gen_and_reg64Low32_reg64Low32(x64GenContext, rRegResult,
											 rRegResult); // trim result to 32bit
		}
		else
		{
			...
			for (sint32 b = 0; b < 6; b++)
			{
				...
				PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionOffset,
														 x64GenContext->codeBufferIndex);
			}
	...
	{
		// registerResult = (sint32)registerOperand1(rA) >> (sint32)registerOperand2(rB) (up to 63
		// bits)
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		sint32 rRegResult = tempToRealRegister(imlInstruction->op_r_r_r.registerResult);
		...
		// reset carry
		x64Gen_mov_mem8Reg64_imm8(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, xer_ca), 0);
		// we use the same shift by register approach as in SLW/SRW, but we have to differentiate by
		// signed/unsigned shift since it influences how the carry flag is set
		...
		// signed shift
		PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionJumpToSignedShift,
												 x64GenContext->codeBufferIndex);
		for (sint32 b = 0; b < 6; b++)
		{
			...
			x64Gen_mov_mem8Reg64_imm8(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, xer_ca),
									  1);
			PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionJumpToAfterCa,
													 x64GenContext->codeBufferIndex);
			// arithmetic shift
			if (b == 5)
			{
		...
		// end
		PPCRecompilerX64Gen_redirectRelativeJump(x64GenContext, jumpInstructionJumpToEnd,
												 x64GenContext->codeBufferIndex);
		x64Gen_mov_reg64_reg64(x64GenContext, rRegResult, REG_RESV_TEMP);
		// update CR if requested
		// todo
			 ...
			 imlInstruction->operation == PPCREC_IML_OP_DIVIDE_UNSIGNED)
	{
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		sint32 rRegResult = tempToRealRegister(imlInstruction->op_r_r_r.registerResult);
		...
		x64Emit_mov_mem32_reg32(x64GenContext, REG_RSP,
								(uint32)offsetof(PPCInterpreter_t, temporaryGPR[0]), REG_EAX);
		x64Emit_mov_mem32_reg32(x64GenContext, REG_RSP,
								(uint32)offsetof(PPCInterpreter_t, temporaryGPR[1]), REG_EDX);
		...
		if (rRegResult != REG_RAX)
			x64Emit_mov_reg64_mem32(x64GenContext, REG_EAX, REG_RSP,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[0]));
		if (rRegResult != REG_RDX)
			x64Emit_mov_reg64_mem32(x64GenContext, REG_EDX, REG_RSP,
									(uint32)offsetof(PPCInterpreter_t, temporaryGPR[1]));
		...
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			...
			PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction, ppcImlGenContext, x64GenContext,
			 ...
			 imlInstruction->operation == PPCREC_IML_OP_MULTIPLY_HIGH_UNSIGNED)
	{
		PPCRecompilerX64Gen_crConditionFlags_forget(PPCRecFunction, ppcImlGenContext,
													x64GenContext);
		sint32 rRegResult = tempToRealRegister(imlInstruction->op_r_r_r.registerResult);
...
bool PPCRecompilerX64Gen_imlInstruction_r_r_s32(PPCRecFunction_t* PPCRecFunction,
												ppcImlGenContext_t* ppcImlGenContext,
												x64GenContext_t* x64GenContext,
												PPCRecImlInstruction_t* imlInstruction)
{
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_ADD_UPDATE_CARRY)
	{
		...
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
		{
			...
			sint32 crRegister = imlInstruction->crRegister;
			// x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_SIGN, REG_RSP,
			// offsetof(PPCInterpreter_t, cr)+sizeof(uint8)*(crRegister*4+PPCREC_CR_BIT_LT));
			// x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_SIGNED_GREATER, REG_RSP,
			// offsetof(PPCInterpreter_t, cr)+sizeof(uint8)*(crRegister*4+PPCREC_CR_BIT_GT));
			// x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_EQUAL, REG_RSP,
			// offsetof(PPCInterpreter_t, cr)+sizeof(uint8)*(crRegister*4+PPCREC_CR_BIT_EQ));
			PPCRecompilerX64Gen_updateCRLogical(PPCRecFunction, ppcImlGenContext, x64GenContext,
												imlInstruction);
		}
	}
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_RLWIMI)
	{
		...
		x64Gen_mov_reg64_reg64(x64GenContext, REG_RESV_TEMP,
							   tempToRealRegister(imlInstruction->op_r_r_s32.registerA));
		...
		x64Gen_and_reg64Low32_imm32(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r_s32.registerResult), ~mask);
		// AND temporary rS register with mask
		x64Gen_and_reg64Low32_imm32(x64GenContext, REG_RESV_TEMP, mask);
		// OR result with temporary
		x64Gen_or_reg64Low32_reg64Low32(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r_s32.registerResult),
			REG_RESV_TEMP);
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_MULTIPLY_SIGNED)
	{
	...
	else if (imlInstruction->operation == PPCREC_IML_OP_SRAW)
	{
		...
		if (imlInstruction->op_r_r_s32.registerA != imlInstruction->op_r_r_s32.registerResult)
			x64Gen_mov_reg64_reg64(x64GenContext,
								   tempToRealRegister(imlInstruction->op_r_r_s32.registerResult),
								   tempToRealRegister(imlInstruction->op_r_r_s32.registerA));
		...
		if (sh >= 31)
			...
			caTestMask = (1 << (sh)) - 1;
		x64Gen_test_reg64Low32_imm32(x64GenContext,
									 tempToRealRegister(imlInstruction->op_r_r_s32.registerResult),
									 caTestMask);
		// SETNE/NZ [ESP+XER_CA]
		x64Gen_setcc_mem8(x64GenContext, X86_CONDITION_NOT_EQUAL, REG_RSP,
						  offsetof(PPCInterpreter_t, xer_ca));
		// SAR registerResult, SH
		x64Gen_sar_reg64Low32_imm8(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_r_s32.registerResult), sh);
		// JNS <skipInstruction> (if sign not set)
		sint32 jumpInstructionOffset = x64GenContext->codeBufferIndex;
		x64Gen_jmpc_near(x64GenContext, X86_CONDITION_SIGN,
						 0); // todo: Can use 2-byte form of jump instruction here
			 ...
			 imlInstruction->operation == PPCREC_IML_OP_RIGHT_SHIFT)
	{
		...
		if (imlInstruction->operation == PPCREC_IML_OP_LEFT_SHIFT)
			x64Gen_shl_reg64Low32_imm8(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r_s32.registerResult),
				imlInstruction->op_r_r_s32.immS32);
		else
			x64Gen_shr_reg64Low32_imm8(
				x64GenContext, tempToRealRegister(imlInstruction->op_r_r_s32.registerResult),
				imlInstruction->op_r_r_s32.immS32);
		// CR update
		if (imlInstruction->crRegister != PPC_REC_INVALID_REGISTER)
...
bool PPCRecompilerX64Gen_imlInstruction_conditionalJump(PPCRecFunction_t* PPCRecFunction,
														ppcImlGenContext_t* ppcImlGenContext,
														x64GenContext_t* x64GenContext,
														...
														PPCRecImlInstruction_t* imlInstruction)
{
	if (imlInstruction->op_conditionalJump.condition == PPCREC_JUMP_CONDITION_NONE)
	{
		...
		if (imlInstruction->op_conditionalJump.jumpAccordingToSegment)
		{
			...
			if (imlSegment->nextSegmentBranchTaken == nullptr)
				assert_dbg();
			PPCRecompilerX64Gen_rememberRelocatableOffset(x64GenContext, X64_RELOC_LINK_TO_SEGMENT,
		...
		else
		{
			...
			PPCRecompilerX64Gen_rememberRelocatableOffset(
				x64GenContext, X64_RELOC_LINK_TO_PPC,
				(void*)(size_t)imlInstruction->op_conditionalJump.jumpmarkAddress);
			x64Gen_jmp_imm32(x64GenContext, 0);
		}
	}
	else
	{
		...
		if (imlInstruction->op_conditionalJump.crRegisterIndex == PPCREC_CR_TEMPORARY ||
			imlInstruction->op_conditionalJump.crRegisterIndex >= 8)
		{
		...
		{
			uint8 crBitIndex = imlInstruction->op_conditionalJump.crRegisterIndex * 4 +
							   imlInstruction->op_conditionalJump.crBitIndex;
			if (imlInstruction->op_conditionalJump.crRegisterIndex ==
				x64GenContext->activeCRRegister)
			{
				if (x64GenContext->activeCRState == PPCREC_CR_STATE_TYPE_UNSIGNED_ARITHMETIC)
				{
					if (imlInstruction->op_conditionalJump.crBitIndex == CR_BIT_LT)
					{
						PPCRecompilerX64Gen_rememberRelocatableOffset(
							x64GenContext, X64_RELOC_LINK_TO_PPC,
							(void*)(size_t)imlInstruction->op_conditionalJump.jumpmarkAddress);
						x64Gen_jmpc_far(x64GenContext,
										imlInstruction->op_conditionalJump.bitMustBeSet
											? X86_CONDITION_CARRY
											: X86_CONDITION_NOT_CARRY,
										0);
						return true;
...
bool PPCRecompilerX64Gen_imlInstruction_conditionalJumpCycleCheck(
	...
	x64GenContext_t* x64GenContext, PPCRecImlInstruction_t* imlInstruction)
{
	...
	// 1) DEC [mem] + JNS has significantly worse performance than BT + JNC (probably due to
	// additional memory write) 2) CMP [mem], 0 + JG has about equal (or slightly worse) performance
	// than BT + JNC
 ...
 */
bool PPCRecompilerX64Gen_imlInstruction_cr(PPCRecFunction_t* PPCRecFunction,
										   ppcImlGenContext_t* ppcImlGenContext,
										   x64GenContext_t* x64GenContext,
										   PPCRecImlInstruction_t* imlInstruction)
{
	PPCRecompilerX64Gen_crConditionFlags_forget(
		PPCRecFunction, ppcImlGenContext, x64GenContext); // while these instruction do not directly
														  // affect eflags, they change the CR bit
	if (imlInstruction->operation == PPCREC_IML_OP_CR_CLEAR)
	{
		// clear cr bit
		x64Gen_mov_mem8Reg64_imm8(
			x64GenContext, REG_RSP,
			offsetof(PPCInterpreter_t, cr) + sizeof(uint8) * imlInstruction->op_cr.crD, 0);
		return true;
	}
	else if (imlInstruction->operation == PPCREC_IML_OP_CR_SET)
	{
		...
		x64Gen_mov_mem8Reg64_imm8(
			x64GenContext, REG_RSP,
			offsetof(PPCInterpreter_t, cr) + sizeof(uint8) * imlInstruction->op_cr.crD, 1);
		return true;
	}
	...
	{
		x64Emit_movZX_reg64_mem8(x64GenContext, REG_RESV_TEMP, REG_RSP,
								 offsetof(PPCInterpreter_t, cr) +
									 sizeof(uint8) * imlInstruction->op_cr.crB);
		if (imlInstruction->operation == PPCREC_IML_OP_CR_ORC ||
			imlInstruction->operation == PPCREC_IML_OP_CR_ANDC)
		{
		...
		if (imlInstruction->operation == PPCREC_IML_OP_CR_OR ||
			imlInstruction->operation == PPCREC_IML_OP_CR_ORC)
			x64Gen_or_reg64Low8_mem8Reg64(x64GenContext, REG_RESV_TEMP, REG_RSP,
										  offsetof(PPCInterpreter_t, cr) +
											  sizeof(uint8) * imlInstruction->op_cr.crA);
		else
			x64Gen_and_reg64Low8_mem8Reg64(x64GenContext, REG_RESV_TEMP, REG_RSP,
										   offsetof(PPCInterpreter_t, cr) +
											   sizeof(uint8) * imlInstruction->op_cr.crA);

		x64Gen_mov_mem8Reg64_reg64Low8(x64GenContext, REG_RESV_TEMP, REG_RSP,
									   offsetof(PPCInterpreter_t, cr) +
										   sizeof(uint8) * imlInstruction->op_cr.crD);
...
void PPCRecompilerX64Gen_imlInstruction_ppcEnter(PPCRecFunction_t* PPCRecFunction,
												 ppcImlGenContext_t* ppcImlGenContext,
												 x64GenContext_t* x64GenContext,
												 PPCRecImlInstruction_t* imlInstruction)
{
	...
	if (ppcImlGenContext->hasFPUInstruction)
	{
		...
		// PPCRecompilerX86_crConditionFlags_saveBeforeOverwrite(PPCRecFunction, ppcImlGenContext,
		// x64GenContext);
		...
		// x64Gen_mov_reg32_imm32(x64GenContext, REG_EAX,
		// imlInstruction->op_ppcEnter.ppcAddress&0x7FFFFFFF);
		// PPCRecompilerX64Gen_rememberRelocatableOffset(x64GenContext, X86_RELOC_MAKE_RELATIVE);
		// x64Gen_jmp_imm32(x64GenContext,
		// (uint32)PPCRecompiler_recompilerCallEscapeAndCallFPUUnavailable);
		//// patch jump
		//*(uint32*)(x64GenContext->codeBuffer+jmpCodeOffset+2) =
		// x64GenContext->codeBufferIndex-jmpCodeOffset-6;
	}
}

void PPCRecompilerX64Gen_imlInstruction_r_name(PPCRecFunction_t* PPCRecFunction,
											   ppcImlGenContext_t* ppcImlGenContext,
											   x64GenContext_t* x64GenContext,
											   PPCRecImlInstruction_t* imlInstruction)
{
	...
	{
		x64Emit_mov_reg64_mem32(
			x64GenContext, tempToRealRegister(imlInstruction->op_r_name.registerIndex), REG_RSP,
			offsetof(PPCInterpreter_t, gpr) + sizeof(uint32) * (name - PPCREC_NAME_R0));
	}
	else if (name >= PPCREC_NAME_SPR0 && name < PPCREC_NAME_SPR0 + 999)
	{
		sint32 sprIndex = (name - PPCREC_NAME_SPR0);
		if (sprIndex == SPR_LR)
			x64Emit_mov_reg64_mem32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_name.registerIndex),
									REG_RSP, offsetof(PPCInterpreter_t, spr.LR));
		else if (sprIndex == SPR_CTR)
			x64Emit_mov_reg64_mem32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_name.registerIndex),
									REG_RSP, offsetof(PPCInterpreter_t, spr.CTR));
		else if (sprIndex == SPR_XER)
			x64Emit_mov_reg64_mem32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_name.registerIndex),
									REG_RSP, offsetof(PPCInterpreter_t, spr.XER));
		else if (sprIndex >= SPR_UGQR0 && sprIndex <= SPR_UGQR7)
		{
			sint32 memOffset = offsetof(PPCInterpreter_t, spr.UGQR) +
							   sizeof(PPCInterpreter_t::spr.UGQR[0]) * (sprIndex - SPR_UGQR0);
			x64Emit_mov_reg64_mem32(x64GenContext,
									tempToRealRegister(imlInstruction->op_r_name.registerIndex),
									REG_RSP, memOffset);
		}
			...
			assert_dbg();
		// x64Emit_mov_reg64_mem32(x64GenContext,
		// tempToRealRegister(imlInstruction->op_r_name.registerIndex), REG_RSP,
		// offsetof(PPCInterpreter_t, spr)+sizeof(uint32)*(name-PPCREC_NAME_SPR0));
	}
	else
		assert_dbg();
}

void PPCRecompilerX64Gen_imlInstruction_name_r(PPCRecFunction_t* PPCRecFunction,
											   ppcImlGenContext_t* ppcImlGenContext,
											   x64GenContext_t* x64GenContext,
											   PPCRecImlInstruction_t* imlInstruction)
{
	...
	if (name >= PPCREC_NAME_R0 && name < PPCREC_NAME_R0 + 32)
	{
		x64Emit_mov_mem32_reg64(x64GenContext, REG_RSP,
								offsetof(PPCInterpreter_t, gpr) +
									sizeof(uint32) * (name - PPCREC_NAME_R0),
								tempToRealRegister(imlInstruction->op_r_name.registerIndex));
	}
	else if (name >= PPCREC_NAME_SPR0 && name < PPCREC_NAME_SPR0 + 999)
	{
		...
		if (sprIndex == SPR_LR)
			x64Emit_mov_mem32_reg64(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, spr.LR),
									tempToRealRegister(imlInstruction->op_r_name.registerIndex));
		else if (sprIndex == SPR_CTR)
			x64Emit_mov_mem32_reg64(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, spr.CTR),
									tempToRealRegister(imlInstruction->op_r_name.registerIndex));
		else if (sprIndex == SPR_XER)
			x64Emit_mov_mem32_reg64(x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, spr.XER),
									tempToRealRegister(imlInstruction->op_r_name.registerIndex));
		else if (sprIndex >= SPR_UGQR0 && sprIndex <= SPR_UGQR7)
		{
			sint32 memOffset = offsetof(PPCInterpreter_t, spr.UGQR) +
							   sizeof(PPCInterpreter_t::spr.UGQR[0]) * (sprIndex - SPR_UGQR0);
			x64Emit_mov_mem32_reg64(x64GenContext, REG_RSP, memOffset,
									tempToRealRegister(imlInstruction->op_r_name.registerIndex));
		}
		else
			assert_dbg();
...
uint8* PPCRecompilerX86_allocateExecutableMemory(sint32 size)
{
	...
	{
		// allocate new block
		codeMemoryBlockSize = std::max(
			1024 * 1024 * 4, size + 1024); // 4MB (or more if the function is larger than 4MB)
		codeMemoryBlockIndex = 0;
		codeMemoryBlock = (uint8*)MemMapper::AllocateMemory(nullptr, codeMemoryBlockSize,
															MemMapper::PAGE_PERMISSION::P_RWX);
	}
...
bool PPCRecompiler_generateX64Code(PPCRecFunction_t* PPCRecFunction,
								   ppcImlGenContext_t* ppcImlGenContext)
{
	...
	for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
	{
		...
		for (sint32 i = 0; i < imlSegment->imlListCount; i++)
		{
			...
			{
				PPCRecompilerX64Gen_imlInstruction_r_name(PPCRecFunction, ppcImlGenContext,
														  &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_NAME_R)
			{
				PPCRecompilerX64Gen_imlInstruction_name_r(PPCRecFunction, ppcImlGenContext,
														  &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_R_R)
			{
				if (PPCRecompilerX64Gen_imlInstruction_r_r(PPCRecFunction, ppcImlGenContext,
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_r_s32(
						PPCRecFunction, ppcImlGenContext, &x64GenContext, imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			}
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_r_r_s32(
						PPCRecFunction, ppcImlGenContext, &x64GenContext, imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_r_r_r(
						PPCRecFunction, ppcImlGenContext, &x64GenContext, imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_conditionalJump(
						PPCRecFunction, ppcImlGenContext, &x64GenContext, imlSegment,
						imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			}
			...
			{
				PPCRecompilerX64Gen_imlInstruction_conditionalJumpCycleCheck(
					PPCRecFunction, ppcImlGenContext, &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_MACRO)
			{
				if (PPCRecompilerX64Gen_imlInstruction_macro(
						PPCRecFunction, ppcImlGenContext, &x64GenContext, imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_store(PPCRecFunction, ppcImlGenContext,
															 &x64GenContext, imlInstruction,
															 false) == false)
				{
					codeGenerationFailed = true;
			...
			{
				PPCRecompilerX64Gen_imlInstruction_mem2mem(PPCRecFunction, ppcImlGenContext,
														   &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_CR)
			{
				if (PPCRecompilerX64Gen_imlInstruction_cr(PPCRecFunction, ppcImlGenContext,
														  &x64GenContext, imlInstruction) == false)
				{
					codeGenerationFailed = true;
				}
			...
			{
				PPCRecompilerX64Gen_imlInstruction_ppcEnter(PPCRecFunction, ppcImlGenContext,
															&x64GenContext, imlInstruction);
			}
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_fpr_load(PPCRecFunction, ppcImlGenContext,
																&x64GenContext, imlInstruction,
																false) == false)
				{
					codeGenerationFailed = true;
			...
			{
				if (PPCRecompilerX64Gen_imlInstruction_fpr_store(PPCRecFunction, ppcImlGenContext,
																 &x64GenContext, imlInstruction,
																 false) == false)
				{
					codeGenerationFailed = true;
			...
			{
				PPCRecompilerX64Gen_imlInstruction_fpr_r_r_r(PPCRecFunction, ppcImlGenContext,
															 &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R_R_R_R)
			{
				PPCRecompilerX64Gen_imlInstruction_fpr_r_r_r_r(PPCRecFunction, ppcImlGenContext,
															   &x64GenContext, imlInstruction);
			}
			else if (imlInstruction->type == PPCREC_IML_TYPE_FPR_R)
			{
				PPCRecompilerX64Gen_imlInstruction_fpr_r(PPCRecFunction, ppcImlGenContext,
														 &x64GenContext, imlInstruction);
			}
			else
			{
	...
	uint8* executableMemory =
		PPCRecompilerX86_allocateExecutableMemory(x64GenContext.codeBufferIndex);
	...
	for (sint32 i = 0; i < x64GenContext.relocateOffsetTableCount; i++)
	{
		...
		else if (x64GenContext.relocateOffsetTable[i].type == X64_RELOC_LINK_TO_PPC ||
				 x64GenContext.relocateOffsetTable[i].type == X64_RELOC_LINK_TO_SEGMENT)
		{
			...
			if (x64GenContext.relocateOffsetTable[i].type == X64_RELOC_LINK_TO_PPC)
			{
				for (sint32 s = 0; s < ppcImlGenContext->segmentListCount; s++)
				{
					if (ppcImlGenContext->segmentList[s]->isJumpDestination &&
						ppcImlGenContext->segmentList[s]->jumpDestinationPPCAddress == ppcOffset)
					{
			...
			{
				PPCRecImlSegment_t* destSegment =
					(PPCRecImlSegment_t*)x64GenContext.relocateOffsetTable[i].extraInfo;
			...
			if (relocInstruction[0] == 0x0F &&
				(relocInstruction[1] >= 0x80 && relocInstruction[1] <= 0x8F))
			{
				// Jcc relativeImm32
				sint32 distanceNearJump =
					(sint32)((baseAddress + x64Offset) - (baseAddress + relocBase + 2));
				...
				{
					// patch offset
					*(uint32*)(relocInstruction + 2) =
						(uint32)((baseAddress + x64Offset) - (baseAddress + relocBase + 6));
				}
			}
			else if (relocInstruction[0] == 0xE9)
			{
				// JMP relativeImm32
				*(uint32*)(relocInstruction + 1) =
					(uint32)((baseAddress + x64Offset) - (baseAddress + relocBase + 5));
			}
			else
				assert_dbg();
		}
...
void PPCRecompilerX64Gen_generateEnterRecompilerCode()
{
	...
	x64GenContext.codeBuffer[jmpPatchOffset] =
		(x64GenContext.codeBufferIndex - (jmpPatchOffset - 4));
...
void* PPCRecompilerX64Gen_generateLeaveRecompilerCode()
{
	...
	// update instruction pointer
	// LR is in EDX
	x64Emit_mov_mem32_reg32(&x64GenContext, REG_RSP, offsetof(PPCInterpreter_t, instructionPointer),
							REG_EDX);
...
void PPCRecompilerX64Gen_generateRecompilerInterfaceFunctions()
{
	...
	PPCRecompiler_leaveRecompilerCode_unvisited =
		(void ATTR_MS_ABI (*)())PPCRecompilerX64Gen_generateLeaveRecompilerCode();
	PPCRecompiler_leaveRecompilerCode_visited =
		(void ATTR_MS_ABI (*)())PPCRecompilerX64Gen_generateLeaveRecompilerCode();
	cemu_assert_debug(PPCRecompiler_leaveRecompilerCode_unvisited !=
					  PPCRecompiler_leaveRecompilerCode_visited);
}