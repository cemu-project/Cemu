														   ...
														   sint32 memRegisterB64, sint32 memImmS32)
{
	...
	// note: Unlike the 32bit version this instruction does not set the upper 32bits of the 64bit
	// register to 0
	x64Gen_writeU8(x64GenContext, 0x66); // 16bit prefix
	x64Gen_movBEZeroExtend_reg64_mem32Reg64PlusReg64(x64GenContext, dstRegister, memRegisterA64,
													 memRegisterB64, memImmS32);
}
								   ...
								   sint32 registerA, sint32 registerB)
{
	...
	x64Gen_writeU8(x64GenContext,
				   0xE2 - ((registerDst >= 8) ? 0x80 : 0) - ((registerA >= 8) ? 0x20 : 0));
