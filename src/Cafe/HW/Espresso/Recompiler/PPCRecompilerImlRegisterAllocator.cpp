...
bool _detectLoop(PPCRecImlSegment_t* currentSegment, sint32 depth, uint32 iterationIndex,
				 PPCRecImlSegment_t* imlSegmentLoopBase)
{
	...
	{
		if (currentSegment->nextSegmentBranchNotTaken->momentaryIndex >
			currentSegment->momentaryIndex)
		{
			currentSegment->raInfo.isPartOfProcessedLoop =
				_detectLoop(currentSegment->nextSegmentBranchNotTaken, depth + 1, iterationIndex,
							imlSegmentLoopBase);
		}
	}
	if (currentSegment->nextSegmentBranchTaken)
	{
		...
		{
			currentSegment->raInfo.isPartOfProcessedLoop =
				_detectLoop(currentSegment->nextSegmentBranchTaken, depth + 1, iterationIndex,
							imlSegmentLoopBase);
		}
	}
...
void PPCRecRA_detectLoop(ppcImlGenContext_t* ppcImlGenContext,
						 PPCRecImlSegment_t* imlSegmentLoopBase)
{
	uint32 iterationIndex = PPCRecRA_getNextIterationIndex();
	imlSegmentLoopBase->raInfo.lastIterationIndex = iterationIndex;
	if (_detectLoop(imlSegmentLoopBase->nextSegmentBranchTaken, 0, iterationIndex,
					imlSegmentLoopBase))
	{
...
void PPCRecRA_identifyLoop(ppcImlGenContext_t* ppcImlGenContext, PPCRecImlSegment_t* imlSegment)
{
	...
	if (imlSegment->nextSegmentBranchTaken &&
		imlSegment->nextSegmentBranchTaken->momentaryIndex < imlSegment->momentaryIndex)
	{
...
void PPCRecRA_insertGPRLoadInstruction(PPCRecImlSegment_t* imlSegment, sint32 insertIndex,
...
void PPCRecRA_insertGPRLoadInstructions(PPCRecImlSegment_t* imlSegment, sint32 insertIndex,
										raLoadStoreInfo_t* loadList, sint32 loadCount)
{
	PPCRecompiler_pushBackIMLInstructions(imlSegment, insertIndex, loadCount);
	memset(imlSegment->imlList + (insertIndex + 0), 0x00,
		   sizeof(PPCRecImlInstruction_t) * loadCount);
...
void PPCRecRA_insertGPRStoreInstruction(PPCRecImlSegment_t* imlSegment, sint32 insertIndex,
										sint32 registerIndex, sint32 registerName)
{
	PPCRecompiler_pushBackIMLInstructions(imlSegment, insertIndex, 1);
	PPCRecImlInstruction_t* imlInstructionItr = imlSegment->imlList + (insertIndex + 0);
...
void PPCRecRA_insertGPRStoreInstructions(PPCRecImlSegment_t* imlSegment, sint32 insertIndex,
										 raLoadStoreInfo_t* storeList, sint32 storeCount)
{
	PPCRecompiler_pushBackIMLInstructions(imlSegment, insertIndex, storeCount);
	memset(imlSegment->imlList + (insertIndex + 0), 0x00,
		   sizeof(PPCRecImlInstruction_t) * storeCount);
...
// count how many instructions there are until physRegister is used by any subrange (returns 0 if
// register is in use at startIndex, and INT_MAX if not used for the remainder of the segment)
sint32 PPCRecRA_countInstructionsUntilNextLocalPhysRegisterUse(PPCRecImlSegment_t* imlSegment,
															   sint32 startIndex,
															   sint32 physRegister)
{
...
uint32 PPCRecRA_getAllowedRegisterMaskForFullRange(raLivenessRange_t* range)
{
	...
	for (auto& subrange : range->list_subranges)
	{
		...
		while (subrangeItr)
		{
			...
			if (subrange->start.index < subrangeItr->end.index &&
					subrange->end.index > subrangeItr->start.index ||
				(subrange->start.index == RA_INTER_RANGE_START &&
				 subrange->start.index == subrangeItr->start.index) ||
				(subrange->end.index == RA_INTER_RANGE_END &&
				 subrange->end.index == subrangeItr->end.index))
			{
...
bool _livenessRangeStartCompare(raLivenessSubrange_t* lhs, raLivenessSubrange_t* rhs)
{
									 ...
									 PPCRecImlSegment_t* imlSegment)
{
	...
	// std::sort(imlSegment->raInfo.list_subranges.begin(), imlSegment->raInfo.list_subranges.end(),
	// _sortSubrangesByStartIndexDepr);
	...
	while (subrangeItr)
	{
		...
		if (subrangeItr->range->physicalRegister >= 0)
		{
			...
			for (sint32 f = 0; f < liveInfo.liveRangesCount; f++)
			{
				...
				{
					// this should never happen because we try to preventively avoid register
					// conflicts
					assert_dbg();
				}
			}
		...
		uint32 unusedRegisterMask = physRegisterMask; // mask of registers that are currently not
													  // used (does not include range checks)
		if (physRegisterMask != 0)
		{
			allowedPhysRegisterMask =
		...
		if (physRegisterMask == 0)
		{
			struct
			{
				...
				// split current range (this is generally only a good choice when the current range
				// is long but rarely used)
				struct
				{
					sint32 cost;
					sint32 physRegister;
				...
				// explode a inter-segment range (prefer ranges that are not read/written in this
				// segment)
				struct
				{
					...
					sint32 cost;
					sint32 distance; // size of hole
					// note: If we explode a range, we still have to check the size of the hole that
					// becomes available, if too small then we need to add cost of splitting local
					// subrange
				} explodeRange;
				// todo - add more strategies, make cost estimation smarter (for example, in some
				// cases splitting can have reduced or no cost if read/store can be avoided due to
				// data flow)
			} spillStrategies;
			...
			if (subrangeItr->end.index != RA_INTER_RANGE_END)
			{
				...
				// 1) Get the size of the largest possible hole that we can cut into any of the live
				// local subranges 1.1) Check if the hole is large enough to hold the current
				// subrange 2) If yes, cut hole and return false (full retry) 3) If no, try to reuse
				// free register (need to determine how large the region is we can use) 4) If there
				// is no free register or the range is extremely short go back to step 1+2 but
				// additionally split the current subrange at where the hole ends
				...
				if (currentIndex >= 0)
				{
					for (sint32 f = 0; f < liveInfo.liveRangesCount; f++)
					{
						...
						if (candidate->end.index == RA_INTER_RANGE_END)
							continue;
						sint32 distance =
							PPCRecRA_countInstructionsUntilNextUse(candidate, currentIndex);
						if (distance < 2)
							continue; // not even worth the consideration
									  // calculate split cost of candidate
						sint32 cost = PPCRecRARange_estimateAdditionalCostAfterSplit(
							candidate, currentIndex + distance);
						// calculate additional split cost of currentRange if hole is not large
						// enough
						if (distance < requiredSize)
						{
							cost += PPCRecRARange_estimateAdditionalCostAfterSplit(
								subrangeItr, currentIndex + distance);
							// we also slightly increase cost in relation to the remaining length
							// (in order to make the algorithm prefer larger holes)
				...
				if (currentIndex >= 0)
				{
					if (unusedRegisterMask != 0)
					{
						for (sint32 t = 0; t < PPC_X64_GPR_USABLE_REGISTERS; t++)
						{
							...
							sint32 distance =
								PPCRecRA_countInstructionsUntilNextLocalPhysRegisterUse(
									imlSegment, currentIndex, t);
							if (distance < 2)
								continue; // not worth consideration
							// calculate additional cost due to split
							if (distance >= requiredSize)
								assert_dbg(); // should not happen or else we would have selected
											  // this register
							sint32 cost = PPCRecRARange_estimateAdditionalCostAfterSplit(
								subrangeItr, currentIndex + distance);
							// add small additional cost for the remaining range (prefer larger
							// holes)
							cost += (requiredSize - distance) / 10;
				...
				for (sint32 f = 0; f < liveInfo.liveRangesCount; f++)
				{
					...
					if (candidate->end.index != RA_INTER_RANGE_END)
						continue;
					sint32 distance = PPCRecRA_countInstructionsUntilNextUse(
						liveInfo.liveRangeList[f], currentIndex);
					if (distance < 2)
						continue;
					sint32 cost;
				...
				if (spillStrategies.explodeRange.cost != INT_MAX &&
					spillStrategies.explodeRange.cost <=
						spillStrategies.localRangeHoleCutting.cost &&
					spillStrategies.explodeRange.cost <= spillStrategies.availableRegisterHole.cost)
				{
					// explode range
					...
					if (requiredSize > spillStrategies.explodeRange.distance)
						PPCRecRA_splitLocalSubrange(
							ppcImlGenContext, subrangeItr,
							currentIndex + spillStrategies.explodeRange.distance, true);
				}
				else if (spillStrategies.availableRegisterHole.cost != INT_MAX &&
						 spillStrategies.availableRegisterHole.cost <=
							 spillStrategies.explodeRange.cost &&
						 spillStrategies.availableRegisterHole.cost <=
							 spillStrategies.localRangeHoleCutting.cost)
				{
					...
					PPCRecRA_splitLocalSubrange(
						ppcImlGenContext, subrangeItr,
						currentIndex + spillStrategies.availableRegisterHole.distance, true);
				}
				else if (spillStrategies.localRangeHoleCutting.cost != INT_MAX &&
						 spillStrategies.localRangeHoleCutting.cost <=
							 spillStrategies.explodeRange.cost &&
						 spillStrategies.localRangeHoleCutting.cost <=
							 spillStrategies.availableRegisterHole.cost)
				{
					// cut hole
					PPCRecRA_splitLocalSubrange(
						ppcImlGenContext, spillStrategies.localRangeHoleCutting.largestHoleSubrange,
						currentIndex + spillStrategies.localRangeHoleCutting.distance, true);
					// split current subrange if necessary
			...
			else
			{
				...
				// simple but bad solution -> explode the entire range (no longer allow it to cross
				// segment boundaries) better solutions: 1) Depending on the situation, we can
				// explode other ranges to resolve the conflict. Thus we should explode the range
				// with the lowest extra cost
...
void _findSubrangeWriteEndings(raLivenessSubrange_t* subrange, uint32 iterationIndex, sint32 depth,
							   subrangeEndingInfo_t* info)
{
	if (depth >= 30)
	{
	...
	if (imlSegment->nextSegmentBranchNotTaken)
	{
		...
		{
			_findSubrangeWriteEndings(subrange->subrangeBranchNotTaken, iterationIndex, depth + 1,
									  info);
		}
	}
	if (imlSegment->nextSegmentBranchTaken)
	{
		...
		{
			_findSubrangeWriteEndings(subrange->subrangeBranchTaken, iterationIndex, depth + 1,
									  info);
		}
	}
}
...
void PPCRecRA_generateSegmentInstructions(ppcImlGenContext_t* ppcImlGenContext,
										  PPCRecImlSegment_t* imlSegment)
{
	...
	liveInfo.liveRangesCount = 0;
	sint32 index = 0;
	sint32 suffixInstructionCount =
		(imlSegment->imlListCount > 0 &&
		 PPCRecompiler_isSuffixInstruction(imlSegment->imlList + imlSegment->imlListCount - 1))
			? 1
			: 0;
	...
	while (subrangeItr)
	{
		if (subrangeItr->start.index == RA_INTER_RANGE_START)
		{
			...
			if (virtualReg2PhysReg[subrangeItr->range->virtualRegister] != -1)
				assert_dbg();
#endif
			virtualReg2PhysReg[subrangeItr->range->virtualRegister] =
				subrangeItr->range->physicalRegister;
		}
		// next
		subrangeItr = subrangeItr->link_segmentSubrangesGPR.next;
	}
	...
	while (index < imlSegment->imlListCount + 1)
	{
			...
			if (liverange->end.index <= index)
			{
				...
				if (liverange->hasStore)
				{
					PPCRecRA_insertGPRStoreInstruction(
						imlSegment,
						std::min(index, imlSegment->imlListCount - suffixInstructionCount),
						liverange->range->physicalRegister, liverange->range->name);
					index++;
				}
				// remove entry
				liveInfo.liveRangesCount--;
		...
		while (subrangeItr)
		{
			if (subrangeItr->start.index == index)
			{
				...
				if (subrangeItr->_noLoad == false)
				{
					PPCRecRA_insertGPRLoadInstruction(
						imlSegment,
						std::min(index, imlSegment->imlListCount - suffixInstructionCount),
						subrangeItr->range->physicalRegister, subrangeItr->range->name);
					index++;
		...
		if (index < imlSegment->imlListCount)
		{
			...
			PPCRecompiler_replaceGPRRegisterUsageMultiple(
				ppcImlGenContext, imlSegment->imlList + index, inputGpr, replaceGpr);
		}
	...
	for (sint32 f = 0; f < liveInfo.liveRangesCount; f++)
	{
		...
		if (liverange->end.index == RA_INTER_RANGE_END)
		{
			...
			if (liverange->hasStore)
			{
				loadStoreList[storeLoadListLength].registerIndex =
					liverange->range->physicalRegister;
	...
	{
		PPCRecRA_insertGPRStoreInstructions(imlSegment,
	...
	if (storeLoadListLength > 0)
	{
		PPCRecRA_insertGPRLoadInstructions(imlSegment,
										   imlSegment->imlListCount - suffixInstructionCount,
										   loadStoreList, storeLoadListLength);
	}
}
...
{
	// insert empty segments after every non-taken branch if the linked segment has more than one
	// input this gives the register allocator more room to create efficient spill code
	...
	while (segmentIndex < ppcImlGenContext->segmentListCount)
	{
		...
		if (imlSegment->nextSegmentBranchTaken == nullptr ||
			imlSegment->nextSegmentBranchNotTaken == nullptr)
		{
			segmentIndex++;
			continue;
		}
