...
#define PPC_REC_CODE_AREA_START                                                                    \
	(0x00000000) // lower bound of executable memory area. Recompiler expects this address to be 0
...
#define PPC_REC_MAX_VIRTUAL_GPR                                                                    \
	(40) // enough to store 32 GPRs + a few SPRs + temp registers (usually only 1-2)
...
#define PPCREC_IML_OP_FLAG_NOT_EXPANDED                                                            \
	(1 << 2) // set single-precision load instructions to indicate that the value should not be
...
typedef struct
{
	...
	uint8 crMode; // only used when crRegister is valid, used to differentiate between various forms
				  // of condition flag set/clear behavior
	...
	union
	{
		...
		struct
		{
			...
			bool jumpAccordingToSegment; // PPCRecImlSegment_t* destinationSegment; // if set, this
										 // replaces jumpmarkAddress
			uint8 condition; // only used when crRegisterIndex is 8 or above (update: Apparently
							 // only used to mark jumps without a condition? -> Cleanup)
...
{
	sint32 momentaryIndex{}; // index in segment list, generally not kept up to date except if
							 // needed (necessary for loop detection)
	...
	sint32 imlListCount{};
	// segment link
	_PPCRecImlSegment_t* nextSegmentBranchNotTaken{}; // this is also the default for segments where
													  // there is no branch
	...
	uint32 ppcAddrMax{};
	// enterable segments
	bool isEnterable{}; // this segment can be entered from outside the recompiler (no preloaded
						// registers necessary)
	...
	// CR use mask
	uint32 crBitsInput{};	// bits that are expected to be set from the previous segment (read in
							// this segment but not overwritten)
...
struct ppcImlGenContext_t
{
	...
	bool hasFPUInstruction; // if true, PPCEnter macro will create FP_UNAVAIL checks -> Not needed
							// in user mode
	// register allocator info
	struct
	{
...
typedef struct
{
	PPCRecFunction_t* ppcRecompilerFuncTable[PPC_REC_ALIGN_TO_4MB(
		PPC_REC_CODE_AREA_SIZE /
		4)]; // one virtual-function pointer for each potential ppc instruction
	PPCREC_JUMP_ENTRY ppcRecompilerDirectJumpTable[PPC_REC_ALIGN_TO_4MB(
		PPC_REC_CODE_AREA_SIZE / 4)]; // lookup table for ppc offset to native code function
	// x64 data
...
extern void ATTR_MS_ABI (*PPCRecompiler_enterRecompilerCode)(uint64 codeMem,
