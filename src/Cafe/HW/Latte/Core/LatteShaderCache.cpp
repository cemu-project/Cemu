...
FileCache* fc_shaderCacheGeneric =
	nullptr; // contains hardware and Cemu version independent shader information

#define SHADER_CACHE_GENERIC_EXTRA_VERSION                                                         \
	2 // changing this constant will invalidate all hardware-independent cache files
...
void LatteShaderCache_ShowProgress(const std::function<bool(void)>& loadUpdateFunc,
								   bool isPipelines);

void LatteShaderCache_handleDeprecatedCacheFiles(fs::path pathGeneric,
												 fs::path pathGenericPre1_25_0,
												 fs::path pathGenericPre1_16_0);
...
uint32 LatteShaderCache_getShaderCacheExtraVersion(uint64 titleId)
{
	...
	uint32 extraVersion =
		((uint32)(titleId >> 32) + ((uint32)titleId) * 3) + cacheFileVersion + 0xe97af1ad;
	return extraVersion;
}
...
void LatteShaderCache_load()
{
	...
	// get cache file name
	const auto pathGeneric =
		ActiveSettings::GetPath("shaderCache/transferable/{:016x}_shaders.bin", cacheTitleId);
	const auto pathGenericPre1_25_0 = ActiveSettings::GetPath(
		"shaderCache/transferable/{:016x}.bin", cacheTitleId); // before 1.25.0
	const auto pathGenericPre1_16_0 = ActiveSettings::GetPath(
		"shaderCache/transferable/{:08x}.bin", CafeSystem::GetRPXHashBase()); // before 1.16.0

	LatteShaderCache_handleDeprecatedCacheFiles(pathGeneric, pathGenericPre1_25_0,
												pathGenericPre1_16_0);
	...
	fc_shaderCacheGeneric =
		FileCache::Open(pathGeneric.generic_wstring(), false,
						transferableExtraVersion); // legacy extra version (1.25.0 - 1.25.1b)
	if (!fc_shaderCacheGeneric)
		fc_shaderCacheGeneric =
			FileCache::Open(pathGeneric.generic_wstring(), true,
							LatteShaderCache_getShaderCacheExtraVersion(cacheTitleId));
	if (!fc_shaderCacheGeneric)
	{
	...
	std::string tvTexPath = fmt::format(
		"{}/meta/bootTvTex.tga", CafeSystem::GetMlcStoragePath(CafeSystem::GetForegroundTitleId()));
	sint32 status;
	auto fscfile = fsc_open(tvTexPath.c_str(),
	...
	if (fscfile)
	{
		...
		if (size > 0)
		{
			...
			if (backgroundLoaded)
				g_shaderCacheLoaderState.textureId = g_renderer->GenerateTexture(
					file.imageData, {file.imageWidth, file.imageHeight});
		}
	...
	const auto timeLoad =
		std::chrono::duration_cast<std::chrono::milliseconds>(timeLoadEnd - timeLoadStart).count();
	...
	forceLog_printf("Shader cache loaded with %d shaders. Commited mem %dMB. Took %dms",
	...
	auto& io = ImGui::GetIO();
	const auto kPopupFlags = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration |
			...
			if (ImGui::Begin("Background texture", nullptr, kPopupFlags))
			{
				if (g_shaderCacheLoaderState.textureId)
				{
					...
					ImGui::GetWindowDrawList()->AddImage(
						g_shaderCacheLoaderState.textureId,
						ImVec2(paddingLeftAndRight, paddingTopAndBottom),
								   ...
								   bool isPipelines)
{
	...
	while (true)
	{
		...
		// in order to slightly speed up shader loading, we don't update the display if little time
		// passed this also avoids delayed loading in case third party software caps the framerate
		// at 30
		if ((tick_cached() - lastFrameUpdate) <
			std::chrono::milliseconds(1000 / 20)) // -> aim for 20 FPS
			continue;
		...
		if (g_renderer->ImguiBegin(true))
		{
			...
			if (g_shaderCacheLoaderState.textureId)
			{
				...
				ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0);
				ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, {0, 0});
				if (ImGui::Begin("Background texture", nullptr,
								 kPopupFlags | ImGuiWindowFlags_NoBringToFrontOnFocus))
				{
					float imageDisplayWidth = io.DisplaySize.x;
					float imageDisplayHeight = 720 * imageDisplayWidth / 1280;
			...
			if (ImGui::Begin(titleText.c_str(), nullptr, kPopupFlags))
			{
				...
				float percentLoaded;
				if (isPipelines)
					percentLoaded = (float)g_shaderCacheLoaderState.loadedPipelines /
									(float)g_shaderCacheLoaderState.pipelineFileCount;
				else
					percentLoaded = (float)g_shaderCacheLoaderState.loadedShaderFiles /
									(float)g_shaderCacheLoaderState.shaderFileCount;
				ImGui::ProgressBar(percentLoaded, {-1, 0}, "");

				if (isPipelines)
					text = fmt::format("{}/{} ({}%%)", g_shaderCacheLoaderState.loadedPipelines,
									   g_shaderCacheLoaderState.pipelineFileCount,
									   (int)(percentLoaded * 100));
				else
					text = fmt::format("{}/{} ({}%%)", g_shaderCacheLoaderState.loadedShaderFiles,
...
bool LatteShaderCache_updatePipelineLoadingProgress()
{
	...
	return VulkanPipelineStableCache::GetInstance().UpdateLoading(
		g_shaderCacheLoaderState.loadedPipelines, pipelinesMissingShaders);
}
												 ...
												 uint32* contextRegisters, bool usesGeometryShader)
{
	...
	// header
	streamWriter.writeBE<uint8>(1 |
								(SHADER_CACHE_TYPE_VERTEX << 4)); // version and type (shared field)
	...
	// write to cache
	uint64 shaderCacheName =
		LatteShaderCache_getShaderNameInTransferableCache(shaderBaseHash, SHADER_CACHE_TYPE_VERTEX);
	std::span<uint8> dataBlob = streamWriter.getResult();
	fc_shaderCacheGeneric->AddFileAsync({shaderCacheName, shaderAuxHash}, dataBlob.data(),
										dataBlob.size());
}
												   ...
												   uint32 vsRingParameterCount)
{
	...
	MemStreamWriter streamWriter(128 * 1024);
	// header
	streamWriter.writeBE<uint8>(
		1 | (SHADER_CACHE_TYPE_GEOMETRY << 4)); // version and type (shared field)
	streamWriter.writeBE<uint64>(shaderBaseHash);
	streamWriter.writeBE<uint64>(shaderAuxHash);
	...
	// write to cache
	uint64 shaderCacheName = LatteShaderCache_getShaderNameInTransferableCache(
		shaderBaseHash, SHADER_CACHE_TYPE_GEOMETRY);
	std::span<uint8> dataBlob = streamWriter.getResult();
	fc_shaderCacheGeneric->AddFileAsync({shaderCacheName, shaderAuxHash}, dataBlob.data(),
										dataBlob.size());
}
												...
												uint32* contextRegisters, bool usesGeometryShader)
{
	...
	MemStreamWriter streamWriter(128 * 1024);
	streamWriter.writeBE<uint8>(1 |
								(SHADER_CACHE_TYPE_PIXEL << 4)); // version and type (shared field)
	streamWriter.writeBE<uint64>(shaderBaseHash);
	streamWriter.writeBE<uint64>(shaderAuxHash);
	streamWriter.writeBE<uint8>(usesGeometryShader ? 1 : 0);
	// register state
	...
	// write to cache
	uint64 shaderCacheName =
		LatteShaderCache_getShaderNameInTransferableCache(shaderBaseHash, SHADER_CACHE_TYPE_PIXEL);
	std::span<uint8> dataBlob = streamWriter.getResult();
	fc_shaderCacheGeneric->AddFileAsync({shaderCacheName, shaderAuxHash}, dataBlob.data(),
										dataBlob.size());
}

void LatteShaderCache_loadOrCompileSeparableShader(LatteDecompilerShader* shader,
												   uint64 shaderBaseHash, uint64 shaderAuxHash)
{
...
bool LatteShaderCache_readSeparableVertexShader(MemStreamReader& streamReader, uint8 version)
{
	...
	// get fetch shader
	LatteFetchShader::CacheHash fsHash = LatteFetchShader::CalculateCacheHash(
		(uint32*)fetchShaderData.data(), fetchShaderData.size());
	LatteFetchShader* fetchShader = LatteShaderRecompiler_createFetchShader(
		fsHash, lcr->GetRawView(), (uint32*)fetchShaderData.data(), fetchShaderData.size());
	...
	LatteDecompiler_DecompileVertexShader(
		shaderBaseHash, lcr->GetRawView(), vertexShaderData.data(), vertexShaderData.size(),
		fetchShaderList, 1, lcr->GetSpecialStateValues(), usesGeometryShader, &decompilerOutput);
	LatteDecompilerShader* vertexShader = LatteShader_CreateShaderFromDecompilerOutput(
		decompilerOutput, shaderBaseHash, false, shaderAuxHash, lcr->GetRawView());
	// compile
	LatteShader_DumpShader(shaderBaseHash, shaderAuxHash, vertexShader);
	LatteShader_DumpRawShader(shaderBaseHash, shaderAuxHash, SHADER_DUMP_TYPE_VERTEX,
...
bool LatteShaderCache_readSeparableGeometryShader(MemStreamReader& streamReader, uint8 version)
{
	...
	LatteDecompiler_DecompileGeometryShader(
		shaderBaseHash, lcr->GetRawView(), geometryShaderData.data(), geometryShaderData.size(),
		geometryCopyShaderData.data(), geometryCopyShaderData.size(), lcr->GetSpecialStateValues(),
		vsRingParameterCount, &decompilerOutput);
	...
	LatteShader_DumpRawShader(shaderBaseHash, shaderAuxHash, SHADER_DUMP_TYPE_GEOMETRY,
							  geometryShaderData.data(), geometryShaderData.size());
...
bool LatteShaderCache_readSeparablePixelShader(MemStreamReader& streamReader, uint8 version)
{
	...
	LatteDecompilerOutput_t decompilerOutput{};
	LatteDecompiler_DecompilePixelShader(shaderBaseHash, lcr->GetRawView(), pixelShaderData.data(),
										 pixelShaderData.size(), lcr->GetSpecialStateValues(),
	...
	LatteShader_DumpRawShader(shaderBaseHash, shaderAuxHash, SHADER_DUMP_TYPE_PIXEL,
							  pixelShaderData.data(), pixelShaderData.size());
												 ...
												 fs::path pathGenericPre1_16_0)
{
	...
	bool hasOldCacheFiles =
		fs::exists(pathGenericPre1_25_0, ec) || fs::exists(pathGenericPre1_16_0, ec);
	...
	if (hasOldCacheFiles && !hasNewCacheFiles)
	{
		...
		const auto infoMsg =
			L"Outdated shader cache\n\nCemu detected that the shader cache for this game is "
		...
		dialog.SetYesNoLabels(_("Delete outdated cache file [recommended]"),
							  _("Keep outdated cache file"));
