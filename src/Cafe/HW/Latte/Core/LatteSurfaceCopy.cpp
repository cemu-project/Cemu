	...
	Latte::E_DIM dstDim, Latte::E_HWTILEMODE dstTilemode, sint32 dstAA, sint32 dstLevel)
{
	...
	if (srcDim == Latte::E_DIM::DIM_3D && (srcDepth >> srcLevel) == 0 &&
		(srcWidth >> srcLevel) == 0 && (srcHeight >> srcLevel) == 0)
		return;
	else if ((srcWidth >> srcLevel) == 0 && (srcHeight >> srcLevel) == 0)
		return;
	// look up source texture
	LatteTexture* sourceTexture = nullptr;
	LatteTextureView* sourceView = LatteTC_GetTextureSliceViewOrTryCreate(
		srcPhysAddr, srcMipAddr, srcSurfaceFormat, srcTilemode, srcWidth, srcHeight, srcDepth,
		srcPitch, srcSwizzle, srcSlice, srcLevel);
	...
	LatteTextureView* destinationView = LatteTextureViewLookupCache::lookupSlice(
		dstPhysAddr, dstWidth, dstHeight, dstPitch, dstLevel, dstSlice, dstSurfaceFormat);
	if (destinationView)
	...
	if (!destinationTexture)
	{
		...
		destinationView =
			LatteTexture_CreateMapping(dstPhysAddr, dstMipAddr, dstWidth, dstHeight, dstDepth,
									   dstPitch, dstTilemode, dstSwizzle, dstLevel, 1, dstSlice, 1,
									   dstSurfaceFormat, dstDim, Latte::E_DIM::DIM_2D, false);
		destinationTexture = destinationView->baseTexture;
	}
	// copy texture
	if (sourceTexture && destinationTexture)
	{
		...
		if (LatteTexture_doesEffectiveRescaleRatioMatch(
				sourceTexture, sourceView->firstMip, destinationTexture, destinationView->firstMip))
		{
			// adjust copy size
			...
			LatteTexture_scaleToEffectiveSize(sourceTexture, &effectiveCopyWidth,
											  &effectiveCopyHeight, 0);
			// copy slice
			if (sourceView->baseTexture->isDepth != destinationView->baseTexture->isDepth)
			{
				g_renderer->surfaceCopy_copySurfaceWithFormatConversion(
					sourceTexture, sourceView->firstMip, sourceView->firstSlice, destinationTexture,
					destinationView->firstMip, destinationView->firstSlice, copyWidth, copyHeight);
				uint64 eventCounter = LatteTexture_getNextUpdateEventCounter();
				LatteTexture_MarkDynamicTextureAsChanged(destinationTexture->baseView,
														 destinationView->firstSlice,
														 destinationView->firstMip, eventCounter);
			}
			else
			{
				...
				g_renderer->texture_copyImageSubData(
					sourceTexture, sourceView->firstMip, 0, 0, realSrcSlice, destinationTexture,
					texDstMipLevel, 0, 0, realDstSlice, effectiveCopyWidth, effectiveCopyHeight, 1);
				uint64 eventCounter = LatteTexture_getNextUpdateEventCounter();
				LatteTexture_MarkDynamicTextureAsChanged(destinationTexture->baseView,
														 destinationView->firstSlice,
														 texDstMipLevel, eventCounter);
			}
		}
		else
		{
			debug_printf(
				"gx2CP_itHLECopySurface(): Copy texture with non-matching effective size\n");
		}
	...
	if (destinationTexture->width == 8 && destinationTexture->height == 8 &&
		destinationTexture->tileMode == Latte::E_HWTILEMODE::TM_1D_TILED_THIN1)
	{
		forceLogDebug_printf("Texture readback after copy for Bayonetta 2 (phys: 0x%08x)",
							 destinationTexture->physAddress);
