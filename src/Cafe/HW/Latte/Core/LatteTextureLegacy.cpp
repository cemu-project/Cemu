...
{
	TexScaleXY
		perUnit[Latte::GPU_LIMITS::NUM_TEXTURES_PER_STAGE]; // stores
															// actualResolution/effectiveResolution
															// ratio for each texture
...
void LatteTexture_setEffectiveTextureScale(LatteConst::ShaderType shaderType, sint32 texUnit,
										   float u, float v)
{
	cemu_assert_debug(texUnit >= 0 && texUnit < Latte::GPU_LIMITS::NUM_TEXTURES_PER_STAGE);
...
void LatteTexture_ReloadData(LatteTexture* tex, uint32 textureUnit)
{
	...
	{
		if (tex->dim == Latte::E_DIM::DIM_2D_ARRAY || tex->dim == Latte::E_DIM::DIM_2D_ARRAY_MSAA)
		{
			sint32 numSlices = std::max(tex->depth, 1);
			for (sint32 s = 0; s < numSlices; s++)
				LatteTextureLoader_UpdateTextureSliceData(
					tex, textureUnit, s, mip, tex->physAddress, tex->physMipAddress, tex->dim,
					tex->width, tex->height, tex->depth, tex->mipLevels, tex->pitch, tex->tileMode,
					tex->swizzle, true);
		}
		else if (tex->dim == Latte::E_DIM::DIM_CUBEMAP)
		{
			cemu_assert_debug((tex->depth % 6) == 0);
			sint32 numFullCubeMaps = tex->depth / 6; // number of cubemaps (if numFullCubeMaps is >1
													 // then this texture is a cubemap array)
		...
		else
		{
			// load slice 0
			LatteTextureLoader_UpdateTextureSliceData(
				tex, textureUnit, 0, mip, tex->physAddress, tex->physMipAddress, tex->dim,
				tex->width, tex->height, tex->depth, tex->mipLevels, tex->pitch, tex->tileMode,
				tex->swizzle, true);
		}
	}
...
{
	const auto tex = g_renderer->texture_createTextureEx(
		textureUnit, dim, physAddress, physMipAddress, format, width, height, depth, pitch,
		mipLevels, swizzle, tileMode, isDepth);
...
Latte::E_GX2SURFFMT
LatteTexture_ReconstructGX2Format(const Latte::LATTE_SQ_TEX_RESOURCE_WORD1_N& texUnitWord1,
								  const Latte::LATTE_SQ_TEX_RESOURCE_WORD4_N& texUnitWord4)
{
	...
	if (texUnitWord4.get_FORMAT_COMP_X() ==
		Latte::LATTE_SQ_TEX_RESOURCE_WORD4_N::E_FORMAT_COMP::COMP_SIGNED)
		gx2Format |= Latte::E_GX2SURFFMT::FMT_BIT_SIGNED;

	return gx2Format;
}

void LatteTexture_updateTexturesForStage(LatteDecompilerShader* shaderContext,
										 uint32 glBackendBaseTexUnit,
										 _LatteRegisterSetTextureUnit* texRegBase)
{
	for (sint32 z = 0; z < shaderContext->textureUnitListCount; z++)
	{
		...
		if (dim == Latte::E_DIM::DIM_2D_ARRAY || dim == Latte::E_DIM::DIM_3D ||
			dim == Latte::E_DIM::DIM_2D_ARRAY_MSAA || dim == Latte::E_DIM::DIM_1D_ARRAY)
		{
			depth = depth + 1;
		}
		else
		...
		if (dim == Latte::E_DIM::DIM_2D_MSAA)
		{
			...
			// without this we encounter a crash in The Mysterious Cities of Gold: Secret Paths due
			// to it setting mip count to 2 and leaving mip pointer on an invalid uninitialized
			// value
			viewFirstMip = 0;
			viewNumMips = 1;
		}
		...
		if (isDepthSampler == false)
			textureView = LatteTextureViewLookupCache::lookup(
				physAddr, width, height, depth, pitch, viewFirstMip, viewNumMips, viewFirstSlice,
				viewNumSlices, format, dim);
		else
			textureView = LatteTextureViewLookupCache::lookup(
				physAddr, width, height, depth, pitch, viewFirstMip, viewNumMips, viewFirstSlice,
				viewNumSlices, format, dim, true);
		if (textureView == nullptr)
		{
			// create new mapping
			textureView = LatteTexture_CreateMapping(
				physAddr, physMipAddr, width, height, depth, pitch, tileMode, swizzle, viewFirstMip,
				viewNumMips, viewFirstSlice, viewNumSlices, format, dim, dim, isDepthSampler);
			if (textureView == nullptr)
				continue;
		...
		{
			// on OpenGL, texture views and sampler parameters are tied together (we are avoiding
			// sampler objects due to driver bugs) in order to emulate different sampler parameters
			// when a texture is bound multiple times we create extra views
			...
			{
				// Intel driver has issues with textures that have multiple views bound and used by
				// a shader, causes a softlock in BotW therefore we disable this on Intel
			...
			rendererGL->renderstate_updateTextureSettingsGL(shaderContext, textureView,
															textureIndex + glBackendBaseTexUnit,
															word4, textureIndex, isDepthSampler);
		}
		...
		if (textureView->baseTexture->swizzle != swizzle)
		{
			debug_printf("BaseSwizzle diff prev %08x new %08x rt %08x tm %d\n",
						 ...
						 textureView->baseTexture->lastRenderTargetSwizzle,
						 textureView->baseTexture->tileMode);
			if (swizzle == textureView->baseTexture->lastRenderTargetSwizzle)
			{
				// last render to texture updated the swizzle and we can assume the texture data is
				// still valid
				textureView->baseTexture->swizzle =
					textureView->baseTexture->lastRenderTargetSwizzle;
			}
			else
			{
		...
		else if ((viewFirstMip + viewNumMips) > 1 &&
		...
		if (LatteTC_HasTextureChanged(textureView->baseTexture) || swizzleChanged)
		{
			...
			debug_printf("Reload texture 0x%08x res %dx%d memRange %08x-%08x SwizzleChange: %s\n",
						 ...
						 textureView->baseTexture->height, textureView->baseTexture->texDataPtrLow,
						 textureView->baseTexture->texDataPtrHigh, swizzleChanged ? "yes" : "no");
#endif
			...
			g_renderer->texture_bindAndActivateRawTex(textureView->baseTexture,
													  textureIndex + glBackendBaseTexUnit);
		...
		LatteTexture_setEffectiveTextureScale(shaderContext->shaderType, textureIndex, texScaleU,
											  texScaleV);
	}
}

// initialize textures used by the current drawcall
// Sets LatteGPUState.repeatTextureInitialization to true if a new texture mapping was created
// (indicating that this function must be called again) also sets
// LatteGPUState.requiresTextureBarrier to true if texture barrier is required
void LatteTexture_updateTextures()
{
	...
	if (pixelShader)
		LatteTexture_updateTexturesForStage(pixelShader, CEMU_PS_TEX_UNIT_BASE,
											LatteGPUState.contextNew.SQ_TEX_START_PS);
	// vertex shader
	LatteDecompilerShader* vertexShader = LatteSHRC_GetActiveVertexShader();
	cemu_assert_debug(vertexShader != nullptr);
	LatteTexture_updateTexturesForStage(vertexShader, CEMU_VS_TEX_UNIT_BASE,
										LatteGPUState.contextNew.SQ_TEX_START_VS);
	// geometry shader
	LatteDecompilerShader* geometryShader = LatteSHRC_GetActiveGeometryShader();
	if (geometryShader)
		LatteTexture_updateTexturesForStage(geometryShader, CEMU_GS_TEX_UNIT_BASE,
											LatteGPUState.contextNew.SQ_TEX_START_GS);
}
...
void LatteTexture_getEffectiveSize(LatteTexture* texture, sint32* effectiveWidth,
								   sint32* effectiveHeight, sint32* effectiveDepth, sint32 mipLevel)
{
...
void LatteTexture_scaleToEffectiveSize(LatteTexture* texture, sint32* x, sint32* y, sint32 mipLevel)
{
	if (texture->overwriteInfo.hasResolutionOverwrite == false)
		return;
	*x = *x * std::max(1, texture->overwriteInfo.width >> mipLevel) /
		 std::max(1, texture->width >> mipLevel);
	*y = *y * std::max(1, texture->overwriteInfo.height >> mipLevel) /
		 std::max(1, texture->height >> mipLevel);
}
