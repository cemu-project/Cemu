...
#define GPU7_CF_INST_ALU_MASK                                                                      \
	(0x10000) // this mask is used to make sure that there is no collision between any
			  // GPU7_CF_INST_* values (e.g. GPU7_CF_INST_ALU_BREAK and GPU7_CF_INST_POP would
			  // collide due to different encoding but identical value)
...
#define GPU7_ALU_SRC_IS_ANY_CONST(__v)                                                             \
	((__v) >= 248 && (__v) < 253) // special macro to handle all GPU7_ALU_SRC_IS_CONST_*
