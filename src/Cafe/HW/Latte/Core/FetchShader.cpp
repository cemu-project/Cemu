...
void _fetchShaderDecompiler_parseInstruction_VTX_SEMANTIC(LatteFetchShader* parsedFetchShader,
														  uint32* contextRegister,
														  const LatteClauseInstruction_VTX* instr)
{
	uint32 semanticId =
		instr->getFieldSEM_SEMANTIC_ID();		   // location (attribute index inside shader)
	...
	bool isSigned =
		instr->getField_FORMAT_COMP_ALL() == LatteClauseInstruction_VTX::FORMAT_COMP::COMP_SIGNED;
	...
	{
		auto bufferGroupItr = std::find_if(
			parsedFetchShader->bufferGroups.begin(), parsedFetchShader->bufferGroups.end(),
			[bufferIndex](LatteParsedFetchShaderBufferGroup_t& bufferGroup)
			{ return bufferGroup.attributeBufferIndex == bufferIndex; });
	...
	{
		auto bufferGroupItr =
			std::find_if(parsedFetchShader->bufferGroupsInvalid.begin(),
						 parsedFetchShader->bufferGroupsInvalid.end(),
						 [bufferIndex](LatteParsedFetchShaderBufferGroup_t& bufferGroup)
						 { return bufferGroup.attributeBufferIndex == bufferIndex; });
	...
	if (attribGroup->attribCount < (groupAttribIndex + 1))
	{
		...
		attribGroup->attrib = (LatteParsedFetchShaderAttribute_t*)realloc(
			attribGroup->attrib,
			sizeof(LatteParsedFetchShaderAttribute_t) * attribGroup->attribCount);
	}
	...
	if (srcSelX == LatteClauseInstruction_VTX::SRC_SEL::SEL_X)
	{
		cemu_assert_debug(
			fetchType !=
			LatteConst::VertexFetchType2::INSTANCE_DATA); // aluDivisor 0 in combination with
														  // instanced data is not allowed?
	...
	else if (srcSelX == LatteClauseInstruction_VTX::SRC_SEL::SEL_W)
	{
		cemu_assert_debug(
			fetchType ==
			LatteConst::VertexFetchType2::INSTANCE_DATA); // using constant divisor 1 with
														  // per-vertex data seems strange? (divisor
														  // is instance-only)
	...
	{
		// use alu divisor 1
		attribGroup->attrib[groupAttribIndex].aluDivisor =
			(sint32)contextRegister[mmVGT_INSTANCE_STEP_RATE_0 + 0];
	...
	else if (srcSelX == LatteClauseInstruction_VTX::SRC_SEL::SEL_Z)
	{
		// use alu divisor 2
		attribGroup->attrib[groupAttribIndex].aluDivisor =
			(sint32)contextRegister[mmVGT_INSTANCE_STEP_RATE_0 + 1];
		cemu_assert_debug(attribGroup->attrib[groupAttribIndex].aluDivisor > 0);
	}
}

void _fetchShaderDecompiler_parseVTXClause(LatteFetchShader* parsedFetchShader,
										   uint32* contextRegister, std::span<uint8> clauseCode,
										   size_t numInstructions)
{
	...
	while (instr < end)
	{
		...
		{
			_fetchShaderDecompiler_parseInstruction_VTX_SEMANTIC(parsedFetchShader, contextRegister,
																 instr);
		}
		else
		{
...
void _fetchShaderDecompiler_parseCF(LatteFetchShader* parsedFetchShader, uint32* contextRegister,
									std::span<uint8> programCode)
{
	...
	while (cfInstruction < end)
	{
		...
		{
			auto vtxInstruction =
				cfInstruction->getParserIfOpcodeMatch<LatteCFInstruction_DEFAULT>();
			cemu_assert_debug(vtxInstruction->getField_COND() ==
							  LatteCFInstruction::CF_COND::CF_COND_ACTIVE);
			_fetchShaderDecompiler_parseVTXClause(parsedFetchShader, contextRegister,
												  vtxInstruction->getClauseCode(programCode),
												  vtxInstruction->getField_COUNT());
		}
		...
		{
			cemu_assert_debug(
				false); // unusual for fetch shader? They should end with a return instruction
			break;
		}
		cfInstruction++;
	}
...
// can be assumed to be thread-safe, if called simultaneously on the same fetch shader only one
// shader will become registered. The others will be destroyed
														  ...
														  uint32 fsProgramSize)
{
	...
	// otherwise fall back to our broken legacy method (where we assumed fetch shaders had no CF
	// program) this workaround is required to make sure old shader caches dont break
	...
	//...} 			{0x00000002, 0x01800000, 0x00000000, 0x8a000000, 0x1c00a001, 0x27961000,
	// 0x000a0000, ...} 			{0x00000002, 0x01800c00, 0x00000000, 0x8a000000, 0x2c00a001,
	// 0x2c151000, 0x000a0000,
	//...} // size 0x50
	//          {0x00000002, 0x01801000, 0x00000000, 0x8a000000, 0x1c00a001, 0x280d1000, 0x00090000,
	//          ...} // size 0x60
	//			{0x00000002, 0x01801c00, 0x00000000, 0x8a000000, 0x1c00a001, 0x280d1000, 0x00090000,
	//...} // size 0x90
	...
	if ((fsProgramCode[0] & 1) == 0 && fsProgramCode[0] <= 0x30 &&
		(fsProgramCode[1] & ~((3 << 10) | (1 << 19))) == 0x01800000)
	{
		// very likely a CF instruction
		_fetchShaderDecompiler_parseCF(newFetchShader, contextRegister,
									   {(uint8*)fsProgramCode, fsProgramSize});
	}
	else
	{
		while (index < (fsProgramSize / 4))
		{
			...
			{
				// this might be the clause initialization instruction? (Seems to be the first
				// instruction always) todo - upon further investigation, it seems like fetch
				// shaders also start with a CF program. Our implementation doesnt emit one right
				// now
			...
			{
				_fetchShaderDecompiler_parseInstruction_VTX_SEMANTIC(
					newFetchShader, contextRegister,
					(const LatteClauseInstruction_VTX*)(fsProgramCode + index - 1));
				index += 3;
			}
		}
	}
	...
	for (auto& bufferGroup : newFetchShader->bufferGroups)
	{
		for (sint32 i = 0; i < bufferGroup.attribCount; i++)
		{
			...
			uint32 attribAlignment =
				LatteShaderRecompiler_getAttributeAlignment(bufferGroup.attrib + i);
			// fix alignment
		...
		{
			uint32 attribAlignment =
				LatteShaderRecompiler_getAttributeAlignment(bufferGroup.attrib + 0);
			vboOffset = (vboOffset + attribAlignment - 1) & ~(attribAlignment - 1);
		}
	...
	// register in cache
	// its possible that during multi-threaded shader cache loading, two identical (same hash) fetch
	// shaders get created simultaneously we catch and handle this case here. RegisterInCache() is
	// atomic and if another fetch shader is already registered, we abandon the local instance
...
LatteFetchShader::CacheHash LatteFetchShader::CalculateCacheHash(void* programCode,
																 uint32 programSize)
{
	...
	// todo - we should incorporate the value of VGT_INSTANCE_STEP_RATE_0/1 into the hash since it
	// affects the generated LatteFetchShader object
	//        However, this would break compatibility with shader caches and gfx packs due to
	//        altering the shader base hashes
...
{
	// does not hold s_fetchShaderCache for better performance. Be careful not to call this while
	// another thread invokes RegisterInCache()
...
{
	return memory_getPointerFromPhysicalOffset(LatteGPUState.contextRegister[mmSQ_PGM_START_FS + 0]
											   << 8);
}
...
LatteFetchShader* LatteFetchShader::FindByGPUState()
{
	...
	cemu_assert_debug(fsPhysAddr24 < 0x1000000); // should only contain the upper 24 bit of the
												 // address in the lower 24 bit of the register
	...
	if (lookupInfo)
	{
		...
		if (!fetchShader)
		{
			fetchShader = LatteShaderRecompiler_createFetchShader(
				fsHash, LatteGPUState.contextNew.GetRawView(), (uint32*)_getFSProgramPtr(),
				_getFSProgramSize());
...
std::unordered_map<LatteFetchShader::CacheHash, LatteFetchShader*>
	LatteFetchShader::s_fetchShaderByHash;
