...
bool gxShader_checkIfSuccessfullyLinked(GLuint glProgram)
{
	...
	glGetProgramInfoLog(glProgram, std::min(tempLength, infoLogLength), (GLsizei*)&tempLength,
						(GLcharARB*)infoLog);
...
void LatteShader_prepareSeparableUniforms(LatteDecompilerShader* shader)
{
	...
	{
		shader->uniform.loc_remapped =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_remappedVS");
		shader->uniform.loc_uniformRegister =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_uniformRegisterVS");
	}
	else if (shader->shaderType == LatteConst::ShaderType::Geometry)
	{
		shader->uniform.loc_remapped =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_remappedGS");
		shader->uniform.loc_uniformRegister =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_uniformRegisterGS");
	}
	else if (shader->shaderType == LatteConst::ShaderType::Pixel)
	{
		shader->uniform.loc_remapped =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_remappedPS");
		shader->uniform.loc_uniformRegister =
			glGetUniformLocation(shaderGL->GetProgram(), "uf_uniformRegisterPS");
	}
	catchOpenGLError();
	shader->uniform.loc_windowSpaceToClipSpaceTransform =
		glGetUniformLocation(shaderGL->GetProgram(), "uf_windowSpaceToClipSpaceTransform");
	shader->uniform.loc_alphaTestRef =
		glGetUniformLocation(shaderGL->GetProgram(), "uf_alphaTestRef");
	shader->uniform.loc_pointSize = glGetUniformLocation(shaderGL->GetProgram(), "uf_pointSize");
	shader->uniform.loc_fragCoordScale =
		glGetUniformLocation(shaderGL->GetProgram(), "uf_fragCoordScale");
...
GLuint gpu7ShaderGLDepr_compileShader(const std::string& source, uint32_t type)
{
	...
	if (log_length > 0)
	{
		char log[2048]{};
		GLsizei log_size;
		glGetShaderInfoLog(shader_object, std::min(log_length, (GLint)sizeof(log) - 1), &log_size,
						   log);
...
GLuint gpu7ShaderGLDepr_compileVertexShader(const char* shaderSource, sint32 shaderSourceLength)
{
	...
	if (shaderLogLengthInfo > 0)
	{
		...
		glGetShaderInfoLog(shaderObject,
						   std::min<uint32>(shaderLogLengthInfo, sizeof(messageLog) - 1),
						   (GLsizei*)&shaderLogLen, (GLcharARB*)messageLog);
		forceLog_printf("Error/Warning in vertex shader:");
...
GLuint gxShaderDepr_compileRaw(StringBuf* strSourceVS, StringBuf* strSourceFS)
{
	...
	GLuint vertexShader =
		gpu7ShaderGLDepr_compileVertexShader(strSourceVS->c_str(), strSourceVS->getLen());
	glAttachShader(glShaderProgram, vertexShader);
	GLuint fragmentShader =
		gpu7ShaderGLDepr_compileFragmentShader(strSourceFS->c_str(), strSourceFS->getLen());
...
GLuint gxShaderDepr_compileRaw(const std::string& vertex_source, const std::string& fragment_source)
{
	...
	auto vertex_shader = std::async(std::launch::deferred, gpu7ShaderGLDepr_compileShader,
									vertex_source, GL_VERTEX_SHADER);
	auto fragment_shader = std::async(std::launch::deferred, gpu7ShaderGLDepr_compileShader,
