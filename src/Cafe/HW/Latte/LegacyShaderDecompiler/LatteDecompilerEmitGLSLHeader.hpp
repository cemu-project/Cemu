...
{
void _emitUniformVariables(LatteDecompilerShaderContext* decompilerContext,
						   RendererAPI rendererType,
						   LatteDecompilerOutputUniformOffsets& uniformOffsets)
{
	LatteDecompilerShaderResourceMapping& resourceMapping =
		(rendererType == RendererAPI::Vulkan) ? decompilerContext->output->resourceMappingVK
											  : decompilerContext->output->resourceMappingGL;
	...
	if (rendererType == RendererAPI::Vulkan)
	{
		...
		if (decompilerContext->hasUniformVarBlock)
		{
			...
			decompilerContext->shaderSource->addFmt(
				"layout(set = {}, binding = {}) uniform ufBlock" _CRLF "{{" _CRLF,
				(sint32)resourceMapping.setIndex,
				(sint32)resourceMapping.uniformVarsBufferBindingPoint);
		}
	}
	...
	if (decompilerContext->shader->uniformMode == LATTE_DECOMPILER_UNIFORM_MODE_REMAPPED)
	{
		...
		if (shaderType == LatteConst::ShaderType::Vertex)
			shaderSrc->addFmt("uniform ivec4 uf_remappedVS[{}];" _CRLF,
							  (sint32)shader->list_remappedUniformEntries.size());
		else if (shaderType == LatteConst::ShaderType::Pixel)
			shaderSrc->addFmt("uniform ivec4 uf_remappedPS[{}];" _CRLF,
							  (sint32)shader->list_remappedUniformEntries.size());
		else if (shaderType == LatteConst::ShaderType::Geometry)
			shaderSrc->addFmt("uniform ivec4 uf_remappedGS[{}];" _CRLF,
							  (sint32)shader->list_remappedUniformEntries.size());
		else
			debugBreakpoint();
	...
	{
		// here we try to predict the accessed range so we dont have to upload the whole register
		// file we assume that if there is a fixed-index access on an index higher than a relative
		// access, it bounds the prior relative access
		...
		{
			if (accessItr.index > highestAccessIndex ||
				(accessItr.index == highestAccessIndex && accessItr.isRelative &&
				 !highestAccessIndexIsRel))
			{
	...
	{
		if ((decompilerContext->shaderType == LatteConst::ShaderType::Vertex &&
			 !decompilerContext->usesGeometryShader) ||
	...
	// define uf_fragCoordScale which holds the xy scale for render target resolution vs effective
	// resolution
...
void _emitUniformBuffers(LatteDecompilerShaderContext* decompilerContext)
{
	...
	if (decompilerContext->shader->uniformMode == LATTE_DECOMPILER_UNIFORM_MODE_FULL_CBANK)
	{
		for (uint32 i = 0; i < LATTE_NUM_MAX_UNIFORM_BUFFERS; i++)
		{
			...
			cemu_assert_debug(
				decompilerContext->output->resourceMappingGL.uniformBuffersBindingPoint[i] >= 0);
			cemu_assert_debug(
				decompilerContext->output->resourceMappingVK.uniformBuffersBindingPoint[i] >= 0);

			shaderSrc->addFmt(
				"UNIFORM_BUFFER_LAYOUT({}, {}, {}) ",
				(sint32)decompilerContext->output->resourceMappingGL.uniformBuffersBindingPoint[i],
				(sint32)decompilerContext->output->resourceMappingVK.setIndex,
				(sint32)decompilerContext->output->resourceMappingVK.uniformBuffersBindingPoint[i]);

			shaderSrc->addFmt("uniform {}{}" _CRLF,
							  _getShaderUniformBlockInterfaceName(decompilerContext->shaderType),
							  i);
			shaderSrc->add("{" _CRLF);
			shaderSrc->addFmt("vec4 {}{}[{}];" _CRLF,
							  _getShaderUniformBlockVariableName(decompilerContext->shaderType), i,
							  LATTE_GLSL_DYNAMIC_UNIFORM_BLOCK_SIZE);
...
void _emitTextureDefinitions(LatteDecompilerShaderContext* shaderContext)
{
	...
	for (sint32 i = 0; i < LATTE_NUM_MAX_TEX_UNITS; i++)
	{
		...
		src->addFmt("TEXTURE_LAYOUT({}, {}, {}) ",
					(sint32)shaderContext->output->resourceMappingGL.textureUnitToBindingPoint[i],
					(sint32)shaderContext->output->resourceMappingVK.setIndex,
					(sint32)shaderContext->output->resourceMappingVK.textureUnitToBindingPoint[i]);
		...
		if (shaderContext->shader->textureIsIntegerFormat[i])
		{
			...
			if (shaderContext->shader->textureUnitDim[i] == Latte::E_DIM::DIM_1D)
				src->add("usampler1D");
			else if (shaderContext->shader->textureUnitDim[i] == Latte::E_DIM::DIM_2D ||
					 shaderContext->shader->textureUnitDim[i] == Latte::E_DIM::DIM_2D_MSAA)
				src->add("usampler2D");
			else
...
void _emitAttributes(LatteDecompilerShaderContext* decompilerContext)
{
	...
	if (decompilerContext->shader->shaderType == LatteConst::ShaderType::Vertex)
	{
		...
		for (uint32 i = 0; i < LATTE_NUM_MAX_ATTRIBUTE_LOCATIONS; i++)
		{
			...
			{
				cemu_assert_debug(
					decompilerContext->output->resourceMappingGL.attributeMapping[i] >= 0);
				cemu_assert_debug(
					decompilerContext->output->resourceMappingVK.attributeMapping[i] >= 0);
				cemu_assert_debug(
					decompilerContext->output->resourceMappingGL.attributeMapping[i] ==
					decompilerContext->output->resourceMappingVK.attributeMapping[i]);

				shaderSrc->addFmt(
					"ATTR_LAYOUT({}, {}) in uvec4 attrDataSem{};" _CRLF,
					(sint32)decompilerContext->output->resourceMappingVK.setIndex,
					(sint32)decompilerContext->output->resourceMappingVK.attributeMapping[i], i);
			}
		}
	}
}
...
void _emitHeaderMacros(LatteDecompilerShaderContext* decompilerContext)
{
	...
	// Vulkan defines
	src->add("#define ATTR_LAYOUT(__vkSet, __location) layout(set = __vkSet, location = "
			 "__location)" _CRLF);
	src->add("#define UNIFORM_BUFFER_LAYOUT(__glLocation, __vkSet, __vkLocation) layout(set = "
			 "__vkSet, binding = __vkLocation, std140)" _CRLF);
	src->add("#define TEXTURE_LAYOUT(__glLocation, __vkSet, __vkLocation) layout(set = __vkSet, "
			 "binding = __vkLocation)" _CRLF);
	if (decompilerContext->shaderType == LatteConst::ShaderType::Vertex ||
		decompilerContext->shaderType == LatteConst::ShaderType::Geometry)
	{
		...
		if (decompilerContext->analyzer.hasStreamoutWrite)
			src->add("#define XFB_BLOCK_LAYOUT(__bufferIndex, __stride, __location) "
					 "layout(location = __location, xfb_buffer = __bufferIndex, xfb_stride = "
					 "__stride, xfb_offset = 0)" _CRLF);
		...
		{
			src->add("#define SET_POSITION(_v) gl_Position = _v; gl_Position.z = (gl_Position.z + "
					 "gl_Position.w) / 2.0" _CRLF);
		}
	...
	{
		// fCStr_shaderSource->append("#define GET_FRAGCOORD()
		// vec4(gl_FragCoord.xy*uf_fragCoordScale.xy+uf_fragCoordScale.zw,gl_FragCoord.zw)"
		// STR_LINEBREAK);
		src->add("#define GET_FRAGCOORD() "
				 "vec4(gl_FragCoord.xy*uf_fragCoordScale.xy,gl_FragCoord.zw)" _CRLF);
	}
	src->add("#else" _CRLF);
	// OpenGL defines
	src->add("#define ATTR_LAYOUT(__vkSet, __location) layout(location = __location)" _CRLF);
	src->add("#define UNIFORM_BUFFER_LAYOUT(__glLocation, __vkSet, __vkLocation) layout(binding = "
			 "__glLocation, std140) " _CRLF);
	src->add("#define TEXTURE_LAYOUT(__glLocation, __vkSet, __vkLocation) layout(binding = "
			 "__glLocation)" _CRLF);
	if (decompilerContext->shaderType == LatteConst::ShaderType::Vertex ||
		decompilerContext->shaderType == LatteConst::ShaderType::Geometry)
	{
		if (decompilerContext->analyzer.hasStreamoutWrite)
			src->add("#define XFB_BLOCK_LAYOUT(__bufferIndex, __stride, __location) "
					 "layout(xfb_buffer = __bufferIndex, xfb_stride = __stride)" _CRLF);
	...
	{
		src->add("#define GET_FRAGCOORD() "
				 "vec4(gl_FragCoord.xy*uf_fragCoordScale,gl_FragCoord.zw)" _CRLF);
	}
	src->add("#endif" _CRLF);
	...
	{
		auto drawPrimitiveMode =
			decompilerContext->contextRegistersNew->VGT_PRIMITIVE_TYPE.get_PRIMITIVE_MODE();
		...
		// note: The input primitive type is not stored with the geometry shader object (and
		// registers). Therefore we have to rely on whatever primitive mode was set for the draw
		// call.
		...
		uint32 maxVerticesInGS =
			((decompilerContext->contextRegisters[mmSQ_GSVS_RING_ITEMSIZE] & 0x7FFF) * 4) /
			bytesPerVertex;

		if (!decompilerContext->analyzer.hasLoops &&
			decompilerContext->analyzer.numEmitVertex < maxVerticesInGS)
...
void _emitVSExports(LatteDecompilerShaderContext* shaderContext)
{
	...
	for (uint32 i = 0; i < 32; i++)
	{
		if ((parameterMask & (1 << i)) == 0)
			continue;
		uint32 vsSemanticId =
			_getVertexShaderOutParamSemanticId(shaderContext->contextRegisters, i);
		...
		src->add("out");
		src->addFmt(" vec4 passParameterSem{};" _CRLF,
					psInputTable->import[psInputIndex].semanticId);
	}
}
...
void _emitMisc(LatteDecompilerShaderContext* decompilerContext)
{
	...
	else
	{
		...
		if (decompilerContext->shaderType == LatteConst::ShaderType::Geometry)
		{
			...
			uint32 ringItemSize =
				decompilerContext->contextRegisters[mmSQ_GSVS_RING_ITEMSIZE] & 0x7FFF;
			if ((ringItemSize & 0xF) != 0)
				debugBreakpoint();
			if (((decompilerContext->contextRegisters[mmSQ_GSVS_RING_ITEMSIZE] & 0x7FFF) & 0xF) !=
				0)
				debugBreakpoint();
			...
			for (sint32 p = 0; p < decompilerContext->parsedGSCopyShader->numParam; p++)
			{
				if (decompilerContext->parsedGSCopyShader->paramMapping[p].exportType != 2)
					continue;
				src->addFmt(
					"layout(location = {}) out vec4 passG2PParameter{};" _CRLF,
					decompilerContext->parsedGSCopyShader->paramMapping[p].exportParam & 0x7F,
					(sint32)decompilerContext->parsedGSCopyShader->paramMapping[p].exportParam);
			}
		}
		else if (decompilerContext->shaderType == LatteConst::ShaderType::Pixel)
		{
			...
			for (sint32 i = 0; i < psInputTable->count; i++)
			{
				if (psInputTable->import[i].semanticId > LATTE_ANALYZER_IMPORT_INDEX_PARAM_MAX)
					continue;
				uint32 location = psInputTable->import[i].semanticId &
								  0x7F; // todo - the range above 128 has special meaning?
	...
	if ((decompilerContext->shaderType == LatteConst::ShaderType::Vertex ||
		 decompilerContext->shaderType == LatteConst::ShaderType::Geometry) &&
		decompilerContext->analyzer.hasStreamoutEnable)
	{
		if (decompilerContext->useTFViaSSBO)
		{
			...
			{
				src->addFmt(
					"layout(set = {}, binding = {}) buffer StreamoutBuffer" _CRLF,
					decompilerContext->output->resourceMappingVK.setIndex,
					decompilerContext->output->resourceMappingVK.getTFStorageBufferBindingPoint());
				src->add("{" _CRLF);
		...
		else
		{
			...
			for (uint32 i = 0; i < LATTE_NUM_STREAMOUT_BUFFER; i++)
			{
				...
				src->addFmt("XFB_BLOCK_LAYOUT({}, {}, {}) out XfbBlock{} " _CRLF, i, bufferStride,
							locationOffset, i);
				src->add("{" _CRLF);
				src->addFmt("layout(xfb_buffer = {}, xfb_offset = 0) int sb{}[{}];" _CRLF, i, i,
							decompilerContext->output->streamoutBufferStride[i] / 4);
				src->add("};" _CRLF);
...
void emitHeader(LatteDecompilerShaderContext* decompilerContext)
{
	...
	if (dump_shaders_enabled)
		decompilerContext->shaderSource->add(
			"// start of shader inputs/outputs, predetermined by Cemu. Do not touch" _CRLF);
	// macros
	_emitHeaderMacros(decompilerContext);
	// uniform variables
	decompilerContext->shaderSource->add("#ifdef VULKAN" _CRLF);
	_emitUniformVariables(decompilerContext, RendererAPI::Vulkan,
						  decompilerContext->output->uniformOffsetsVK);
	decompilerContext->shaderSource->add("#else" _CRLF);
	_emitUniformVariables(decompilerContext, RendererAPI::OpenGL,
						  decompilerContext->output->uniformOffsetsGL);
