...
class LatteTCGenIR
{
  ...
  public:
	...
	void setVertexShaderContext(const LatteFetchShader* parsedFetchShader,
								const uint32* vtxSemanticTable);

	ZpIR::ZpIRFunction* transcompileLatteToIR(const void* programData, uint32 programSize,
  ...
  private:
	...
	ZpIR::IRReg loadALUOperand(LatteALUSrcSel srcSel, uint8 srcChan, bool isNeg, bool isAbs,
							   bool isRel, uint8 indexMode, const uint32* literalData,
							   ZpIR::DataType typeHint, bool convertOnTypeMismatch);
	void emitALUGroup(const class LatteClauseInstruction_ALU* aluUnit[5],
	...
	// helpers
	void CF_CALL_FS_emitFetchAttribute(LatteParsedFetchShaderAttribute_t& attribute,
