...
/*
	...
	- Since there are 4 bank and 2 pipe bits this means 4*2 = 8 micro tiles (or 8*4 for thick?). But
   the arrangement varies per tilemode (aspect ratio) Allowed layouts: 1x8, 2x4, 4x2
...
void AdjustSurfaceInfo(uint32 elemMode, uint32 expandX, uint32 expandY, uint32* pBpp,
...
void ComputeMipLevelDimensions(uint32* pWidth, uint32* pHeight, uint32* pNumSlices,
							   AddrSurfaceFlags flags, Latte::E_HWSURFFMT format, uint32 mipLevel)
{
	bool isBCn = (uint32)format >= (uint32)Latte::E_HWSURFFMT::HWFMT_BC1 &&
				 (uint32)format <= (uint32)Latte::E_HWSURFFMT::U_HWFMT_BC7;
	...
	else if (mipLevel && flags.inputIsBase)
	{
		...
		if (format != E_HWSURFFMT::U_HWFMT_32_32_32 &&
			format != E_HWSURFFMT::U_HWFMT_32_32_32_FLOAT)
		{
...
E_HWTILEMODE _ComputeSurfaceMipLevelTileMode(E_HWTILEMODE baseTileMode, uint32 bpp, uint32 level,
											 uint32 width, uint32 height, uint32 numSlices,
											 uint32 numSamples, bool isDepth, bool noRecursive)
{
	E_HWTILEMODE result;
	...
	else
	{
		...
		if (level)
		{
			...
			case E_HWTILEMODE::TM_2D_TILED_THIN1:
			case E_HWTILEMODE::TM_3D_TILED_THIN1:
				if (widthAligned < widthAlignFactor * macroTileWidth ||
					heightAligned < macroTileHeight)
					expTileMode = E_HWTILEMODE::TM_1D_TILED_THIN1;
				break;
			case E_HWTILEMODE::TM_2D_TILED_THIN2:
				macroTileWidth >>= 1;
			...
			result = _ComputeSurfaceMipLevelTileMode(expTileMode, bpp, level, widthAligned,
													 heightAligned, numSlicesAligned, numSamples,
													 isDepth, true);
		}
		else
		{
...
void _ComputeSurfaceAlignmentsMacroTiled(E_HWTILEMODE tileMode, uint32 bpp, AddrSurfaceFlags flags,
										 uint32 numSamples, uint32* pBaseAlign, uint32* pPitchAlign,
										 uint32* pHeightAlign, uint32* pMacroWidth,
										 uint32* pMacroHeight)
{
	...
	uint32 pitchAlign = std::max(macroTileWidth, macroTileWidth * (m_pipeInterleaveBytes / bpp /
																   (8 * thickness) / numSamples));
	...
	if (thickness == 1)
		baseAlign =
			std::max(macroTileBytes, (numSamples * heightAlign * bpp * pitchAlign + 7) >> 3);
	else
...
uint32 ComputeSurfaceBankSwappedWidth(E_HWTILEMODE tileMode, uint32 bpp, uint32 numSamples,
									  uint32 pitch)
{
...
void PadDimensions(E_HWTILEMODE tileMode, uint32 padDims, int isCube, int cubeAsArray,
				   uint32* pPitch, uint32 pitchAlign, uint32* pHeight, uint32 heightAlign,
				   uint32* pSlices, uint32 sliceAlign)
{
	uint32 thickness = TM_GetThickness(tileMode);
	if (!padDims)
		padDims = 3;
...
void _ComputeSurfaceAlignmentsMicroTiled(E_HWTILEMODE tileMode, uint32 bpp, AddrSurfaceFlags flags,
										 uint32 numSamples, uint32& baseAlignOut,
										 uint32& pitchAlignOut, uint32& heightAlignOut)
{
	if (bpp == 96 || bpp == 48 || bpp == 24)
		bpp /= 3u;
	uint32 thickness = TM_GetThickness(tileMode);
...
void _ComputeSurfaceAlignmentsLinear(E_HWTILEMODE tileMode, uint32 bpp, AddrSurfaceFlags flags,
									 uint32* pBaseAlign, uint32* pPitchAlign, uint32* pHeightAlign)
{
	cemu_assert_debug(tileMode == E_HWTILEMODE::TM_LINEAR_GENERAL ||
					  tileMode == E_HWTILEMODE::TM_LINEAR_ALIGNED);
...
void _ComputeSurfaceInfoLinear(E_HWTILEMODE tileMode, uint32 bpp, uint32 numSamples, uint32 pitch,
							   uint32 height, uint32 numSlices, uint32 mipLevel, uint32 padDims,
							   AddrSurfaceFlags flags, AddrSurfaceInfo_OUT* pOut)
{
	uint32 heightAlign;
	uint32 pitchAlign;
	uint32 baseAlign;
	...
	PadDimensions(tileMode, padDims, flags.dimCube, flags.cubeAsArray, &expPitch, pitchAlign,
				  &expHeight, heightAlign, &expNumSlices, microTileThickness);
	if (flags.linearWA && mipLevel == 0)
		expPitch *= 3;
...
void _ComputeSurfaceInfoMicroTiled(E_HWTILEMODE tileMode, uint32 bpp, uint32 numSamples,
								   uint32 pitch, uint32 height, uint32 numSlices, uint32 mipLevel,
								   uint32 padDims, AddrSurfaceFlags flags,
								   AddrSurfaceInfo_OUT* pOut)
{
	...
	uint32 pitchAlign;
	uint32 baseAlign;
	_ComputeSurfaceAlignmentsMicroTiled(expTileMode, bpp, flags, numSamples,
										/* outputs: */ baseAlign, pitchAlign, heightAlign);
	PadDimensions(expTileMode, padDims, flags.dimCube, flags.cubeAsArray, &expPitch, pitchAlign,
				  &expHeight, heightAlign, &expNumSlices, microTileThickness);
	pOut->pitch = expPitch;
	pOut->height = expHeight;
	pOut->depth = expNumSlices;
...
void _ComputeSurfaceInfoMacroTiled(E_HWTILEMODE tileMode, E_HWTILEMODE baseTileMode, uint32 bpp,
								   uint32 numSamples, uint32 pitch, uint32 height, uint32 numSlices,
								   uint32 mipLevel, uint32 padDims, AddrSurfaceFlags flags,
								   AddrSurfaceInfo_OUT* pOut)
{
	...
	if (tileMode != baseTileMode && mipLevel != 0 && TM_IsThickAndMacroTiled(baseTileMode) &&
		!TM_IsThickAndMacroTiled(tileMode))
	{
		_ComputeSurfaceAlignmentsMacroTiled(baseTileMode, bpp, flags, numSamples, &baseAlign,
											&pitchAlign, &heightAlign, &macroWidth, &macroHeight);
		if (expPitch < pitchAlign * pitchAlignFactor || expHeight < heightAlign)
		{
			_ComputeSurfaceInfoMicroTiled(E_HWTILEMODE::TM_1D_TILED_THIN1, bpp, numSamples, pitch,
										  height, numSlices, mipLevel, padDims, flags, pOut);
			return;
		}
	}
	_ComputeSurfaceAlignmentsMacroTiled(tileMode, bpp, flags, numSamples, &baseAlign, &pitchAlign,
										&heightAlign, &macroWidth, &macroHeight);
...
COMPUTE_SURFACE_RESULT ComputeSurfaceInfoEx(const AddrSurfaceInfo_IN* pIn,
											AddrSurfaceInfo_OUT* pOut)
{
	...
	if (flags.fmask)
		...
		tileMode = _ComputeSurfaceMipLevelTileMode(tileMode, bpp, mipLevel, pitch, height,
												   numSlices, numSamples, flags.depth, false);
	switch (tileMode)
	{
	case E_HWTILEMODE::TM_LINEAR_GENERAL:
	case E_HWTILEMODE::TM_LINEAR_ALIGNED:
		_ComputeSurfaceInfoLinear(tileMode, bpp, numSamples, pitch, height, numSlices, mipLevel,
								  padDims, flags, pOut);
		pOut->hwTileMode = tileMode;
		break;
	case E_HWTILEMODE::TM_1D_TILED_THIN1:
	case E_HWTILEMODE::TM_1D_TILED_THICK:
		_ComputeSurfaceInfoMicroTiled(tileMode, bpp, numSamples, pitch, height, numSlices, mipLevel,
									  padDims, flags, pOut);
		break;
	case E_HWTILEMODE::TM_2D_TILED_THIN1:
	case E_HWTILEMODE::TM_2D_TILED_THIN2:
	...
	case E_HWTILEMODE::TM_3B_TILED_THIN1:
	case E_HWTILEMODE::TM_3B_TILED_THICK:
		_ComputeSurfaceInfoMacroTiled(tileMode, baseTileMode, bpp, numSamples, pitch, height,
									  numSlices, mipLevel, padDims, flags, pOut);
		break;
	default:
...
void RestoreSurfaceInfo(uint32 elemMode, uint32 expandX, uint32 expandY, uint32* pBpp,
						uint32* pWidth, uint32* pHeight)
{
	if (pBpp)
	{
...
{
	if (GetFillSizeFieldsFlags() == 1 &&
		(pIn->size != sizeof(AddrSurfaceInfo_IN) || pOut->size != sizeof(AddrSurfaceInfo_OUT)))
		return COMPUTE_SURFACE_RESULT::BAD_SIZE_FIELD;
	cemu_assert_debug(pIn->bpp <= 128);
	ComputeMipLevelDimensions(&pIn->width, &pIn->height, &pIn->numSlices, pIn->flags, pIn->format,
							  pIn->mipLevel);
...
void GX2CalculateSurfaceInfo(Latte::E_GX2SURFFMT surfaceFormat, uint32 surfaceWidth,
							 uint32 surfaceHeight, uint32 surfaceDepth, E_DIM surfaceDim,
							 E_GX2TILEMODE surfaceTileMode, uint32 surfaceAA, uint32 level,
							 AddrSurfaceInfo_OUT* pSurfOut, bool optimizeForDepthBuffer,
							 bool optimizeForScanBuffer)
{
	...
	if (surfaceTileMode == E_GX2TILEMODE::TM_LINEAR_SPECIAL)
	{
		...
		pSurfOut->height =
			(~(blockSize - 1) & (pSurfOut->height + blockSize - 1)) / (uint64)blockSize;
		...
		pSurfOut->surfSize =
			pSurfOut->bpp * numSamples * pSurfOut->depth * pSurfOut->height * pSurfOut->pitch >> 3;
	...
	else
	{
		...
		{
			// cemuLog_log(LogType::Force, "Unexpected TileMode {} in AddrLib",
			// (uint32)surfaceTileMode);
...
uint32 CalculateMipOffset(Latte::E_GX2SURFFMT format, uint32 width, uint32 height, uint32 depth,
						  E_DIM dim, E_HWTILEMODE tileMode, uint32 swizzle, uint32 surfaceAA,
						  sint32 mipIndex)
{
	...
	{
		GX2CalculateSurfaceInfo(format, width, height, depth, dim, MakeGX2TileMode(tileMode),
								surfaceAA, level, &surfaceInfo);
		if (level)
		{
			uint32 pad = 0;
			...
			pad += (surfaceInfo.baseAlign - (currentMipOffset % surfaceInfo.baseAlign)) %
				   surfaceInfo.baseAlign;
...
// For thick-tiled surfaces this returns the area of the whole thick tile (4 slices per thick tile)
// and the relative slice index within the tile is returned in subSliceIndex
void CalculateMipAndSliceAddr(uint32 physAddr, uint32 physMipAddr, Latte::E_GX2SURFFMT format,
							  uint32 width, uint32 height, uint32 depth, Latte::E_DIM dim,
							  Latte::E_HWTILEMODE tileMode, uint32 swizzle, uint32 surfaceAA,
							  sint32 mipIndex, sint32 sliceIndex, uint32* outputSliceOffset,
							  uint32* outputSliceSize, sint32* subSliceIndex)
{
	...
	if (mipIndex == 0) // make sure surfaceInfo is initialized
		GX2CalculateSurfaceInfo(format, width, height, depth, dim, MakeGX2TileMode(tileMode),
								surfaceAA, 0, &surfaceInfo);
	uint32 sliceOffset = 0;
	uint32 sliceSize = 0;

	// surfaceInfo.sliceSize isn't always correct (especially when depth is misaligned with 4 for
	// THICK tile modes?) so we calculate it manually this formula only works because both pitch and
	// height are aligned to micro/macro blocks by GX2CalculateSurfaceInfo, normally we would have
	// to use the tile dimensions to calculate the size
