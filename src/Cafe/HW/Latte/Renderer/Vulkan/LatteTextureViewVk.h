...
{
  public:
	LatteTextureViewVk(VkDevice device, class LatteTextureVk* texture, Latte::E_DIM dim,
	...
	{
		list_descriptorSets.erase(
			std::remove(list_descriptorSets.begin(), list_descriptorSets.end(), dsInfo),
			list_descriptorSets.end());
	};
  ...
  private:
	...
	// each texture view holds one Vulkan image view per swizzle mask. Image views are only
	// instantiated when requested via GetViewRGBA/GetSamplerView since a large majority of texture
	// views will only have 1 or 2 instantiated image views, we use a small fixed-size cache and
	// only allocate the larger map (m_fallbackCache) if necessary
	...
	VkDevice m_device;
	VkFormat m_format;
	std::vector<struct VkDescriptorSetInfo*>
		list_descriptorSets; // list of descriptors sets referencing this view
