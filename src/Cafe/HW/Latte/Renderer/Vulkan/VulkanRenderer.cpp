...
bool VulkanRenderer::ImguiBegin(bool mainWindow)
{
	...
	ImGui_ImplVulkan_NewFrame(m_state.currentCommandBuffer, chainInfo.GetBackBuffer().frameBuffer, chainInfo.getExtent());
...
void VulkanRenderer::ClearColorbuffer(bool padView)
{
	...
	VkClearColorValue clearColor{ 0, 0, 0, 0 };
	ClearColorImageRaw(chainInfo.GetBackBuffer().image, 0, 0, clearColor, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
}
