...
struct FSAIpcCommand
{
	union
	{
		...
		struct
		{
			...
			uint8 mode[12]; // +0x284 note: code seems to access this value like it has a size of
							// 0x10 but the actual struct element is only 12 bytes? Maybe a typo (10
							// instead of 0x10 in the struct def)
