...
std::string __FSATranslatePath(FSAClient* fsaClient, std::string_view input,
...
FSCVirtualFile* __FSAOpenNode(FSAClient* client, std::string_view path, FSC_ACCESS_FLAG accessFlags,
							  sint32& fscStatus)
{
...
FSStatus __FSAOpenFile(FSAClient* client, const char* path, const char* accessModifierStr,
					   sint32* fileHandle)
{
	*fileHandle = FS_INVALID_HANDLE_VALUE;
	...
	bool isAppend = false; // todo: proper support for this (all write operations should move cursor
	...
	else if (strcmp(accessModifierStr, "r+") == 0)
	{
		...
		accessModifier = FSC_ACCESS_FLAG::READ_PERMISSION | FSC_ACCESS_FLAG::WRITE_PERMISSION |
						 FSC_ACCESS_FLAG::FILE_ALLOW_CREATE; // create if non exists, read, write
	}
	else if (strcmp(accessModifierStr, "w") == 0)
	{
		accessModifier = FSC_ACCESS_FLAG::READ_PERMISSION | FSC_ACCESS_FLAG::WRITE_PERMISSION |
						 FSC_ACCESS_FLAG::FILE_ALWAYS_CREATE; // create new file & write
		truncateFile = true;
	}
	else if (strcmp(accessModifierStr, "w+") == 0)
	{
		accessModifier = FSC_ACCESS_FLAG::READ_PERMISSION | FSC_ACCESS_FLAG::WRITE_PERMISSION |
						 FSC_ACCESS_FLAG::FILE_ALWAYS_CREATE; // create new file & read & write
		truncateFile = true;
	}
	...
	{
		cemu_assert_debug(false); // a+ is kind of special. Writing always happens at the end but
								  // the read cursor can dynamically move
		// but Cafe OS might not support this. Needs investigation.
		// this also used to be FILE_ALWAYS_CREATE in 1.26.2 and before
		accessModifier = FSC_ACCESS_FLAG::READ_PERMISSION | FSC_ACCESS_FLAG::WRITE_PERMISSION |
						 FSC_ACCESS_FLAG::FILE_ALLOW_CREATE;
		isAppend = true;
	}
	else
		cemu_assert_debug(false);
	...
	*fileHandle = fsFileHandle;
	cemuLog_log(LogType::File, "Open file {} (access: {} result: ok handle: 0x{})", path,
				accessModifierStr, (uint32)*fileHandle);
	return (FSStatus)FS_RESULT::SUCCESS;
}
...
FSStatus __FSAOpenDirectory(FSAClient* client, std::string_view path, sint32* dirHandle)
{
	*dirHandle = FS_INVALID_HANDLE_VALUE;
	sint32 fscStatus;
	FSCVirtualFile* fscFile = __FSAOpenNode(
		client, path, FSC_ACCESS_FLAG::OPEN_DIR | FSC_ACCESS_FLAG::OPEN_FILE, fscStatus);
	if (!fscFile)
	...
	*dirHandle = fsDirHandle;
	cemuLog_log(LogType::File, "Open directory {} (result: ok handle: 0x{})", path,
				(uint32)*dirHandle);
	return (FSStatus)FS_RESULT::SUCCESS;
}
...
bool __FSA_GetStatFromFSCFile(FSCVirtualFile* fscFile, FSStat_t* fsStatOut)
{
	...
	{
		// note: Only quota (save) directories have the size field set. For other directories it's
		// zero. Hyrule Warriors relies on the size field being zero for /vol/content/data/.
		// Otherwise it will try to read it like a file and get stuck in an endless loop. Art
		// Academy reads the size for save directories
...
FSStatus __FSA_GetFileStat(FSAClient* client, const char* path, FSStat_t* fsStatOut)
{
	sint32 fscStatus;
	FSCVirtualFile* fscFile = __FSAOpenNode(
		client, path, FSC_ACCESS_FLAG::OPEN_FILE | FSC_ACCESS_FLAG::OPEN_DIR, fscStatus);
	if (!fscFile)
		return FSA_convertFSCtoFSStatus(fscStatus);
...
FSStatus FSAProcessCmd_queryInfo(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	void* queryResult =
		memory_getPointerFromVirtualOffset(_swapEndianU32(fullCmd->returnValueMPTR));
	// handle query
...
FSStatus FSAProcessCmd_getStatFile(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	FSStat_t* statOut =
		(FSStat_t*)memory_getPointerFromVirtualOffset(_swapEndianU32(fullCmd->returnValueMPTR));
	FSCVirtualFile* fscFile = sFileHandleTable.GetByHandle(fileHandle);
	if (!fscFile)
		return (FSStatus)FS_RESULT::NOT_FOUND;
...
FSStatus FSAProcessCmd_openFile(FSAClient* client, FSAIpcCommand* cmd)
{
	coreinit::FSCmdBlockBody_t* fullCmd = (coreinit::FSCmdBlockBody_t*)cmd;
	sint32 fileHandle = 0;
	FSStatus fsStatus = __FSAOpenFile(client, (char*)cmd->cmdOpenFile.path,
									  (char*)cmd->cmdOpenFile.mode, &fileHandle);
...
FSStatus FSAProcessCmd_readDir(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	FSDirEntry_t* dirEntryOut =
		(FSDirEntry_t*)memory_getPointerFromVirtualOffset(_swapEndianU32(fullCmd->returnValueMPTR));
...
FSStatus FSAProcessCmd_closeDir(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	if (!fscFile)
	{
		forceLogDebug_printf("CloseDir: Invalid handle (0x%08x)",
							 (sint32)cmd->cmdReadDir.dirHandle);
		return (FSStatus)FS_RESULT::ERR_PLACEHOLDER;
	}
...
FSStatus FSAProcessCmd_appendFile(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	cemuLog_force("FSAProcessCmd_appendFile(): size 0x{:08x} count 0x{:08x} (todo)\n",
				  _swapEndianU32(cmd->cmdAppendFile.size),
				  _swapEndianU32(cmd->cmdAppendFile.count));
#endif
...
FSStatus FSAProcessCmd_getCwd(FSAClient* client, FSAIpcCommand* cmd)
{
	...
	char* pathOutput =
		(char*)memory_getPointerFromVirtualOffset(_swapEndianU32(fullCmd->returnValueMPTR));
	sint32 pathOutputMaxLen = _swapEndianU32(fullCmd->transferSize);
	cemu_assert(pathOutputMaxLen > 0);
	sint32 fscStatus = FSC_STATUS_OK;
	strncpy(pathOutput, client->workingDirectory.data(),
			std::min(client->workingDirectory.size() + 1, (size_t)pathOutputMaxLen));
...
void FSAHandleCommandIoctl(FSAClient* client, IPCCommandBody* cmd, uint32 operationId, void* ptrIn,
						   void* ptrOut)
{
...
void FSAIoThread()
{
	...
	while (true)
	{
		...
		else if (cmd->cmdId == IPCCommandId::IOS_IOCTL)
		{
			...
			FSAHandleCommandIoctl(sFSAClientArray.data() + clientHandle, cmd, cmd->args[0],
								  MEMPTR<void>(cmd->args[1]), MEMPTR<void>(cmd->args[3]));
		}
...
void Initialize()
{
	...
	sFSAIoMsgQueue = (IOSMsgQueueId)IOS_CreateMessageQueue(_m_sFSAIoMsgQueueMsgBuffer.GetPtr(),
														   _m_sFSAIoMsgQueueMsgBuffer.GetCount());
