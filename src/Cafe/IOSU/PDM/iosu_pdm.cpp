...
{
	chrono_d::sys_days startDateDay(chrono_d::year(2000) / chrono_d::January / chrono_d::day(1));
	chrono_d::sys_days currentDateDay =
		chrono_d::floor<chrono_d::days>(std::chrono::system_clock::now());
...
void LoadPlaystats()
{
	...
	if (PlayStats.fs->GetSize() != (NUM_PLAY_STATS_ENTRIES * 20 + 4))
	{
		...
		// dont delete the existing file in case it could still be salvaged (todo) and instead just
		// dont track play time
		return;
	}
...
PlayStatsEntry* PlayStats_GetEntry(uint64 titleId)
{
	...
	{
		if (PlayStats.entry[i].titleIdHigh == titleIdHigh &&
			PlayStats.entry[i].titleIdLow == titleIdLow)
...
void CreatePlayDiary()
{
	...
	if (!PlayDiaryData.fs)
	{
		cemuLog_log(
			LogType::Force,
			"Failed to read or write PlayDiary.dat, playtime tracking will not be possible");
	}
	// write header
...
void LoadPlayDiary()
{
	...
	// read header
	if (PlayDiaryData.fs->readData(&PlayDiaryData.header, sizeof(PlayDiaryHeader)) !=
		sizeof(PlayDiaryHeader))
	{
	...
	if (PlayDiaryData.header.readIndex > NUM_PLAY_DIARY_ENTRIES_MAX ||
		PlayDiaryData.header.writeIndex > NUM_PLAY_DIARY_ENTRIES_MAX)
	{
		cemuLog_log(LogType::Force, "Bad value in play diary header (read={} write={})",
					(uint32)PlayDiaryData.header.readIndex,
					(uint32)PlayDiaryData.header.writeIndex);
		PlayDiaryData.header.readIndex =
			PlayDiaryData.header.readIndex % NUM_PLAY_DIARY_ENTRIES_MAX;
		PlayDiaryData.header.writeIndex =
			PlayDiaryData.header.writeIndex % NUM_PLAY_DIARY_ENTRIES_MAX;
	}
	// read entries and set any not-yet-written entries to zero
	uint32 readBytes = PlayDiaryData.fs->readData(PlayDiaryData.entry, NUM_PLAY_DIARY_ENTRIES_MAX *
																		   sizeof(PlayDiaryEntry));
...
bool GetStatForGamelist(uint64 titleId, GameListStat& stat)
{
	...
	for (auto& gameEntry : GetConfig().game_cache_entries)
	{
		...
		if (gameEntry.legacy_last_played != 0)
		{
			...
			if (stat.last_played.year == 0 ||
				std::tie(legacyYear, legacyMonth, legacyDay) >
					std::tie(stat.last_played.year, stat.last_played.month, stat.last_played.day))
			{
...
void TimeTrackingThread(uint64 titleId)
{
	...
	while (true)
	{
		...
		uint32 elapsedMinutes =
			std::chrono::duration_cast<std::chrono::minutes>(currentTime - startTime).count();
