...
void IPCService::ServiceThread()
{
	...
	while (true)
	{
		...
		else if (cmd->cmdId == IPCCommandId::IOS_IOCTLV)
		{
			...
			IPCServiceResponse* serviceResponse =
				MEMPTR<IPCServiceResponse>(vecOut->basePhys).GetPtr();

			serviceResponse->nnResultCode =
				(uint32)ServiceCall(serviceRequest->serviceId, nullptr, nullptr);
			IOS_ResourceReply(cmd, IOS_ERROR_OK);
			continue;
		}
		else
