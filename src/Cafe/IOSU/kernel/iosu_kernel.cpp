...
IOSMsgQueueId IOS_CreateMessageQueue(IOSMessage* messageArray, uint32 messageCount)
{
	...
	auto it = std::find_if(sMsgQueuePool.begin(), sMsgQueuePool.end(),
						   [](const IOSMessageQueue& q) { return q.msgArraySize == 0; });
...
IOS_ERROR _IPCDestroyResourceHandle(IOSDevHandle devHandle)
{
	...
	{
		cemuLog_log(LogType::Force, "_IPCDispatchToResourceManager(): Resource manager destroyed "
									"before all IPC commands were processed");
		return IOS_ERROR_INVALID;
	}
...
IOS_ERROR _IPCDispatchToResourceManager(IOSDevHandle devHandle, IOSDispatchableCommand* dispatchCmd)
{
	...
	if (r != IOS_ERROR_OK)
		cemuLog_log(LogType::Force, "_IPCDispatchToResourceManager(): SendMessage returned {}",
					(sint32)r);
	return r;
}
...
{
	cemu_assert(dispatchCmd >= sIPCDispatchableCommandPool.GetPtr() &&
				dispatchCmd <
					sIPCDispatchableCommandPool.GetPtr() + sIPCDispatchableCommandPool.GetCount());
...
void IPCSubmitFromCOS(uint32 ppcCoreIndex, IPCCommandBody* cmd)
{
	...
	if (r < 0)
	{
		...
		// in non-error case the device handler will send the result asynchronously via
		// IOS_ResourceReply
	}
}
...
IOS_ERROR IOS_ResourceReply(IPCCommandBody* cmd, IOS_ERROR result)
{
	...
	if (dispatchCmd->originalBody->cmdId == IPCCommandId::IOS_OPEN)
	{
		...
		if (IOS_ResultIsError(result))
		{
			cemuLog_log(LogType::Force,
						"IOS_ResourceReply(): Target device triggered an error on IOS_OPEN");
		...
		{
			cemu_assert(_IPCAssignDispatchTargetHandle(devHandle, (IOSDevHandle)result) ==
						IOS_ERROR_OK);
	...
	else if (dispatchCmd->originalBody->cmdId == IPCCommandId::IOS_CLOSE)
	{
		...
		{
			cemuLog_log(LogType::Force,
						"IOS_ResourceReply(): Target device triggered an error on IOS_CLOSE");
		}
