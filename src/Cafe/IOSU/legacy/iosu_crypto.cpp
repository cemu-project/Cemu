...
{
	// load OTP dump
	if (std::ifstream otp_file(GetPath(ActiveSettings::GetUserDataPath(), "otp.bin"), std::ifstream::in | std::ios::binary); otp_file.is_open())
	{
	...
	if (std::ifstream seeprom_file(GetPath(ActiveSettings::GetUserDataPath(),"seeprom.bin"), std::ifstream::in | std::ios::binary); seeprom_file.is_open())
	{
...
sint32 iosuCrypt_checkRequirementsForOnlineMode(std::wstring& additionalErrorInfo)
{
	...
	const auto otp_file = GetPath(ActiveSettings::GetUserDataPath(), "otp.bin");
	...
	const auto seeprom_file = GetPath(ActiveSettings::GetUserDataPath(), "seeprom.bin");
