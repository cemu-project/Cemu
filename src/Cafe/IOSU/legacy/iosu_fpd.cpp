	...
	if (GetConfig().notification.friends)
	{
		...
		if (type == NexFriends::NOTIFICATION_TYPE::NOTIFICATION_TYPE_ONLINE)
		{
			...
			if (g_fpd.nexFriendSession &&
				g_fpd.nexFriendSession->getPendingFriendRequestCount() > 0)
				g_friend_notifications.emplace_back(
					fmt::format("You have {} pending friend request(s)",
								g_fpd.nexFriendSession->getPendingFriendRequestCount()),
					5000);
		}
		else
		{
			...
			if (!msg_format.empty())
			{
				...
				g_friend_notifications.emplace_back(fmt::format(fmt::runtime(msg_format), name),
													5000);
			}
		}
	}
...
void convertMultiByteStringToBigEndianWidechar(const char* input, uint16be* output,
											   sint32 maxOutputLength)
{
...
void startFriendSession()
{
	...
	NAPI::ACTGetNexTokenResult nexTokenResult =
		NAPI::ACT_GetNexToken_WithCache(authInfo, 0x0005001010001C00, 0x0000, 0x00003200);
	if (nexTokenResult.isValid())
	{
		...
		g_fpd.nexFriendSession = new NexFriends(
			hostIp, nexTokenResult.nexToken.port, "ridfebb9", myPid,
			nexTokenResult.nexToken.nexPassword, nexTokenResult.nexToken.token, accountId,
			(uint8*)&miiData, (wchar_t*)screenName, (uint8)countryCode, g_fpd.myPresence);
...
void handleRequest_GetFriendList(iosuFpdCemuRequest_t* fpdCemuRequest, bool getAll)
{
	...
	uint32 pidCount = 0;
	g_fpd.nexFriendSession->getFriendPIDs(
		temporaryPidList, &pidCount, fpdCemuRequest->getFriendList.startIndex,
		std::min<uint32>(sizeof(temporaryPidList) / sizeof(temporaryPidList[0]),
						 fpdCemuRequest->getFriendList.maxCount),
		getAll);
...
void handleRequest_GetFriendRequestList(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	g_fpd.nexFriendSession->getFriendRequestPIDs(
		temporaryPidList, &pidCount, fpdCemuRequest->getFriendList.startIndex,
		std::min<uint32>(sizeof(temporaryPidList) / sizeof(temporaryPidList[0]),
						 fpdCemuRequest->getFriendList.maxCount),
		true, false);
...
void setFriendDataFromNexFriend(friendData_t* friendData, nexFriend* frd)
{
	...
	// screenname
	convertMultiByteStringToBigEndianWidechar(frd->nnaInfo.principalInfo.mii.miiNickname,
											  friendData->screenname,
											  sizeof(friendData->screenname) / sizeof(uint16be));
...
void setFriendDataFromNexFriendRequest(friendData_t* friendData, nexFriendRequest* frdReq,
									   bool isIncoming)
{
	memset(friendData, 0, sizeof(friendData_t));
	// setup friend data
	...
	// screenname
	convertMultiByteStringToBigEndianWidechar(frdReq->principalInfo.mii.miiNickname,
											  friendData->screenname,
											  sizeof(friendData->screenname) / sizeof(uint16be));

	convertMultiByteStringToBigEndianWidechar(
		frdReq->message.commentStr.c_str(), friendData->requestExtraData.comment,
		sizeof(friendData->requestExtraData.comment) / sizeof(uint16be));
	...
	convertFPDTimestampToDate(frdReq->message.expireTimestamp,
							  &friendData->requestExtraData.uknData1);
}

void setFriendRequestFromNexFriendRequest(friendRequest_t* friendRequest, nexFriendRequest* frdReq,
										  bool isIncoming)
{
	...
	memcpy(friendRequest->miiData, frdReq->principalInfo.mii.miiData,
		   sizeof(friendRequest->miiData));

	convertMultiByteStringToBigEndianWidechar(
		frdReq->message.commentStr.c_str(), friendRequest->message,
		sizeof(friendRequest->message) / sizeof(friendRequest->message[0]));
	convertMultiByteStringToBigEndianWidechar(
		frdReq->principalInfo.mii.miiNickname, friendRequest->screenname,
		sizeof(friendRequest->screenname) / sizeof(friendRequest->screenname[0]));
...
void handleRequest_GetFriendRequestListEx(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	{
		uint32 pid = fpdCemuRequest->getFriendListEx.pidList[i];
		friendRequest_t* friendRequest =
			fpdCemuRequest->getFriendRequestListEx.friendRequest.GetPtr() + i;
		nexFriendRequest frdReq;
		bool incoming = false;
		if (!g_fpd.nexFriendSession->getFriendRequestByPID(frdReq, &incoming, pid))
		{
...
SysAllocator<uint32, 32>
	_fpCallbackResultArray; // use a ring buffer of results to avoid overwriting the result when
							// multiple callbacks are queued at the same time
...
void handleFPCallback(fpAsyncCallback_t* fpCallback, uint32 resultCode)
{
	...
	*resultPtr = resultCode;
	...
	coreinitAsyncCallback_add(fpCallback->funcMPTR, 2,
							  memory_getVirtualOffsetFromPointer(resultPtr),
							  fpCallback->customParam);
}
...
void handleResultCB_GetBasicInfoAsync(NexFriends* nexFriends, uint32 result, void* custom)
{
	...
	if (result != 0)
	{
		handleFPCallback(
			&cbInfo->fpCallback,
			0x80000000); // todo - properly translate internal error to nn::fp error code
	...
	for (sint32 i = 0; i < cbInfo->count; i++)
	{
		...
		convertMultiByteStringToBigEndianWidechar(principalBasicInfo->mii.miiNickname,
												  basicInfo->screenname,
												  sizeof(basicInfo->screenname) / sizeof(uint16be));
...
void handleRequest_GetBasicInfoAsync(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	getBasicInfoAsyncParams_t* cbInfo =
		(getBasicInfoAsyncParams_t*)malloc(sizeof(getBasicInfoAsyncParams_t));
	...
	g_fpd.nexFriendSession->requestPrincipleBaseInfoByPID(principalBasicInfo, pidList, count,
														  handleResultCB_GetBasicInfoAsync, cbInfo);
}
...
void handleRequest_RemoveFriendAsync(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	g_fpd.nexFriendSession->removeFriend(fpdCemuRequest->addOrRemoveFriend.pid,
										 std::bind(handleResponse_addOrRemoveFriend,
												   std::placeholders::_1,
												   fpdCemuRequest->addOrRemoveFriend.funcPtr,
												   fpdCemuRequest->addOrRemoveFriend.custom));
}
...
void handleRequest_MarkFriendRequestAsReceivedAsync(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	// skipped for now
	g_fpd.nexFriendSession->markFriendRequestsAsReceived(
		messageIds, count,
		std::bind(handleResponse_MarkFriendRequestAsReceivedAsync, std::placeholders::_1,
				  fpdCemuRequest->markFriendRequest.funcPtr,
				  fpdCemuRequest->markFriendRequest.custom));
}
...
void handleRequest_CancelFriendRequestAsync(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	bool isIncoming;
	if (g_fpd.nexFriendSession->getFriendRequestByMessageId(
			frq, &isIncoming, fpdCemuRequest->cancelOrAcceptFriendRequest.messageId))
	{
		g_fpd.nexFriendSession->removeFriend(
			frq.principalInfo.principalId,
			std::bind(handleResponse_cancelMyFriendRequest, std::placeholders::_1,
					  frq.principalInfo.principalId,
					  fpdCemuRequest->cancelOrAcceptFriendRequest.funcPtr,
					  fpdCemuRequest->cancelOrAcceptFriendRequest.custom));
	}
	else
	{
		fpdCemuRequest->returnCode = 0x80000000;
...
void handleRequest_AcceptFriendRequestAsync(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	if (g_fpd.nexFriendSession->getFriendRequestByMessageId(
			frq, &isIncoming, fpdCemuRequest->cancelOrAcceptFriendRequest.messageId))
	{
		g_fpd.nexFriendSession->acceptFriendRequest(
			fpdCemuRequest->cancelOrAcceptFriendRequest.messageId,
			std::bind(handleResponse_acceptFriendRequest, std::placeholders::_1,
					  frq.principalInfo.principalId,
					  fpdCemuRequest->cancelOrAcceptFriendRequest.funcPtr,
					  fpdCemuRequest->cancelOrAcceptFriendRequest.custom));
	}
...
void handleRequest_AddFriendRequest(iosuFpdCemuRequest_t* fpdCemuRequest)
{
	...
	g_fpd.nexFriendSession->addFriendRequest(fpdCemuRequest->addFriendRequest.pid, msg.data(),
											 std::bind(handleResponse_addFriendRequest,
													   std::placeholders::_1,
													   fpdCemuRequest->addFriendRequest.funcPtr,
													   fpdCemuRequest->addFriendRequest.custom));
}
		...
		if (g_fpd.asyncLoginCallback.func != MPTR_NULL)
		{
			if (g_fpd.nexFriendSession->isOnline())
			{
				*_fpdAsyncLoginRetCode.GetPtr() = 0x00000000;
				coreinitAsyncCallback_add(g_fpd.asyncLoginCallback.func, 2,
										  _fpdAsyncLoginRetCode.GetMPTR(),
										  g_fpd.asyncLoginCallback.customParam);
	...
	while (true)
	{
		...
		if (ioQueueEntry->request == IOSU_FPD_REQUEST_CEMU)
		{
			iosuFpdCemuRequest_t* fpdCemuRequest =
				(iosuFpdCemuRequest_t*)ioQueueEntry->bufferVectors[0].buffer.GetPtr();
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_LOGIN_ASYNC)
			{
				...
				{
					// offline mode
					*_fpdAsyncLoginRetCode.GetPtr() =
						0; // if we return 0x80000000 here then Splatoon softlocks during boot
					coreinitAsyncCallback_add(fpdCemuRequest->loginAsync.funcPtr, 2,
											  _fpdAsyncLoginRetCode.GetMPTR(),
											  fpdCemuRequest->loginAsync.custom);
				}
			}
			else if (fpdCemuRequest->requestCode == IOSU_FPD_IS_ONLINE)
			{
				fpdCemuRequest->resultU32.u32 =
					g_fpd.nexFriendSession ? g_fpd.nexFriendSession->isOnline() : 0;
			}
			else if (fpdCemuRequest->requestCode == IOSU_FPD_IS_PREFERENCE_VALID)
			{
				fpdCemuRequest->resultU32.u32 =
					1; // todo (if this returns 0, the friend app will show the first-time-setup
					   // screen and ask the user to configure preferences)
			}
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_GET_FRIEND_ACCOUNT_ID)
			{
				...
				{
					char* nnidOutput =
						fpdCemuRequest->getFriendAccountId.accountIds.GetPtr() + i * 17;
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_GET_FRIEND_SCREENNAME)
			{
				...
				for (sint32 i = 0; i < fpdCemuRequest->getFriendScreenname.count; i++)
				{
					uint16be* screennameOutput =
						fpdCemuRequest->getFriendScreenname.nameList.GetPtr() + i * 11;
					...
					{
						convertMultiByteStringToBigEndianWidechar(
							frd.nnaInfo.principalInfo.mii.miiNickname, screennameOutput, 11);
						if (fpdCemuRequest->getFriendScreenname.languageList.IsNull() == false)
							fpdCemuRequest->getFriendScreenname.languageList.GetPtr()[i] =
								frd.nnaInfo.principalInfo.regionGuessed;
						continue;
					}
					bool incoming = false;
					if (g_fpd.nexFriendSession->getFriendRequestByPID(frdReq, &incoming, pid))
					{
						convertMultiByteStringToBigEndianWidechar(
							frdReq.principalInfo.mii.miiNickname, screennameOutput, 11);
						if (fpdCemuRequest->getFriendScreenname.languageList.IsNull() == false)
							fpdCemuRequest->getFriendScreenname.languageList.GetPtr()[i] =
								frdReq.principalInfo.regionGuessed;
						continue;
					}
					screennameOutput[0] = '\0';
				}
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_GET_FRIEND_PRESENCE)
			{
				...
				{
					friendPresence_t* presenceOutput =
						(friendPresence_t*)(fpdCemuRequest->getFriendPresence.presenceList +
											i * sizeof(friendPresence_t));
					...
					if (g_fpd.nexFriendSession->getFriendByPID(frd, pid))
					{
						...
						memcpy(presenceOutput->gameMode.appSpecificData,
							   frd.presence.appSpecificData, 0x14);
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_GET_FRIEND_RELATIONSHIP)
			{
				...
				{
					uint8* relationshipOutput =
						(fpdCemuRequest->getFriendRelationship.relationshipList + i);
			...
			else if (fpdCemuRequest->requestCode == IOSU_FPD_ADD_FRIEND)
			{
				...
				*_fpdAsyncAddFriendRetCode.GetPtr() = 0;
				coreinitAsyncCallback_add(fpdCemuRequest->addOrRemoveFriend.funcPtr, 2,
										  _fpdAsyncAddFriendRetCode.GetMPTR(),
										  fpdCemuRequest->addOrRemoveFriend.custom);
			}
