...
struct
{
	...
	// nim packages
	// note: Seems like scope.rpx expects the number of packages to never change after the initial
	// GetNum call?
...
void iosuNim_thread()
{
	...
	while (true)
	{
		...
		{
			iosuNimCemuRequest_t* nimCemuRequest =
				(iosuNimCemuRequest_t*)ioQueueEntry->bufferVectors[0].buffer.GetPtr();
			if (nimCemuRequest->requestCode == IOSU_NIM_GET_ICON_DATABASE_ENTRY)
			{
				nimCemuRequest->returnCode =
					nim_getIconDatabaseEntry(nimCemuRequest->titleId, nimCemuRequest->ptr.GetPtr());
			}
			...
			else if (nimCemuRequest->requestCode == IOSU_NIM_GET_PACKAGES_INFO)
			{
				...
				uint32 maxCount = nimCemuRequest->maxCount;
				uint64* titleIdList = (uint64*)nimCemuRequest->ptr.GetPtr();
				titlePackageInfo_t* packageInfo =
					(titlePackageInfo_t*)nimCemuRequest->ptr2.GetPtr();
