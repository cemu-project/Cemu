...
static_assert(offsetof(BossNbdlHeader, encryptedHeader) == 0x20,
			  "offsetof(BossNbdlHeader, encryptedHeader)");
...
curl_slist* append_header_param(struct curl_slist* list, const char* format, TArgs&&... args)
{
	return curl_slist_append(
		list, fmt::format(fmt::runtime(format), std::forward<TArgs>(args)...).c_str());
}
...
size_t task_write_callback(char* ptr, size_t size, size_t nmemb, void* userdata)
{
	...
	// forceLogDebug_printf("task_write_callback: %d (processed: %d)", writeByteSize,
	// task->processed_length);
	if (writeByteSize > 0)
	{
...
size_t task_header_callback(char* ptr, size_t size, size_t nitems, void* userdata)
{
	...
	// forceLogDebug_printf("task_header_callback: len %d (%d) and type %d", task->content_length,
	// task->result_buffer.capacity(), task->content_type); forceLogDebug_printf("\t%s", ptr);
	return size * nitems;
}
...
static CURLcode task_sslctx_function(CURL* curl, void* sslctx, void* param)
{
	...
	{
		forceLogDebug_printf("sslctx_function: adding client cert: %d",
							 (int)task_settings->settings[TaskSetting::kClientCert]);
		if (!iosuCrypto_addClientCertificate(sslctx,
											 task_settings->settings[TaskSetting::kClientCert]))
			assert_dbg();
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			{
				forceLogDebug_printf("sslctx_function: adding ca cert: %d",
									 (int)task_settings->settings[location]);
...
{
	const auto it =
		std::find_if(g_boss.tasks.begin(), g_boss.tasks.end(),
					 [taskId, accountId, titleId](const Task& task)
					 {
						 return 0 == strncmp(taskId, task.task_id, sizeof(Task::task_id)) &&
								accountId == task.account_id && titleId == task.title_id;
					 });
...
bool parse_xml_content(Task& task)
{
	...
	if (doc.Parse((const char*)task.result_buffer.data(), task.processed_length) !=
		tinyxml2::XML_SUCCESS)
		return false;

	for (tinyxml2::XMLElement* sheet = doc.FirstChildElement("TaskSheet"); sheet;
		 sheet = sheet->NextSiblingElement("TaskSheet"))
	{
		const auto files = sheet->FirstChildElement("Files");
		if (!files)
			continue;

		for (tinyxml2::XMLElement* file = files->FirstChildElement("File"); file;
			 file = file->NextSiblingElement("File"))
		{
			...
			task.queued_files.emplace(file_name->GetText(), data_id->IntText(), file_type,
									  url->GetText(), size->IntText());
		}
	}
...
void boss_storage_fad_append_or_update(BossStorageFadFile& fad_file, const char* name,
									   uint32 data_id, uint64 timestamp)
{
	for (auto& entry : fad_file.entries)
	{
		if (entry.fileNameId == 0 || strcmp(entry.name, name) == 0)
		{
			...
			entry.timestampRelated =
				(uint32)(timestamp - kTimeStampConvertSeconds); // time since 2000
			return;
		}
	}
}
...
uint32 task_run(const char* taskId, uint32 accountId, uint64 titleId)
{
	...
	forceLogDebug_printf("task run state: %d | exec: %d (tasks: %d)", it->turn_state,
						 it->exec_count, g_boss.tasks.size());
...
https://npts.app.nintendo.net/p01/tasksheet/%s/%s/%s?c=%s&l=%s
	...
	list_headerParam = append_header_param(list_headerParam, "X-Boss-UniqueId: {:05x}",
										   ((titleId >> 8) & 0xFFFFF)); // %05x
	list_headerParam =
		append_header_param(list_headerParam, "X-BOSS-TitleId: /usr/packages/title/{:016x}",
							titleId); // "/usr/packages/title/%016llx"
	...
	if (it->task_settings.taskType == kRawDlTaskSetting)
	{
		...
		strncpy(serviceToken, (char*)&it->task_settings.settings[TaskSetting::kServiceToken],
				TaskSetting::kServiceTokenLen);
		list_headerParam =
			append_header_param(list_headerParam, "X-Nintendo-ServiceToken: {}", serviceToken);
	...
	else
	{
		...
		const char* countryCode =
			NCrypto::GetCountryAsString(Account::GetCurrentAccount().GetCountry());

		char boss_code[0x20];
		strncpy(boss_code, (char*)&it->task_settings.settings[TaskSetting::kBossCode],
				TaskSetting::kBossCodeLen);

		sprintf(url, "https://npts.app.nintendo.net/p01/tasksheet/%s/%s/%s?c=%s&l=%s", "1",
				boss_code, it->task_id, countryCode, languageCode);
	}
	...
	case ContentType::kText:
		forceLogDebug_printf("task_run returns text: %.*s", it->content_length,
							 it->result_buffer.data());
		break;
	}
	...
	if (!it->queued_files.empty())
	{
		...
		try
		{
			fs::path path = ActiveSettings::GetMlcPath(
				"usr/boss/{:08x}/{:08x}/user/common/data/{}", (uint32)(it->title_id >> 32),
				(uint32)(it->title_id & 0xFFFFFFFF), taskIdStr);
			if (!fs::exists(path))
				fs::create_directories(path);

			char targetFileName[TaskSetting::kFileNameLen + 1]{};
			strncpy(targetFileName, (char*)&it->task_settings.settings[TaskSetting::kNbdlFileName],
					TaskSetting::kFileNameLen);
			...
			while (!it->queued_files.empty())
			{
				...
				fs::path db_file = ActiveSettings::GetMlcPath(
					"usr/boss/{:08x}/{:08x}/user/common/{:08x}/{}", (uint32)(it->title_id >> 32),
					(uint32)(it->title_id & 0xFFFFFFFF), it->account_id, taskIdStr);
				...
				{
					uint64 timestamp =
						(uint64)currentEntry->timestampRelated + kTimeStampConvertSeconds;
				...
				if (curl_result != CURLE_OK)
				{
					forceLogDebug_printf("task_run curl failed on file download (%d): %s > %s",
										 curl_result, file.file_name.c_str(), file.url.c_str());
					if (hasFileName)
					{
						turnstate = kError;
						break;
				...
				if (it->processed_file_size != file.size)
				{
					forceLogDebug_printf(
						"task_run file download size mismatch: %s > %s > %d from %d bytes",
						file.file_name.c_str(), file.url.c_str(), it->processed_file_size,
						file.size);
					if (hasFileName)
					{
						turnstate = kError;
				...
				uint8 bossAesKey[16] = {
					0x39, 0x70, 0x57, 0x35, 0x58, 0x70, 0x34, 0x58,
					0x37, 0x41, 0x7a, 0x30, 0x71, 0x5a, 0x70, 0x74}; // "9pW5Xp4X7Az0qZpt"
				...
				// file must be padded to 16 byte alignment for AES decryption (padding is cut off
				// after decryption)
				...
				// decrypt header
				AES128CTR_transform(it->file_buffer.data() +
										offsetof(BossNbdlHeader, encryptedHeader),
									sizeof(BossNbdlHeader::encryptedHeader), bossAesKey, aesNonce);
				// decrypt everything else
				AES128CTR_transform(it->file_buffer.data() + sizeof(BossNbdlHeader),
									file_size - sizeof(BossNbdlHeader), bossAesKey, aesNonce);
				...
				try
				{
					...
					new_file.write((char*)it->file_buffer.data() + sizeof(BossNbdlHeader),
								   it->processed_file_size - sizeof(BossNbdlHeader));
					new_file.flush();
					new_file.close();

					boss_storage_fad_append_or_update(fad_content, file.file_name.c_str(),
													  file.data_id, filetime);
					std::ofstream fad_file_updated(db_file, std::ios::out | std::ios::binary |
																std::ios::trunc);
	...
	if (it->task_settings.taskType == kRawDlTaskSetting)
	{
		...
		if (it->task_settings.settings[TaskSetting::kDirectoryName] != '\0')
			strncpy(directoryName, (char*)&it->task_settings.settings[TaskSetting::kDirectoryName],
					TaskSetting::kDirectoryNameLen);
		else
			strncpy(directoryName, it->task_id, TaskSetting::kDirectoryNameLen);

		char fileName[TaskSetting::kFileNameLen + 1]{};
		strncpy(fileName, (char*)&it->task_settings.settings[TaskSetting::kFileName],
				TaskSetting::kFileNameLen);

		//  mcl01\usr\boss\00050000\1018dd00\user\<persistentId>\<storageName>\<filename>
		fs::path path = ActiveSettings::GetMlcPath(
			"usr/boss/{:08x}/{:08x}/user/{:08x}", (uint32)(it->title_id >> 32),
...
bool task_wait(const char* taskId, uint32 accountId, uint64 titleId, uint32 wait_state,
			   uint32 timeout = 0)
{
	const auto it = get_task(taskId, accountId, titleId);
	if (it == g_boss.tasks.cend())
	...
	{
		if (timeout != 0 &&
			(uint32)std::chrono::duration_cast<std::chrono::seconds>(tick_cached() - start)
					.count() >= timeout)
		{
...
uint32 task_register_immediate_run(const char* taskId, uint32 accountId, uint64 titleId,
								   void* settings)
{
	g_boss.tasks.emplace_back(taskId, accountId, titleId, (TaskSetting*)settings);
...
std::pair<uint32, uint64> task_get_content_length(const char* taskId, uint32 accountId,
												  uint64 titleId)
{
	const auto it = get_task(taskId, accountId, titleId);
	return it != g_boss.tasks.cend() ? std::make_pair(it->exec_count, it->content_length)
									 : std::make_pair(0u, (uint64)0);
}

std::pair<uint32, uint64> task_get_processed_length(const char* taskId, uint32 accountId,
													uint64 titleId)
{
	const auto it = get_task(taskId, accountId, titleId);
	return it != g_boss.tasks.cend() ? std::make_pair(it->exec_count, it->processed_length)
									 : std::make_pair(0u, (uint64)0);
}

std::pair<uint32, uint32> task_get_http_status_code(const char* taskId, uint32 accountId,
													uint64 titleId)
{
	const auto it = get_task(taskId, accountId, titleId);
	return it != g_boss.tasks.cend() ? std::make_pair(it->exec_count, it->http_status_code)
									 : std::make_pair(0u, (uint32)0);
}

std::pair<uint32, uint32> task_get_turn_state(const char* taskId, uint32 accountId, uint64 titleId)
{
	const auto it = get_task(taskId, accountId, titleId);
	return it != g_boss.tasks.cend() ? std::make_pair(it->exec_count, it->turn_state)
									 : std::make_pair(0u, (uint32)0);
}
...
void boss_thread()
{
	...
	while (true)
	{
		...
		{
			iosuBossCemuRequest_t* cemu_request =
				(iosuBossCemuRequest_t*)ioQueueEntry->bufferVectors[0].buffer.GetPtr();
			...
			{
				cemu_request->returnCode =
					task_wait(cemu_request->taskId, account_id, title_id, cemu_request->waitState,
							  cemu_request->timeout);
			}
			else if (cemu_request->requestCode == IOSU_NN_BOSS_TASK_REGISTER)
			{
				cemu_request->returnCode = task_register(cemu_request->taskId, account_id, title_id,
														 cemu_request->settings);
			}
			else if (cemu_request->requestCode == IOSU_NN_BOSS_TASK_IS_REGISTERED)
			{
				cemu_request->returnCode =
					task_is_registered(cemu_request->taskId, account_id, title_id) ? TRUE : FALSE;
			}
			else if (cemu_request->requestCode == IOSU_NN_BOSS_TASK_REGISTER_FOR_IMMEDIATE_RUN)
			{
				cemu_request->returnCode = task_register_immediate_run(
			...
			{
				cemu_request->returnCode =
					task_stop_scheduling(cemu_request->taskId, account_id, title_id);
			}
			else
				assert_dbg();
		}
