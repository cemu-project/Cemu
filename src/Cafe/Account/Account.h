	...
	auto format(const OnlineAccountError v, FormatContext& ctx)
	{
		...
		case OnlineAccountError::kNoAccountId:
			return formatter<string_view>::format(
				"AccountId missing (The account is not connected to a NNID)", ctx);
		case OnlineAccountError::kNoPasswordCached:
			return formatter<string_view>::format(
				"IsPasswordCacheEnabled is set to false (The remember password option on your Wii "
				"U must be enabled for this account before dumping it)",
				ctx);
		case OnlineAccountError::kPasswordCacheEmpty:
			return formatter<string_view>::format(
				"AccountPasswordCache is empty (The remember password option on your Wii U must be "
				"enabled for this account before dumping it)",
				ctx);
		case OnlineAccountError::kNoPrincipalId:
			return formatter<string_view>::format("PrincipalId missing", ctx);
		default:
			break;
		}
...
struct OnlineValidator
{
	...
	bool IsValid() const
	{
		return valid_account && otp == FileState::Ok && seeprom == FileState::Ok &&
			   missing_files.empty();
	}
