...
bool FSCVirtualFile_Host::fscDirNext(FSCDirEntry* dirEntry)
{
	...
	if (!m_dirIterator)
	{
		...
		if (!m_dirIterator)
		{
			cemuLog_force("Failed to iterate directory: {}",
						  _utf8Wrapper(m_path->generic_u8string()));
			return false;
		}
	}
...
FSCVirtualFile* FSCVirtualFile_Host::OpenFile(const fs::path& path, FSC_ACCESS_FLAG accessFlags,
											  sint32& fscStatus)
{
	if (!HAS_FLAG(accessFlags, FSC_ACCESS_FLAG::OPEN_FILE) &&
		!HAS_FLAG(accessFlags, FSC_ACCESS_FLAG::OPEN_DIR))
...
class fscDeviceHostFSC : public fscDeviceC
{
  public:
	...
	bool fscDeviceCreateDir(std::wstring_view path, void* ctx, sint32* fscStatus) override
	{
		...
		if (fs::exists(path))
		{
			if (!fs::is_directory(dirPath))
				cemuLog_force("CreateDir: {} already exists but is not a directory",
							  _utf8Wrapper(dirPath));
	...
	bool fscDeviceRename(std::wstring_view srcPath, std::wstring_view dstPath, void* ctx,
						 sint32* fscStatus) override
	{
		*fscStatus = FSC_STATUS_OK;
...
void fscDeviceHostFS_mapBaseDirectories_deprecated()
{
	...
	fsc_mount("/cemuBossStorage/", (mlc / "usr/boss/").generic_wstring().c_str(),
			  &fscDeviceHostFSC::instance(), NULL, FSC_PRIORITY_BASE);
	fsc_mount("/vol/storage_mlc01/", (mlc / "").generic_wstring().c_str(),
			  &fscDeviceHostFSC::instance(), NULL, FSC_PRIORITY_BASE);
}
...
bool FSCDeviceHostFS_Mount(const char* mountPath, const wchar_t* hostFSPath, sint32 priority)
{
	...
	return fsc_mount(mountPath, hostTargetPath.c_str(), &fscDeviceHostFSC::instance(), nullptr,
