...
void LoadMainExecutable()
{
	...
	uint8* baseRpxData = fsc_extractFile(
		!_pathToBaseExecutable.empty() ? _pathToBaseExecutable.c_str() : _pathToExecutable.c_str(),
		&baseRpxSize, FSC_PRIORITY_BASE);
...
fs::path getTitleSavePath()
{
	...
	return ActiveSettings::GetMlcPath("usr/save/{:08X}/{:08X}/user/", (uint32)(titleId >> 32),
...
void InfoLog_TitleLoaded()
{
	...
	cemuLog_log(LogType::Force, "TitleId: {:08x}-{:08x}", (uint32)(titleId >> 32),
				(uint32)(titleId & 0xFFFFFFFF));
	...
	cemuLog_force("Save path:   {}{}", _utf8Wrapper(effectiveSavePath),
				  saveDirExists ? "" : " (not present)");
	...
	cemuLog_log(LogType::Force, "gameprofile path: {}",
				g_current_game_profile->IsLoaded() ? gameProfilePath
...
void InfoLog_PrintActiveSettings()
{
	...
	// settings to log:
	forceLog_printf("CPU-Mode: %s%s", fmt::format("{}", ActiveSettings::GetCPUMode()).c_str(),
					g_current_game_profile->GetCPUMode().has_value() ? " (gameprofile)" : "");
	forceLog_printf(
		"Load shared libraries: %s%s",
		ActiveSettings::LoadSharedLibrariesEnabled() ? "true" : "false",
		g_current_game_profile->ShouldLoadSharedLibraries().has_value() ? " (gameprofile)" : "");
	forceLog_printf(
		"Use precompiled shaders: %s%s",
		fmt::format("{}", ActiveSettings::GetPrecompiledShadersOption()).c_str(),
		g_current_game_profile->GetPrecompiledShadersState().has_value() ? " (gameprofile)" : "");
	forceLog_printf("Full sync at GX2DrawDone: %s",
					ActiveSettings::WaitForGX2DrawDoneEnabled() ? "true" : "false");
	if (ActiveSettings::GetGraphicsAPI() == GraphicAPI::kVulkan)
	{
		forceLog_printf("Async compile: %s",
						GetConfig().async_compile.GetValue() ? "true" : "false");
...
struct
{
	...
	const char* mlcPath;
} shareddataDef[] = {0xFFCAFE01,
					 2,
					 "CafeCn.ttf",
					 "resources/sharedFonts/CafeCn.ttf",
					 "sys/title/0005001b/10042400/content/CafeCn.ttf",
					 0xFFCAFE02,
					 2,
					 "CafeKr.ttf",
					 "resources/sharedFonts/CafeKr.ttf",
					 "sys/title/0005001b/10042400/content/CafeKr.ttf",
					 0xFFCAFE03,
					 2,
					 "CafeStd.ttf",
					 "resources/sharedFonts/CafeStd.ttf",
					 "sys/title/0005001b/10042400/content/CafeStd.ttf",
					 0xFFCAFE04,
					 2,
					 "CafeTw.ttf",
					 "resources/sharedFonts/CafeTw.ttf",
					 "sys/title/0005001b/10042400/content/CafeTw.ttf"};
...
uint32 loadSharedData()
{
	...
	if (hasAllShareddataFiles)
	{
		...
		SharedDataEntry* shareddataTable =
			(SharedDataEntry*)memory_getPointerFromVirtualOffset(0xF8000000);
		memset(shareddataTable, 0, sizeof(SharedDataEntry) * numEntries);
		uint8* dataWritePtr =
			memory_getPointerFromVirtualOffset(0xF8000000 + sizeof(SharedDataEntry) * numEntries);
		// setup entries
		for (sint32 i = 0; i < numEntries; i++)
		{
			...
			if (!fontFile)
			{
				cemuLog_log(LogType::Force, "Failed to load shared font {}",
							shareddataDef[i].fileName);
				continue;
			}
		...
		forceLog_printfW(L"COS: System fonts found. Generated shareddata (%dKB)",
						 (uint32)(dataWritePtr - (uint8*)shareddataTable) / 1024);
...
void cemu_initForGame()
{
	...
	PPCTimer_start();
	// this must happen after the RPX/RPL files are mapped to memory (coreinit sets up heaps so that
	// they don't overwrite RPX/RPL data)
	osLib_load();
	// link all modules
	...
	if (isLaunchTypeELF)
	{
		...
		memory_writeU32(0x801504,
						rpl_mapHLEImport(nullptr, "coreinit", "OSDynLoad_FindExport", true));
	}
	else
	{
		// replace any known function signatures with our HLE implementations and patch bugs in the
		// games
		GamePatch_scan();
	}
	...
	coreinit::OSRunThread(initialThread, PPCInterpreter_makeCallableExportDepr(coreinit_start), 0,
						  nullptr);
...
STATUS_CODE LoadAndMountForegroundTitle(TitleId titleId)
{
	...
	if (!sGameInfo_ForegroundTitle.IsValid())
	{
		cemuLog_log(LogType::Force,
					"Mounting failed: Game meta information is either missing, inaccessible or not "
					"valid (missing or invalid .xml files in code and meta folder)");
	...
	// mount base
	if (!titleBase.Mount("/vol/content", "content", FSC_PRIORITY_BASE) ||
		!titleBase.Mount(GetInternalVirtualCodeFolder(), "code", FSC_PRIORITY_BASE))
	{
	...
	if (titleUpdate.IsValid())
	{
		...
		// mount update
		if (!titleUpdate.Mount("/vol/content", "content", FSC_PRIORITY_PATCH) ||
			!titleUpdate.Mount(GetInternalVirtualCodeFolder(), "code", FSC_PRIORITY_PATCH))
		{
			cemuLog_log(LogType::Force, "Mounting failed");
			return STATUS_CODE::UNABLE_TO_MOUNT;
		}
	...
	if (!aocList.empty())
	{
		...
		// mount AOC
		if (!titleAOC.Mount(fmt::format("/vol/aoc{:016x}", titleAOC.GetAppTitleId()), "content",
							FSC_PRIORITY_PATCH))
		{
			cemuLog_log(LogType::Force, "Mounting failed");
			return STATUS_CODE::UNABLE_TO_MOUNT;
	...
	if (!sLaunchModeIsStandalone)
	{
		...
		if (argstr && *argstr != '\0')
		{
			...
			if (index != std::string::npos)
			{
				...
				int status;
				const auto file = fsc_open(
					rpxPath.c_str(), FSC_ACCESS_FLAG::OPEN_FILE | FSC_ACCESS_FLAG::READ_PERMISSION,
					&status);
				if (file)
				{
...
STATUS_CODE PrepareForegroundTitle(TitleId titleId)
{
	...
	if (tip.GetType() == TitleIdParser::TITLE_TYPE::AOC ||
		tip.GetType() == TitleIdParser::TITLE_TYPE::BASE_TITLE_UPDATE)
...
STATUS_CODE PrepareForegroundTitleFromStandaloneRPX(const fs::path& path)
{
	...
	cemuLog_log(
		LogType::Force,
		"Launching executable in standalone mode due to incorrect layout or missing meta files");
	...
	if (boost::iequals(dirName, "code"))
	{
		...
		if (fs::is_directory(contentPath, ec))
		{
			...
			bool r = FSCDeviceHostFS_Mount(std::string("/vol/content").c_str(),
										   boost::nowide::widen(_utf8Wrapper(contentPath)).c_str(),
										   FSC_PRIORITY_BASE);
			if (!r)
			{
				cemuLog_log(LogType::Force, "Failed to mount {}",
							_utf8Wrapper(contentPath).c_str());
				return STATUS_CODE::UNABLE_TO_MOUNT;
			}
		}
	}
	// mount code folder to a virtual temporary path
	FSCDeviceHostFS_Mount(std::string("/internal/code/").c_str(),
						  boost::nowide::widen(_utf8Wrapper(executablePath.parent_path())).c_str(),
						  FSC_PRIORITY_BASE);
	...
	// since a lot of systems (including save folder location) rely on a TitleId, we derive a
	// placeholder id from the executable hash
...
void InitVirtualMlcStorage()
{
	// starting with Cemu 1.27.0 /vol/storage_mlc01/ is virtualized, meaning that it doesn't point
	// to one singular host os folder anymore instead it now uses a more complex solution to source
	// titles with various formats (folder, wud, wua) from the game paths and host mlc path
	...
	// only the current title gets mounted immediately, every other title should be mounted lazily
	// on first access
	...
	// when something iterates /vol/storage_mlc01/usr/title/ we can use a fake FS device mounted to
	// /vol/storage_mlc01/usr/title and sys/title that simulates the title id folders the same
	// device would then have to mount titles when their folders are actually accessed
}
...
{
	TitleIdParser tip(titleId);
	return fmt::format("/vol/storage_mlc01/{}/title/{:08x}/{:08x}",
					   tip.IsSystemTitle() ? "sys" : "usr", (uint32)(titleId >> 32),
					   (uint32)(titleId & 0xFFFFFFFF));
}
