...
static_assert(offsetof(iosu::nim::titlePackageInfo_t, ukn28DLProgressRelatedMax_u64be) == 0x28,
			  "ukn28_u64be has invalid offset");
...
void export_QuerySchedulerStatus(PPCInterpreter_t* hCPU)
{
	...
	// scope.rpx only checks the second byte and if it matches 0x01 then the scheduler is considered
	// paused/stopped (displays that downloads are inactive)
...
typedef struct
{
	...
	uint8 uknByte0C;
	uint8 applicationBoxDevice1; // this is what E-Shop app reads to determine device (0 -> mlc, 1
								 // -> extern storage?)
	uint8 uknByte0E;
...
void load()
{
	...
	osLib_addFunction("nn_nim",
					  "NeedsNotifyToUsers__Q3_2nn3nim4utilFPCQ3_2nn3nim16TitlePackageInfoPCQ3_"
					  "2nn3nim11ResultError",
					  export_NeedsNotifyToUsersTitlePackage);

	osLib_addFunction("nn_nim", "ListTitlePackagesStatically__Q2_2nn3nimFPULUi",
					  export_ListTitlePackagesStatically);

	osLib_addFunction("nn_nim", "GetECommerceInfrastructureCountry__Q2_2nn3nimFPQ3_2nn3nim7Country",
					  export_GetECommerceInfrastructureCountry);

	osLib_addFunction("nn_nim", "QuerySchedulerStatus__Q2_2nn3nimFPQ3_2nn3nim15SchedulerStatus",
					  export_QuerySchedulerStatus);

	osLib_addFunction("nn_nim",
					  "GetIconDatabaseEntries__Q2_2nn3nimFPQ3_2nn3nim17IconDatabaseEntryPCULUi",
					  export_GetIconDatabaseEntries);

	osLib_addFunction("nn_nim", "Construct__Q3_2nn3nim11ResultErrorFQ2_2nn6Resulti",
					  export_ConstructResultError);

	osLib_addFunction("nn_nim",
					  "MakeTitlePackageTaskConfigAutoUsingBgInstallPolicy__Q3_2nn3nim4utilFULiQ3_"
					  "2nn4Cafe9TitleType",
