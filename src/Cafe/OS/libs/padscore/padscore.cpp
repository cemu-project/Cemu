...
sint32 _KPADRead(uint32 channel, KPADStatus_t* samplingBufs, uint32 length,
				 betype<KPAD_ERROR>* errResult)
{
	if (channel >= InputManager::kMaxWPADControllers)
	{
		debugBreakpoint();
...
void export_KPADEnableDPD(PPCInterpreter_t* hCPU)
{
	...
	if (const auto controller = InputManager::instance().get_wpad_controller(channel))
	{
		if (controller->get_data_format() != kDataFormat_FREESTYLE &&
			controller->get_data_format() != 0x1F)
...
WPADStatus WPADIsMplsAttached(sint32 index, uint32be* attached, void* callback)
{
	...
	*attached = controller && controller->is_mpls_attached();
	...
	if (callback)
		PPCCoreCallback(MEMPTR(callback), index,
						controller ? WPADStatus::Success : WPADStatus::NoController);
...
void TickFunction(PPCInterpreter_t* hCPU)
{
	...
	for (auto i = 0; i < InputManager::kMaxWPADControllers; ++i)
	{
		if (g_padscore.controller_data[i].connectCallback)
		{
			if (!g_padscore.controller_data[i].disconnectCalled)
			{
				...
				PPCCoreCallback(g_padscore.controller_data[i].connectCallback, i,
			...
			{
				if (controller->m_status ==
						WPADController::ConnectCallbackStatus::ReportDisconnect ||
					controller->was_home_button_down()) // fixed!
				{
					controller->m_status = WPADController::ConnectCallbackStatus::ReportConnect;

					inputLog_printf("Calling WPADConnectCallback(%d, %d)", i,
									WPAD_ERR_NO_CONTROLLER);
					PPCCoreCallback(g_padscore.controller_data[i].connectCallback, i,
									WPAD_ERR_NO_CONTROLLER);
				}
				else if (controller->m_status ==
						 WPADController::ConnectCallbackStatus::ReportConnect)
				{
	...
	for (auto i = 0; i < InputManager::kMaxWPADControllers; ++i)
	{
		if (g_padscore.controller_data[i].extension_callback)
		{
			if (const auto controller = instance.get_wpad_controller(i))
			{
				if (controller->m_extension_status ==
					WPADController::ConnectCallbackStatus::ReportConnect)
				{
					controller->m_extension_status = WPADController::ConnectCallbackStatus::None;
					inputLog_printf("Calling WPADextensionCallback(%d)", i);
					PPCCoreCallback(g_padscore.controller_data[i].extension_callback, i,
									controller->get_device_type());
				}
			}
		}
	}
...
void load()
{
	...
	osLib_addFunction("padscore", "KPADGetUnifiedWpadStatus",
					  padscoreExport_KPADGetUnifiedWpadStatus);
	osLib_addFunction("padscore", "KPADSetSamplingCallback",
					  padscoreExport_KPADSetSamplingCallback);
	...
	osLib_addFunction("padscore", "WPADSetExtensionCallback",
					  padscoreExport_WPADSetExtensionCallback);
	osLib_addFunction("padscore", "WPADSetSamplingCallback",
					  padscoreExport_KPADSetSamplingCallback);
