...
static_assert(offsetof(TaskSetting_t, uknExt_vTableProbably) == 0x1000,
			  "offsetof(TaskSetting_t, uknExt)");
...
{
	forceLogDebug_printf("nn_boss_NetTaskSetting_SetServiceToken(0x%x, 0x%x)",
						 MEMPTR(thisptr).GetMPTR(), MEMPTR(serviceToken).GetMPTR());
	forceLogDebug_printf("\t->%s", serviceToken);
	memcpy(&thisptr->settings[TaskSetting_t::kServiceToken], serviceToken,
		   TaskSetting_t::kServiceTokenLen);
	return 0x200080;
}

Result AddInternalCaCert(NetTaskSetting_t* thisptr, char certId)
{
	forceLogDebug_printf("nn_boss_NetTaskSetting_AddInternalCaCert(0x%x, 0x%x)",
						 MEMPTR(thisptr).GetMPTR(), (int)certId);
...
{
	forceLogDebug_printf("nn_boss_NetTaskSetting_SetInternalClientCert(0x%x, 0x%x)",
						 MEMPTR(thisptr).GetMPTR(), (int)certId);
...
void export_Initialize(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_NbdlTaskSetting_Initialize(0x%08x, %s, 0x%llx, 0x%08x)",
						 thisptr.GetMPTR(), bossCode.GetPtr(), directorySizeLimit,
						 directoryName.GetMPTR());

	if (!bossCode || strnlen(bossCode.GetPtr(), 0x20) == 0x20)
	{
		osLib_returnFromFunction(
			hCPU, BUILD_NN_RESULT(NN_RESULT_LEVEL_LVL6, NN_RESULT_MODULE_NN_BOSS, 0x3780));
		return;
	}
	...
	strncpy((char*)&thisptr->settings[TaskSetting_t::kBossCode], bossCode.GetPtr(),
			TaskSetting_t::kBossCodeLen);

	*(uint64be*)&thisptr->settings[TaskSetting_t::kDirectorySizeLimit] =
		directorySizeLimit; // uint64be
	if (directoryName)
		strncpy((char*)&thisptr->settings[TaskSetting_t::kDirectoryName], directoryName.GetPtr(),
				TaskSetting_t::kDirectoryNameLen);

	osLib_returnFromFunction(
		hCPU, BUILD_NN_RESULT(NN_RESULT_LEVEL_SUCCESS, NN_RESULT_MODULE_NN_BOSS, 0x80));
}

Result SetFileName(NbdlTaskSetting_t* thisptr, const char* fileName)
{
	forceLogDebug_printf("nn_boss_NbdlTaskSetting_t_SetFileName(0x%08x, %s)",
						 MEMPTR(thisptr).GetMPTR(), fileName ? fileName : "\"\"");
	...
	strncpy((char*)&thisptr->settings[TaskSetting_t::kNbdlFileName], fileName,
			TaskSetting_t::kFileNameLen);
	return BUILD_NN_RESULT(NN_RESULT_LEVEL_SUCCESS, NN_RESULT_MODULE_NN_BOSS, 0x80);
}

} // namespace NbdlTaskSetting
...
Result Initialize(RawDlTaskSetting_t* thisptr, const char* url, bool newArrival, bool led,
				  const char* fileName, const char* directoryName)
{
	forceLogDebug_printf("nn_boss_RawDlTaskSetting_Initialize(0x%x, 0x%x, %d, %d, 0x%x, 0x%x)",
						 MEMPTR(thisptr).GetMPTR(), MEMPTR(url).GetMPTR(), newArrival, led,
	...
	if (directoryName && strnlen(directoryName, TaskSetting_t::kDirectoryNameLen) ==
							 TaskSetting_t::kDirectoryNameLen)
	{
		return 0xC0203780;
	}
...
void export_Initialize(PPCInterpreter_t* hCPU)
{
	...
	ppcDefineParamMEMPTR(directoryName, const char, 4);
	// forceLogDebug_printf("nn_boss_PlayReportSetting_Initialize(0x%08x, %s, 0x%llx, 0x%08x)",
	// thisptr.GetMPTR(), ptr.GetPtr(), directorySizeLimit, directoryName.GetMPTR());
...
void export_Set(PPCInterpreter_t* hCPU)
{
	...
	// TODO
	forceLogDebug_printf("nn_boss_PlayReportSetting_Set(0x%08x, %s, 0x%x) TODO", thisptr.GetMPTR(),
						 key.GetPtr(), value);

	osLib_returnFromFunction(hCPU, 1);
}

} // namespace PlayReportSetting
...
{
	forceLogDebug_printf("nn_boss_DirectoryName_operator_const_char(0x%x)",
						 MEMPTR(thisptr).GetMPTR());
	return thisptr->name;
}
} // namespace DirectoryName
...
void export_Initialize3(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_Initialize3(0x%08x, %s, 0x%x)", thisptr.GetMPTR(),
						 taskId.GetPtr(), accountId);
...
void export_Initialize2(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_Initialize2(0x%08x, %d, %s)", thisptr.GetMPTR(), slotId,
						 taskId.GetPtr());
...
void export_Initialize1(PPCInterpreter_t* hCPU)
{
	ppcDefineParamMEMPTR(thisptr, Task_t, 0);
	ppcDefineParamMEMPTR(taskId, const char, 1);
	forceLogDebug_printf("nn_boss_Task_Initialize1(0x%08x, %s)", thisptr.GetMPTR(),
						 taskId.GetPtr());
...
void export_StartScheduling(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_StartScheduling(0x%08x, %d)", thisptr.GetMPTR(),
						 executeImmediately);
...
void export_IsRegistered(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_IsRegistered(0x%08x) -> %d", thisptr.GetMPTR(),
						 bossRequest->returnCode);
...
void export_Wait(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_Wait(0x%08x, 0x%x, %d)", thisptr.GetMPTR(), timeout,
						 waitState);
...
void export_RegisterForImmediateRun(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_RegisterForImmediateRun(0x%08x, 0x%08x)", thisptr.GetMPTR(),
						 settings.GetMPTR());
...
void export_Register(PPCInterpreter_t* hCPU)
{
	ppcDefineParamMEMPTR(thisptr, Task_t, 0);
	ppcDefineParamMEMPTR(settings, TaskSetting_t, 1);
	forceLogDebug_printf("nn_boss_Task_Register(0x%08x, 0x%08x)", thisptr.GetMPTR(),
						 settings.GetMPTR());
...
void export_GetTurnState(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_GetTurnState(0x%08x, 0x%08x) -> %d", thisptr.GetMPTR(),
						 execCount.GetMPTR(), bossRequest->u32.result);

	osLib_returnFromFunction(hCPU, bossRequest->u32.result);
	// osLib_returnFromFunction(hCPU, 7); // 7 -> finished? 0x11 -> Error (Splatoon doesn't like it
	// when we return 0x11 for Nbdl tasks) RETURN FINISHED
}
...
void export_GetContentLength(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_GetContentLength(0x%08x, 0x%08x) -> 0x%llx",
						 thisptr.GetMPTR(), execCount.GetMPTR(), bossRequest->u64.result);
...
void export_GetProcessedLength(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_GetProcessedLength(0x%08x, 0x%08x) -> 0x%llx",
						 thisptr.GetMPTR(), execCount.GetMPTR(), bossRequest->u64.result);
...
void export_GetHttpStatusCode(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nn_boss_Task_GetHttpStatusCode(0x%08x, 0x%08x) -> %d", thisptr.GetMPTR(),
						 execCount.GetMPTR(), bossRequest->u32.result);
...
Result Initialize(bossStorage_t* thisptr, const char* dirName, uint32 accountId, StorageKind type)
{
	...
	forceLogDebug_printf("boss::Storage::Initialize(%s, 0x%08x, %d)", dirName, accountId, type);
...
uint32 Initialize(AlmightyStorage_t* thisptr, TitleId_t* titleId, const char* storageName,
				  uint32 accountId, StorageKind storageKind)
{
...
FSCVirtualFile* nnBossStorageFile_open(nn::boss::Storage::bossStorage_t* storage, uint32 fileNameId)
{
	...
	sprintf(storageFilePath, "/cemuBossStorage/%08x/%08x/user/common/data/%s/%08x",
			(uint32)(storage->titleId.u64 >> 32), (uint32)(storage->titleId.u64),
			storage->storageName, fileNameId);
	sint32 fscStatus;
	FSCVirtualFile* fscStorageFile =
		fsc_open(storageFilePath,
				 FSC_ACCESS_FLAG::OPEN_FILE | FSC_ACCESS_FLAG::READ_PERMISSION |
					 FSC_ACCESS_FLAG::WRITE_PERMISSION,
				 &fscStatus);
...
bossStorageFadEntry_t* nnBossStorageFad_getTable(nn::boss::Storage::bossStorage_t* storage)
{
	...
	char fadPath[1024];
	sprintf(fadPath, "/cemuBossStorage/%08x/%08x/user/common/%08x/%s/fad.db",
			(uint32)(storage->titleId.u64 >> 32), (uint32)(storage->titleId.u64), accountId,
			storage->storageName);

	sint32 fscStatus;
	FSCVirtualFile* fscFadFile = fsc_open(
		fadPath, FSC_ACCESS_FLAG::OPEN_FILE | FSC_ACCESS_FLAG::READ_PERMISSION, &fscStatus);
	if (!fscFadFile)
	{
		return nullptr;
	}
	...
	// read entries
	bossStorageFadEntry_t* fadTable =
		(bossStorageFadEntry_t*)malloc(sizeof(bossStorageFadEntry_t) * FAD_ENTRY_MAX_COUNT);
...
bool nnBossStorageFad_getEntryByName(nn::boss::Storage::bossStorage_t* storage, char* name,
...
std::string nnBossNsDataExport_GetPath(nsData_t* nsData)
{
	...
	fs::path path =
		fmt::format(L"cemuBossStorage/{:08x}/{:08x}/user/{:08x}", (uint32)(title_id >> 32),
					(uint32)(title_id & 0xFFFFFFFF), accountId);
...
void nnBossNsDataExport_Exist(PPCInterpreter_t* hCPU)
{
	...
	{
		sint32 fscStatus;
		auto fscStorageFile = fsc_open((char*)nnBossNsDataExport_GetPath(nsData).c_str(),
									   FSC_ACCESS_FLAG::OPEN_FILE, &fscStatus);
...
void nnBossNsDataExport_getSize(PPCInterpreter_t* hCPU)
{
	...
	else
	{
		sint32 fscStatus;
		fscStorageFile =
			fsc_open((char*)nnBossNsDataExport_GetPath(nsData).c_str(),
...
void nnBossNsDataExport_readWithSizeOut(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf(
		"nsData readWithSizeOut LR %08x (filename %s length 0x%x) Result: %d Sizeout: %llx",
		hCPU->spr.LR, nsData->name, length, r, _swapEndianU64(*sizeOut));
...
void nnBossNsDataExport_seek(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nsData seek LR %08x (filename %s seek 0x%x) Result: %d", hCPU->spr.LR,
						 nsData->name, (uint32)seekPos, r);
...
void nnBoss_load()
{
	...
	osLib_addFunction("nn_boss", "Wait__Q3_2nn4boss4TaskFUiQ3_2nn4boss13TaskWaitState",
					  nn::boss::Task::export_Wait);
	osLib_addFunction("nn_boss", "GetTurnState__Q3_2nn4boss4TaskCFPUi",
					  nn::boss::Task::export_GetTurnState);
	osLib_addFunction("nn_boss", "GetHttpStatusCode__Q3_2nn4boss4TaskCFPUi",
					  nn::boss::Task::export_GetHttpStatusCode);
	osLib_addFunction("nn_boss", "GetContentLength__Q3_2nn4boss4TaskCFPUi",
					  nn::boss::Task::export_GetContentLength);
	osLib_addFunction("nn_boss", "GetProcessedLength__Q3_2nn4boss4TaskCFPUi",
					  nn::boss::Task::export_GetProcessedLength);
	osLib_addFunction("nn_boss", "Register__Q3_2nn4boss4TaskFRQ3_2nn4boss11TaskSetting",
					  nn::boss::Task::export_Register);
	osLib_addFunction("nn_boss", "Unregister__Q3_2nn4boss4TaskFv",
					  nn::boss::Task::export_Unregister);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss4TaskFPCc",
					  nn::boss::Task::export_Initialize1);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss4TaskFUcPCc",
					  nn::boss::Task::export_Initialize2);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss4TaskFPCcUi",
					  nn::boss::Task::export_Initialize3);
	osLib_addFunction("nn_boss", "IsRegistered__Q3_2nn4boss4TaskCFv",
					  nn::boss::Task::export_IsRegistered);
	osLib_addFunction("nn_boss",
					  "RegisterForImmediateRun__Q3_2nn4boss4TaskFRCQ3_2nn4boss11TaskSetting",
					  nn::boss::Task::export_RegisterForImmediateRun);
	osLib_addFunction("nn_boss", "StartScheduling__Q3_2nn4boss4TaskFb",
					  nn::boss::Task::export_StartScheduling);
	osLib_addFunction("nn_boss", "StopScheduling__Q3_2nn4boss4TaskFv",
					  nn::boss::Task::export_StopScheduling);

	// Nbdl task setting
	osLib_addFunction("nn_boss", "__ct__Q3_2nn4boss15NbdlTaskSettingFv",
					  nn::boss::NbdlTaskSetting::export_ctor);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss15NbdlTaskSettingFPCcLT1",
					  nn::boss::NbdlTaskSetting::export_Initialize);
	// osLib_addFunction("nn_boss", "SetFileName__Q3_2nn4boss15NbdlTaskSettingFPCc",
	// nn::boss::NbdlTaskSetting::export_SetFileName);
	cafeExportRegisterFunc(nn::boss::NbdlTaskSetting::SetFileName, "nn_boss",
						   "SetFileName__Q3_2nn4boss15NbdlTaskSettingFPCc", LogType::Placeholder);

	// play task setting
	osLib_addFunction("nn_boss", "__ct__Q3_2nn4boss17PlayReportSettingFv",
					  nn::boss::PlayReportSetting::export_ctor);
	osLib_addFunction("nn_boss", "Set__Q3_2nn4boss17PlayReportSettingFPCcUi",
					  nn::boss::PlayReportSetting::export_Set);
	// osLib_addFunction("nn_boss", "Set__Q3_2nn4boss17PlayReportSettingFUiT1",
	// nn::boss::PlayReportSetting::export_Set);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss17PlayReportSettingFPvUi",
					  nn::boss::PlayReportSetting::export_Initialize);

	cafeExportRegisterFunc(nn::boss::RawDlTaskSetting::ctor, "nn_boss",
						   "__ct__Q3_2nn4boss16RawDlTaskSettingFv", LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::RawDlTaskSetting::Initialize, "nn_boss",
						   "Initialize__Q3_2nn4boss16RawDlTaskSettingFPCcbT2N21",
						   LogType::Placeholder);

	cafeExportRegisterFunc(nn::boss::NetTaskSetting::SetServiceToken, "nn_boss",
						   "SetServiceToken__Q3_2nn4boss14NetTaskSettingFPCUc",
						   LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::NetTaskSetting::AddInternalCaCert, "nn_boss",
						   "AddInternalCaCert__Q3_2nn4boss14NetTaskSettingFSc",
						   LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::NetTaskSetting::SetInternalClientCert, "nn_boss",
						   "SetInternalClientCert__Q3_2nn4boss14NetTaskSettingFSc",
						   LogType::Placeholder);

	// Title
	cafeExportRegisterFunc(nn::boss::Title::ctor, "nn_boss", "__ct__Q3_2nn4boss5TitleFv",
						   LogType::Placeholder);
	// cafeExportMakeWrapper<nn::boss::Title::SetNewArrivalFlagOff>("nn_boss",
	// "SetNewArrivalFlagOff__Q3_2nn4boss5TitleFv"); SMM bookmarks

	// TitleId
	cafeExportRegisterFunc(nn::boss::TitleId::ctor1, "nn_boss", "__ct__Q3_2nn4boss7TitleIDFv",
						   LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::TitleId::ctor2, "nn_boss", "__ct__Q3_2nn4boss7TitleIDFUL",
						   LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::TitleId::cctor, "nn_boss",
						   "__ct__Q3_2nn4boss7TitleIDFRCQ3_2nn4boss7TitleID", LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::TitleId::operator_ne, "nn_boss",
	...
	// DirectoryName
	cafeExportRegisterFunc(nn::boss::DirectoryName::ctor, "nn_boss",
						   "__ct__Q3_2nn4boss13DirectoryNameFv", LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::DirectoryName::operator_const_char, "nn_boss",
						   "__opPCc__Q3_2nn4boss13DirectoryNameCFv", LogType::Placeholder);

	// Account
	cafeExportRegisterFunc(nn::boss::Account::ctor, "nn_boss", "__ct__Q3_2nn4boss7AccountFUi",
						   LogType::Placeholder);

	// storage
	osLib_addFunction("nn_boss", "__ct__Q3_2nn4boss7StorageFv", nnBossStorageExport_ct);
	// osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss7StorageFPCcQ3_2nn4boss11StorageKind",
	// nnBossStorageExport_initialize);
	osLib_addFunction("nn_boss", "Exist__Q3_2nn4boss7StorageCFv", nnBossStorageExport_exist);
	osLib_addFunction("nn_boss", "GetDataList__Q3_2nn4boss7StorageCFPQ3_2nn4boss8DataNameUiPUiT2",
					  nnBossStorageExport_getDataList);
	osLib_addFunction("nn_boss", "GetDataList__Q3_2nn4boss7StorageCFPQ3_2nn4boss8DataNameUiPUiT2",
					  nnBossStorageExport_getDataList);
	cafeExportRegisterFunc(nn::boss::Storage::Initialize, "nn_boss",
						   "Initialize__Q3_2nn4boss7StorageFPCcUiQ3_2nn4boss11StorageKind",
						   LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::Storage::Initialize2, "nn_boss",
						   "Initialize__Q3_2nn4boss7StorageFPCcQ3_2nn4boss11StorageKind",
						   LogType::Placeholder);

	// AlmightyStorage
	cafeExportRegisterFunc(nn::boss::AlmightyStorage::ctor, "nn_boss",
						   "__ct__Q3_2nn4boss15AlmightyStorageFv", LogType::Placeholder);
	cafeExportRegisterFunc(
		nn::boss::AlmightyStorage::Initialize, "nn_boss",
		"Initialize__Q3_2nn4boss15AlmightyStorageFQ3_2nn4boss7TitleIDPCcUiQ3_2nn4boss11StorageKind",
		LogType::Placeholder);

	// AlmightyTask
	cafeExportRegisterFunc(nn::boss::AlmightyTask::ctor, "nn_boss",
						   "__ct__Q3_2nn4boss12AlmightyTaskFv", LogType::Placeholder);
	cafeExportRegisterFunc(nn::boss::AlmightyTask::Initialize, "nn_boss",
						   "Initialize__Q3_2nn4boss12AlmightyTaskFQ3_2nn4boss7TitleIDPCcUi",
						   LogType::Placeholder);
	// cafeExportRegisterFunc(nn::boss::AlmightyTask::dtor, "nn_boss",
	// "__dt__Q3_2nn4boss12AlmightyTaskFv", LogType::Placeholder);

	// NsData
	osLib_addFunction("nn_boss", "__ct__Q3_2nn4boss6NsDataFv", nnBossNsDataExport_ct);
	osLib_addFunction("nn_boss", "Initialize__Q3_2nn4boss6NsDataFRCQ3_2nn4boss7StoragePCc",
					  nnBossNsDataExport_initialize);
	osLib_addFunction("nn_boss", "DeleteRealFileWithHistory__Q3_2nn4boss6NsDataFv",
					  nnBossNsDataExport_DeleteRealFileWithHistory);
	...
	osLib_addFunction("nn_boss", "Read__Q3_2nn4boss6NsDataFPLPvUi",
					  nnBossNsDataExport_readWithSizeOut);
	osLib_addFunction("nn_boss", "Seek__Q3_2nn4boss6NsDataFLQ3_2nn4boss12PositionBase",
					  nnBossNsDataExport_seek);
}