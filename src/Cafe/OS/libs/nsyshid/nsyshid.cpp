...
DEFINE_GUID(GUID_DEVINTERFACE_HID, 0x4D1E55B2L, 0xF16F, 0x11CF, 0x88, 0xCB, 0x00, 0x11, 0x11, 0x00,
			0x00, 0x30);
...
{
	return CreateFile(devicePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
					  NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
}

void attachClientToList(HIDClient_t* hidClient)
{
	// todo - append at the beginning or end of the list? List order matters because it also
	// controls the order in which attach callbacks are called
	if (firstHIDClient)
	{
...
void checkAndAddDevice(wchar_t* devicePath, HANDLE hDevice)
{
	...
	{
		forceLog_printf("HID: Input packet length not available or out of range (length = %d)",
						maxPacketInputLength);
		maxPacketInputLength = 0x20;
	}
	if (maxPacketOutputLength <= 0 || maxPacketOutputLength >= 0xF000)
	{
		forceLog_printf("HID: Output packet length not available or out of range (length = %d)",
...
void initDeviceList()
{
	...
	hDevInfo =
		SetupDiGetClassDevs(&GUID_DEVINTERFACE_HID, NULL, 0, DIGCF_DEVICEINTERFACE | DIGCF_PRESENT);
	...
	if (hDevInfo != INVALID_HANDLE_VALUE)
	{
		...
		SetupDiEnumDeviceInterfaces(hDevInfo, NULL, &GUID_DEVINTERFACE_HID, dwMemberIdx,
									&DevIntfData);
		...
		while (GetLastError() != ERROR_NO_MORE_ITEMS)
		{
			...
			DevIntfDetailData = (PSP_DEVICE_INTERFACE_DETAIL_DATA)HeapAlloc(
				GetProcessHeap(), HEAP_ZERO_MEMORY, dwSize);
			DevIntfDetailData->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);

			if (SetupDiGetDeviceInterfaceDetail(hDevInfo, &DevIntfData, DevIntfDetailData, dwSize,
												&dwSize, &DevData))
			{
...
uint32 doAttachCallback(HIDClient_t* hidClient, HIDDeviceInfo_t* deviceInfo)
{
	return PPCCoreCallback(hidClient->callbackFunc, memory_getVirtualOffsetFromPointer(hidClient),
						   memory_getVirtualOffsetFromPointer(deviceInfo->hidDevice),
						   HID_CALLBACK_ATTACH);
}
...
void doHIDTransferCallback(MPTR callbackFuncMPTR, MPTR callbackParamMPTR, uint32 hidHandle,
						   uint32 errorCode, MPTR buffer, sint32 length)
{
	coreinitAsyncCallback_add(callbackFuncMPTR, 5, hidHandle, errorCode, buffer, length,
							  callbackParamMPTR);
}
...
void _hidSetReportAsync(HIDDeviceInfo_t* hidDeviceInfo, uint8* reportData, sint32 length,
						uint8* originalData, sint32 originalLength, MPTR callbackFuncMPTR,
						MPTR callbackParamMPTR)
{
	...
	while (true)
	{
		...
		if (retryCount >= 40)
		{
			...
			sint32 errorCode = -1;
			doHIDTransferCallback(callbackFuncMPTR, callbackParamMPTR, hidDeviceInfo->handle,
								  errorCode, memory_getVirtualOffsetFromPointer(originalData), 0);
			free(reportData);
			return;
		}
	}
...
sint32 _hidSetReportSync(HIDDeviceInfo_t* hidDeviceInfo, uint8* reportData, sint32 length,
						 uint8* originalData, sint32 originalLength, OSThread_t* osThread)
{
...
void export_HIDSetReport(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nsyshid.HIDSetReport(%d,0x%02x,0x%02x,...)", hidHandle, reportRelatedUkn,
						 reportId);
	...
	{
		forceLog_printf("nsyshid.HIDSetReport(): Unable to find device with hid handle %d",
						hidHandle);
	...
	// prepare report data
	// note: Currently we need to pad the data to 0x20 bytes for it to work (plus one extra byte for
	// HidD_SetOutputReport) Does IOSU pad data to 0x20 byte? Also check if this is specific to
	// Skylanders portal
	...
	{
		std::future<sint32> res = std::async(std::launch::async, &_hidSetReportSync, hidDeviceInfo,
											 reportData, paddedLength + 1, data, dataLength,
	...
	{
		// asynchronous
		std::thread(&_hidSetReportAsync, hidDeviceInfo, reportData, paddedLength + 1, data,
					dataLength, callbackFuncMPTR, callbackParamMPTR)
			.detach();
		returnCode = 0;
	}
...
sint32 _hidReadInternalSync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength)
{
	...
	if (readResult != FALSE)
	{
		...
		if (bt == 0)
			...
			transferLength = bt - 1;
		forceLogDebug_printf("HidRead Result received immediately (error 0x%08x) Length 0x%08x",
	...
	else
	{
		...
		// async hid read is never supposed to return unless there is an response? Lego Dimensions
		// stops HIDRead calls as soon as one of them fails with a non-zero error (which includes
		// time out)
...
void _hidReadAsync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength,
...
sint32 _hidReadSync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength,
					OSThread_t* osThread)
{
	sint32 returnCode = _hidReadInternalSync(hidDeviceInfo, data, maxLength);
	coreinit_resumeThread(osThread, 1000);
...
void export_HIDRead(PPCInterpreter_t* hCPU)
{
	...
	ppcDefineParamMPTR(callbackFuncMPTR, 3);  // r6
	ppcDefineParamMPTR(callbackParamMPTR, 4); // r7
	forceLogDebug_printf("nsyshid.HIDRead(0x%x,0x%08x,0x%08x,0x%08x,0x%08x) LR %08x", hCPU->gpr[3],
	...
	if (callbackFuncMPTR != MPTR_NULL)
	{
		...
		std::thread(&_hidReadAsync, hidDeviceInfo, data, maxLength, callbackFuncMPTR,
					callbackParamMPTR)
			.detach();
		returnCode = 0;
	}
	else
	{
		...
		std::future<sint32> res = std::async(std::launch::async, &_hidReadSync, hidDeviceInfo, data,
											 maxLength, coreinitThread_getCurrentThreadDepr(hCPU));
		coreinit_suspendThread(coreinitThread_getCurrentThreadDepr(hCPU), 1000);
		PPCCore_switchToScheduler();
		returnCode = res.get();
	}
...
sint32 _hidWriteInternalSync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength)
{
	...
	{
		// sometimes we get the result immediately
		forceLogDebug_printf("HidWrite Result received immediately (error 0x%08x) Length 0x%08x",
							 GetLastError());
	}
	else
	{
...
void _hidWriteAsync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength,
					MPTR callbackFuncMPTR, MPTR callbackParamMPTR)
{
...
sint32 _hidWriteSync(HIDDeviceInfo_t* hidDeviceInfo, uint8* data, sint32 maxLength,
					 OSThread_t* osThread)
{
	sint32 returnCode = _hidWriteInternalSync(hidDeviceInfo, data, maxLength);
	coreinit_resumeThread(osThread, 1000);
...
void export_HIDWrite(PPCInterpreter_t* hCPU)
{
	...
	ppcDefineParamMPTR(callbackFuncMPTR, 3);  // r6
	ppcDefineParamMPTR(callbackParamMPTR, 4); // r7
	forceLogDebug_printf("nsyshid.HIDWrite(0x%x,0x%08x,0x%08x,0x%08x,0x%08x)", hCPU->gpr[3],
	...
	{
		// asynchronous transfer
		std::thread(&_hidWriteAsync, hidDeviceInfo, data, maxLength, callbackFuncMPTR,
					callbackParamMPTR)
			.detach();
		returnCode = 0;
	}
	...
	{
		// synchronous transfer
		std::future<sint32> res =
			std::async(std::launch::async, &_hidWriteSync, hidDeviceInfo, data, maxLength,
					   coreinitThread_getCurrentThreadDepr(hCPU));
		coreinit_suspendThread(coreinitThread_getCurrentThreadDepr(hCPU), 1000);
		PPCCore_switchToScheduler();
		returnCode = res.get();
...
void export_HIDDecodeError(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("nsyshid.HIDDecodeError(0x%08x,0x%08x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
						 hCPU->gpr[5]);
