...
void dmaeExport_DMAECopyMem(PPCInterpreter_t* hCPU)
{
	if (hCPU->gpr[6] == DMAE_ENDIAN_NONE)
	{
		...
		memcpy(memory_getPointerFromVirtualOffset(hCPU->gpr[3]),
			   memory_getPointerFromVirtualOffset(hCPU->gpr[4]), hCPU->gpr[5] * 4);
	}
...
void dmaeExport_DMAESemaphore(PPCInterpreter_t* hCPU)
{
	...
	std::atomic<uint64le>* semaphore =
		_rawPtrToAtomic((uint64le*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]));
