...
int TCLSubmitToRing(uint32be* cmd, uint32 cmdLen, uint32be* controlFlags,
					uint64* submissionTimestamp)
{
	// todo - figure out all the bits of *controlFlags
	// if submissionTimestamp != nullptr then set it to the timestamp of the submission. Note: We
	// should make sure that uint64's are written atomically by the GPU command processor
