...
uint32 _GX2CalculateSliceSize(GX2Surface* surface,
							  const LatteAddrLib::AddrSurfaceInfo_OUT* surfaceInfo)
{
...
void GX2CalculateSurfaceInfo(GX2Surface* surfacePtr, uint32 level,
							 LatteAddrLib::AddrSurfaceInfo_OUT* pSurfOut)
{
	...
	LatteAddrLib::GX2CalculateSurfaceInfo(surfacePtr->format, surfacePtr->width, surfacePtr->height,
										  surfacePtr->depth, surfacePtr->dim, surfacePtr->tileMode,
										  surfacePtr->aa, level, pSurfOut, optimizeForDepthBuffer,
...
uint32 _GX2AdjustLevelCount(GX2Surface* surfacePtr)
{
	if (surfacePtr->numLevels <= 1)
		return 1;
	uint32 levels =
		std::max(_CalculateLevels(surfacePtr->width), _CalculateLevels(surfacePtr->height));
...
void GX2CalcSurfaceSizeAndAlignment(GX2Surface* surface)
{
	...
	{
		if (surface->dim != Latte::E_DIM::DIM_1D ||
			(surface->resFlag & GX2_RESFLAG_USAGE_DEPTH_BUFFER) != 0 || surface->aa)
		{
			if (surface->dim != Latte::E_DIM::DIM_3D ||
				(surface->resFlag & GX2_RESFLAG_USAGE_COLOR_BUFFER) != 0)
	...
	{
		uint32 padding =
			(surface->alignment - surface->imageSize % surface->alignment) % surface->alignment;
...
{
	const uint8 table[0x40] = {0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01,
							   0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01,
							   0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01,
							   0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void GX2SetSurfaceSwizzle(GX2Surface* surface, uint32 newSwizzle)
{
	...
	currentSwizzle |=
		(newSwizzle
		 << 8); // newSwizzle isn't actually masked and some games set it to values above 0xFF
