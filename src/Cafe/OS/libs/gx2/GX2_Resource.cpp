...
bool GX2RCreateBuffer(GX2RBuffer* buffer)
{
	...
	MPTR allocResult =
		PPCCoreCallback(GX2RAllocateFunc, (uint32)buffer->resFlags, bufferSize, bufferAlignment);
...
void GX2RSetAttributeBuffer(GX2RBuffer* buffer, uint32 bufferIndex, uint32 stride, uint32 offset)
{
	...
	GX2SetAttribBuffer(bufferIndex, bufferSize - offset, stride,
					   ((uint8be*)buffer->GetPtr()) + offset);
}
...
bool GX2RCreateSurface(GX2Surface* surface, uint32 resFlags)
{
	...
	MPTR allocResult = PPCCoreCallback(GX2RAllocateFunc, (uint32)surface->resFlag,
									   (uint32)surface->imageSize + (uint32)surface->mipSize,
									   (uint32)surface->alignment);
...
void GX2RDestroySurfaceEx(GX2Surface* surface, uint32 resFlags)
{
	...
	resFlags &= (GX2R_RESFLAG_UKN_BIT_19 | GX2R_RESFLAG_UKN_BIT_20 | GX2R_RESFLAG_UKN_BIT_21 |
				 GX2R_RESFLAG_UKN_BIT_22 | GX2R_RESFLAG_UKN_BIT_23);
...
bool GX2RSurfaceExists(GX2Surface* surface)
{
	...
	if (surface->imagePtr == MPTR_NULL)
		return false;
	if ((surface->resFlag & (GX2R_RESFLAG_USAGE_CPU_READ | GX2R_RESFLAG_USAGE_CPU_WRITE |
							 GX2R_RESFLAG_USAGE_GPU_READ | GX2R_RESFLAG_USAGE_GPU_WRITE)) == 0)
		return false;
	return true;
}
...
void GX2RDrawIndexed(GX2PrimitiveMode2 primitiveMode, GX2RBuffer* indexBuffer,
					 Latte::LATTE_VGT_DMA_INDEX_TYPE::E_INDEX_TYPE indexType, uint32 count,
					 uint32 baseIndex, uint32 baseVertex, uint32 numInstances)
{
	GX2DrawIndexedEx(primitiveMode, count, indexType,
					 (uint8be*)indexBuffer->GetPtr() +
						 (baseIndex * (uint32)indexBuffer->elementSize),
					 baseVertex, numInstances);
}
