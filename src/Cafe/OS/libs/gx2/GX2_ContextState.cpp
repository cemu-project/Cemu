...
GX2RegLoadPktEntry_t configReg_loadPktEntries[0xF] = // base: 0x2000
	{{0x300, 0x6}, {0x900, 0x48}, {0x980, 0x48}, {0xA00, 0x48}, {0x310, 0xC},
	 {0x542, 0x1}, {0x235, 0x1},  {0x232, 0x2},	 {0x23A, 0x1},	{0x256, 0x1},
	 {0x60C, 0x1}, {0x5C5, 0x1},  {0x2C8, 0x1},	 {0x363, 0x1},	{0x404, 0x2}};

GX2RegLoadPktEntry_t contextReg_loadPktEntries[0x2D] = // base: 0xA000
	{{0x0, 0x2},   {0x3, 0x3},	 {0xA, 0x4},   {0x10, 0x38},  {0x50, 0x34},	 {0x8E, 0x4},
	 {0x94, 0x40}, {0x100, 0x9}, {0x10C, 0x3}, {0x10F, 0x60}, {0x185, 0xA},	 {0x191, 0x27},
	 {0x1E0, 0x9}, {0x200, 0x1}, {0x202, 0x7}, {0xE0, 0x20},  {0x210, 0x29}, {0x250, 0x34},
	 {0x290, 0x1}, {0x292, 0x2}, {0x2A1, 0x1}, {0x2A5, 0x1},  {0x2A8, 0x2},	 {0x2AC, 0x3},
	 {0x2CA, 0x1}, {0x2CC, 0x1}, {0x2CE, 0x1}, {0x300, 0x9},  {0x30C, 0x1},	 {0x312, 0x1},
	 {0x316, 0x2}, {0x343, 0x2}, {0x349, 0x3}, {0x34C, 0x2},  {0x351, 0x1},	 {0x37E, 0x6},
	 {0x2B4, 0x3}, {0x2B8, 0x3}, {0x2BC, 0x3}, {0x2C0, 0x3},  {0x2C8, 0x1},	 {0x29B, 0x1},
	 {0x8C, 0x1},  {0xD5, 0x1},	 {0x284, 0xC}};

GX2RegLoadPktEntry_t resourceReg_loadPktEntries[9] =			 // base: 0xE000
	{{0, 0x70},													 // ps tex
	 {0x380, 0x70}, {0x460, 0x70},								 // vs tex
	 {0x7E0, 0x70}, {0x8B9, 0x7},  {0x8C0, 0x70}, {0x930, 0x70}, // gs tex
	 {0xCB0, 0x70}, {0xD89, 0x7}};
...
typedef struct
{
	...
	uint8 ukn9C00[0x200];
	/* +0x9E00 */ uint8 loadDL_buffer[0x300]; // this displaylist caches the IT_LOAD_* commands for
											  // swapping out context
} GX2ContextState_t;
...
void _GX2Context_cmdLoad(void* gx2ukn, uint32 pm4Header, MPTR physAddrRegArea, uint32 waitForIdle,
						 uint32 numRegOffsetEntries, GX2RegLoadPktEntry_t* regOffsetEntries)
{
...
#define __cmdStateLoad(__gx2State, __pm4Command, __regArea, __waitForIdle, __regLoadPktEntries)    \
	_GX2Context_cmdLoad(                                                                           \
		NULL,                                                                                      \
		pm4HeaderType3(__pm4Command,                                                               \
					   2 + sizeof(__regLoadPktEntries) / sizeof(__regLoadPktEntries[0]) * 2),      \
		memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(__regArea)), __waitForIdle,    \
		sizeof(__regLoadPktEntries) / sizeof(__regLoadPktEntries[0]), __regLoadPktEntries)
...
void _GX2Context_WriteCmdRestoreState(GX2ContextState_t* gx2ContextState, uint32 ukn)
{
	...
	MPTR physAddrContextState =
		memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(gx2ContextState));
	_GX2Context_CreateLoadDL();
	__cmdStateLoad(NULL, IT_LOAD_CONFIG_REG, gx2ContextState->hwContext.areaConfigReg, 0x80000000,
				   configReg_loadPktEntries);
	__cmdStateLoad(NULL, IT_LOAD_CONTEXT_REG, gx2ContextState->hwContext.areaContextReg, 0,
				   contextReg_loadPktEntries);
	__cmdStateLoad(NULL, IT_LOAD_ALU_CONST, gx2ContextState->hwContext.areaALUConst, 0,
				   aluConst_loadPktEntries);
	__cmdStateLoad(NULL, IT_LOAD_LOOP_CONST, gx2ContextState->hwContext.areaLoopConst, 0,
				   loopConst_loadPktEntries);
	__cmdStateLoad(NULL, IT_LOAD_RESOURCE, gx2ContextState->hwContext.areaResource, 0,
				   resourceReg_loadPktEntries);
	__cmdStateLoad(NULL, IT_LOAD_SAMPLER, gx2ContextState->hwContext.areaSampler, 0,
				   samplerReg_loadPktEntries);
}
...
void GX2SetDefaultState()
{
	...
	GX2::GX2SetPolygonControl(Latte::LATTE_PA_SU_SC_MODE_CNTL::E_FRONTFACE::CCW, GX2_DISABLE,
							  GX2_DISABLE, Latte::LATTE_PA_SU_SC_MODE_CNTL::E_POLYGONMODE::UKN0,
							  Latte::LATTE_PA_SU_SC_MODE_CNTL::E_PTYPE::TRIANGLES,
							  Latte::LATTE_PA_SU_SC_MODE_CNTL::E_PTYPE::TRIANGLES, GX2_DISABLE,
	...
	// Set clear depth to 1.0 (workaround for Darksiders 2. Investigate whether actual GX2 driver
	// also sets this)
	float depth = 1.0;
	gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 1));
...
void _GX2ContextCreateRestoreStateDL(GX2ContextState_t* gx2ContextState)
{
	...
	if (GX2::GX2WriteGather_isDisplayListActive())
		assert_dbg();
	GX2::GX2BeginDisplayList((void*)gx2ContextState->loadDL_buffer,
							 sizeof(gx2ContextState->loadDL_buffer));
...
void gx2Export_GX2SetupContextStateEx(PPCInterpreter_t* hCPU)
{
	...
	GX2ContextState_t* gx2ContextState =
		(GX2ContextState_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]);
...
void gx2Export_GX2SetContextState(PPCInterpreter_t* hCPU)
{
	...
	bool boiWorkaround = CafeSystem::GetRPXHashBase() ==
						 0x6BCD618E; // workaround for a bug in Binding of Isaac to avoid flicker
	if (boiWorkaround)
	{
	...
	else
	{
		...
		{
			_GX2Context_CreateLoadDL();
			GX2::GX2CallDisplayList(
				memory_getVirtualOffsetFromPointer(gx2ContextState->loadDL_buffer),
				gx2ContextState->loadDL_size);
		}
		// set new context state
		gx2CurrentContextStateMPTR = hCPU->gpr[3];
	}
