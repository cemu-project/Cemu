...
void GX2SetGPUFence(uint32be* fencePtr, uint32 mask, uint32 compareOp, uint32 compareValue)
{
	...
	gx2WriteGather_submitU32AsBE((uint32)(compareOpTable[compareOp & 7]) |
								 0x10); // compare operand + memory select (0x10)
	gx2WriteGather_submitU32AsBE(
		memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(fencePtr)) |
		2);										// physical address + type size flag(?)
...
void GX2SubmitUserTimeStamp(uint64* timestampOut, uint64 value, GX2PipeEventType eventType,
							uint32 triggerInterrupt)
{
	GX2ReserveCmdSpace(7);

	MPTR physTimestampAddr =
		memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(timestampOut));
	...
	else if (eventType == GX2PipeEventType::BOTTOM_AFTER_FLUSH)
	{
		...
		if (triggerInterrupt != 0)
		{
			// bottom callback
			// todo -> Fix this
			gx2WriteGather_submitU32AsBE(0x0000304B); // hax -> This event is handled differently
													  // and uses a different packet?
			gx2WriteGather_submitU32AsBE(0x40000000);
...
void GX2GetEventCallback(GX2CallbackEventType eventType, MEMPTR<void>* callbackFuncPtrOut,
						 MEMPTR<void>* userDataOut)
{
	if ((size_t)eventType >= GX2CallbackEventTypeCount)
	{
...
void __GX2CallbackThread(PPCInterpreter_t* hCPU)
{
	while (coreinit::OSWaitSemaphore(s_eventCbQueueSemaphore))
	{
		...
		if (!s_eventCallback[(size_t)entry.eventType].callbackFuncPtr)
			continue;
		PPCCoreCallback(s_eventCallback[(size_t)entry.eventType].callbackFuncPtr,
						(sint32)entry.eventType, s_eventCallback[(size_t)entry.eventType].userData);
	}
...
void GX2WaitForFlip()
{
	if ((sint32)(_swapEndianU32(LatteGPUState.sharedArea->flipRequestCountBE) ==
...
void GX2Init_event()
{
	...
	coreinit::OSCreateThreadType(
		s_eventCallbackThread, PPCInterpreter_makeCallableExportDepr(__GX2CallbackThread), 0,
		nullptr,
		(uint8*)s_eventCallbackThreadStack.GetPtr() + s_eventCallbackThreadStack.GetByteSize(),
		(sint32)s_eventCallbackThreadStack.GetByteSize(), 16, OSThread_t::ATTR_DETACHED,
...
void GX2EventInit()
{
	...
	coreinit::OSInitEvent(s_updateRetirementEvent,
						  coreinit::OSEvent::EVENT_STATE::STATE_NOT_SIGNALED,
