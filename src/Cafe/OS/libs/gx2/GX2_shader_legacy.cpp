...
void gx2Export_GX2SetFetchShader(PPCInterpreter_t* hCPU)
{
	...
	GX2FetchShader_t* fetchShaderPtr =
		(GX2FetchShader_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]);
	cemu_assert_debug((_swapEndianU32(fetchShaderPtr->shaderPtr) & 0xFF) == 0);

	gx2WriteGather_submit(
		// setup fetch shader
		pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 5), mmSQ_PGM_START_FS - 0xA000,
		...
		*(uint32be*)&(fetchShaderPtr->_regs[0]),
		...
		// write instance step
		pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 2), mmVGT_INSTANCE_STEP_RATE_0 - 0xA000,
		*(uint32be*)&(fetchShaderPtr->divisors[0]), *(uint32be*)&(fetchShaderPtr->divisors[1]));
...
void gx2Export_GX2GetVertexShaderGPRs(PPCInterpreter_t* hCPU)
{
	...
	GX2VertexShader_t* vertexShader =
		(GX2VertexShader_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]);
...
void gx2Export_GX2GetPixelShaderGPRs(PPCInterpreter_t* hCPU)
{
	...
	GX2PixelShader_t* pixelShader =
		(GX2PixelShader_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]);
...
void gx2Export_GX2SetVertexShader(PPCInterpreter_t* hCPU)
{
	...
	{
		// in geometry shader mode the vertex shader is written to _ES register and almost all vs
		// control registers are set by GX2SetGeometryShader
	...
	else
	{
		gx2WriteGather_submit(
			...
			pm4HeaderType3(IT_SET_CONTEXT_REG, 6), mmSQ_PGM_START_VS - 0xA000,
			...
			/* primitive id enable */
			pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmVGT_PRIMITIVEID_EN - 0xA000,
			_swapEndianU32(vertexShader->regs[1]),
			/* output config */
			pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmSPI_VS_OUT_CONFIG - 0xA000,
		...
		if (_swapEndianU32(vertexShader->usesStreamOut) != 0)
		{
			...
			gx2WriteGather_submitU32AsBE(_swapEndianU32(vertexShader->streamOutVertexStride[0]) >>
										 2);
			// stride 1
			gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 2));
			gx2WriteGather_submitU32AsBE(mmVGT_STRMOUT_VTX_STRIDE_1 - 0xA000);
			gx2WriteGather_submitU32AsBE(_swapEndianU32(vertexShader->streamOutVertexStride[1]) >>
										 2);
			// stride 2
			gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 2));
			gx2WriteGather_submitU32AsBE(mmVGT_STRMOUT_VTX_STRIDE_2 - 0xA000);
			gx2WriteGather_submitU32AsBE(_swapEndianU32(vertexShader->streamOutVertexStride[2]) >>
										 2);
			// stride 3
			gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 2));
			gx2WriteGather_submitU32AsBE(mmVGT_STRMOUT_VTX_STRIDE_3 - 0xA000);
			gx2WriteGather_submitU32AsBE(_swapEndianU32(vertexShader->streamOutVertexStride[3]) >>
										 2);
		}
	}
	...
	if (vsSemanticTableSize > 0)
	{
		...
		if (vsSemanticTableSize == 0)
		{
			// todo: Figure out how this is done on real SW/HW (some vertex shaders don't have a
			// semantic table)
			gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 1));
		...
		else
		{
			...
			gx2WriteGather_submitU32AsBE(
				pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + vsSemanticTableSize));
			gx2WriteGather_submitU32AsBE(mmSQ_VTX_SEMANTIC_0 - 0xA000);
...
void gx2Export_GX2SetPixelShader(PPCInterpreter_t* hCPU)
{
	...
	gx2WriteGather_submit(
		...
		pm4HeaderType3(IT_SET_CONTEXT_REG, 6), mmSQ_PGM_START_PS - 0xA000,
		...
		pm4HeaderType3(IT_SET_CONTEXT_REG, 3), mmSPI_PS_IN_CONTROL_0 - 0xA000,
		_swapEndianU32(pixelShader->regs[2]), _swapEndianU32(pixelShader->regs[3]));
	...
	gx2WriteGather_submit(
		/* mmCB_SHADER_MASK */
		pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmCB_SHADER_MASK - 0xA000,
		_swapEndianU32(pixelShader->regs[37]),
		/* mmCB_SHADER_CONTROL */
		pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmCB_SHADER_CONTROL - 0xA000,
		_swapEndianU32(pixelShader->regs[38]),
		/* mmDB_SHADER_CONTROL */
		pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmDB_SHADER_CONTROL - 0xA000,
		_swapEndianU32(pixelShader->regs[39]),
		/* SPI_INPUT_Z */
		pm4HeaderType3(IT_SET_CONTEXT_REG, 2), mmSPI_INPUT_Z - 0xA000,
...
void gx2Export_GX2SetGeometryShader(PPCInterpreter_t* hCPU)
{
	...
	GX2GeometryShader_t* geometryShader =
		(GX2GeometryShader_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[3]);
	...
	gx2WriteGather_submitU32AsBE(0x100000);
	gx2WriteGather_submitU32AsBE(
		_swapEndianU32(geometryShader->regs[0])); // unknown content (SQ_PGM_RESOURCES_GS)
	...
	gx2WriteGather_submit(pm4HeaderType3(IT_SET_CONTEXT_REG, 2),
						  Latte::REGADDR::VGT_GS_MODE - 0xA000, geometryShader->reg.VGT_GS_MODE);
...
void gx2Export_GX2SetComputeShader(PPCInterpreter_t* hCPU)
{
	...
	gx2WriteGather_submit(pm4HeaderType3(IT_SET_CONTEXT_REG, 6), mmSQ_PGM_START_ES - 0xA000,
						  memory_virtualToPhysical(shaderPtr) >> 8, shaderSize >> 3, 0x100000,
						  0x100000, computeShader->regs[0]);
...
{
	gx2Log_printf("GX2SetVertexUniformReg(0x%08x,0x%x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
...
{
	gx2Log_printf("GX2SetPixelUniformReg(0x%08x,0x%x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
...
void _GX2SubmitUniformBlock(uint32 registerBase, uint32 index, MPTR virtualAddress, uint32 size)
{
	...
	gx2WriteGather_submit(pm4HeaderType3(IT_SET_RESOURCE, 8), registerBase + index * 7,
						  memory_virtualToPhysical(virtualAddress), size - 1, 0, 1,
						  0, // ukn
						  0, // ukn
						  0xC0000000);
}
...
{
	gx2Log_printf("GX2SetVertexUniformBlock(0x%08x,0x%x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
	_GX2SubmitUniformBlock(mmSQ_VTX_UNIFORM_BLOCK_START - mmSQ_TEX_RESOURCE_WORD0, hCPU->gpr[3],
						   hCPU->gpr[5], hCPU->gpr[4]);
	osLib_returnFromFunction(hCPU, 0);
}

void gx2Export_GX2SetPixelUniformBlock(PPCInterpreter_t* hCPU)
{
	gx2Log_printf("GX2SetPixelUniformBlock(0x%08x,0x%x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
	_GX2SubmitUniformBlock(mmSQ_PS_UNIFORM_BLOCK_START - mmSQ_TEX_RESOURCE_WORD0, hCPU->gpr[3],
						   hCPU->gpr[5], hCPU->gpr[4]);
	osLib_returnFromFunction(hCPU, 0);
}
...
{
	gx2Log_printf("GX2SetGeometryUniformBlock(0x%08x,0x%x,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
	_GX2SubmitUniformBlock(mmSQ_GS_UNIFORM_BLOCK_START - mmSQ_TEX_RESOURCE_WORD0, hCPU->gpr[3],
						   hCPU->gpr[5], hCPU->gpr[4]);
	osLib_returnFromFunction(hCPU, 0);
}
...
void gx2Export_GX2RSetVertexUniformBlock(PPCInterpreter_t* hCPU)
{
	...
	_GX2SubmitUniformBlock(mmSQ_VTX_UNIFORM_BLOCK_START - mmSQ_TEX_RESOURCE_WORD0, index,
						   bufferPtr->GetVirtualAddr() + offset, bufferPtr->GetSize() - offset);
...
void gx2Export_GX2SetShaderModeEx(PPCInterpreter_t* hCPU)
{
	...
	gx2WriteGather_submit((uint32)(pm4HeaderType3(IT_SET_CONFIG_REG, 7)),
						  (uint32)(mmSQ_CONFIG - 0x2000), sqConfig,
						  0, // ukn / todo
						  0, // ukn / todo
						  0, // ukn / todo
						  0, // ukn / todo
	...
	{
		// update VGT_GS_MODE only if no geometry shader is used (else this register is already set
		// by GX2SetGeometryShader)
		gx2WriteGather_submitU32AsBE(pm4HeaderType3(IT_SET_CONTEXT_REG, 2));
		gx2WriteGather_submitU32AsBE(Latte::REGADDR::VGT_GS_MODE - 0xA000);
		if (mode == GX2_SHADER_MODE_COMPUTE_SHADER)
			gx2WriteGather_submitU32AsBE(
				Latte::LATTE_VGT_GS_MODE()
					.set_MODE(Latte::LATTE_VGT_GS_MODE::E_MODE::SCENARIO_G)
