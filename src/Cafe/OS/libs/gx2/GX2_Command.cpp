...
void GX2Init_writeGather() // init write gather, make current core
{
	...
	for (sint32 i = 0; i < PPC_CORE_COUNT; i++)
	{
		if (i == sGX2MainCoreIndex)
		{
			...
			gx2WriteGatherPipe.writeGatherPtrWrite[i] =
				&gx2WriteGatherPipe.writeGatherPtrGxBuffer[i];
		}
		else
		{
...
void GX2WriteGather_beginDisplayList(PPCInterpreter_t* hCPU, MPTR buffer, uint32 maxSize)
{
	...
	gx2WriteGatherPipe.writeGatherPtrDisplayList[coreIndex] =
		memory_getPointerFromVirtualOffset(gx2WriteGatherPipe.displayListStart[coreIndex]);
	gx2WriteGatherPipe.writeGatherPtrWrite[coreIndex] =
		&gx2WriteGatherPipe.writeGatherPtrDisplayList[coreIndex];
}
...
{
	return (
		uint32)(*gx2WriteGatherPipe.writeGatherPtrWrite[coreIndex] -
				memory_getPointerFromVirtualOffset(gx2WriteGatherPipe.displayListStart[coreIndex]));
}

uint32 GX2WriteGather_getFifoWriteDistance(uint32 coreIndex)
{
	uint32 writeDistance = (uint32)(gx2WriteGatherPipe.writeGatherPtrGxBuffer[coreIndex] -
...
uint32 GX2WriteGather_endDisplayList(PPCInterpreter_t* hCPU, MPTR buffer)
{
	...
	if (gx2WriteGatherPipe.displayListStart[coreIndex] != MPTR_NULL)
	{
		...
		// pad to 32 byte
		if (gx2WriteGatherPipe.displayListMaxSize[coreIndex] >=
			((gx2WriteGatherPipe.displayListMaxSize[coreIndex] + 0x1F) & ~0x1F))
		{
		...
		if (sGX2MainCoreIndex == coreIndex)
			gx2WriteGatherPipe.writeGatherPtrWrite[coreIndex] =
				&gx2WriteGatherPipe.writeGatherPtrGxBuffer[coreIndex];
		else
...
uint32 GX2WriteGather_getReadWriteDistance()
{
	...
	uint32 writeDistance = (uint32)(gx2WriteGatherPipe.writeGatherPtrGxBuffer[coreIndex] +
									GX2_COMMAND_RING_BUFFER_SIZE - gxRingBufferReadPtr);
...
void GX2WriteGather_checkAndInsertWrapAroundMark()
{
	...
	if (writeDistance >= (GX2_COMMAND_RING_BUFFER_SIZE * 3 / 5))
	{
		...
		gx2WriteGather_submitU32AsBE(
			0); // empty word since we can't send commands with zero data words
...
uint32 GX2EndDisplayList(MEMPTR<void> displayListAddr)
{
	...
	uint32 displayListSize =
		GX2WriteGather_endDisplayList(ppcInterpreterCurrentInstance, displayListAddr.GetMPTR());
...
void GX2DirectCallDisplayList(void* addr, uint32 size)
{
	...
	coreIndex = sGX2MainCoreIndex; // always submit to main queue which is owned by GX2 main core
								   // (TCLSubmitToRing does not need this workaround)
...
void GX2PatchDisplayList(uint32be* displayData, GX2_PATCH_TYPE patchType, uint32 patchOffset,
						 void* obj)
{
