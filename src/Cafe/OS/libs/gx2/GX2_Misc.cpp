...
void gx2Export_GX2SampleBottomGPUCycle(PPCInterpreter_t* hCPU)
{
	...
	osLib_returnFromFunction(hCPU, 0);
	return;
	// seems like implementing this correctly causes more harm than good as games will try to
	// dynamically scale their resolution, which our texture cache and graphic packs cant handle
	// well. If we just never return a valid timestamp, it seems like games stop dynamically scaling
	// resolution Whats a good solution here? Should we implement it correctly and instead rely on
	// graphic pack patches to patch out the dynamic scaling? some known affected games: Wind Waker
	// HD, Super Mario 3D World
...
void GX2Init(void* initSettings)
{
	...
	cemuLog_log(LogType::GX2, "GX2Init() on core {} by thread 0x{:08x}", coreIndex,
				MEMPTR<OSThread_t>(coreinit::OSGetCurrentThread()).GetMPTR());
	...
	if (LatteGPUState.sharedAreaAddr == MPTR_NULL)
	{
		...
		LatteGPUState.sharedArea =
			(gx2GPUSharedArea_t*)memory_getPointerFromVirtualOffset(LatteGPUState.sharedAreaAddr);
	}
...
void GX2SetTVBuffer(void* imageBuffePtr, uint32 imageBufferSize, E_TVRES tvResolutionMode,
					uint32 _surfaceFormat, E_TVBUFFERMODE bufferMode)
{
...
void GX2Invalidate(uint32 invalidationFlags, MPTR invalidationAddr, uint32 invalidationSize)
{
	...
	if (surfaceSyncFlags != 0)
	{
		...
		gx2WriteGather_submitU32AsBE(
			pm4HeaderType3(IT_SURFACE_SYNC, 4)); // IT_SURFACE_SYNC + 4 data dwords
		gx2WriteGather_submitU32AsBE(surfaceSyncFlags);
		gx2WriteGather_submitU32AsBE((invalidationSize + 0xFF) >> 8); // size
		gx2WriteGather_submitU32AsBE(memory_virtualToPhysical(invalidationAddr) >>
									 8);		  // base address (divided by 0x100)
