...
void _BeginOcclusionQuery(GX2Query* queryInfo, bool isGPUQuery)
{
	if (isGPUQuery)
	{
		...
		if (titleId == 0x00050000101c4c00ULL || titleId == 0x00050000101c4d00 ||
			titleId == 0x0005000010116100) // XCX EU, US, JPN
		{
			// in XCX queries are used to determine if certain objects are visible
			// if we are not setting the result fast enough and the query still holds a value of 0
			// (which is the default for GPU queries) then XCX will not render affected objects,
			// causing flicker note: This is a very old workaround. It may no longer be necessary
			// since the introduction of full sync. Investigate
		...
		else
		{
			...
			MPTR queryInfoPhys =
				memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(queryInfo));
...
{
	if (query->reg[LATTE_GC_NUM_RB * 4 + 1] == _swapEndianU32('OCPU') &&
		query->reg[LATTE_GC_NUM_RB * 4 + 0] == 0)
	{
...
void GX2QueryBeginConditionalRender(uint32 queryType, GX2Query* query, uint32 dontWaitBool,
									uint32 pixelsMustPassBool)
{
