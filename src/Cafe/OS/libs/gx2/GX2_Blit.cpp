...
void GX2SetClearDepthStencil(GX2DepthBuffer* depthBuffer, float depthClearValue,
							 uint8 stencilClearValue)
{
	...
	*(uint32*)&depthBuffer->clearDepth = _swapEndianU32(*(uint32*)&depthClearValue);
	...
	gx2WriteGather_submit(pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 2),
						  Latte::REGADDR::DB_STENCIL_CLEAR - 0xA000, stencilClearReg,
						  depthClearReg);
}
...
void GX2SetClearDepth(GX2DepthBuffer* depthBuffer, float depthClearValue)
{
	...
	*(uint32*)&depthBuffer->clearDepth = _swapEndianU32(*(uint32*)&depthClearValue);
	...
	gx2WriteGather_submit(pm4HeaderType3(IT_SET_CONTEXT_REG, 1 + 1),
						  Latte::REGADDR::DB_DEPTH_CLEAR - 0xA000, depthClearReg);
}
...
void _updateDepthStencilClearRegs(float depthClearValue, uint8 stencilClearValue,
								  GX2ClearFlags clearFlags)
{
	if ((clearFlags & GX2ClearFlags::SET_DEPTH_REG) != 0 &&
		(clearFlags & GX2ClearFlags::SET_STENCIL_REG) != 0)
	{
		GX2ReserveCmdSpace(4);
		Latte::LATTE_DB_STENCIL_CLEAR stencilClearReg;
		stencilClearReg.set_clearValue(stencilClearValue);
...
void GX2ClearBuffersEx(GX2ColorBuffer* colorBuffer, GX2DepthBuffer* depthBuffer, float r, float g,
					   float b, float a, float depthClearValue, uint8 stencilClearValue,
					   GX2ClearFlags clearFlags)
{
	GX2ReserveCmdSpace(50);
...
// always uses passed depthClearValue/stencilClearValue for clearing, even if clear flags dont
// specify value updates
void GX2ClearDepthStencilEx(GX2DepthBuffer* depthBuffer, float depthClearValue,
							uint8 stencilClearValue, GX2ClearFlags clearFlags)
{
	GX2ReserveCmdSpace(50);
