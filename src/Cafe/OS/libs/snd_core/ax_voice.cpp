...
AXVPBInternal_t*
	__AXVPBInternalVoiceShadowCopyArrayPtr; // this is the array used by audio mixing (it's synced
											// at the beginning of every audio frame with
...
sint32 AXUserBegin()
{
	// some games (e.g. Color Splash) can block themselves from calling AXSetVoice* API in time if a
	// thread gets rescheduled while inside a AXUserBegin() + AXUserEnd() block to prevent this from
	// happening we extend the current thread's quantum while the protection is raised
		...
		if (__AXUserProtectionArray[i].threadMPTR == currentThreadMPTR)
		{
			...
			if (__AXUserProtectionArray[i].count == 0)
			{
				...
				else
				{
					...
					//{
					//	__AXUserProtectionArray[f].threadMPTR = __AXUserProtectionArray[f +
					// 1].threadMPTR;
					//	__AXUserProtectionArray[f].count = __AXUserProtectionArray[f + 1].count;
					//}
					cemu_assert_debug(false);
				}
				// remove entries associated with the current thread from __AXVoiceProtection[] if
				// the count is zero
				for (sint32 f = 0; f < AX_MAX_VOICES; f++)
				{
					if (__AXVoiceProtection[f].threadMPTR == currentThreadMPTR &&
						__AXVoiceProtection[f].count == 0)
					{
...
bool AXVoiceProtection_IsProtectedByCurrentThread(AXVPB* vpb)
{
	...
	if (AXIst_IsFrameBeingProcessed())
		...
		isProtected = __AXVoiceProtection[index].threadMPTR !=
					  coreinitThread_getCurrentThreadMPTRDepr(ppcInterpreterCurrentInstance);
	return isProtected;
}
...
void AXVoiceProtection_Acquire(AXVPB* vpb)
{
	...
	{
		__AXVoiceProtection[index].threadMPTR =
			coreinitThread_getCurrentThreadMPTRDepr(ppcInterpreterCurrentInstance);
		// does not set count?
	}
}
...
AXVPB* AXVPB_DropVoice(sint32 priority)
{
	for (sint32 i = 1; i < priority; i++)
	{
		...
		if (voiceItr)
		{
			...
			{
				PPCCoreCallback(voiceItr->callbackEx, voiceItr, voiceItr->userParam,
								voiceItr->ukn4C_dropReason);
			}
...
void AXVPBInit()
{
	...
	for (sint32 i = 0; i < AX_MAX_VOICES; i++)
	{
		...
		MPTR internalShadowCopyPhys =
			memory_virtualToPhysical(memory_getVirtualOffsetFromPointer(internalShadowCopy));
...
void AXSetVoiceOffsets(AXVPB* vpb, AXPBOFFSET_t* pbOffset)
{
	cemuLog_log(
		LogType::SoundAPI,
		fmt::format(
			"AXSetVoiceOffsets() -> Format: {0:04x} Current: {1:08x} End: {2:08x} Loop: {3:08x}",
...
void AXGetVoiceOffsets(AXVPB* vpb, AXPBOFFSET_t* pbOffset)
{
	...
	{
		uint32 loopOffset =
			(loopOffsetPtrLow | (loopOffsetPtrHigh << 16) | (ptrHighExtension << 29)) - sampleBase;
		uint32 endOffset =
			(endOffsetPtrLow | (endOffsetPtrHigh << 16) | (ptrHighExtension << 29)) - sampleBase;
		uint32 currentOffset =
			(currentOffsetPtrLow | (currentOffsetPtrHigh << 16) | (ptrHighExtension << 29)) -
			sampleBase;
	...
	{
		uint32 loopOffset =
			(loopOffsetPtrLow | (loopOffsetPtrHigh << 16) | ((ptrHighExtension << 29) / 2)) -
			sampleBase / 2;
		uint32 endOffset =
			(endOffsetPtrLow | (endOffsetPtrHigh << 16) | ((ptrHighExtension << 29) / 2)) -
			sampleBase / 2;
		uint32 currentOffset =
			(currentOffsetPtrLow | (currentOffsetPtrHigh << 16) | ((ptrHighExtension << 29) / 2)) -
			sampleBase / 2;
	...
	sndApiLog_printf("Retrieved voice offsets for voice %08x - base %08x current %08x loopFlag "
					 "%04x loop %08x end %08x",
					 memory_getVirtualOffsetFromPointer(vpb), _swapEndianU32(pbOffset->samples),
					 _swapEndianU32(pbOffset->currentOffset), _swapEndianU16(pbOffset->loopFlag),
					 _swapEndianU32(pbOffset->loopOffset), _swapEndianU32(pbOffset->endOffset));
}
...
sint32 AXGetVoiceLoopCount(AXVPB* vpb)
{
	...
	if (loopOffset < endOffset && srcRatio != 0 && loopLength <= voiceSamplesPerFrame)
	{
		...
		uint32 loopsInFrame =
			((prevCurrentOffset - loopOffset) + voiceSamplesPerFrame) / loopLength;
		vpbLoopTracker_loopCount[voiceIndex] += loopsInFrame;
	}
	else // loopOffset >= endOffset
	{
		if (prevCurrentOffset <=
			endOffset) // loop could only happen if playback cursor was before end
		{
