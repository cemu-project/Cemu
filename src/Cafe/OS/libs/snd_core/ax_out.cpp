...
{
	*drcAudioMode = 1; // apparently the default is Stereo(?), MH3U exits if AXGetDeviceMode doesn't
					   // return 0 (DRCSystemAudioMode must return 1 to set DRC mode to 0)
	return true;
}
...
sint16* __AXTVDMABuffers[3] = {__buf_AXTVDMABuffers_0, __buf_AXTVDMABuffers_1,
							   __buf_AXTVDMABuffers_2};
...
sint16* __AXDRCDMABuffers[3] = {__buf_AXDRCDMABuffers_0, __buf_AXDRCDMABuffers_1,
								__buf_AXDRCDMABuffers_2};
...
{
	sint32 sampleCount =
		size / sizeof(sint16); // sample count in total (summed up for all channels)
	...
	sint16* outputChannel =
		tempTVChannelData + AX_SAMPLES_PER_3MS_48KHZ * tempAudioBlockCounter * channels;
...
void AXOut_SubmitTVFrame(sint32 frameIndex)
{
	...
	if (__AXMode[AX_DEV_TV] == AX_MODE_6CH)
	{
		...
		for (sint32 i = 0; i < numSamples; i++)
		{
			...
			* DirectSound surround order
			...
			Back Right - BR			5
			*/
			dmaOutputBuffer[0] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel0), -32768), 32767));
			dmaOutputBuffer[1] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel1), -32768), 32767));

			dmaOutputBuffer[4] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel2), -32768), 32767));
			dmaOutputBuffer[5] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel3), -32768), 32767));

			dmaOutputBuffer[2] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel4), -32768), 32767));
			dmaOutputBuffer[3] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel5), -32768), 32767));
			dmaOutputBuffer += 6;
			// next sample
			inputChannel0++;
			inputChannel1++;
	...
	else if (__AXMode[AX_DEV_TV] == AX_MODE_MONO)
	{
		...
		AIInitDMA(__AXTVDMABuffers[frameIndex],
				  numSamples * 1 * sizeof(sint16)); // 1ch (output as stereo)
	}
	else
		assert_dbg();
}
...
void AIInitDRCDMA(sint16* sampleData, sint32 size)
{
	...
	sint16* outputChannel =
		tempDRCChannelData + AX_SAMPLES_PER_3MS_48KHZ * tempDRCAudioBlockCounter * channels;
	for (sint32 i = 0; i < sampleCount; ++i)
	{
		outputChannel[i] = _swapEndianS16(sampleData[i]);
	}
...
void AXOut_SubmitDRCFrame(sint32 frameIndex)
{
	...
	else if (__AXMode[AX_DEV_DRC] == AX_MODE_MONO)
	{
		...
		for (sint32 i = 0; i < numSamples; i++)
		{
			...
			dmaOutputBuffer[1] = dmaOutputBuffer[0] = _swapEndianS16(
				(sint16)std::min(std::max(_swapEndianS32(*inputChannel0), -32768), 32767));
			dmaOutputBuffer += 2;
			// next sample
			inputChannel0++;
		}
		AIInitDRCDMA(__AXDRCDMABuffers[frameIndex],
					 numSamples * 2 * sizeof(sint16)); // 1ch (output as stereo)
	}
	else
		assert_dbg();
}
...
void AXOut_init()
{
	...
	if (!g_tvAudio)
	{
		...
		if (IAudioAPI::IsAudioAPIAvailable(audio_api))
		{
			...
			const auto it = std::find_if(devices.begin(), devices.end(),
										 [&config](const auto& d)
										 { return d->GetIdentifier() == config.tv_device; });
			if (it != devices.end())
		...
		if (device_description)
		{
			try
			{
				g_tvAudio = IAudioAPI::CreateDevice(
					(IAudioAPI::AudioAPI)config.audio_api, device_description, 48000, channels,
					snd_core::AX_SAMPLES_PER_3MS_48KHZ * AX_FRAMES_PER_GROUP, 16);
	...
	if (!g_padAudio)
	{
		...
		if (IAudioAPI::IsAudioAPIAvailable(audio_api))
		{
			...
			const auto it = std::find_if(devices.begin(), devices.end(),
										 [&config](const auto& d)
										 { return d->GetIdentifier() == config.pad_device; });
			if (it != devices.end())
				device_description = *it;
		}
...
void AXOut_update()
{
	constexpr auto kTimeout = std::chrono::duration_cast<std::chrono::nanoseconds>(
		std::chrono::milliseconds(((IAudioAPI::kBlockCount * 3) / 4) * (AX_FRAMES_PER_GROUP * 3)));
	constexpr auto kWaitDuration =
		std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::milliseconds(3));
	constexpr auto kWaitDurationFast =
		std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::microseconds(2900));
	constexpr auto kWaitDurationMinimum =
		std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::microseconds(1700));
	...
	bool additional_blocks_required = false;
	{
		...
		if (lock)
			additional_blocks_required = (g_tvAudio && g_tvAudio->NeedAdditionalBlocks()) ||
										 (g_padAudio && g_padAudio->NeedAdditionalBlocks());
	}
	...
	// it can lag behind by multiple periods (up to kTimeout) if there is minor stutter in the CPU
	// thread s_last_check is always set to the timestamp at the time of firing it's used to enforce
	// the minimum wait delay (we want to avoid calling AX update in quick succession because other
	// threads may need to do work first)
