...
const uint32 AX_SYNCFLAG_ITD40 =
	0x40; // Voice initial time delay (AXSetVoiceItdOn, AXSetVoiceItdTarget)
...
struct axOffsetsInternal_t
{
	...
	/* +0x04 / 0x182 */ uint16 ptrHighExtension; // defines 512mb range (highest 3 bit of current
												 // offset ptr counted in bytes)
	// offsets (relative to NULL, counted in sample words)
	// note: All offset ptr variables can only store values up to 512MB (PCM8 mask -> 0x1FFFFFFF,
	// PCM16 mask -> 0x0FFFFFFF, ADPCM mask -> 0x3FFFFFFF)
...
struct AXVPBInternal_t
{
	...
	/* +0x000 */ uint16be
		nextAddrHigh; // points to next shadow copy (physical pointer, NULL for last voice in list)
	...
	/* +0x088 */ AXCHMIX_DEPR
	deviceMixDRC[AX_BUS_COUNT * AX_DRC_CHANNEL_COUNT * 2]; // DRC device mix
