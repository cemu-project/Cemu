...
#define handleAdpcmDecodeLoop()                                                                    \
	...
	{                                                                                              \
		...
		coefIndex = (scale >> 4) & 7;                                                              \
		coefA =                                                                                    \
			(sint32)(sint16)_swapEndianU16(internalShadowCopy->adpcmData.coef[coefIndex * 2 + 0]); \
		coefB =                                                                                    \
			(sint32)(sint16)_swapEndianU16(internalShadowCopy->adpcmData.coef[coefIndex * 2 + 1]); \
...
{
	// How to Survive uses an end offset of 0x40000 which is not a valid adpcm sample offset and
	// thus can never be reached (the ppc side decoder jumps from 0x3FFFF to 0x40002) todo -
	// investigate if the DSP decoder routines can handle invalid ADPCM end offsets
...
void AX_readADPCMSamples(AXVPBInternal_t* internalShadowCopy, sint16* output, sint32 sampleCount)
{
	...
	uint8* sampleBase =
		(uint8*)memory_getPointerFromVirtualOffset(_swapEndianU32(vpb->offsets.samples));
	...
	uint32 internalCurrentOffset =
		_swapEndianU32(*(uint32*)&internalShadowCopy->internalOffsets.currentOffsetPtrHigh);
	uint32 internalLoopOffset =
		_swapEndianU32(*(uint32*)&internalShadowCopy->internalOffsets.loopOffsetPtrHigh);
	...
	if (!(playbackNibbleOffset <= vpbEndOffset &&
		  ((uint64)playbackNibbleOffset + (uint64)(sampleCount * 16 / 14)) >= (uint64)vpbEndOffset))
	{
		while (sampleCount >= 14) // 14 samples per 16 byte block
		{
			...
			delta = 1 << (scale & 0xF);
			coefIndex = (scale >> 4) & 7;
			coefA = (sint32)(sint16)_swapEndianU16(
				internalShadowCopy->adpcmData.coef[coefIndex * 2 + 0]);
			coefB = (sint32)(sint16)_swapEndianU16(
				internalShadowCopy->adpcmData.coef[coefIndex * 2 + 1]);
	...
	uint32 newInternalCurrentOffset =
		internalCurrentOffset + (playbackNibbleOffset - playbackNibbleOffsetStart);
	*(uint32*)&internalShadowCopy->internalOffsets.currentOffsetPtrHigh =
		_swapEndianU32(newInternalCurrentOffset);
}

void AX_DecodeSamplesADPCM_NoSrc(AXVPBInternal_t* internalShadowCopy, float* output,
								 sint32 sampleCount)
{
...
void AX_DecodeSamplesADPCM_Linear(AXVPBInternal_t* internalShadowCopy, float* output,
								  sint32 sampleCount)
{
	...
	internalShadowCopy->src.historySamples[0] =
		_swapEndianS16(historySamples[(historyIndex + 0) & 3]);
	internalShadowCopy->src.historySamples[1] =
		_swapEndianS16(historySamples[(historyIndex + 1) & 3]);
	internalShadowCopy->src.historySamples[2] =
		_swapEndianS16(historySamples[(historyIndex + 2) & 3]);
	internalShadowCopy->src.historySamples[3] =
		_swapEndianS16(historySamples[(historyIndex + 3) & 3]);
}

void AX_DecodeSamplesADPCM_Tap(AXVPBInternal_t* internalShadowCopy, float* output,
							   sint32 sampleCount)
{
	// todo - implement this
...
void AX_DecodeSamplesPCM8_Linear(AXVPBInternal_t* internalShadowCopy, float* output,
								 sint32 sampleCount)
{
	// get variables
	uint32 currentFracPos = (uint32)_swapEndianU16(internalShadowCopy->src.currentFrac);
	...
	uint32 endOffsetPtr =
		_swapEndianU32(*(uint32*)&internalShadowCopy->internalOffsets.endOffsetPtrHigh);
			...
			if (internalShadowCopy->playbackState)
			{
				...
				if (currentOffsetAddr == endOffsetAddr)
				{
					if (internalShadowCopy->internalOffsets.loopFlag)
					{
						// loop
						currentOffsetAddr =
							memory_base + (loopOffsetPtr | (ptrHighExtension << 29));
					}
					else
					{
						// stop playing
	...
	*(uint32*)&internalShadowCopy->internalOffsets.currentOffsetPtrHigh =
		_swapEndianU32(currentOffsetPtr);
}

void AX_DecodeSamplesPCM8_Tap(AXVPBInternal_t* internalShadowCopy, float* output,
							  sint32 sampleCount)
{
	// todo - implement this
...
void AX_DecodeSamplesPCM8_NoSrc(AXVPBInternal_t* internalShadowCopy, float* output,
								sint32 sampleCount)
{
	// get variables
	uint32 currentFracPos = (uint32)_swapEndianU16(internalShadowCopy->src.currentFrac);
...
void AX_DecodeSamplesPCM16_Linear(AXVPBInternal_t* internalShadowCopy, float* output,
								  sint32 sampleCount)
{
	uint32 currentFracPos = (uint32)_swapEndianU16(internalShadowCopy->src.currentFrac);
	uint32 ratio = _swapEndianU32(*(uint32*)&internalShadowCopy->src.ratioHigh);
	...
	uint16* endOffsetAddr =
		(uint16*)(memory_base + ((endOffsetPtr * 2) | (ptrHighExtension << 29)));
	uint16* currentOffsetAddr =
		(uint16*)(memory_base + ((currentOffsetPtr * 2) | (ptrHighExtension << 29)));

	uint16* loopOffsetAddrDebug =
		(uint16*)(memory_base + ((loopOffsetPtr * 2) | (ptrHighExtension << 29)));
		...
		while (currentFracPos >= 0x10000)
		{
			...
			if (internalShadowCopy->playbackState)
			{
				...
				if (currentOffsetAddr == endOffsetAddr)
				{
					...
					{
						// loop
						currentOffsetAddr = (uint16*)(memory_base + ((loopOffsetPtr * 2) |
																	 (ptrHighExtension << 29)));
					}
					else
					{
...
void AX_DecodeSamplesPCM16_Tap(AXVPBInternal_t* internalShadowCopy, float* output,
							   sint32 sampleCount)
{
	// todo - implement this
...
void AX_DecodeSamplesPCM16_NoSrc(AXVPBInternal_t* internalShadowCopy, float* output,
								 sint32 sampleCount)
{
	// get variables
	uint32 currentFracPos = (uint32)_swapEndianU16(internalShadowCopy->src.currentFrac);
...
void AXVoiceMix_DecodeSamples(AXVPBInternal_t* internalShadowCopy, float* output,
							  sint32 sampleCount)
{
...
sint32 AXVoiceMix_MergeInto(float* inputSamples, float* outputSamples, sint32 sampleCount,
							AXCHMIX_DEPR* mix, sint16 deltaI)
{
...
void AXVoiceMix_ApplyADSR(AXVPBInternal_t* internalShadowCopy, float* sampleData,
						  sint32 sampleCount)
{
...
void AXVoiceMix_ApplyBiquad(AXVPBInternal_t* internalShadowCopy, float* sampleData,
							sint32 sampleCount)
{
	...
	{
		forceLogDebug_printf("AX_ApplyBiquad() with incorrect biquad.on value 0x%04x",
							 _swapEndianU16(internalShadowCopy->biquad.on));
	}
#endif
	...
	{
		// optimized variant where voiceInternal->biquad.b1 is hardcoded as zero (used heavily in
		// BotW and Splatoon)
		for (sint32 i = 0; i < sampleCount; i++)
		{
			float inputSample = sampleData[i] / 256.0f;
...
void AXVoiceMix_ApplyLowPass(AXVPBInternal_t* internalShadowCopy, float* sampleData,
							 sint32 sampleCount)
{
	...
	float prevSample =
		(float)_swapEndianS16((sint16)internalShadowCopy->lpf.yn1) * 256.0f / 32767.0f;
	for (sint32 i = 0; i < sampleCount; i++)
	{
...
void AXVoiceMix_MixIntoBuses(AXVPBInternal_t* internalShadowCopy, float* sampleData,
							 sint32 sampleCount, sint32 samplesPerFrame)
{
	// TV mixing
	for (sint32 busIndex = 0; busIndex < AX_BUS_COUNT; busIndex++)
	{
		...
		{
			uint32 channelMixMask =
				(_swapEndianU16(internalShadowCopy->deviceMixMaskTV[busIndex]) >> (channel * 2)) &
				3;
			if (channelMixMask == 0)
			{
	...
	for (sint32 busIndex = 0; busIndex < AX_BUS_COUNT; busIndex++)
	{
		...
		{
			uint32 channelMixMask =
				(_swapEndianU16(internalShadowCopy->deviceMixMaskDRC[busIndex]) >> (channel * 2)) &
				3;
			...
			float* output =
				__AXMixBufferDRC + (busIndex * AX_DRC_CHANNEL_COUNT + channel) * samplesPerFrame;
			AXVoiceMix_MergeInto(sampleData, output, sampleCount, mix, _swapEndianS16(mix->delta));
		}
	}
...
void AXMix_MergeBusSamples(float* input, sint32* output, sint32 sampleCount, uint16& volume,
						   sint16 delta)
{
...
{
	// Not 100% sure why but we need to temporarily right shift the aux samples by 8 to get the
	// sample range the games expect for the AUX callback without this, Color Splash will apply it's
	// effects incorrectly Its probably because AUX mixing always goes through the DSP which uses
	// 16bit arithmetic?
...
void AXAuxMix_MixProcessedAuxSamplesIntoOutput(sint32be* input, float* output, sint32 sampleCount,
											   uint16* volumePtr, sint16 delta)
{
...
void AXMix_mergeTVBuses()
{
	...
	for (sint32 auxBus = 0; auxBus < AX_AUX_BUS_COUNT; auxBus++)
	{
		...
		AXAuxMix_MixProcessedAuxSamplesIntoOutput(auxOutput, __AXMixBufferTV,
												  sampleCount * AX_TV_CHANNEL_COUNT,
												  &auxReturnVolume, auxReturnDelta);
	}
	...
	cemu_assert_debug(masterVolume ==
					  0x8000); // todo -> Calculate delta between old master volume and new volume
	sint16 delta = 0;
	uint16 volVar;
