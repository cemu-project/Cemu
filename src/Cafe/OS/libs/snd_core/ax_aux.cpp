...
struct AUXTVBuffer
{
	...
	sint32be* GetBuffer(uint32 auxBus, uint32 auxFrame, uint32 channel = 0)
	{
		...
		return _buf + auxFrame * samplesPerFrame + auxBus * samplesPerBus +
			   channel * samplesPerChannel;
	}
...
sint32be* AXAux_GetInputBuffer(sint32 device, sint32 deviceIndex, sint32 auxBus)
{
	...
	if (device == AX_DEV_TV)
	{
		...
		if (__AXOldAuxTVCallbackFunc[auxBus] == MPTR_NULL &&
			__AXAuxTVCallbackFunc[auxBus] == MPTR_NULL)
			return nullptr;
	...
	else if (device == AX_DEV_DRC)
	{
		...
		if (__AXOldAuxDRCCallbackFunc[deviceIndex * AX_AUX_BUS_COUNT + auxBus] == MPTR_NULL &&
			__AXAuxDRCCallbackFunc[deviceIndex * AX_AUX_BUS_COUNT + auxBus] == MPTR_NULL)
			return nullptr;
...
sint32 AXRegisterAuxCallback(sint32 device, sint32 deviceIndex, uint32 auxBusIndex, MPTR funcMPTR,
							 MPTR userParam)
{
...
sint32 AXGetAuxCallback(sint32 device, sint32 deviceIndex, uint32 auxBusIndex,
						MEMPTR<uint32be> funcPtrOut, MEMPTR<uint32be> contextPtrOut)
{
	sint32 r = AXIsValidDevice(device, deviceIndex);
	if (r != 0)
		return r;
...
void AXAux_Process()
{
	...
	for (sint32 auxBusIndex = 0; auxBusIndex < AX_AUX_BUS_COUNT; auxBusIndex++)
	{
		...
		for (sint32 channelIndex = 0; channelIndex < AX_TV_CHANNEL_COUNT; channelIndex++)
			__AXAuxCB_dataPtrs[channelIndex] =
				__AXAuxTVBuffer->GetBuffer(auxBusIndex, processedAuxFrameIndex, channelIndex);
		// do callback
	...
	for (sint32 drcIndex = 0; drcIndex < 2; drcIndex++)
	{
		...
		for (sint32 auxBusIndex = 0; auxBusIndex < AX_AUX_BUS_COUNT; auxBusIndex++)
		{
			...
			{
				void* auxOutput =
					__AXAuxDRCBuffer[drcIndex].GetBuffer(auxBusIndex, processedAuxFrameIndex);
			...
			if (__AXAuxDRCCallbackFunc[auxBusIndex + drcIndex * 3] != MPTR_NULL)
			{
				...
				for (sint32 channelIndex = 0; channelIndex < AX_DRC_CHANNEL_COUNT; channelIndex++)
					__AXAuxCB_dataPtrs[channelIndex] = __AXAuxDRCBuffer[drcIndex].GetBuffer(
						auxBusIndex, processedAuxFrameIndex, channelIndex);
				AXAUXCBCHANNELINFO* cbStruct = __AXAuxCB_auxCBStruct.GetPtr();
				cbStruct->numChannels = drcChannelCount;
				cbStruct->numSamples = sampleCount;
				ppcInterpreterCurrentInstance->gpr[3] = __AXAuxCB_dataPtrs.GetMPTR();
				ppcInterpreterCurrentInstance->gpr[4] =
					__AXAuxDRCCallbackUserParam[auxBusIndex + drcIndex * 3];
				ppcInterpreterCurrentInstance->gpr[5] = __AXAuxCB_auxCBStruct.GetMPTR();
				PPCCore_executeCallbackInternal(auxCBFuncMPTR);
			}
			else
