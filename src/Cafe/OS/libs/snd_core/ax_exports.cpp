...
void AXQuit()
{
	...
	// todo - should we wait to make sure any active callbacks are finished with execution before we
	// exit AXQuit?
...
{
	sndApiLog_printf("AXRegisterAuxCallback(0x%08x,0x%08x,0x%08x,0x%08x,0x%08x) LR %08x",
...
{
	sndApiLog_printf("AXGetAuxCallback(0x%08x,0x%08x,0x%08x,0x%08x,0x%08x)", hCPU->gpr[3],
...
{
	sndApiLog_printf("AXSetAuxReturnVolume(0x%08x,0x%08x,0x%08x,0x%04x)", hCPU->gpr[3],
...
void export_AXCheckVoiceOffsets(PPCInterpreter_t* hCPU)
{
	...
	uint32 highExtLoop =
		__AXCalculatePointerHighExtension(format, sampleBase, _swapEndianU32(pbOffset->loopOffset));
	uint32 highExtEnd =
		__AXCalculatePointerHighExtension(format, sampleBase, _swapEndianU32(pbOffset->endOffset));
	uint32 highExtCurrent = __AXCalculatePointerHighExtension(
		format, sampleBase, _swapEndianU32(pbOffset->currentOffset));
...
void export_AXSetDeviceRemixMatrix(PPCInterpreter_t* hCPU)
{
	...
	sndApiLog_printf("AXSetDeviceRemixMatrix(%d,%d,%d,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
...
void export_AXGetDeviceRemixMatrix(PPCInterpreter_t* hCPU)
{
	...
	sndApiLog_printf("AXGetDeviceRemixMatrix(%d,%d,%d,0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
					 hCPU->gpr[5], hCPU->gpr[6]);
...
sint32 AXGetDeviceFinalOutput(uint32 device, sint16be* sampleBufferOutput, uint32 bufferSize,
							  AXGetDeviceFinalOutput_t* output)
{
...
void loadExportsSndCore1()
{
	...
	osLib_addFunction("snd_core", "AXGetDeviceFinalMixCallback",
					  export_AXGetDeviceFinalMixCallback);
	osLib_addFunction("snd_core", "AXRegisterDeviceFinalMixCallback",
					  export_AXRegisterDeviceFinalMixCallback);

	osLib_addFunction("snd_core", "AXRegisterAppFrameCallback", export_AXRegisterAppFrameCallback);
	osLib_addFunction("snd_core", "AXDeregisterAppFrameCallback",
					  export_AXDeregisterAppFrameCallback);
...
{
	cafeExportRegisterFunc(sndcore2_AXInitWithParams, "sndcore2", "AXInitWithParams",
						   LogType::SoundAPI);
	...
	osLib_addFunction("sndcore2", "AXGetDeviceFinalMixCallback",
					  export_AXGetDeviceFinalMixCallback);
	osLib_addFunction("sndcore2", "AXRegisterDeviceFinalMixCallback",
					  export_AXRegisterDeviceFinalMixCallback);

	osLib_addFunction("sndcore2", "AXRegisterAppFrameCallback", export_AXRegisterAppFrameCallback);
	osLib_addFunction("sndcore2", "AXDeregisterAppFrameCallback",
					  export_AXDeregisterAppFrameCallback);
