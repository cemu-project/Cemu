...
sint32 __AXFinalMixOutputChannelCount[AX_DEV_COUNT] = {
	0}; // number of output channels returned by final mix callback
...
sint32 AXSetDeviceRemixMatrix(sint32 deviceId, uint32 inputChannelCount, uint32 outputChannelCount,
							  const MEMPTR<float32be>& matrix)
{
	// validate parameters
	...
	{
		if (g_remix_matrices[i].deviceEntry[deviceId].channelIn == inputChannelCount &&
	...
	for (uint32 i = 0; i < g_remix_matrices.GetCount(); ++i)
	{
		if (g_remix_matrices[i].deviceEntry[deviceId].channelIn == 0 &&
			g_remix_matrices[i].deviceEntry[deviceId].channelOut == 0)
		{
...
sint32 AXGetDeviceRemixMatrix(uint32 deviceId, uint32 inputChannelCount, uint32 outputChannelCount,
							  MEMPTR<MEMPTR<float32be>>& matrix)
{
	// validate parameters
	if (deviceId == AX_DEV_TV)
	{
...
sint32 AXUpsampleLinear32To48(sint32* inputBuffer, sint32* outputBuffer, sint32* sampleHistory,
							  sint32 sampleCount, bool shiftSamples, sint32 channelCount)
{
	if (shiftSamples)
	{
...
void AXIst_ProcessFinalMixCallback()
{
	...
	if (isRenderer48 || __AXDeviceUpsampleStage[AX_DEV_TV] != AX_UPSAMPLE_STAGE_BEFORE_FINALMIX)
	{
		...
		{
			__AXFinalMixCBStructTV_dataPtrArray[c] =
				__AXTempFinalMixTVBuffer.GetPtr() + c * inputSampleCount;
		}
	...
	else
	{
		...
		{
			upsampledSampleCount = AXUpsampleLinear32To48(
				__AXTVOutputBuffer, __AXTVBuffer48.GetPtr(), __AXTVUpsamplerSampleHistory,
				inputSampleCount, true, AX_TV_CHANNEL_COUNT);
		}
		else
		{
		...
		{
			__AXFinalMixCBStructTV_dataPtrArray[c] =
				__AXTVBuffer48.GetPtr() + c * upsampledSampleCount;
		}
	...
	if (isRenderer48 || __AXDeviceUpsampleStage[AX_DEV_DRC] != AX_UPSAMPLE_STAGE_BEFORE_FINALMIX)
	{
		...
		{
			__AXFinalMixCBStructDRC_dataPtrArray[c] =
				__AXTempFinalMixDRCBuffer.GetPtr() + c * inputSampleCount;
		}
	...
	else
	{
		...
		if (__AXDRCUpsampler[0].useLinearUpsampler || forceLinearUpsampler)
			upsampledSampleCount = AXUpsampleLinear32To48(
				__AXDRCOutputBuffer, __AXDRCBuffer48.GetPtr(), __AXDRC0UpsamplerSampleHistory,
				inputSampleCount, true, AX_DRC_CHANNEL_COUNT);
		else
		{
			cemu_assert(false); // todo
		}
		...
		if (__AXDRCUpsampler[1].useLinearUpsampler || forceLinearUpsampler)
			upsampledSampleCount = AXUpsampleLinear32To48(
				__AXDRCOutputBuffer + (upsampledSampleCount * AX_DRC_CHANNEL_COUNT),
				__AXDRCBuffer48.GetPtr() + inputSampleCount * AX_DRC_CHANNEL_COUNT,
				__AXDRC1UpsamplerSampleHistory, inputSampleCount, true, AX_DRC_CHANNEL_COUNT);
		else
		{
			cemu_assert(false); // todo
		}
		for (sint32 c = 0; c < AX_DRC_CHANNEL_COUNT * 2; c++)
			__AXFinalMixCBStructDRC_dataPtrArray[c] =
				__AXDRCBuffer48.GetPtr() + c * upsampledSampleCount;
	...
	{
		// copy TV
		AXTransferSamples(__AXTempFinalMixTVBuffer.GetPtr(), __AXTVBuffer48.GetPtr(),
						  AX_SAMPLES_PER_3MS_48KHZ * AX_TV_CHANNEL_COUNT, false);
		// copy DRC 0
		AXTransferSamples(__AXTempFinalMixDRCBuffer.GetPtr(), __AXDRCBuffer48.GetPtr(),
						  AX_SAMPLES_PER_3MS_48KHZ * AX_DRC_CHANNEL_COUNT, false);
	}
	else
	{
		...
		else
		{
			...
			{
				upsampledSampleCount = AXUpsampleLinear32To48(
					__AXTempFinalMixTVBuffer.GetPtr(), __AXTVBuffer48.GetPtr(),
					__AXTVUpsamplerSampleHistory, inputSampleCount, false, AX_TV_CHANNEL_COUNT);
			}
			else
			{
				cemu_assert(false);
			...
			if (__AXDRCUpsampler[0].useLinearUpsampler || forceLinearUpsampler)
			{
				AXUpsampleLinear32To48(__AXTempFinalMixDRCBuffer.GetPtr(), __AXDRCBuffer48.GetPtr(),
									   __AXDRC0UpsamplerSampleHistory, inputSampleCount, false,
									   AX_DRC_CHANNEL_COUNT);
			}
			else
			{
				cemu_assert(false);
...
void AXIst_SyncSingleVPB(AXVPB* vpb)
{
	...
	if (internalVPB->nextToProcess != nullptr)
		internalShadowCopy->nextToProcess =
			__AXVPBInternalVoiceShadowCopyArrayPtr + (sint32)internalVPB->nextToProcess->index;
	else
	...
	{
		internalVPB->internalOffsets.currentOffsetPtrHigh =
			internalShadowCopy->internalOffsets.currentOffsetPtrHigh;
		internalVPB->internalOffsets.currentOffsetPtrLow =
			internalShadowCopy->internalOffsets.currentOffsetPtrLow;
	}
	// sync volume
	...
	// the part below could be incorrect (it seems strange that the delta flag overwrites the full
	// ve flag? But PPC code looks like this)
	...
	if ((sync & AX_SYNCFLAG_OFFSETS) != 0)
	{
		...
		memcpy(&internalShadowCopy->internalOffsets, &internalVPB->internalOffsets,
			   sizeof(axOffsetsInternal_t));
	}
	else
	{
		...
		{
			// sync loop offset
			internalShadowCopy->internalOffsets.loopOffsetPtrLow =
				internalVPB->internalOffsets.loopOffsetPtrLow;
			internalShadowCopy->internalOffsets.loopOffsetPtrHigh =
				internalVPB->internalOffsets.loopOffsetPtrHigh;
		}
		if ((sync & AX_SYNCFLAG_ENDOFFSET) != 0)
		{
			// sync end offset
			internalShadowCopy->internalOffsets.endOffsetPtrLow =
				internalVPB->internalOffsets.endOffsetPtrLow;
			internalShadowCopy->internalOffsets.endOffsetPtrHigh =
				internalVPB->internalOffsets.endOffsetPtrHigh;
		}
...
void AXIst_SyncVPB(AXVPBInternal_t** lastProcessedDSPShadowCopy,
				   AXVPBInternal_t** lastProcessedPPCShadowCopy)
{
	...
	for (auto vpb : freeVoicesArray)
	{
		...
		AXVPBInternal_t* internalShadowCopy =
			__AXVPBInternalVoiceShadowCopyArrayPtr + (sint32)vpb->index;
	...
	if (lastProcessedDSPShadowCopy)
	{
		...
		{
			AXVPBInternal_t* internalShadowCopy =
				__AXVPBInternalVoiceShadowCopyArrayPtr + (sint32)previousInternalDSP->index;
	...
	if (lastProcessedPPCShadowCopy)
	{
		...
		{
			AXVPBInternal_t* internalShadowCopy =
				__AXVPBInternalVoiceShadowCopyArrayPtr + (sint32)previousInternalPPC->index;
...
void AXIst_ApplyDeviceRemix(sint32be* samples, float32be* matrix, sint32 inputChannelCount,
							sint32 outputChannelCount, sint32 sampleCount)
{
...
void AXIst_HandleDeviceRemix()
{
	...
	for (uint32 i = 0; i < g_remix_matrices.GetCount(); ++i)
	{
		...
		if (entry.deviceEntry[0].channelIn == __AXFinalMixCBStructTV->numChannelInput &&
			entry.deviceEntry[0].channelOut == __AXOutTVOutputChannelCount &&
			!entry.deviceEntry[0].matrix.IsNull())
		{
			AXIst_ApplyDeviceRemix((sint32be*)__AXTVBuffer48.GetPtr(),
								   entry.deviceEntry[0].matrix.GetPtr(),
								   __AXFinalMixCBStructTV->numChannelInput,
								   __AXOutTVOutputChannelCount, AX_SAMPLES_PER_3MS_48KHZ);
			break;
		}
	}
	...
	{
		const auto& entry = g_remix_matrices[i];
		if (entry.deviceEntry[1].channelIn == __AXFinalMixCBStructDRC->numChannelInput &&
			entry.deviceEntry[1].channelOut == __AXOutDRCOutputChannelCount &&
			!entry.deviceEntry[0].matrix.IsNull())
		{
			AXIst_ApplyDeviceRemix((sint32be*)__AXDRCBuffer48.GetPtr(),
								   entry.deviceEntry[1].matrix.GetPtr(),
								   __AXFinalMixCBStructDRC->numChannelInput,
								   __AXOutDRCOutputChannelCount, AX_SAMPLES_PER_3MS_48KHZ);
			break;
		}
	}
}
...
void AXIst_InitThread()
{
	...
	coreinit::OSCreateThreadType(__AXIstThread.GetPtr(),
								 PPCInterpreter_makeCallableExportDepr(AXIst_ThreadEntry), 0,
								 &__AXIstThreadMsgQueue, __AXIstThreadStack.GetPtr() + 0x4000,
								 0x4000, 14, istThreadAttr, OSThread_t::THREAD_TYPE::TYPE_DRIVER);
...
void AXIst_GenerateFrame()
{
	...
	memset(__AXTVOutputBuffer.GetPtr(), 0,
		   AX_SAMPLES_PER_3MS_48KHZ * AX_TV_CHANNEL_COUNT * sizeof(sint32));
	memset(__AXDRCOutputBuffer.GetPtr(), 0,
		   AX_SAMPLES_PER_3MS_48KHZ * AX_DRC_CHANNEL_COUNT * sizeof(sint32));
