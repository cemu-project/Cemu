...
namespace acp
{
ACPStatus _ACPConvertResultToACPStatus(uint32* nnResult, const char* functionName,
									   uint32 someConstant)
{
	// todo
...
nnResult __ACPUpdateSaveTimeStamp(uint32 persistentId, uint64 titleId, ACPDeviceType deviceType)
{
	...
	if (saveinfoData && !saveinfoData->empty())
	{
		...
		tinyxml2::XMLError xmlError =
			doc.Parse((const char*)saveinfoData->data(), saveinfoData->size());
		if (xmlError == xml::XML_SUCCESS || xmlError == xml::XML_ERROR_EMPTY_DOCUMENT)
		{
			...
			for (xml::XMLElement* account = info->FirstChildElement("account"); account;
...
void CreateSaveMetaFiles(uint32 persistentId, uint64 titleId)
{
	...
	FSCVirtualFile* fscFile =
		fsc_open((titlePath + "/meta/meta.xml").c_str(),
	...
	if (fscFile)
	{
		...
		const auto outPath =
			ActiveSettings::GetMlcPath("usr/save/{:08x}/{:08x}/meta/meta.xml",
									   GetTitleIdHigh(titleId), GetTitleIdLow(titleId));
	...
	fscFile = fsc_open((titlePath + "/meta/iconTex.tga").c_str(),
					   FSC_ACCESS_FLAG::OPEN_FILE | FSC_ACCESS_FLAG::READ_PERMISSION, &fscStatus);
	if (fscFile)
	{
		sint32 fileSize = fsc_getFileSize(fscFile);
		...
		const auto outPath =
			ActiveSettings::GetMlcPath("usr/save/{:08x}/{:08x}/meta/iconTex.tga",
									   GetTitleIdHigh(titleId), GetTitleIdLow(titleId));
...
void nnACPExport_ACPCreateSaveDirEx(PPCInterpreter_t* hCPU)
{
	...
	osLib_returnFromFunction(hCPU,
							 _ACPConvertResultToACPStatus(&result, "ACPCreateSaveDirEx", 0x300));
}
...
void export_ACPGetTitleMetaDirByDevice(PPCInterpreter_t* hCPU)
{
	...
	{
		sprintf(path, "/vol/storage_mlc01/sys/title/%08x/%08x/meta", (uint32)(titleId >> 32),
				(uint32)(titleId & 0xFFFFFFFF));
	}
	else
	{
		sprintf(path, "/vol/storage_mlc01/usr/title/%08x/%08x/meta", (uint32)(titleId >> 32),
				(uint32)(titleId & 0xFFFFFFFF));
	}
...
void export_ACPGetNetworkTime(PPCInterpreter_t* hCPU)
{
	...
	ppcDefineParamU32BEPtr(
		ukn, 1); // probably timezone or offset? Could also be a bool for success/failed

	uint64 t = coreinit::coreinit_getOSTime() +
			   (uint64)((sint64)(ppcCyclesSince2000_UTC - ppcCyclesSince2000) / 20LL);
...
void load()
{
	...
	osLib_addFunction("nn_acp", "ACPCheckTitleNotReferAccountLaunch",
					  export_ACPCheckTitleNotReferAccountLaunch);
	...
	osLib_addFunction("nn_acp", "ACPGetTitleIdOfMainApplication",
					  export_ACPGetTitleIdOfMainApplication);
	...
	osLib_addFunction("nn_acp", "ACPConvertNetworkTimeToOSCalendarTime",
					  export_ACPConvertNetworkTimeToOSCalendarTime);
}
} // namespace acp
} // namespace nn
