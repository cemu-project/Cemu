...
void export_DownloadIconFile(PPCInterpreter_t* hCPU)
{
	...
	auto asyncTask = std::async(std::launch::async, asyncDownloadIconFile, titleId,
								encryptedIconData, coreinit::OSGetCurrentThread());
...
bool decryptIcon(nnIdbeEncryptedIcon_t* iconInput, nnIdbeIconDataV0_t* iconOutput)
{
	...
	AES128_CBC_decrypt_updateIV(decryptedSHA256, iconInput->hashSHA256, sizeof(decryptedSHA256),
								idbeAesKeys + 16 * idbeHeader->keyIndex, iv);
	AES128_CBC_decrypt((uint8*)iconOutput, (uint8*)&iconInput->iconData,
					   sizeof(iconInput->iconData), idbeAesKeys + 16 * idbeHeader->keyIndex, iv);
