...
bool H264_IsBotW()
{
	// Cemuhook has a hack where it always returns a small size for H264DECMemoryRequirement (256
	// bytes) it also outputs images pre-cropped instead of giving the game raw uncropped images
	// both of these are required to allow Breath of the Wild to playback the higher res (1080p)
	// videos from the Switch version we mirror these hacks for user convenience and because there
	// are no downsides
	uint64 currentTitleId = CafeSystem::GetForegroundTitleId();
	if (currentTitleId == 0x00050000101c9500 || currentTitleId == 0x00050000101c9400 ||
		currentTitleId == 0x00050000101c9300)
		return true;
	return false;
}

uint32 H264DECMemoryRequirement(uint32 codecProfile, uint32 codecLevel, uint32 width, uint32 height,
								uint32be* sizeRequirementOut)
{
	if (H264_IsBotW())
	{
	...
	// note: On console this seems to check if maxWidth or maxHeight < 64 but Pikmin 3 passes 32x32
	// and crashes if this function fails ?
	if (width < 0x20 || height < 0x20 || width > 2800 || height > 1408 ||
		sizeRequirementOut == MPTR_NULL || codecLevel >= 52 ||
		(codecProfile != 0x42 && codecProfile != 0x4D && codecProfile != 0x64))
		return 0x1010000;
...
H264DEC_STATUS H264DECFindIdrpoint(uint8* ptr, uint32 length, uint32be* offsetOut)
{
	...
	for (uint32 i = 0; i < length - 4; ++i)
	{
		...
		b &= 0x9F;
		if (b != 5 && b != 7 &&
			b != 8) // check for NAL type IDR slice, but also accept SPS or PPS slices
			continue;
...
class H264AVCDecoder
{
  ...
  public:
	...
	void Init(bool isBufferedMode)
	{
		...
		s_create_ip.s_ivd_create_ip_t.u4_share_disp_buf =
			1; // shared display buffer mode -> We give the decoder a list of buffers that it will
			   // use (?)
	...
	static bool GetImageInfo(uint8* stream, uint32 length, uint32& imageWidth, uint32& imageHeight)
	{
		...
		// cemu_assert(status == 0); -> This errors when not both the headers are present, but it
		// will still set the parameters we need
		bool isValid = false;
		if (true) // status == 0)
		{
	...
	void Decode(void* data, uint32 length, double timestamp, void* imageOutput,
				DecodeResult& decodeResult)
	{
		...
		cemu_assert_debug(
			m_isBufferedMode ||
			s_dec_op.u4_output_present); // if buffered mode is disabled, then every input should
										 // output a frame (except for partial slices?)
		...
		if (s_dec_op.u4_output_present)
		{
			...
			bt.Start();
			CopyImageToResultBuffer((uint8*)s_dec_op.s_disp_frm_buf.pv_y_buf,
									(uint8*)s_dec_op.s_disp_frm_buf.pv_u_buf,
									(uint8*)m_imageBuffers[s_dec_op.u4_ts], s_dec_op);
			bt.Stop();
			...
			cemuLog_log(LogType::H264, "H264Bench | DecodeTime {}ms CopyTime {}ms", decodeTime,
						copyTime);
		}
		else
		{
		...
		if (m_isBufferedMode)
		{
			// in buffered mode, always buffer 5 frames regardless of actual reordering and decoder
			// latency
		...
		// status = ih264d_api_function(mCodecCtx, &s_ctl_get_vui_params_ip,
		// &s_ctl_get_vui_params_op); cemu_assert(status == 0);
	}
	...
	void CopyImageToResultBuffer(uint8* yIn, uint8* uvIn, uint8* bufOut,
								 ivd_video_decode_op_t& decodeInfo)
	{
  ...
  private:
	...
	void ReinitBuffers()
	{
		...
		{
			m_displayBuf.emplace_back().resize(s_ctl_op.u4_min_out_buf_size[0] +
											   s_ctl_op.u4_min_out_buf_size[1]);
		}
		// set
		ivd_set_display_frame_ip_t s_set_display_frame_ip{
			0}; // make sure to zero-initialize this. The codec seems to check the first 3
				// pointers/sizes per frame, regardless of the value of u4_num_bufs
		...
		cemu_assert_debug(s_ctl_op.u4_min_out_buf_size[0] != 0 &&
		...
		for (uint32 i = 0; i < s_ctl_op.u4_num_disp_bufs; i++)
		{
			...
			s_set_display_frame_ip.s_disp_buffer[i].u4_min_out_buf_size[0] =
				s_ctl_op.u4_min_out_buf_size[0];
			s_set_display_frame_ip.s_disp_buffer[i].u4_min_out_buf_size[1] =
				s_ctl_op.u4_min_out_buf_size[1];
			s_set_display_frame_ip.s_disp_buffer[i].pu1_bufs[0] = m_displayBuf[i].data() + 0;
			s_set_display_frame_ip.s_disp_buffer[i].pu1_bufs[1] =
				m_displayBuf[i].data() + s_ctl_op.u4_min_out_buf_size[0];
		}
...
H264DEC_STATUS H264DECGetImageSize(uint8* stream, uint32 length, uint32 offset,
								   uint32be* outputWidth, uint32be* outputHeight)
{
...
void _async_H264DECEnd(coreinit::OSEvent* executeDoneEvent, H264AVCDecoder* session,
					   H264Context* ctx,
					   std::vector<H264AVCDecoder::DecodeResult>* decodeResultsOut)
{
...
H264DEC_STATUS H264DECEnd(void* workMemory)
{
	...
	coreinit::OSInitEvent(executeDoneEvent, coreinit::OSEvent::EVENT_STATE::STATE_NOT_SIGNALED,
						  coreinit::OSEvent::EVENT_MODE::MODE_MANUAL);
	std::vector<H264AVCDecoder::DecodeResult> results;
	auto asyncTask = std::async(std::launch::async, _async_H264DECEnd,
								executeDoneEvent.GetPointer(), session, ctx, &results);
...
void H264DoFrameOutputCallback(H264Context* ctx, H264AVCDecoder::DecodeResult& decodeResult)
{
	...
	stack_fptrOutputData->resultArray =
		(MEMPTR<H264DECFrameOutput>*)stack_resultPtrArray.GetPointer();
	...
	if (!ctx->Param.outputFunc.IsNull())
	{
		cemuLog_log(
			LogType::H264,
			"H264: Outputting frame via callback. Timestamp: {} Buffer 0x{:08x} UserParam 0x{:08x}",
			(double)decodeResult.timestamp, (uint32)frameOutput->imagePtr.GetMPTR(),
			ctx->Param.userMemoryParam.GetMPTR());
		PPCCoreCallback(ctx->Param.outputFunc.GetMPTR(), stack_fptrOutputData.GetMPTR());
	}
}

void _async_H264DECExecute(coreinit::OSEvent* executeDoneEvent, H264AVCDecoder* session,
						   H264Context* ctx, void* imageOutput,
						   H264AVCDecoder::DecodeResult* decodeResult)
{
	session->Decode(ctx->BitStream.ptr.GetPtr(), ctx->BitStream.length, ctx->BitStream.timestamp,
					imageOutput, *decodeResult);
	coreinit::OSSignalEvent(executeDoneEvent);
}
...
uint32 H264DECExecute(void* workMemory, void* imageOutput)
{
	...
	H264AVCDecoder::DecodeResult decodeResult;
	auto asyncTask =
		std::async(std::launch::async, _async_H264DECExecute, executeDoneEvent.GetPointer(),
				   session, ctx, imageOutput, &decodeResult);
	coreinit::OSWaitEvent(executeDoneEvent);
	_ReleaseDecoderSession(session);
	if (decodeResult.frameReady)
...
H264DEC_STATUS H264DECCheckDecunitLength(void* workMemory, uint8* data, uint32 maxLength,
										 uint32 offset, uint32be* unitLengthOut)
{
	// todo: our implementation for this currently doesn't parse slice headers and instead assumes
	// that each frame is encoded into a single NAL slice. For all known cases this is sufficient
	...
	while (true)
	{
		...
		{
			*unitLengthOut =
				(sint32)((rbspStream.getBasePtr() + rbspStream.getBaseLength()) - data) +
				startCodeOffset;
			return H264DEC_STATUS::SUCCESS;
		}
		case 6:
