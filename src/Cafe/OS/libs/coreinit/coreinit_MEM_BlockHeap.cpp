...
namespace coreinit
{
MEMHeapHandle MEMInitBlockHeap(MEMBlockHeap2_t* memStart, void* startAddr, void* endAddr,
							   void* initTrackMem, uint32 initTrackMemSize, uint32 createFlags)
{
	if (memStart == nullptr || startAddr == nullptr || endAddr == nullptr ||
	...
	{
		sint32 result =
			MEMAddBlockHeapTracking(MEMPTR<void>(memStart).GetMPTR(),
									MEMPTR<void>(initTrackMem).GetMPTR(), initTrackMemSize);
		if (result != 0)
		{
...
sint32 MEMGetAllocatableSizeForBlockHeapEx(MEMBlockHeap2_t* blockHeap, sint32 alignment)
{
	if (!blockHeap || blockHeap->magic != MEMHeapMagic::BLOCK_HEAP)
	{
		cemuLog_log(LogType::Force,
					"MEMGetAllocatableSizeForBlockHeapEx(): Not a valid block heap");
		return 0;
	}
	if (alignment < 0)
		alignment = -alignment;
...
void _blockHeapDebugVerifyIfBlockIsLinked(MEMBlockHeapDEPR* blockHeapHead,
										  MEMBlockHeapTrackDEPR* track)
{
	// MEMBlockHeapTrack_t* trackItr =
	// (MEMBlockHeapTrack_t*)memory_getPointerFromVirtualOffsetAllowNull(_swapEndianU32(blockHeapHead->firstBlock));
	...
	//	// next
	//	trackItr =
	//(MEMBlockHeapTrack_t*)memory_getPointerFromVirtualOffsetAllowNull(_swapEndianU32(trackItr->nextBlock));
	// }
}
...
sint32 MEMAddBlockHeapTracking(MPTR heap, MPTR trackMem, uint32 trackMemSize)
{
	...
	if (trackMem == MPTR_NULL ||
		trackMemSize <= (sizeof(MEMBlockHeapGroupDEPR) + sizeof(MEMBlockHeapTrackDEPR)))
	{
		return -4;
	}
	uint32 trackCount =
		(trackMemSize - sizeof(MEMBlockHeapGroupDEPR)) / sizeof(MEMBlockHeapTrackDEPR);
	MEMBlockHeapGroupDEPR* group =
		(MEMBlockHeapGroupDEPR*)memory_getPointerFromVirtualOffset(trackMem);
	...
	blockHeapHead->freeBlocksLeft =
		_swapEndianU32(_swapEndianU32(blockHeapHead->freeBlocksLeft) + trackCount);
	__OSUnlockScheduler();
	return 0;
}
...
MPTR _MEMBlockHeap_GetFreeBlockTrack(MEMBlockHeapDEPR* blockHeapHead)
{
	...
	MEMBlockHeapTrackDEPR* track =
		(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(trackMPTR);
	MPTR nextFreeBlockMPTR = _swapEndianU32(
		track->nextBlock); // for unreserved tracks, nextBlock holds the next unreserved block
	...
	blockHeapHead->freeBlocksLeft =
		_swapEndianU32(_swapEndianU32(blockHeapHead->freeBlocksLeft) - 1);
	return trackMPTR;
}
...
sint32 _MEMBlockHeap_AllocAtBlock(MEMBlockHeapDEPR* blockHeapHead, MEMBlockHeapTrackDEPR* track,
								  MPTR allocationAddress, uint32 size)
{
	...
	if (track->isFree != _swapEndianU32(0))
	{
		...
		MEMBlockHeapTrackDEPR* trackItr =
			(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
				_swapEndianU32(blockHeapHead->nextFreeBlock));
		while (trackItr)
		{
			if (trackItr == track)
				assert_dbg();
			// next
			trackItr = (MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
				_swapEndianU32(trackItr->nextBlock));
		}
	}
	...
	if (prefixBlockSize > 0)
	{
		...
		MEMBlockHeapTrackDEPR* prefixTrack =
			(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(prefixTrackMPTR);
		...
		{
			MEMBlockHeapTrackDEPR* tempTrack =
				(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(
					_swapEndianU32(track->previousBlock));
	...
	if (suffixBlockSize > 0)
	{
		...
		MEMBlockHeapTrackDEPR* suffixTrack =
			(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(suffixTrackMPTR);
		...
		if (_swapEndianU32(track->nextBlock) != MPTR_NULL)
		{
			MEMBlockHeapTrackDEPR* tempTrack =
				(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(
					_swapEndianU32(track->nextBlock));
...
MEMBlockHeapTrackDEPR* _MEMBlockHeap_FindBlockContaining(MEMBlockHeapDEPR* blockHeapHead,
														 MPTR memAddr)
{
	...
	// todo: If distanceToStart < distanceToEnd -> Iterate starting from firstBlock, else iterate
	// starting from lastBlock (and continue via track->previousBlock)
	MEMBlockHeapTrackDEPR* track =
		(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
			_swapEndianU32(blockHeapHead->headBlock));
	if (track == NULL)
		return NULL;
	while (track)
	{
		...
		track = (MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
			_swapEndianU32(track->nextBlock));
	}
	return NULL;
}
...
MPTR MEMAllocFromBlockHeapEx(MPTR heap, uint32 size, sint32 alignment)
{
	...
	if (allocateAtEndOfBlock)
		track = (MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
			_swapEndianU32(blockHeapHead->tailBlock));
	else
		track = (MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
			_swapEndianU32(blockHeapHead->headBlock));

	cemu_assert_debug(std::popcount<unsigned int>(alignment) ==
					  1); // not a supported alignment value
	while (track)
	{
		if (track->isFree != _swapEndianU32(0))
		{
			...
			if (allocateAtEndOfBlock == false)
			{
				...
				uint32 alignedBlockRangeStart =
					(blockRangeStart + alignment - 1) & ~(alignment - 1);
				if (alignedBlockRangeStart < blockRangeEnd)
				{
					uint32 allocRange = blockRangeEnd - alignedBlockRangeStart;
					if (allocRange >= size)
					{
						sint32 allocError = _MEMBlockHeap_AllocAtBlock(
							blockHeapHead, track, alignedBlockRangeStart, size);
						_blockHeapDebugVerifyLinkOrder(blockHeapHead);
						__OSUnlockScheduler();
						if (allocError == 0)
			...
			else
			{
				...
				{
					sint32 allocError = _MEMBlockHeap_AllocAtBlock(blockHeapHead, track,
																   alignedBlockRangeStart, size);
					_blockHeapDebugVerifyLinkOrder(blockHeapHead);
					__OSUnlockScheduler();
					if (allocError == 0)
						return alignedBlockRangeStart;
...
void MEMFreeToBlockHeap(MPTR heap, MPTR memAddr)
{
	...
	if (block != NULL)
	{
		...
		if (_swapEndianU32(block->previousBlock) != NULL)
		{
			...
			MEMBlockHeapTrackDEPR* previousBlock =
				(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(previousBlockMPTR);
			if (_swapEndianU32(previousBlock->isFree) != 0)
			{
				...
				{
					MEMBlockHeapTrackDEPR* tempNextBlock =
						(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(
							_swapEndianU32(block->nextBlock));
				...
				// debug
				_blockHeapDebugVerifyIfBlockIsLinked(
					blockHeapHead,
					(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(blockMPTR));
		...
		if (_swapEndianU32(block->nextBlock) != NULL)
		{
			...
			MEMBlockHeapTrackDEPR* nextBlock =
				(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(nextBlockMPTR);
			if (_swapEndianU32(nextBlock->isFree) != 0)
			{
				...
				if (_swapEndianU32(nextBlock->nextBlock) != MPTR_NULL)
				{
					MEMBlockHeapTrackDEPR* tempNextBlock =
						(MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffset(
							_swapEndianU32(nextBlock->nextBlock));
...
uint32 MEMGetTotalFreeSizeForBlockHeap(MEMBlockHeapDEPR* blockHeap)
{
	...
	while (blockMPTR)
	{
		...
		if (track->isFree != _swapEndianU32(0))
		{
			// get block size
			uint32 blockSize =
				_swapEndianU32(track->addrEnd) - _swapEndianU32(track->addrStart) + 1;
			// add to totalSize
			totalSize += blockSize;
		}
		// next
...
void MEMDumpBlockHeap(MPTR heap)
{
	...
	while (track)
	{
		...
		debug_printf(" %08x %08x - %08x prev/next %08x %08x isFree: %d\n",
					 memory_getVirtualOffsetFromPointer(track), blockRangeStart, blockRangeEnd,
					 _swapEndianU32(track->previousBlock), _swapEndianU32(track->nextBlock),
					 _swapEndianU32(track->isFree));
		// next
		track = (MEMBlockHeapTrackDEPR*)memory_getPointerFromVirtualOffsetAllowNull(
			_swapEndianU32(track->nextBlock));
	}
