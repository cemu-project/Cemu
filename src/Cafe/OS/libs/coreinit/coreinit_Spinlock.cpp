...
void __OSBoostThread(OSThread_t* thread)
{
	...
	__OSUpdateThreadEffectivePriority(
		thread); // sets thread->effectivePriority to zero since boostCount != 0
	__OSUnlockScheduler();
}
...
bool OSTryAcquireSpinLockWithTimeout(OSSpinLock* spinlock, uint64 timeout)
{
	...
	else
	{
		...
		uint64 timeoutValue =
			coreinit_getTimerTick() + coreinit::EspressoTime::ConvertNsToTimerTicks(timeout);
		while (!spinlock->ownerThread.atomic_compare_exchange(nullptr, currentThread))
		{
			OSYieldThread();
...
void InitializeSpinlock()
{
	...
	cafeExportRegister("coreinit", OSUninterruptibleSpinLock_TryAcquireWithTimeout,
					   LogType::Placeholder);
