...
struct
{
	...
	MPTR rangeStart;
	uint32 rangeSize;
	uint8* cacheStateCopy; // holds a copy of the entire range, simulates icache state (updated via
						   // ICBI)
} coreinitCodeGen = {0};
...
void coreinitExport_OSCodegenCopy(PPCInterpreter_t* hCPU)
{
	...
	if (codeAddrDest < coreinitCodeGen.rangeStart ||
		codeAddrDest >= (coreinitCodeGen.rangeStart + coreinitCodeGen.rangeSize))
		assert_dbg();
	if ((codeAddrDest + size) < coreinitCodeGen.rangeStart ||
		(codeAddrDest + size) > (coreinitCodeGen.rangeStart + coreinitCodeGen.rangeSize))
		assert_dbg();

	memcpy(memory_getPointerFromVirtualOffset(codeAddrDest),
		   memory_getPointerFromVirtualOffset(memAddrSrc), size);
...
void codeGenHandleICBI(uint32 ea)
{
	...
	if (ea >= coreinitCodeGen.rangeStart &&
		ea < (coreinitCodeGen.rangeStart + coreinitCodeGen.rangeSize))
	{
