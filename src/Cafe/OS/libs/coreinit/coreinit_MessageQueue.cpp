...
void OSInitMessageQueueEx(OSMessageQueue* msgQueue, OSMessage* msgArray, uint32 msgCount,
						  void* userData)
{
	msgQueue->magic = 'mSgQ';
...
sint32 OSSendMessage(OSMessageQueue* msgQueue, OSMessage* msg, uint32 flags)
{
	...
	{
		// decrease firstIndex
		sint32 newFirstIndex =
			(sint32)((sint32)msgQueue->firstIndex + (sint32)msgQueue->msgCount - 1) %
			(sint32)msgQueue->msgCount;
	...
	{
		sint32 messageIndex =
			(uint32)(msgQueue->firstIndex + msgQueue->usedCount) % (uint32)msgQueue->msgCount;
		msgQueue->usedCount = (uint32)msgQueue->usedCount + 1;
...
{
	OSInitMessageQueue(g_systemMessageQueue.GetPtr(), _systemMessageQueueArray.GetPtr(),
					   _systemMessageQueueArray.GetCount());
