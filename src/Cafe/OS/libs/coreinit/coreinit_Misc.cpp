...
sint32 ppcSprintf(const char* formatStr, char* strOut, sint32 maxLength, PPCInterpreter_t* hCPU,
				  sint32 initialParamIndex)
{
	char tempStr[4096];
	...
	while (*formatStr)
	{
		...
		if (c == '%')
		{
			...
			// length + specifier
			char tempFormat[64];
			if (*formatStr == 'X' || *formatStr == 'x' || *formatStr == 'u' || *formatStr == 'd' ||
				*formatStr == 'p' || *formatStr == 'i' ||
				(formatStr[0] == 'l' && formatStr[1] == 'd'))
			{
				// number
				formatStr++;
				strncpy(tempFormat, formatStart,
						std::min((std::ptrdiff_t)sizeof(tempFormat) - 1, formatStr - formatStart));
				if ((formatStr - formatStart) < sizeof(tempFormat))
					tempFormat[(formatStr - formatStart)] = '\0';
				else
					tempFormat[sizeof(tempFormat) - 1] = '\0';
				sint32 tempLen = sprintf(tempStr, tempFormat,
										 PPCInterpreter_getCallParamU32(hCPU, integerParamIndex));
				integerParamIndex++;
			...
			else if (*formatStr == 's')
			{
				...
				if (strOffset == MPTR_NULL)
					...
					tempLen =
						sprintf(tempStr, tempFormat, memory_getPointerFromVirtualOffset(strOffset));
				integerParamIndex++;
				for (sint32 i = 0; i < tempLen; i++)
				{
					if (writeIndex >= maxLength)
			...
			else if (*formatStr == 'f')
			{
				...
				if ((formatStr - formatStart) < sizeof(tempFormat))
					...
					tempFormat[sizeof(tempFormat) - 1] = '\0';
				sint32 tempLen =
					sprintf(tempStr, tempFormat, (float)hCPU->fpr[1 + floatParamIndex].fp0);
				floatParamIndex++;
				for (sint32 i = 0; i < tempLen; i++)
				{
					if (writeIndex >= maxLength)
			...
			else if (formatStr[0] == 'l' && formatStr[1] == 'f')
			{
				...
				if ((formatStr - formatStart) < sizeof(tempFormat))
					...
					tempFormat[sizeof(tempFormat) - 1] = '\0';
				sint32 tempLen =
					sprintf(tempStr, tempFormat, (double)hCPU->fpr[1 + floatParamIndex].fp0);
				floatParamIndex++;
				for (sint32 i = 0; i < tempLen; i++)
				{
					if (writeIndex >= maxLength)
			...
			else if ((formatStr[0] == 'l' && formatStr[1] == 'l' &&
					  (formatStr[2] == 'x' || formatStr[2] == 'X')))
			{
				formatStr += 3;
				// number (64bit)
				strncpy(tempFormat, formatStart,
				...
				if (integerParamIndex & 1)
					integerParamIndex++;
				sint32 tempLen = sprintf(tempStr, tempFormat,
										 PPCInterpreter_getCallParamU64(hCPU, integerParamIndex));
				integerParamIndex += 2;
				for (sint32 i = 0; i < tempLen; i++)
				{
					if (writeIndex >= maxLength)
...
void WriteCafeConsole(CafeLogType cafeLogType, const char* msg, sint32 len)
{
	...
	{
		cemuLog_log(
			LogType::CoreinitLogging, "[{0}] {1}", cafeLogName,
...
void miscInit()
{
	...
	// Disney Infinity 2.0 actually relies on home button menu being enabled by default. If it's
	// false it will crash due to calling erreula->IsAppearHomeNixSign() before initializing erreula
