...
{
	CoreinitAsyncCallback(MPTR functionMPTR, uint32 numParameters, uint32 r3, uint32 r4, uint32 r5,
						  uint32 r6, uint32 r7, uint32 r8, uint32 r9, uint32 r10)
		: m_functionMPTR(functionMPTR),
		  m_numParameters(numParameters), m_gprParam{r3, r4, r5, r6, r7, r8, r9, r10} {};

	static void queue(MPTR functionMPTR, uint32 numParameters, uint32 r3, uint32 r4, uint32 r5,
					  uint32 r6, uint32 r7, uint32 r8, uint32 r9, uint32 r10)
	{
		s_asyncCallbackSpinlock.acquire();
		s_asyncCallbackQueue.emplace_back(
  ...
  private:
	void doCall()
	{
		PPCCoreCallback(m_functionMPTR, m_gprParam[0], m_gprParam[1], m_gprParam[2], m_gprParam[3],
						m_gprParam[4], m_gprParam[5], m_gprParam[6], m_gprParam[7]);
	}

	static CoreinitAsyncCallback* allocateAndInitFromPool(MPTR functionMPTR, uint32 numParameters,
														  uint32 r3, uint32 r4, uint32 r5,
														  ...
														  uint32 r9, uint32 r10)
	{
		...
		{
			CoreinitAsyncCallback* cb = new CoreinitAsyncCallback(functionMPTR, numParameters, r3,
...
void coreinitAsyncCallback_addWithLock(MPTR functionMPTR, uint32 numParameters, uint32 r3,
									   uint32 r4, uint32 r5, uint32 r6, uint32 r7, uint32 r8,
									   uint32 r9, uint32 r10)
{
	...
	if (functionMPTR >= 0x10000000)
	{
		cemuLog_log(
			LogType::Force,
			fmt::format(
				"Suspicious callback address {0:08x} params: {1:08x} {2:08x} {3:08x} {4:08x}",
				functionMPTR, r3, r4, r5, r6));
...
void coreinitAsyncCallback_add(MPTR functionMPTR, uint32 numParameters, uint32 r3, uint32 r4,
							   uint32 r5, uint32 r6, uint32 r7, uint32 r8, uint32 r9, uint32 r10)
{
...
void InitializeAsyncCallback()
{
	...
	coreinit::OSCreateThreadType(
		g_coreinitCallbackThread.GetPtr(),
		PPCInterpreter_makeCallableExportDepr(_coreinitCallbackThread), 0, nullptr,
		_g_coreinitCallbackThreadStack.GetPtr() + _g_coreinitCallbackThreadStack.GetByteSize(),
		(sint32)_g_coreinitCallbackThreadStack.GetByteSize(), 0, 7,
