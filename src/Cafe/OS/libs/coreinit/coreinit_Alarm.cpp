...
{
  public:
	OSHostAlarm(uint64 nextFire, uint64 period,
				void (*callbackFunc)(uint64 currentTick, void* context), void* context)
		: m_nextFire(nextFire), m_period(period), m_callbackFunc(callbackFunc), m_context(context)
	{
  ...
  public:
	struct ComparatorFireTime
	{
		bool operator()(OSHostAlarm* const& p1, OSHostAlarm* const& p2) const
		{
			...
			if (p1Fire == p2Fire)
				return (uintptr_t)p1 <
					   (uintptr_t)p2; // if time is equal, differ by pointer (to allow storing
									  // multiple alarms with same firing time)
...
OSHostAlarm* OSHostAlarmCreate(uint64 nextFire, uint64 period,
							   void (*callbackFunc)(uint64 currentTick, void* context),
							   void* context)
{
...
{
#ifdef ALARM_LOGGING
	cemuLog_log(LogType::Force, "[Alarm] Alarm ready and alarm thread signalled. Current tick: {}",
				currentTick);
#endif
	OSSignalEventInternal(g_alarmEvent.GetPtr());
}

void __OSInitiateAlarm(OSAlarm_t* alarm, uint64 startTime, uint64 period, MPTR handlerFunc,
					   bool isPeriodic)
{
	cemu_assert_debug(__OSHasSchedulerLock());
	...
	cemuLog_log(LogType::Force, "[Alarm] Start alarm 0x{:08x}. Func 0x{:08x}. Period: {}ms",
				MEMPTR(alarm).GetMPTR(), handlerFunc, periodInMS);
#endif
		...
		while (true)
		{
			...
			while (itr != g_activeAlarms.end())
			{
				if (currentTick >= _swapEndianU64(itr->first->nextTime))
				{
					...
					{
						alarm->nextTime = _swapEndianU64(_swapEndianU64(alarm->nextTime) +
														 _swapEndianU64(alarm->period));
					}
					break;
				}
				++itr;
...
#ifdef ALARM_LOGGING
			double periodInMS = (double)_swapEndianU64(alarm->period) * 1000.0 /
								(double)EspressoTime::GetTimerClock();
			cemuLog_log(
				LogType::Force,
				"[Alarm] Callback 0x{:08x} for alarm 0x{:08x}. Current tick: {}. Period: {}ms",
				_swapEndianU32(alarm->handler), MEMPTR(alarm).GetMPTR(), currentTick, periodInMS);
#endif
			PPCCoreCallback(_swapEndianU32(alarm->handler), alarm,
							&(g_alarmThread.GetPtr()->context));
		}
	}
}
...
void InitializeAlarm()
{
	...
	// init event
	OSInitEvent(g_alarmEvent.GetPtr(), OSEvent::EVENT_STATE::STATE_NOT_SIGNALED,
				OSEvent::EVENT_MODE::MODE_AUTO);

	// create alarm callback handler thread
	coreinit::OSCreateThreadType(
		g_alarmThread.GetPtr(), RPLLoader_MakePPCCallable(_OSAlarmThread), 0, nullptr,
		_g_alarmThreadStack.GetPtr() + _g_alarmThreadStack.GetByteSize(),
		(sint32)_g_alarmThreadStack.GetByteSize(), 0, 0x7, OSThread_t::THREAD_TYPE::TYPE_IO);
