...
// titles that utilize MP task queue: Yoshi's Woolly World, Fast Racing Neo, Tokyo Mirage Sessions,
// Mii Maker
...
/*
	...
	The race condition then happens when a worker thread calls MPDequeTask()/MPRunTask while
   MPInitTaskQ() is being executed on the main thread. Since MPInitTaskQ() (re)initializes the
   internal spinlock it's not thread-safe, leading to a corruption of the spinlock state for other
   threads
...
bool MPRunTask(MPTask* task)
{
	...
	if (taskQ->state != MP_TASKQ_STATE_STOPPING && taskQ->state != MP_TASKQ_STATE_STOP)
	{
		...
		task->taskFunc.result =
			PPCCoreCallback(task->taskFunc.func, task->taskFunc.data, task->taskFunc.size);
...
bool MPEnqueTask(MPTaskQ* taskq, MPTask* task)
{
	...
	if (task->taskState == MP_TASK_STATE_INIT)
	{
		...
		if ((uint32)taskq->endIndex[1] < taskq->taskQueueSize &&
			(taskQState == MP_TASKQ_STATE_INIT || taskQState == MP_TASKQ_STATE_RUN ||
			 taskQState == MP_TASKQ_STATE_STOPPING || taskQState == MP_TASKQ_STATE_STOP ||
			 taskQState == MP_TASKQ_STATE_DONE))
		{
			task->taskQ = taskq;
...
bool MPRunTasksFromTaskQ(MPTaskQ* taskq, int granularity)
{
	...
	while (true)
	{
		...
		// since we are having a granularity parameter, we might want to give the scheduler the
		// chance for other stuff when having multiple tasks
		if (result != 0)
...
bool MPWaitTaskQ(MPTaskQ* taskQ, uint32 waitState)
{
	...
	while (waitStop || waitDone || waitRun)
	{
		...
		if (loopCounter > 0)
			coreinit::OSSleepTicks(EspressoTime::ConvertNsToTimerTicks(
				50000)); // sleep thread for 0.05ms to give other threads a chance to run (avoids
						 // softlocks in YWW)
		else
			PPCCore_switchToScheduler();
		loopCounter++;
	...
	while (waitStop || waitDone || waitRun)
	{
		...
		if (OSGetSystemTime() >= endTime)
		{
			if (waitState == MP_TASKQ_STATE_DONE)
				cemuLog_log(
					LogType::Force,
					"MPWaitTaskQWithTimeout(): Timeout occurred while waiting for done-only state");
			return false;
		}
