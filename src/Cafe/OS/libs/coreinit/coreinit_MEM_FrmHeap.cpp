...
MEMFrmHeap* MEMCreateFrmHeapEx(void* memStart, uint32 size, uint32 createFlags)
{
	...
	MEMInitHeapBase(frmHeap, MEMHeapMagic::FRAME_HEAP,
					(void*)((uintptr_t)startAddr + sizeof(MEMFrmHeap)), (void*)endAddr,
					createFlags);
...
void* __FrmHeap_AllocFromTail(MEMFrmHeap* frmHeap, uint32 size, sint32 alignment)
{
	...
	if (allocStart >= head)
	{
		...
		if (frmHeap->HasOptionClear())
			memset(frmHeap->allocationTail.GetPtr(), 0,
				   prevTail.GetMPTR() - frmHeap->allocationTail.GetMPTR());
		return MEMPTR<void>(allocStart).GetPtr();
	}
	return nullptr;
}
...
bool MEMRecordStateForFrmHeap(MEMFrmHeap* frmHeap, uint32 id)
{
	...
	MEMFrmHeapRecordedState* rState = (MEMFrmHeapRecordedState*)__FrmHeap_AllocFromHead(
		frmHeap, sizeof(MEMFrmHeapRecordedState), 4); // modifies memHeap->allocationHead
