...
SysAllocator<OSThreadQueue>
	g_activeThreadQueue; // list of all threads (can include non-detached inactive threads)
...
void __OSActivateThread(OSThread_t* thread)
{
	...
	g_activeThreadQueue->addThread(
		thread, &thread->activeThreadChain); // todo - check if thread already in queue
...
void __OSDeactivateThread(OSThread_t* thread)
{
	...
	g_activeThreadQueue->removeThread(
		thread, &thread->activeThreadChain); // todo - check if thread in queue
...
void threadEntry(PPCInterpreter_t* hCPU)
{
	...
	// const uint32 im = OSDisableInterrupts(); -> on an actual Wii U interrupts are disabled for
	// this callback, but there are games that yield the thread in the callback (see Angry Birds
	// Star Wars)
	if (gCoreinitData->__cpp_exception_init_ptr != MPTR_NULL)
	{
		PPCCoreCallback(_swapEndianU32(gCoreinitData->__cpp_exception_init_ptr),
						&currentThread->crt.eh_globals);
	}
...
void OSCreateThreadInternal(OSThread_t* thread, uint32 entryPoint, MPTR stackLowerBaseAddr,
							uint32 stackSize, uint8 affinityMask,
							OSThread_t::THREAD_TYPE threadType)
{
	cemu_assert_debug(thread !=
					  nullptr); // make thread struct mandatory. Caller can always use SysAllocator
	__OSLockScheduler();
	...
	if (isThreadStillActive)
	{
		...
		while ((thread->state == OSThread_t::THREAD_STATE::STATE_READY ||
				thread->state == OSThread_t::THREAD_STATE::STATE_RUNNING) &&
		...
		{
			forceLog_printf("Calling OSCreateThread() on thread which is still active (Thread "
							"exited without detached flag). Forcing OSDetachThread()...");
			__OSUnlockScheduler();
	...
	if (thread == nullptr)
		thread = (OSThread_t*)memory_getPointerFromVirtualOffset(
			coreinit_allocFromSysArea(sizeof(OSThread_t), 32));
	...
	thread->state = (entryPoint != MPTR_NULL) ? OSThread_t::THREAD_STATE::STATE_READY
											  : OSThread_t::THREAD_STATE::STATE_NONE;
	...
	// for duplicated threads)
	thread->stackBase = _swapEndianU32(
		stackLowerBaseAddr +
		stackSize); // these fields are quite important and lots of games rely on them being
					// accurate (Examples: Darksiders 2, SMW3D, Batman Arkham City)
	thread->stackEnd = _swapEndianU32(stackLowerBaseAddr);
	// init stackpointer
	thread->context.gpr[GPR_SP] =
		_swapEndianU32(stackLowerBaseAddr + stackSize -
					   0x20); // how many free bytes should there be at the beginning of the stack?
	// init misc stuff
	...
	thread->id = 0x8000; // Warriors Orochi 3 softlocks if this is zero due to confusing threads
						 // (_OSActivateThread should set this?)
	// init ugqr
...
bool OSCreateThreadType(OSThread_t* thread, MPTR entryPoint, sint32 numParam, void* ptrParam,
						void* stackTop2, sint32 stackSize, sint32 priority, uint32 attr,
						OSThread_t::THREAD_TYPE threadType)
{
	OSCreateThreadInternal(thread, entryPoint,
						   memory_getVirtualOffsetFromPointer(stackTop2) - stackSize, stackSize,
						   attr, threadType);
	thread->context.gpr[3] = _swapEndianU32(numParam); // num arguments
	thread->context.gpr[4] =
		_swapEndianU32(memory_getVirtualOffsetFromPointer(ptrParam)); // arguments pointer
...
bool OSCreateThread(OSThread_t* thread, MPTR entryPoint, sint32 numParam, void* ptrParam,
					void* stackTop2, sint32 stackSize, sint32 priority, uint32 attr)
{
	return OSCreateThreadType(thread, entryPoint, numParam, ptrParam, stackTop2, stackSize,
							  priority, attr, OSThread_t::THREAD_TYPE::TYPE_APP);
}
...
bool OSRunThread(OSThread_t* thread, MPTR funcAddress, sint32 numParam, void* ptrParam)
{
	...
	cemu_assert_debug(ppcInterpreterCurrentInstance == nullptr ||
					  OSGetCurrentThread() !=
						  thread); // called on self, what should this function do?
...
void OSExitThread(sint32 exitValue)
{
	...
	if (!threadBE->cleanupCallback2.IsNull())
	{
		...
		PPCCoreCallback(threadBE->cleanupCallback2.GetMPTR(), threadBE,
						_swapEndianU32(threadBE->stackEnd));
	}
	// cpp exception cleanup
	if (gCoreinitData->__cpp_exception_cleanup_ptr != 0 && threadBE->crt.eh_globals != nullptr)
	{
		PPCCoreCallback(_swapEndianU32(gCoreinitData->__cpp_exception_cleanup_ptr),
						&threadBE->crt.eh_globals);
...
void OSSleepTicks(uint64 ticks)
{
	...
	__OSLockScheduler();
	OSHostAlarm* hostAlarm = OSHostAlarmCreate(
		coreinit_getOSTime() + ticks, 0, _OSSleepTicks_alarmHandler, _threadQueue.GetPointer());
...
bool OSJoinThread(OSThread_t* thread, uint32be* exitValue)
{
	...
	if ((thread->attr & OSThread_t::ATTR_DETACHED) == 0 &&
		thread->state != OSThread_t::THREAD_STATE::STATE_MORIBUND)
	{
...
sint32 __OSResumeThreadInternal(OSThread_t* thread, sint32 resumeCount)
{
	...
	if (thread->suspendCounter == 0)
	{
		...
		if (ppcInterpreterCurrentInstance &&
			__OSCoreShouldSwitchToThread(coreinit::OSGetCurrentThread(), thread))
			PPCCore_switchToSchedulerWithLock();
	}
...
void __OSSuspendThreadInternal(OSThread_t* thread)
{
	...
	cemu_assert_debug(thread->state != OSThread_t::THREAD_STATE::STATE_NONE &&
					  thread->state !=
						  OSThread_t::THREAD_STATE::STATE_MORIBUND); // how to handle these?
...
{
	__OSLockScheduler();
	cemu_assert_debug(thread->requestFlags == 0 ||
					  thread->requestFlags ==
						  OSThread_t::REQUEST_FLAG_CANCEL); // todo - how to handle cases where
															// other flags are already set?
...
void __OSStoreThread(OSThread_t* thread, PPCInterpreter_t* hCPU)
{
	...
	thread->requestFlags = (OSThread_t::REQUEST_FLAG_BIT)(
		thread->requestFlags &
		OSThread_t::REQUEST_FLAG_CANCEL); // remove all flags except cancel flag

	// update total cycles
	uint64 remainingCycles = std::min((uint64)ppcInterpreterCurrentInstance->remainingCycles,
									  (uint64)thread->quantumTicks);
...
// this is necessary since we can't block in __OSThreadSwitchToNext() (__OSStoreThread + thread
// switch must happen inside same scheduler lock)
void __OSThreadCoreIdle(void* unusedParam)
{
	...
	while (true)
	{
		if (!g_coreRunQueueThreadCount[coreIndex].isZero()) // avoid hammering the lock on the main
															// core if there is no runable thread
		{
			__OSLockScheduler();
...
void __OSThreadSwitchToNext()
{
	...
	// if (ppcInterpreterCurrentInstance)
	//	debug_printf("Core %d store thread %08x (t = %d)\n", hostThread->ppcInstance.sprNew.UPIR,
	// memory_getVirtualOffsetFromPointer(hostThread->thread), t_assignedCoreIndex);
	...
	cemu_assert_debug(__OSHasSchedulerLock());
	cemu_assert_debug(g_isMulticoreMode == false ||
					  hostThread->selectedCore == t_assignedCoreIndex);
...
void __OSFiberThreadEntry(void* _thread)
{
	...
	__OSUnlockScheduler(); // lock is always held when switching to a fiber, so we need to unlock it
						   // here
	while (true)
	{
		// run one timeslice
		...
		// we add a slight randomized variance to the thread quantum to avoid getting stuck in
		// repeated code sequences where the duration matches the thread quantum perfectly this was
		// seen in Mario Party 10 on launch where several OSLockMutex operations would align in such
		// a way that one thread would never successfully acquire the lock
...
void OSSchedulerEnd()
{
	...
	for (size_t i = 0; i < Espresso::CORE_COUNT; i++)
		g_coreRunQueueThreadCount[i].increment(); // make sure to wake up cores if they are paused
												  // and waiting for runnable threads
...
struct TerminatorThread
{
	struct DeallocatorQueueEntry
	{
		...
		DeallocatorQueueEntry(OSThread_t* thread, MEMPTR<void> stack, MEMPTR<void> deallocatorFunc)
			: thread(thread), stack(stack), deallocatorFunc(deallocatorFunc){};
...
void __OSQueueThreadDeallocation(OSThread_t* thread)
{
	uint32 coreIndex = OSGetCoreId();
	TerminatorThread::DeallocatorQueueEntry queueEntry(
		thread, memory_getPointerFromVirtualOffset(_swapEndianU32(thread->stackEnd)),
		thread->deallocatorFunc);
	s_terminatorThreads[coreIndex].queueDeallocators.push(queueEntry);
	OSSignalSemaphoreInternal(s_terminatorThreads[coreIndex].semaphoreQueuedDeallocators.GetPtr(),
							  false); // do not reschedule here! Current thread must not be
									  // interrupted otherwise deallocator will run too early
}
...
void __OSInitTerminatorThreads()
{
	for (sint32 i = 0; i < PPC_CORE_COUNT; i++)
	{
		...
		OSCreateThreadType(s_terminatorThreads[i].terminatorThread.GetPtr(),
						   PPCInterpreter_makeCallableExportDepr(__OSTerminatorThreadFunc), 0,
						   nullptr,
						   (uint8*)s_terminatorThreads[i].threadStack.GetPtr() +
							   s_terminatorThreads[i].threadStack.GetByteSize(),
						   (sint32)s_terminatorThreads[i].threadStack.GetByteSize(), 0, 1 << i,
						   OSThread_t::THREAD_TYPE::TYPE_IO);
		OSSetThreadName(s_terminatorThreads[i].terminatorThread.GetPtr(),
						s_terminatorThreads[i].threadName.GetPtr());
...
void __OSInitDefaultThreads()
{
	for (sint32 i = 0; i < PPC_CORE_COUNT; i++)
	{
		...
		coreinit::OSCreateThreadType(coreDefaultThread, MPTR_NULL, 0, nullptr,
									 (uint8*)stackBase + stackSize, stackSize, 16, 1 << i,
									 OSThread_t::THREAD_TYPE::TYPE_APP);
...
{
	return memory_getVirtualOffsetFromPointer(
		coreinit::__currentCoreThread[PPCInterpreter_getCoreIndex(hCPU)]);
}
