...
struct
{
	...
	uint32 startOffset;
	uint32 size;
} fgAreaEntries[FG_BUCKET_AREA_COUNT] = {{0, 0, 0x2800000},		   {7, 0x2800000, 0x400000},
										 {1, 0x2C00000, 0x900000}, {2, 0x3500000, 0x3C0000},
										 {3, 0x38C0000, 0x1C0000}, {4, 0x3A80000, 0x3C0000},
										 {5, 0x3E40000, 0x1BF000}, {6, 0x3FFF000, 0x1000}};
...
void coreinitExport_OSGetForegroundBucket(PPCInterpreter_t* hCPU)
{
	...
	bool r = OSGetForegroundBucket(
		(MEMPTR<void>*)memory_getPointerFromVirtualOffsetAllowNull(areaOutput),
		(uint32be*)memory_getPointerFromVirtualOffsetAllowNull(areaSize));
...
void coreinitExport_OSGetForegroundBucketFreeArea(PPCInterpreter_t* hCPU)
{
	...
	ppcDefineParamMPTR(areaOutput, 0);
	ppcDefineParamMPTR(areaSize, 1);
	bool r = OSGetForegroundBucketFreeArea(
		(MPTR*)memory_getPointerFromVirtualOffsetAllowNull(areaOutput),
		(MPTR*)memory_getPointerFromVirtualOffsetAllowNull(areaSize));
	osLib_returnFromFunction(hCPU, r ? 1 : 0);
}
...
void InitializeFG()
{
	...
	osLib_addFunction("coreinit", "OSGetForegroundBucketFreeArea",
					  coreinitExport_OSGetForegroundBucketFreeArea);
