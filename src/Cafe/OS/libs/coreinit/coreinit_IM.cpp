...
void coreinitExport_IM_GetHomeButtonParams(PPCInterpreter_t* hCPU)
{
	...
	// todo
	// note: No idea what these values mean. But they were chosen so that the Browser (surf.rpx)
	// does not OSPanic()
...
void InitializeIM()
{
	osLib_addFunction("coreinit", "IMIsAPDEnabledBySysSettings",
					  coreinitExport_IMIsAPDEnabledBySysSettings);
