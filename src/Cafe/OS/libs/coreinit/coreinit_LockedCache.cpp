...
namespace coreinit
{
uint8 lcCacheMask[PPC_CORE_COUNT][(LC_LOCKED_CACHE_SIZE + LC_LOCKED_CACHE_GRANULARITY - 1) /
								  LC_LOCKED_CACHE_GRANULARITY] = {0};
...
{
	// debug_printf("LCAlloc(0x%04x) Thread %08x Core %d", hCPU->gpr[3],
	// coreinitThread_getCurrentThread(hCPU), PPCInterpreter_getCoreIndex(hCPU));
	...
	for (uint32 i = 0; i <= (LC_LOCKED_CACHE_SIZE / LC_LOCKED_CACHE_GRANULARITY) - entryMaskLength;
		 i++)
	{
		...
		if (rangeIsFree)
		{
			...
			// return allocAddr
			// debug_printf("LCAlloc result %08x Thread %08x Core %d", (uint32)allocAddr,
			// coreinitThread_getCurrentThread(hCPU), PPCInterpreter_getCoreIndex(hCPU));
	...
	// debug_printf("LCAlloc failed Thread %08x Core %d", coreinitThread_getCurrentThread(hCPU),
	// PPCInterpreter_getCoreIndex(hCPU));
	osLib_returnFromFunction(hCPU, MPTR_NULL);
}
...
void coreinitExport_LCDealloc(PPCInterpreter_t* hCPU)
{
	...
	if (deallocAddr < lcAddr[coreIndex] ||
		deallocAddr >= (lcAddr[coreIndex] + LC_LOCKED_CACHE_SIZE))
	{
		// out of bounds
	...
	if (startIndex > 0 && (cacheMask[startIndex - 1] != LC_MASK_RESERVED_END &&
						   cacheMask[startIndex - 1] != LC_MASK_FREE))
	{
...
{
	// on the real HW, LC is shared between processes and not all processes can access it. In the
	// emulator we don't care and always return true.
	osLib_returnFromFunction(hCPU, 1);
}
...
void InitializeLC()
{
	...
	{
		memset(lcCacheMask[f], LC_MASK_FREE,
			   (LC_LOCKED_CACHE_SIZE + LC_LOCKED_CACHE_GRANULARITY - 1) /
				   LC_LOCKED_CACHE_GRANULARITY);
	}
