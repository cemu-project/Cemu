...
static_assert(offsetof(MCPTitleInfo, titleVersion) == 0x48,
			  "title entry titleVersion has invalid offset");
static_assert(offsetof(MCPTitleInfo, osVersion) == 0x4A,
			  "title entry osVersion has invalid offset");
...
void coreinitExport_MCP_GetSysProdSettings(PPCInterpreter_t* hCPU)
{
	...
	sint32 result = MCP_GetSysProdSettings(
		hCPU->gpr[3], (SysProdSettings*)memory_getPointerFromVirtualOffset(hCPU->gpr[4]));
	osLib_returnFromFunction(hCPU, result);
}

void coreinitExport_MCP_TitleListByAppType(PPCInterpreter_t* hCPU)
{
	forceLogDebug_printf("MCP_TitleListByAppType(0x%08x,0x%08x,0x%08x,0x%08x,0x%08x)", hCPU->gpr[3],
...
void coreinitExport_MCP_GetTitleInfoByTitleAndDevice(PPCInterpreter_t* hCPU)
{
	...
	if (mcpRequest->titleListRequest.titleCount == 0)
	{
		...
		osLib_returnFromFunction(
			hCPU, BUILD_NN_RESULT(
					  NN_RESULT_LEVEL_STATUS, NN_RESULT_MODULE_MCP,
					  0)); // E-Shop/nn_vctl.rpl expects error to be returned when no title is found
		return;
	}
...
void export_MCP_Get4SecondOffStatus(PPCInterpreter_t* hCPU)
{
	...
	// if this returns 1 then Barista will display the warning about cold-shutdown ('Holding the
	// POWER button for at least four seconds...')
	osLib_returnFromFunction(hCPU, 0);
}
...
typedef struct
{
	...
	/* +0x30F */ uint32be
		storageSubindexOrMask; // the id in the storage path, but this might also be a MASK of
							   // indices (e.g. 1 -> Only device 1, 7 -> Device 1,2,3) men.rpx
							   // expects 0xF (or 0x7?) to be set for MLC, SLC and USB for
							   // MLC_FullDeviceList
	uint8 ukn313[4];
	uint8 ukn317[4];
} MCPDevice_t;
...
static_assert(offsetof(MCPDevice_t, storagePath) == 0x090,
			  "MCPDevice_t.storagePath has invalid offset");
static_assert(offsetof(MCPDevice_t, storageSubindexOrMask) == 0x30F,
			  "MCPDevice_t.storageSubindex has invalid offset");
...
void MCP_DeviceListEx(uint32 mcpHandle, uint32be* deviceCount, MCPDevice_t* deviceList,
					  uint32 deviceListSize, bool returnFullList)
{
	...
	for (sint32 f = 0; f < 1; f++)
	{
		...
		sprintf(deviceList[index].storagePath, "/vol/storage_%s%02x", deviceList[index].storageName,
				(sint32)deviceList[index].storageSubindexOrMask);
		index++;
		// 1
...
void export_MCP_DeviceList(PPCInterpreter_t* hCPU)
{
	...
	sprintf(deviceList[0].storagePath, "/vol/storage_%s%02x", deviceList[0].storageName,
			(sint32)deviceList[0].storageSubindexOrMask);
	// 1
	strcpy(deviceList[1].storageName, "slc");
	deviceList[1].storageSubindexOrMask = (0x01); // bitmask?
	sprintf(deviceList[1].storagePath, "/vol/storage_%s%02x", deviceList[1].storageName,
			(sint32)deviceList[1].storageSubindexOrMask);

	// 2
	// strcpy(deviceList[2].storageName, "usb");
	// deviceList[2].storageSubindex = 1;
	// sprintf(deviceList[2].storagePath, "/vol/storage_%s%02x", deviceList[2].storageName,
	// (sint32)deviceList[2].storageSubindex);
...
void InitializeMCP()
{
	...
	osLib_addFunction("coreinit", "MCP_GetTitleInfoByTitleAndDevice",
					  coreinitExport_MCP_GetTitleInfoByTitleAndDevice);
...
void coreinitExport_UCReadSysConfig(PPCInterpreter_t* hCPU)
{
	...
	// r5 = UCParamStruct_t*
	UCParamStruct_t* ucParamBase =
		(UCParamStruct_t*)memory_getPointerFromVirtualOffset(hCPU->gpr[5]);
	...
	for (uint32 i = 0; i < ucParamCount; i++)
	{
		...
		else if (_strcmpi(ucParam->settingName, "cafe.initial_launch") == 0)
		{
			...
			// 0x01		Inited but no user created yet (setting this will make the home menu go into
			// user creation dialog) 0x02		Inited, with user other values are unknown
		...
		else if (_strcmpi(ucParam->settingName, "p_acct1.net_communication_on_game") == 0)
		{
			...
			// note: This option is account-bound, the p_acct1 prefix indicates that the account in
			// slot 1 is used account in slot 1
			if (ucParam->resultPtr != _swapEndianU32(MPTR_NULL))
		...
		else if (_strcmpi(ucParam->settingName, "p_acct1.network_launcher") == 0)
		{
			...
			if (ucParam->resultPtr != _swapEndianU32(MPTR_NULL))
				memory_writeU8(_swapEndianU32(ucParam->resultPtr),
							   0); // data type is guessed (0 -> no restrictions, 1 -> read only?, 2
								   // -> no access?)
		}
		else if (_strcmpi(ucParam->settingName, "s_acct01.uuid") == 0)
		{
			if (ucParam->resultPtr != _swapEndianU32(MPTR_NULL) && ucParam->ukn4_size >= 37)
			{
				...
				char tempStr[64];
				sprintf(
					tempStr, "%02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X",
					uuid[0], uuid[1], uuid[2], uuid[3], uuid[4], uuid[5], uuid[6], uuid[7], uuid[8],
					uuid[9], uuid[10], uuid[11], uuid[12], uuid[13], uuid[14], uuid[15]);
				strcpy(
					(char*)memory_getPointerFromVirtualOffset(_swapEndianU32(ucParam->resultPtr)),
					tempStr);
			}
		}
