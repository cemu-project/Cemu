...
void* coreinit_memcpy(MEMPTR<void> dst, MEMPTR<void> src, uint32 size)
{
	if (dst.GetMPTR() == 0xFFFFFFFF)
	{
		...
		// this may be a bug in the game. The last few bytes of the address space are writable, but
		// wrap-around behavior of COS memcpy is unknown
...
void OSGetMemBound(sint32 memType, MPTR* offsetOutput, uint32* sizeOutput)
{
	...
	*/
	...
	else if (memType == 2)
	{
		...
		// due to differences in our library implementations we currently allocate less memory for
		// the OS/RPLs than on the actual hardware, as a result more memory is available to games
