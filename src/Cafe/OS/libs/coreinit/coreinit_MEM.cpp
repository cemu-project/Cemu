...
// the system area is a block of memory that exists only in the emulator. It is used to simplify
// dynamic memory allocation for system data this partially overlaps with the system heap from
// coreinit_SysHeap.cpp -> Use SysHeap for everything
...
MPTR coreinit_allocFromSysArea(uint32 size, uint32 alignment)
{
	...
	// deprecated, use OSAllocFromSystem instead (for everything but SysAllocator which probably
	// should use its own allocator?)
...
{
	for (MEMHeapBase* obj = (MEMHeapBase*)MEMGetFirstListObject(list); obj;
		 obj = (MEMHeapBase*)MEMGetNextListObject(list, obj))
	{
...
MEMList* MEMList_FindContainingHeap(MEMHeapBase* head)
{
	...
	if ((uintptr_t)memBound.GetPtr() > (uintptr_t)head ||
		(uintptr_t)head >= (uintptr_t)memBound.GetPtr() + (uint32)memBoundSize)
	{
		if ((uintptr_t)bucket.GetPtr() > (uintptr_t)head ||
			(uintptr_t)head >= (uintptr_t)bucket.GetPtr() + (uint32)bucketSize)
		{
...
MEMHeapBase* MEMSetBaseHeapHandle(uint32 index, MEMHeapBase* heapBase)
{
	...
	if (sHeapBaseHandle[index] != nullptr)
	{
		cemuLog_log(LogType::Force,
					"MEMSetBaseHeapHandle(): Trying to assign heap to non-empty slot");
		return sHeapBaseHandle[index];
	}
...
MEMHeapBase* MEMFindContainHeap(const void* memBlock)
{
	...
	MEMHeapBase* result;
	if ((uintptr_t)memBound.GetPtr() > (uintptr_t)memBlock ||
		(uintptr_t)memBlock >= (uintptr_t)memBound.GetPtr() + (uint32)memBoundSize)
	{
		if ((uintptr_t)bucket.GetPtr() > (uintptr_t)memBlock ||
			(uintptr_t)memBlock >= (uintptr_t)bucket.GetPtr() + (uint32)bucketSize)
		{
...
{
	MEMInitHeapBase((MEMHeapBase*)heapAddress, MEMHeapMagic::USER_HEAP, (uint8*)heapAddress + 0x40,
					(uint8*)heapAddress + heapSize, 0);
	return heapAddress;
}
...
void* default_MEMAllocFromDefaultHeap(uint32 size)
{
	...
	coreinitMemLog_printf("MEMAllocFromDefaultHeap(0x%08x) Result: 0x%08x", size,
						  memory_getVirtualOffsetFromPointer(mem));
	return mem;
}
...
void* default_MEMAllocFromDefaultHeapEx(uint32 size, sint32 alignment)
{
	...
	coreinitMemLog_printf("MEMAllocFromDefaultHeap(0x%08x,%d) Result: 0x%08x", size, alignment,
						  memory_getVirtualOffsetFromPointer(mem));
	return mem;
}
...
{
	MEMPTR<void> r{
		PPCCoreCallback(gCoreinitData->MEMAllocFromDefaultHeapEx.GetMPTR(), size, alignment)};
	return r.GetPtr();
}
...
void coreinitExport_MEMInitAllocatorForDefaultHeap(PPCInterpreter_t* hCPU)
{
	...
	gDefaultHeapAllocator->funcAlloc =
		PPCInterpreter_makeCallableExportDepr(_DefaultHeapAllocator_Alloc);
	gDefaultHeapAllocator->funcFree =
		PPCInterpreter_makeCallableExportDepr(_DefaultHeapAllocator_Free);
					  ...
					  MEMPTR<MEMHeapBase>& mem2Heap)
{
	...
	gCoreinitData->MEMAllocFromDefaultHeap =
		RPLLoader_MakePPCCallable(export_default_MEMAllocFromDefaultHeap);
	gCoreinitData->MEMAllocFromDefaultHeapEx =
		RPLLoader_MakePPCCallable(export_default_MEMAllocFromDefaultHeapEx);
	gCoreinitData->MEMFreeToDefaultHeap =
		RPLLoader_MakePPCCallable(export_default_MEMFreeToDefaultHeap);
	...
	OSDynLoad_SetAllocator(RPLLoader_MakePPCCallable(default_DynLoadAlloc),
						   RPLLoader_MakePPCCallable(default_DynLoadFree));
	OSDynLoad_SetTLSAllocator(RPLLoader_MakePPCCallable(default_DynLoadAlloc),
							  RPLLoader_MakePPCCallable(default_DynLoadFree));
}
...
void InitializeMEM()
{
	...
	osLib_addFunction("coreinit", "MEMInitAllocatorForDefaultHeap",
					  coreinitExport_MEMInitAllocatorForDefaultHeap);
