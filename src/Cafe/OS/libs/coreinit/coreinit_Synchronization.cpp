...
bool OSWaitEventWithTimeout(OSEvent* event, uint64 timeout)
{
	...
	else
	{
		...
		// workaround for a bad implementation in some Unity games (like Qube Directors Cut, see
		// FEventWiiU::Wait) where the the return value of OSWaitEventWithTimeout is ignored and
		// instead the game measures the elapsed time to determine if a timeout occurred
		timeout = timeout * 98ULL /
				  100ULL; // 98% (we want the function to return slightly before the actual timeout)
		...
		auto hostAlarm = coreinit::OSHostAlarmCreate(
			coreinit::coreinit_getOSTime() + coreinit::EspressoTime::ConvertNsToTimerTicks(timeout),
			0, _OSWaitEventWithTimeoutHandler, &data);
		event->threadQueue.queueAndWait(OSGetCurrentThread());
...
void OSLockMutexInternal(OSMutex* mutex)
{
	...
	while (true)
	{
		if (mutex->owner == nullptr)
		{
			...
			mutex->lockCount = 1;
			// cemu_assert_debug(mutex->next == nullptr && mutex->prev == nullptr); -> not zero
			// initialized
...
void OSFastMutex_LockInternal(OSFastMutex* fastMutex)
{
	...
	while (true)
	{
		if (fastMutex->owner.atomic_compare_exchange(nullptr,
													 currentThread)) //(fastMutex->owner == nullptr)
		{
			// acquire lock
...
void OSFastMutex_UnlockInternal(OSFastMutex* fastMutex)
{
	...
	if (fastMutex->owner != currentThread)
	{
		...
		// forceLog_printf("OSFastMutex_Unlock() called on mutex which is not owned by current
		// thread");
		_OSFastMutex_ReleaseContention(fastMutex);
		return;
	}
...
{
	OSInitEvent(g_rendezvousEvent.GetPtr(), OSEvent::EVENT_STATE::STATE_NOT_SIGNALED,
