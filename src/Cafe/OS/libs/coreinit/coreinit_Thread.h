...
struct OSThreadQueueInternal
{
  ...
  private:
	...
	{
		cemu_assert_debug((void*)threadLink >= (void*)thread &&
						  (void*)threadLink < (void*)((uint8*)thread + 0x680));
...
struct OSThread_t
{
	...
	/* +0x325 */ uint8 attr;
	/* +0x326 */ uint16be id; // Warriors Orochi 3 uses this to identify threads. Seems like this is
							  // always set to 0x8000 ?
	...
	/* +0x334 */ uint32be
		exitValue; // set by OSExitThread() or by returning from the thread entrypoint

	/* +0x338 */ MEMPTR<coreinit::OSThreadQueue>
		currentRunQueue[Espresso::CORE_COUNT]; // points to the OSThreadQueue on which this thread
											   // is (null if not in queue)
	/* +0x344 */ coreinit::OSThreadLink linkRun[Espresso::CORE_COUNT];

	// general wait queue / thread dependency queue
	/* +0x35C */ MEMPTR<coreinit::OSThreadQueueInternal>
		currentWaitQueue; // shared between OSThreadQueue and OSThreadQueueSmall
	/* +0x360 */ coreinit::OSThreadLink waitQueueLink;

	/* +0x368 */ coreinit::OSThreadQueue joinQueue;

	/* +0x378 */ MEMPTR<coreinit::OSMutex>
		waitingForMutex; // set when thread is waiting for OSMutex to unlock
	/* +0x37C */ coreinit::OSMutexQueue mutexQueue;

	/* +0x38C */ coreinit::OSThreadLink
		activeThreadChain; // queue of active threads (g_activeThreadQueue)
	...
	/* +0x5D4 */ uint32 stateFlags; // 0x5D4 | various flags? Controls if canceling/suspension is
									// allowed (at cancel points) or not? If 1 -> Cancel/Suspension
									// not allowed, if 0 -> Cancel/Suspension allowed
	...
	/* +0x618 */ uint64be
		wakeUpTime; // time when thread first became active (Should only be set once(?) but we
					// currently set this on every timeslice since thats more useful for debugging)
...
void OSInitEventEx(OSEvent* event, OSEvent::EVENT_STATE initialState, OSEvent::EVENT_MODE mode,
				   void* userData);
