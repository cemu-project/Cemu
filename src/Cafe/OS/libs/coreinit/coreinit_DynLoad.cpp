...
uint32 OSDynLoad_Release(uint32 moduleHandle)
{
	...
	// this function isn't supposed to return anything, but early versions of Cemu did and Cemuhook
	// (up to 0.5.7.6) now relies on it. We still keep the return value around for compatibility
	return 0;
}

uint32 OSDynLoad_FindExport(uint32 moduleHandle, uint32 isData, const char* exportName,
							betype<MPTR>* addrOut)
{
