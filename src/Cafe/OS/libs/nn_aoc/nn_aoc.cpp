	...
	if (volDirIterator)
	{
		...
		while (fsc_nextDir(volDirIterator, &dirEntry))
		{
			...
			if (dirName.size() != (3 + 16))
				continue;
			if (dirName[0] != 'a' || dirName[1] != 'o' || dirName[2] != 'c')
				continue;
			TitleId aocTitleId;
...
AOC_RESULT AOC_ListTitle(uint32be* titleCountOut, AOCTitle* titleList, uint32 maxCount,
						 void* workBuffer, uint32 workBufferSize)
{
	generateAOCList();
...
AOC_RESULT AOC_OpenTitle(char* pathOut, AOCTitle* aocTitleInfo, void* workBuffer,
...
AOC_RESULT AOC_GetPurchaseInfo(uint32be* purchaseBoolArrayOut, uint64 titleId, uint16be* entryIds,
							   uint32 entryCount, void* workBuffer, uint32 workBufferSize)
{
	// open ticket file
	// on an actual Wii U they get stored to SLC but the download manager places these in the code
	// folder currently
