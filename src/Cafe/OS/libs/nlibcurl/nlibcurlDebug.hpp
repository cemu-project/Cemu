...
void curlDebug_markActiveRequest(CURL_t* curl)
{
	...
	wchar_t filePath[256];
	swprintf(filePath, sizeof(filePath) / sizeof(wchar_t),
			 L"dump//curl//session%u//request%04d_param.txt", _curlDebugSessionId,
			 (sint32)curl->debug.activeRequestIndex);
	...
	if (curl->debug.file_requestParam)
	{
		...
		curl->debug.file_requestParam->writeStringFmt(
			"Request %d %d-%d-%d %d:%02d:%02d\r\n", (sint32)curl->debug.activeRequestIndex,
...
void curlDebug_logEasySetOptStr(CURL_t* curl, const char* optName, const char* optValue)
{
	...
	if (curl->debug.file_requestParam)
	{
		curl->debug.file_requestParam->writeStringFmt("SetOpt %s: ", optName,
													  optValue ? optValue : "NULL");
		if (optValue)
...
void curlDebug_resultWrite(CURL_t* curl, char* ptr, size_t size, size_t nmemb)
{
	...
	if (curl->debug.file_responseRaw == nullptr)
	{
		wchar_t filePath[256];
		swprintf(filePath, sizeof(filePath) / sizeof(wchar_t),
				 L"dump//curl//session%u//request%04d_responseRaw.bin", _curlDebugSessionId,
				 (sint32)curl->debug.responseRequestIndex);
...
void curlDebug_headerWrite(CURL_t* curl, char* ptr, size_t size, size_t nmemb)
{
	...
	if (curl->debug.file_responseHeaders == nullptr)
	{
		wchar_t filePath[256];
		swprintf(filePath, sizeof(filePath) / sizeof(wchar_t),
				 L"dump//curl//session%u//request%04d_responseHeaders.txt", _curlDebugSessionId,
				 (sint32)curl->debug.responseRequestIndex);
