...
struct CURL_t
{
	...
	struct
	{
		...
		bool hasDumpedResultInfo{}; // set once the response info has been dumped (reset when next
...
void CurlWorkerThread(CURL_t* curl, PPCConcurrentQueue<QueueMsg_t>* callerQueue,
...
uint32 SendOrderToWorker(CURL_t* curl, QueueOrder order, uint32 arg1 = 0)
{
	...
	while (true)
	{
		...
		if (msg.order == QueueOrder_ProgressCB)
		{
			...
			sendMsg.result = PPCCoreCallback(
				curl->fprogress.GetMPTR(), curl->progress_client.GetMPTR(), msg.progress_cb.dltotal,
				msg.progress_cb.dlnow, msg.progress_cb.ultotal, msg.progress_cb.ulnow);
		...
		else if (msg.order == QueueOrder_HeaderCB)
		{
			...
			memcpy(tmp.GetPointer(), msg.header_cb.buffer,
				   msg.header_cb.size * msg.header_cb.nitems);

			QueueMsg_t sendMsg = {};
			sendMsg.order = QueueOrder_CBDone;
			sendMsg.result =
				PPCCoreCallback(curl->fwrite_header.GetMPTR(), tmp.GetMPTR(), msg.header_cb.size,
								msg.header_cb.nitems, curl->writeheader.GetMPTR());
			threadQueue.push(sendMsg);
		}
		else if (msg.order == QueueOrder_ReadCB)
		{
			...
			QueueMsg_t sendMsg = {};
			sendMsg.order = QueueOrder_CBDone;
			forceLogDebug_printf("QueueOrder_ReadCB size: %d nitems: %d", msg.read_cb.size,
								 msg.read_cb.nitems);
			sendMsg.result =
				PPCCoreCallback(curl->fread_func_set.GetMPTR(), tmp.GetMPTR(), msg.read_cb.size,
								msg.read_cb.nitems, curl->in_set.GetMPTR());
		...
		else if (msg.order == QueueOrder_WriteCB)
		{
			...
			sendMsg.order = QueueOrder_CBDone;
			sendMsg.result =
				PPCCoreCallback(curl->fwrite_func.GetMPTR(), tmp.GetMPTR(), msg.write_cb.size,
								msg.write_cb.nmemb, curl->out.GetMPTR());
			threadQueue.push(sendMsg);
		}
...
void export_curl_multi_add_handle(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("curl_multi_add_handle(0x%08x, 0x%08x) -> 0x%x", curlm.GetMPTR(),
						 curl.GetMPTR(), result);
...
void export_curl_multi_remove_handle(PPCInterpreter_t* hCPU)
{
	...
	if (result == CURLM_OK)
	{
		curlm->curl.erase(std::remove(curlm->curl.begin(), curlm->curl.end(), (void*)curl.GetPtr()),
						  curlm->curl.end());
	}

	forceLogDebug_printf("curl_multi_remove_handle(0x%08x, 0x%08x) -> 0x%x", curlm.GetMPTR(),
						 curl.GetMPTR(), result);
	osLib_returnFromFunction(hCPU, result);
}
...
void export_curl_multi_timeout(PPCInterpreter_t* hCPU)
{
	...
	*timeoParam = (uint32)timeoutLE;
	...
	forceLogDebug_printf("curl_multi_timeout(0x%08x, 0x%08x [%d]) -> 0x%x", curlm.GetMPTR(),
						 timeoParam.GetMPTR(), timeoutLE, result);
	osLib_returnFromFunction(hCPU, result);
}
...
void export_curl_multi_perform(PPCInterpreter_t* hCPU)
{
	...
	// forceLogDebug_printf("curl_multi_perform(0x%08x, 0x%08x)", curlm.GetMPTR(),
	// runningHandles.GetMPTR());
	...
	*(runningHandles.GetPtr()) = tempRunningHandles;
	forceLogDebug_printf(
		"curl_multi_perform(0x%08x, 0x%08x) -> 0x%x (running handles: %d) LR: 0x%08x",
		curlm.GetMPTR(), runningHandles.GetMPTR(), result, tempRunningHandles, hCPU->spr.LR);
...
void export_curl_multi_fdset(PPCInterpreter_t* hCPU)
{
	...
	CURLMcode result =
		curl_multi_fdset(curlm->curlm, &h_readFd, &h_writeFd, &h_exceptionFd, &h_maxFd);
...
void export_curl_multi_setopt(PPCInterpreter_t* hCPU)
{
	...
	case CURLMOPT_MAXCONNECTS:
		...
		break;
	default:
		forceLogDebug_printf("curl_multi_setopt(0x%08x, %d, 0x%08x) unsupported option",
							 curlm.GetMPTR(), option, parameter.GetMPTR());
	}

	forceLogDebug_printf("curl_multi_setopt(0x%08x, %d, 0x%08x) -> 0x%x", curlm.GetMPTR(), option,
						 parameter.GetMPTR(), result);
...
void export_curl_multi_info_read(PPCInterpreter_t* hCPU)
{
	...
	if (msg)
	{
		MEMPTR<CURLMsg_t> result{
			PPCCoreCallback(g_nlibcurl.malloc.GetMPTR(), ppcsizeof<CURLMsg_t>())};
		result->msg = msg->msg;
		...
		forceLogDebug_printf(
			"curl_multi_info_read(0x%08x, 0x%08x [%d]) -> 0x%08x (0x%x, 0x%08x, 0x%x)",
			curlm.GetMPTR(), msgsInQueue.GetMPTR(), *msgsInQueue.GetPtr(), result.GetMPTR(),
			msg->msg, result->curl.GetMPTR(), msg->data.result);
		osLib_returnFromFunction(hCPU, result.GetMPTR());
	}
	else
	{
...
void lock_function(CURL* handle, curl_lock_data data, curl_lock_access access, void* userptr)
{
	...
	PPCCoreCallback(share->lockfunc.GetMPTR(), share->curl.GetMPTR(), (uint32)data, (uint32)access,
					share->data.GetMPTR());
}
...
{
	CURLSH_t* share = (CURLSH_t*)userptr;
	PPCCoreCallback(share->unlockfunc.GetMPTR(), share->curl.GetMPTR(), (uint32)data,
					share->data.GetMPTR());
}
...
void export_curl_share_setopt(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("curl_share_setopt(0x%08x, 0x%x, 0x%08x) -> 0x%x", share.GetMPTR(), option,
						 parameter.GetMPTR(), result);
	osLib_returnFromFunction(hCPU, result);
}
...
{
	CURLSHPtr result{
		PPCCoreCallback(g_nlibcurl.calloc.GetMPTR(), (uint32)1, ppcsizeof<CURLSH_t>())};
...
CURLcode ssl_ctx_callback(CURL* curl, void* sslctx, void* param)
{
	...
	{
		if (iosuCrypto_addCustomCACertificate(sslctx, &customPKI[0], (sint32)customPKI.size()) ==
			false)
		{
			cemu_assert_suspicious();
			return CURLE_SSL_CACERT;
	...
	if (HAS_FLAG(flags, NSSL_VERIFY_PEER))
	{
		::SSL_CTX_set_cipher_list(
			(SSL_CTX*)sslctx,
			"AES256-SHA"); // TLS_RSA_WITH_AES_256_CBC_SHA (in CURL it's called rsa_aes_256_sha)
...
size_t header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
{
	...
	if (g_callerQueue == nullptr || g_threadQueue == nullptr)
	{
		...
		return PPCCoreCallback(curl->fwrite_header.GetMPTR(), tmp.GetMPTR(), (uint32)size,
							   (uint32)nitems, curl->writeheader.GetMPTR());
	}
	...
	debug[size * nitems] = 0;
	forceLogDebug_printf("header_callback(0x%p, 0x%x, 0x%x, 0x%08x) [%s]", (void*)buffer, size,
						 nitems, curl->writeheader.GetMPTR(), debug);
#endif
	return msg.result;
}
...
size_t write_callback(char* ptr, size_t size, size_t nmemb, void* userdata)
{
	...
	forceLogDebug_printf("write_callback(0x%p 0x%x, 0x%x, 0x%08x)", (void*)ptr, size, nmemb,
						 curl->out.GetMPTR());
	...
	if (g_callerQueue == nullptr || g_threadQueue == nullptr)
	{
		...
		memcpy(tmp.GetPointer(), ptr, size * nmemb);
		int r = PPCCoreCallback(curl->fwrite_func.GetMPTR(), tmp.GetMPTR(), (uint32)size,
								(uint32)nmemb, curl->out.GetMPTR());
		return r;
	}
...
size_t read_callback(char* buffer, size_t size, size_t nitems, void* instream)
{
	...
	forceLogDebug_printf("read_callback(0x%p, 0x%x, 0x%x, 0x%08x) [func: 0x%x]", (void*)buffer,
						 size, nitems, curl->in_set.GetMPTR(), curl->fread_func_set.GetMPTR());

	if (g_callerQueue == nullptr || g_threadQueue == nullptr)
	{
		StackAllocator<char> tmp((uint32)(size * nitems));
		const sint32 result = PPCCoreCallback(curl->fread_func_set.GetMPTR(), tmp.GetMPTR(),
											  (uint32)size, (uint32)nitems, curl->in_set.GetMPTR());
	...
	msg.read_cb.nitems = (uint32)std::min<uint32>(
		nitems, 0x4000); // limit this to 16KB which is the limit in nlibcurl.rpl (Super Mario Maker
						 // crashes on level upload if the size is too big)

	forceLogDebug_printf("read_callback(0x%p, 0x%x, 0x%x, 0x%08x) [func: 0x%x] PUSH", (void*)buffer,
						 size, nitems, curl->in_set.GetMPTR(), curl->fread_func_set.GetMPTR());
	g_callerQueue->push(msg, curl->curlThread);
	...
	forceLogDebug_printf("read_callback(0x%p, 0x%x, 0x%x, 0x%08x) DONE Result: %d", (void*)buffer,
						 size, nitems, curl->in_set.GetMPTR(), msg.result);
...
int progress_callback(void* clientp, double dltotal, double dlnow, double ultotal, double ulnow)
{
	// peterBreak();
	CURL_t* curl = (CURL_t*)clientp;
	// int result = PPCCoreCallback(curl->fprogress.GetMPTR(), curl->progress_client.GetMPTR(),
	// dltotal, dlnow, ultotal, ulnow);
	if (g_callerQueue == nullptr || g_threadQueue == nullptr)
		return PPCCoreCallback(curl->fprogress.GetMPTR(), curl->progress_client.GetMPTR(), dltotal,
							   dlnow, ultotal, ulnow);
	...
	forceLogDebug_printf("progress_callback(%.02lf, %.02lf, %.02lf, %.02lf) -> %d", dltotal, dlnow,
						 ultotal, ulnow, msg.result);
	return msg.result;
}
...
void export_curl_easy_setopt(PPCInterpreter_t* hCPU)
{
	...
	{
		curlDebug_logEasySetOptStr(curl.GetPtr(), "CURLOPT_POSTFIELDS",
								   (const char*)parameter.GetPtr());
		forceLogDebug_printf("curl_easy_setopt(%d) [%s]", option, parameter.GetPtr());
		result = ::curl_easy_setopt(curlObj, (CURLoption)option, parameter.GetPtr());
		break;
	}
	...
	case CURLOPT_HTTPHEADER:
	{
		...
		bool isFirst = true;
		for (curl_slist_t* ppcList = (curl_slist_t*)parameter.GetPtr(); ppcList;
			 ppcList = ppcList->next.GetPtr())
		{
			forceLogDebug_printf("curl_slist_append: %s", ppcList->data.GetPtr());
			curlDebug_logEasySetOptStr(
				curl.GetPtr(), isFirst ? "CURLOPT_HTTPHEADER" : "CURLOPT_HTTPHEADER(continue)",
				(const char*)ppcList->data.GetPtr());
	...
	default:
		forceLogDebug_printf("curl_easy_setopt(0x%08x, %d, 0x%08x) unsupported option",
							 curl.GetMPTR(), option, parameter.GetMPTR());
	}

	forceLogDebug_printf("curl_easy_setopt(0x%08x, %d, 0x%08x) -> 0x%x", curl.GetMPTR(), option,
						 parameter.GetMPTR(), result);
...
void export_curl_easy_getinfo(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf("curl_easy_getinfo(0x%08x, 0x%x, 0x%08x) -> 0x%x", curl.GetMPTR(), info,
						 parameter.GetMPTR(), result);
	osLib_returnFromFunction(hCPU, result);
}
...
void export_curl_slist_append(PPCInterpreter_t* hCPU)
{
	...
	if (!dupdata)
	{
		forceLogDebug_printf("curl_slist_append(0x%08x, 0x%08x [%s]) -> 0x00000000", list.GetMPTR(),
							 data.GetMPTR(), data.GetPtr());
		osLib_returnFromFunction(hCPU, 0);
		return;
	}

	MEMPTR<curl_slist_t> result{
		PPCCoreCallback(g_nlibcurl.malloc.GetMPTR(), ppcsizeof<curl_slist_t>())};
	if (result)
	{
		result->data = dupdata;
		result->next = nullptr;
	...
	forceLogDebug_printf("curl_slist_append(0x%08x, 0x%08x [%s]) -> 0x%08x", list.GetMPTR(),
						 data.GetMPTR(), data.GetPtr(), result.GetMPTR());
	if (list)
...
void export_curl_global_init_mem(PPCInterpreter_t* hCPU)
{
	...
	forceLogDebug_printf(
		"curl_global_init_mem(0x%x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x) -> 0x%08x", flags,
		m.GetMPTR(), f.GetMPTR(), r.GetMPTR(), s.GetMPTR(), c.GetMPTR(), result);
	osLib_returnFromFunction(hCPU, result);
}
