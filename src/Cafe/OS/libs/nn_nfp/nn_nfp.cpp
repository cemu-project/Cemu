...
void nnNfpExport_SetActivateEvent(PPCInterpreter_t* hCPU)
{
	...
	coreinit::OSInitEvent(osEvent, coreinit::OSEvent::EVENT_STATE::STATE_NOT_SIGNALED,
...
void nnNfpExport_SetDeactivateEvent(PPCInterpreter_t* hCPU)
{
	...
	nfpLog_printf("SetDeactivateEvent(0x%08x)", osEventMPTR);
...
{
	nfpLog_printf("StartDetection()");
	nnNfpLock();
	nfp_data.isDetecting = true;
	nnNfpUnlock();
	osLib_returnFromFunction(hCPU,
							 BUILD_NN_RESULT(NN_RESULT_LEVEL_SUCCESS, NN_RESULT_MODULE_NN_NFP, 0));
}

void nnNfpExport_StopDetection(PPCInterpreter_t* hCPU)
{
	nfpLog_printf("StopDetection()");
	nnNfpLock();
	nfp_data.isDetecting = false;
	nnNfpUnlock();
	osLib_returnFromFunction(hCPU,
...
{
	nfpLog_printf("GetTagInfo(0x%08x)", hCPU->gpr[3]);
	...
	if (nfp_data.hasActiveAmiibo == false)
	{
		nnNfpUnlock();
		osLib_returnFromFunction(hCPU,
								 BUILD_NN_RESULT(NN_RESULT_LEVEL_STATUS, NN_RESULT_MODULE_NN_NFP,
												 0)); // todo: Return correct error code
		return;
	}
...
{
	nfpLog_printf("NFCGetTagInfo(%d,%d,0x%08x,0x%08x)", index, timeout, functionPtr,
				  userParam ? memory_getVirtualOffsetFromPointer(userParam) : 0);
	...
	memcpy(callbackParam->uid, nfp_data.amiiboProcessedData.uid,
		   nfp_data.amiiboProcessedData.uidLength);
...
static_assert(offsetof(nfpRomInfo_t, amiiboSeries) == 0x3,
			  "nfpRomInfo.seriesId has invalid offset");
...
{
	nfpLog_printf("GetNfpRomInfo(0x%08x)", hCPU->gpr[3]);
	...
	romInfo->characterId[0] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.gameAndCharacterId[0];
	romInfo->characterId[1] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.gameAndCharacterId[1];
	romInfo->characterId[2] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.characterVariation; // guessed

	romInfo->amiiboSeries =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.amiiboSeries; // guessed
	romInfo->number =
		*(uint16be*)nfp_data.amiiboNFCData.amiiboIdentificationBlock.amiiboModelNumber;	  // guessed
...
static_assert(offsetof(nfpCommonData_t, writeCount) == 0x4,
			  "nfpCommonData.writeCount has invalid offset");
static_assert(offsetof(nfpCommonData_t, amiiboSeries) == 0x9,
			  "nfpCommonData.seriesId has invalid offset");
static_assert(offsetof(nfpCommonData_t, nfpType) == 0xC,
			  "nfpCommonData.nfpType has invalid offset");
static_assert(offsetof(nfpCommonData_t, applicationAreaSize) == 0xE,
			  "nfpCommonData.applicationAreaSize has invalid offset");

void nnNfpExport_GetNfpCommonInfo(PPCInterpreter_t* hCPU)
{
	nfpLog_printf("GetNfpCommonInfo(0x%08x)", hCPU->gpr[3]);
	...
	commonInfo->characterId[0] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.gameAndCharacterId[0];
	commonInfo->characterId[1] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.gameAndCharacterId[1];
	commonInfo->characterId[2] =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.characterVariation;

	commonInfo->number =
		*(uint16be*)nfp_data.amiiboNFCData.amiiboIdentificationBlock.amiiboModelNumber;
	commonInfo->amiiboSeries = nfp_data.amiiboNFCData.amiiboIdentificationBlock.amiiboSeries;
	commonInfo->nfpType =
		nfp_data.amiiboNFCData.amiiboIdentificationBlock.amiiboFigureType; // guessed

	commonInfo->applicationAreaSize =
		sizeof(nfp_data.amiiboInternal
				   .applicationData); // not 100% sure if this is always set to the maximum
...
{
	nfpLog_printf("GetNfpRegisterInfo(0x%08x)", hCPU->gpr[3]);
	...
	{
		memcpy(registerInfo->ownerMii, nfp_data.amiiboInternal.amiiboSettings.mii,
			   sizeof(nfp_data.amiiboInternal.amiiboSettings.mii));
		memcpy(registerInfo->nickname, nfp_data.amiiboInternal.amiiboSettings.nickname,
			   sizeof(nfp_data.amiiboInternal.amiiboSettings.nickname));
	}
...
{
	nfpLog_printf("InitializeRegisterInfoSet(0x%08x)", hCPU->gpr[3]);
...
{
	nfpLog_printf("SetNfpRegisterInfo(0x%08x)", hCPU->gpr[3]);
	ppcDefineParamStructPtr(registerInfoSet, nfpRegisterInfoSet_t, 0);

	memcpy(nfp_data.amiiboInternal.amiiboSettings.mii, registerInfoSet->ownerMii,
		   sizeof(nfp_data.amiiboInternal.amiiboSettings.mii));
	memcpy(nfp_data.amiiboInternal.amiiboSettings.nickname, registerInfoSet->nickname,
		   sizeof(nfp_data.amiiboInternal.amiiboSettings.nickname));
...
{
	nfpLog_printf("IsExistApplicationArea()");
...
{
	nfpLog_printf("OpenApplicationArea(0x%08x)", hCPU->gpr[3]);
	...
	if (nfpAppAreaId != appAreaId)
	{
		osLib_returnFromFunction(hCPU,
								 BUILD_NN_RESULT(NN_RESULT_LEVEL_STATUS, NN_RESULT_MODULE_NN_NFP,
												 NN_RESULT_NFP_CODE_APPAREAIDMISMATCH));
		return;
	}
...
{
	nfpLog_printf("ReadApplicationArea(0x%08x, 0x%x)", hCPU->gpr[3], hCPU->gpr[4]);
	...
	if (!nfp_data.hasOpenApplicationArea)
	{
		osLib_returnFromFunction(
			hCPU, BUILD_NN_RESULT(NN_RESULT_LEVEL_STATUS, NN_RESULT_MODULE_NN_NFP, 0));
		return;
	}
...
{
	nfpLog_printf("WriteApplicationArea(0x%08x, 0x%x, 0x%08x)", hCPU->gpr[3], hCPU->gpr[4],
				  hCPU->gpr[5]);
	ppcDefineParamPtr(bufferPtr, uint8*, 0);
	ppcDefineParamU32(len, 1);
...
{
	nfpLog_printf("CreateApplicationArea(0x%08x)", hCPU->gpr[3]);
...
{
	nfpLog_printf("DeleteApplicationArea()");
...
{
	nfpLog_printf("GetAmiiboSettingsArgs(0x%08x)", hCPU->gpr[3]);
...
bool nnNfp_touchNfcTagFromFile(const wchar_t* filePath, uint32* nfcError)
{
	...
	if (rawData.cfg0[0] != 0x00 || rawData.cfg0[1] != 0x00 || rawData.cfg0[2] != 0x00 ||
		rawData.cfg0[3] != 0x04)
	{
		*nfcError = NFC_ERROR_INVALID_FILE_FORMAT;
		return false;
	}
	if (rawData.cfg1[0] != 0x5F || rawData.cfg1[1] != 0x00 || rawData.cfg1[2] != 0x00 ||
		rawData.cfg1[3] != 0x00)
	{
		*nfcError = NFC_ERROR_INVALID_FILE_FORMAT;
		return false;
	...
	if (rawData.cc[0] != 0xF1 || rawData.cc[1] != 0x10 || rawData.cc[2] != 0xFF ||
		rawData.cc[3] != 0xEE)
	{
		*nfcError = NFC_ERROR_INVALID_FILE_FORMAT;
		return false;
	...
	{
		coreinit::OSEvent* osEvent =
			(coreinit::OSEvent*)memory_getPointerFromVirtualOffset(nfp_data.activateEvent);
...
void nnNfp_update()
{
	...
	{
		coreinit::OSEvent* osEvent =
			(coreinit::OSEvent*)memory_getPointerFromVirtualOffset(nfp_data.deactivateEvent);
		coreinit::OSSignalEvent(osEvent);
	}
}
...
{
	nfpLog_printf("GetNfpState()");

	// workaround for Mario Party 10 eating CPU cycles in an infinite loop (maybe due to incorrect
	// NFP detection handling?)
...
void nnNfp_load()
{
	osLib_addFunction("nn_nfp", "SetActivateEvent__Q2_2nn3nfpFP7OSEvent",
					  nnNfpExport_SetActivateEvent);
	osLib_addFunction("nn_nfp", "SetDeactivateEvent__Q2_2nn3nfpFP7OSEvent",
					  nnNfpExport_SetDeactivateEvent);
	...
	osLib_addFunction("nn_nfp", "GetTagInfo__Q2_2nn3nfpFPQ3_2nn3nfp7TagInfo",
					  nnNfpExport_GetTagInfo);
	...
	osLib_addFunction("nn_nfp", "GetNfpRomInfo__Q2_2nn3nfpFPQ3_2nn3nfp7RomInfo",
					  nnNfpExport_GetNfpRomInfo);
	osLib_addFunction("nn_nfp", "GetNfpCommonInfo__Q2_2nn3nfpFPQ3_2nn3nfp10CommonInfo",
					  nnNfpExport_GetNfpCommonInfo);
	osLib_addFunction("nn_nfp", "GetNfpRegisterInfo__Q2_2nn3nfpFPQ3_2nn3nfp12RegisterInfo",
	...
	osLib_addFunction("nn_nfp", "SetNfpRegisterInfo__Q2_2nn3nfpFRCQ3_2nn3nfp15RegisterInfoSet",
					  nnNfpExport_SetNfpRegisterInfo);

	osLib_addFunction("nn_nfp", "IsExistApplicationArea__Q2_2nn3nfpFv",
					  nnNfpExport_IsExistApplicationArea);
	osLib_addFunction("nn_nfp", "OpenApplicationArea__Q2_2nn3nfpFUi",
					  nnNfpExport_OpenApplicationArea);
	osLib_addFunction("nn_nfp",
					  "CreateApplicationArea__Q2_2nn3nfpFRCQ3_2nn3nfp25ApplicationAreaCreateInfo",
					  nnNfpExport_CreateApplicationArea);
	osLib_addFunction("nn_nfp", "DeleteApplicationArea__Q2_2nn3nfpFv",
					  nnNfpExport_DeleteApplicationArea);
	osLib_addFunction("nn_nfp", "ReadApplicationArea__Q2_2nn3nfpFPvUi",
					  nnNfpExport_ReadApplicationArea);
	osLib_addFunction("nn_nfp", "WriteApplicationArea__Q2_2nn3nfpFPCvUiRCQ3_2nn3nfp5TagId",
					  nnNfpExport_WriteApplicationArea);
	...
	osLib_addFunction("nn_nfp", "GetAmiiboSettingsArgs__Q2_2nn3nfpFPQ3_2nn3nfp18AmiiboSettingsArgs",
					  nnNfpExport_GetAmiiboSettingsArgs);
}

void load()
{
	nnNfp_load(); // legacy interface, update these to use cafeExportRegister /
				  // cafeExportRegisterFunc

	cafeExportRegisterFunc(nn::nfp::GetErrorCode, "nn_nfp",
						   "GetErrorCode__Q2_2nn3nfpFRCQ2_2nn6Result", LogType::Placeholder);
