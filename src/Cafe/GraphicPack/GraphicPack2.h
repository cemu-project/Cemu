...
class GraphicPack2
{
  public:
	...
	enum
	{
		...
		GFXPACK_VERSION_7 = 7, // added fine-grained origin control in patch format (no more forced
							   // 4 byte alignment), .string directive (an alias to .byte) and
							   // support for more than one constant per data directive
	};
	...
	struct CustomShader
	{
		...
		GP_SHADER_TYPE type;
		bool isPreVulkanShader{}; // set to true for V3 packs since the shaders are not compatible
	...
	struct Preset
	{
		...
		Preset(std::string_view category, std::string_view name,
			   std::unordered_map<std::string, PresetVar> vars)
			: category(category), name(name), variables(std::move(vars))
		{
		}

		Preset(std::string_view category, std::string_view name, std::string_view condition,
			   std::unordered_map<std::string, PresetVar> vars)
	...
	const fs::path GetFilename2() const
	{
		return fs::path(m_filename);
	}
	bool Reload();
	...
	{
		return m_name.empty() ? m_path : m_name;
	}
	...
	[[nodiscard]] std::optional<PresetVar> GetPresetVariable(const std::vector<PresetPtr>& presets,
															 std::string_view var_name) const;

	void ValidatePresetSelections();
	bool SetActivePreset(std::string_view category, std::string_view name,
	...
	[[nodiscard]] std::unordered_map<std::string, std::vector<PresetPtr>>
	GetCategorizedPresets(std::vector<std::string>& order) const;
	...
	static const std::string* FindCustomShaderSource(uint64 shaderBaseHash, uint64 shaderAuxHash,
  ...
  private:
	...
	std::unique_ptr<RendererOutputShader> m_output_shader, m_upscaling_shader, m_downscaling_shader,
		m_output_shader_ud, m_upscaling_shader_ud, m_downscaling_shader_ud;

	template<typename T>
	bool ParseRule(const ExpressionParser& parser, IniParser& iniParser, const char* option_name,
				   T* value_out) const;

	template<typename T>
	std::vector<T> ParseList(const ExpressionParser& parser, IniParser& iniParser,
...
template<typename T>
bool GraphicPack2::ParseRule(const ExpressionParser& parser, IniParser& iniParser,
							 const char* option_name, T* value_out) const
{
...
template<typename T>
std::vector<T> GraphicPack2::ParseList(const ExpressionParser& parser, IniParser& iniParser,
									   const char* option_name) const
{
	std::vector<T> result;
