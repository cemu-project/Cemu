...
RegisterCtrl::RegisterCtrl(wxWindow* parent, const wxWindowID& id, const wxPoint& pos,
...
void RegisterCtrl::OnDraw(wxDC& dc, sint32 start, sint32 count, const wxPoint& start_position)
{
	...
	for (sint32 i = 0; i <= count; i++)
	{
		...
		if (line < 32)
		{
			...
			if (code_module)
				dc.DrawText(wxString::Format("<%s> + %x", code_module->moduleName2.c_str(),
											 register_value -
												 code_module->regionMappingBase_text.GetMPTR()),
							position);

			// check if address is a string
			uint32 string_offset = register_value;
			if (string_offset >= MEMORY_DATA_AREA_ADDR &&
				string_offset < (MEMORY_DATA_AREA_ADDR + MEMORY_DATA_AREA_SIZE))
			{
				...
				else
				{
					...
					if (is_valid_string && buffer.tellp() > 1)
					{
						dc.DrawText(wxString::Format(L"ws \"%s\"", wbuffer.str().c_str()),
									position);
					}
				}
			}
		...
		else if (32 + 1 <= line && line <= 32 + 1 + 32)
		{
			...
			const uint64 fpr0_value =
				debuggerState.debugSession.ppcSnapshot.fpr[register_index].fp0int;
			...
			dc.DrawText(wxString::Format(
							"%lf", debuggerState.debugSession.ppcSnapshot.fpr[register_index].fp0),
						position);
			...
			const uint64 fpr1_value =
				debuggerState.debugSession.ppcSnapshot.fpr[register_index].fp1int;
			...
			dc.DrawText(wxString::Format(
							"%lf", debuggerState.debugSession.ppcSnapshot.fpr[register_index].fp1),
						position);
		}
		else
		{
...
void RegisterCtrl::OnMouseDClick(const wxPoint& position, uint32 line)
{
	...
	if (line < 32)
	{
		...
		{
			const uint32 register_value =
				debuggerState.debugSession.ppcSnapshot.gpr[register_index];
