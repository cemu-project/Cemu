		...
		m_module_address(0)
{
	...
	const auto file = GetPath(ActiveSettings::GetConfigPath(), "debugger/config.xml");
...
std::wstring DebuggerWindow2::GetModuleStoragePath(std::string module_name, uint32_t crc_hash) const
{
	...
	return GetPath(ActiveSettings::GetConfigPath(), "debugger/{}_{:#10x}.xml", module_name, crc_hash).generic_wstring();
}
