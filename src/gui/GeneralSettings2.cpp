...
{
  public:
	wxDeviceDescription(const IAudioAPI::DeviceDescriptionPtr& description)
...
{
  public:
	wxVulkanUUID(const VulkanRenderer::DeviceInfo& info) : m_device_info(info) {}
	const VulkanRenderer::DeviceInfo& GetDeviceInfo() const
	{
...
{
  public:
	wxAccountData(const Account& account) : m_account(account) {}
...
wxPanel* GeneralSettings2::AddGeneralPage(wxNotebook* notebook)
{
...

	{
...

		{
			...
			first_row->Add(
				new wxStaticText(box, wxID_ANY, _("Language"), wxDefaultPosition, wxDefaultSize, 0),
				0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

			wxString language_choices[] = {_("Default"), _("English")};
			m_language = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
									  std::size(language_choices), language_choices);
			m_language->SetSelection(0);
			m_language->SetToolTip(
				_("Changes the interface language of Cemu\nAvailable languages are stored in the "
...

		{
			...
			m_save_window_position_size =
				new wxCheckBox(box, wxID_ANY, _("Remember main window position"));
			m_save_window_position_size->SetToolTip(
				_("Restores the last known window position and size when starting Cemu"));
			second_row->Add(m_save_window_position_size, 0, topflag, 5);
			second_row->AddSpacer(10);
			m_save_padwindow_position_size =
				new wxCheckBox(box, wxID_ANY, _("Remember pad window position"));
			m_save_padwindow_position_size->SetToolTip(
				_("Restores the last known pad window position and size when opening it"));
			...
			m_discord_presence->SetToolTip(
				_("Enables the Discord Rich Presence feature\nYou will also need to enable it in "
				  "the Discord settings itself!"));
			second_row->Add(m_discord_presence, 0, botflag, 5);
			second_row->AddSpacer(10);
			m_fullscreen_menubar = new wxCheckBox(box, wxID_ANY, _("Fullscreen menu bar"));
			m_fullscreen_menubar->SetToolTip(
				_("Displays the menu bar when Cemu is running in fullscreen mode and the mouse "
			...
			m_save_screenshot->SetToolTip(_("Pressing the screenshot key (F12) will save a "
			...
			m_permanent_storage->SetToolTip(_("Cemu will remember your custom mlc path in "
											  "%LOCALAPPDATA%/Cemu for new installations."));
...

	{
		...
		m_mlc_path = new wxTextCtrl(box, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
									wxTE_READONLY);
		m_mlc_path->SetMinSize(wxSize(150, -1));
		m_mlc_path->Bind(wxEVT_CHAR, &GeneralSettings2::OnMLCPathChar, this);
		m_mlc_path->SetToolTip(
			_("The mlc directory contains your save games and installed game update/dlc data"));
		...
		change_path->SetToolTip(_("Select a custom mlc path\nThe mlc path is used to store Wii U "
								  "related files like save games, game updates and dlc data"));
...

	{
		...
		m_game_paths->SetToolTip(_("Add the root directory of your game(s). It will scan all "
								   "directories in it for games"));
		...
		general_gamepath_add_button->SetToolTip(
			_("Adds a game path to scan for games displayed in the game list\nIf you have unpacked "
			  "games, make sure to select the root folder of a game"));
		general_gamepath_buttons->Add(general_gamepath_add_button, 0, wxALL, 5);

		auto* general_gamepath_remove_button =
			new wxButton(general_gamepath_box, wxID_ANY, _("Remove"));
		general_gamepath_remove_button->Bind(wxEVT_BUTTON, &GeneralSettings2::OnRemovePathClicked,
											 this);
		general_gamepath_remove_button->SetToolTip(
			_("Removes the currently selected game path from the game list"));
...
wxPanel* GeneralSettings2::AddGraphicsPage(wxNotebook* notebook)
{
...

	{
		...
		row->Add(new wxStaticText(box, wxID_ANY, _("Graphics API")), 0,
		...
		m_graphic_api =
			new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize, api_size, choices);
		...
		row->Add(new wxStaticText(box, wxID_ANY, _("Graphics Device")), 0,
				 wxALIGN_CENTER_VERTICAL | wxALL, 5);
		m_graphic_device =
			new wxChoice(box, wxID_ANY, wxDefaultPosition, {230, -1}, api_size, choices);
		...
		row->Add(new wxStaticText(box, wxID_ANY, _("VSync")), 0, wxALIGN_CENTER_VERTICAL | wxALL,
				 5);
		...
		m_async_compile->SetToolTip(
			_("Enables async shader and pipeline compilation. Reduces stutter at the cost of "
		...
		m_gx2drawdone_sync->SetToolTip(
			_("If synchronization is requested by the game, the emulated CPU will wait for the GPU "
			  "to finish all operations.\nThis is more accurate behavior, but may cause lower "
			  "performance"));
...

	{
		...
		m_upscale_filter =
			new wxRadioBox(graphics_panel, wxID_ANY, _("Upscale filter"), wxDefaultPosition,
						   wxDefaultSize, std::size(choices), choices, 5, wxRA_SPECIFY_COLS);
		m_upscale_filter->SetToolTip(_(
			"Upscaling filters are used when the game resolution is smaller than the window size"));
		graphics_panel_sizer->Add(m_upscale_filter, 0, wxALL | wxEXPAND, 5);

		m_downscale_filter =
			new wxRadioBox(graphics_panel, wxID_ANY, _("Downscale filter"), wxDefaultPosition,
						   wxDefaultSize, std::size(choices), choices, 5, wxRA_SPECIFY_COLS);
		m_downscale_filter->SetToolTip(_("Downscaling filters are used when the game resolution is "
...

	{
		...
		m_fullscreen_scaling =
			new wxRadioBox(graphics_panel, wxID_ANY, _("Fullscreen scaling"), wxDefaultPosition,
						   wxDefaultSize, std::size(choices), choices, 5, wxRA_SPECIFY_COLS);
		m_fullscreen_scaling->SetToolTip(
			_("Controls the output aspect ratio when it doesn't match the ratio of the game"));
...
{
	auto audio_panel =
		new wxPanel(notebook, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL);
...

	{
		...
		audio_general_row->Add(new wxStaticText(box, wxID_ANY, _("API")), 0,
							   wxALIGN_CENTER_VERTICAL | wxALL, 5);
		...
		audio_general_row->Add(new wxStaticText(box, wxID_ANY, _("Latency")), 0,
							   wxALIGN_CENTER_VERTICAL | wxALL, 5);
		m_audio_latency = new wxSlider(box, wxID_ANY, 2, 0, IAudioAPI::kBlockCount - 1,
									   wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL);
		m_audio_latency->SetToolTip(
			_("Controls the amount of buffered audio data\nHigher values will create a delay in "
			  "audio playback, but may avoid audio problems when emulation is too slow"));
		...
		m_audio_latency->Bind(wxEVT_SLIDER, &GeneralSettings2::OnLatencySliderChanged, this,
							  wxID_ANY, wxID_ANY, new wxControlObject(latency_text));
	...
	const wxString audio_channel_choices[] = {_("Mono"), _("Stereo"), _("Surround")};
	{
		...
		audio_tv_row->Add(new wxStaticText(box, wxID_ANY, _("Device")), 0,
						  wxALIGN_CENTER_VERTICAL | wxALL, 5);
		...
		audio_tv_row->Add(new wxStaticText(box, wxID_ANY, _("Channels")), 0,
						  wxALIGN_CENTER_VERTICAL | wxALL, 5);

		m_tv_channels = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
									 std::size(audio_channel_choices), audio_channel_choices);
		...
		audio_tv_row->Add(new wxStaticText(box, wxID_ANY, _("Volume")), 0,
						  wxALIGN_CENTER_VERTICAL | wxALL, 5);
		m_tv_volume = new wxSlider(box, wxID_ANY, 100, 0, 100, wxDefaultPosition, wxDefaultSize,
								   wxSL_HORIZONTAL);
		audio_tv_row->Add(m_tv_volume, 0, wxEXPAND | wxALL, 5);
		auto audio_tv_volume_text = new wxStaticText(box, wxID_ANY, wxT("100%"));
		audio_tv_row->Add(audio_tv_volume_text, 0, wxALIGN_CENTER_VERTICAL | wxALL | wxALIGN_RIGHT,
						  5);

		m_tv_volume->Bind(wxEVT_SLIDER, &GeneralSettings2::OnSliderChangedPercent, this, wxID_ANY,
						  wxID_ANY, new wxControlObject(audio_tv_volume_text));
...

	{
		...
		audio_pad_row->Add(new wxStaticText(box, wxID_ANY, _("Device")), 0,
						   wxALIGN_CENTER_VERTICAL | wxALL, 5);
		...
		audio_pad_row->Add(new wxStaticText(box, wxID_ANY, _("Channels")), 0,
						   wxALIGN_CENTER_VERTICAL | wxALL, 5);
		m_pad_channels =
			new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
						 std::size(audio_channel_drc_choices), audio_channel_drc_choices);
		...
		audio_pad_row->Add(new wxStaticText(box, wxID_ANY, _("Volume")), 0,
						   wxALIGN_CENTER_VERTICAL | wxALL, 5);
		...
		audio_pad_row->Add(audio_pad_volume_text, 0,
						   wxALIGN_CENTER_VERTICAL | wxALL | wxALIGN_RIGHT, 5);

		m_pad_volume->Bind(wxEVT_SLIDER, &GeneralSettings2::OnSliderChangedPercent, this, wxID_ANY,
						   wxID_ANY, new wxControlObject(audio_pad_volume_text));
...
wxPanel* GeneralSettings2::AddOverlayPage(wxNotebook* notebook)
{
	...
	const wxString positions[]{_("Disabled"),	 _("Top left"),	   _("Top center"),
							   _("Top right"),	 _("Bottom left"), _("Bottom center"),
							   _("Bottom right")};
	const wxString text_scale[]{"50%",	"75%",	"100%", "125%", "150%", "175%",
								"200%", "225%", "250%", "275%", "300%"};
	{
		...
		{
			position_row->Add(new wxStaticText(box, wxID_ANY, _("Position")), 0,
							  wxALL | wxALIGN_CENTER_VERTICAL, 5);
			m_overlay_position = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
											  std::size(positions), positions);
			m_overlay_position->SetSelection(0);
			m_overlay_position->SetToolTip(
				_("Controls the overlay which displays technical information while playing"));
			position_row->Add(m_overlay_position, 0, wxALL, 5);

			position_row->AddSpacer(25);

			position_row->Add(new wxStaticText(box, wxID_ANY, _("Text Color")), 0,
							  wxALL | wxALIGN_CENTRE_VERTICAL, 5);
			m_overlay_font_color = new wxColourPickerCtrl(
				box, wxID_ANY, *wxWHITE, wxDefaultPosition, wxDefaultSize, wxCLRP_SHOW_ALPHA);
			...
			position_row->Add(new wxStaticText(box, wxID_ANY, _("Scale")), 0,
							  wxALL | wxALIGN_CENTRE_VERTICAL, 5);
			m_overlay_scale = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
										   std::size(text_scale), text_scale);
		...
		auto settings2_row = new wxFlexGridSizer(0, 4, 2, 0);
		{
			...
			m_overlay_fps->SetToolTip(
				_("The number of frames per second. Average over last 5 seconds"));
			settings2_row->Add(m_overlay_fps, 0, wxALL, 5);

			m_overlay_drawcalls = new wxCheckBox(box, wxID_ANY, _("Draw calls per frame"));
			m_overlay_drawcalls->SetToolTip(
				_("The number of draw calls per frame. Average over last 5 seconds"));
			...
			if (pRtlGetVersion && pRtlGetVersion(&info) == 0 &&
				((info.dwMajorVersion == 6 && info.dwMinorVersion >= 3) || info.dwMajorVersion > 6))
...

	{
		...
		position_row->SetNonFlexibleGrowMode(wxFLEX_GROWMODE_SPECIFIED);
		{
			...
			m_notification_position = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
												   std::size(positions), positions);
			m_notification_position->SetSelection(0);
			m_notification_position->SetToolTip(
				_("Controls the notification position while playing"));
			...
			position_row->Add(new wxStaticText(box, wxID_ANY, _("Text Color")), 0,
							  wxALL | wxALIGN_CENTRE_VERTICAL, 5);
			m_notification_font_color = new wxColourPickerCtrl(
				box, wxID_ANY, *wxWHITE, wxDefaultPosition, wxDefaultSize, wxCLRP_SHOW_ALPHA);
			...
			position_row->Add(new wxStaticText(box, wxID_ANY, _("Scale")), 0,
							  wxALL | wxALIGN_CENTRE_VERTICAL, 5);
			m_notification_scale = new wxChoice(box, wxID_ANY, wxDefaultPosition, wxDefaultSize,
												std::size(text_scale), text_scale);
		...
		auto settings1_row = new wxFlexGridSizer(1, 0, 2, 0);
		{
			...
			m_controller_profile_name->SetToolTip(
				_("Displays the active controller profile when starting a game"));
			settings1_row->Add(m_controller_profile_name, 0, wxALL, 5);

			m_controller_low_battery = new wxCheckBox(box, wxID_ANY, _("Low battery"));
			m_controller_low_battery->SetToolTip(
				_("Shows a notification when a low controller battery has been detected"));
			settings1_row->Add(m_controller_low_battery, 0, wxALL, 5);

			m_shader_compiling = new wxCheckBox(box, wxID_ANY, _("Shader compiler"));
			m_shader_compiling->SetToolTip(
				_("Shows a notification after shaders have been compiled"));
...
wxPanel* GeneralSettings2::AddAccountPage(wxNotebook* notebook)
{
...

	{
		...
		content->Add(new wxStaticText(box, wxID_ANY, _("Active account")), 1,
					 wxALIGN_CENTER_VERTICAL | wxALL, 5);
...

	{
		...
		auto* tutorial_link = new wxHyperlinkCtrl(box, wxID_ANY, _("Online play tutorial"),
												  "https://cemu.info/online-guide");
...

	{
		m_account_information =
			new wxCollapsiblePane(online_panel, wxID_ANY, _("Account information"));
#if BOOST_OS_WINDOWS
		...
		m_account_grid = new wxPropertyGrid(win, wxID_ANY, wxDefaultPosition, wxDefaultSize,
		...
		auto* persistent_id_gprop = m_account_grid->Append(
			new wxStringProperty(wxT("PersistentId"), kPropertyPersistentId));
		persistent_id_gprop->SetHelpString(
			_("The persistent id is the internal folder name used for your saves"));
		m_account_grid->SetPropertyReadOnly(persistent_id_gprop);

		m_account_grid->Append(new wxStringProperty(_("Mii name"), kPropertyMiiName))
			->SetHelpString(_("The mii name is the profile name"));
...
wxPanel* GeneralSettings2::AddDebugPage(wxNotebook* notebook)
{
	...
	debug_row->Add(
		new wxStaticText(panel, wxID_ANY, _("Crash dump"), wxDefaultPosition, wxDefaultSize, 0), 0,
		wxALIGN_CENTER_VERTICAL | wxALL, 5);

	wxString dump_choices[] = {_("Disabled"), _("Lite"), _("Full")};
	m_crash_dump = new wxChoice(panel, wxID_ANY, wxDefaultPosition, wxDefaultSize,
								std::size(dump_choices), dump_choices);
	m_crash_dump->SetSelection(0);
	m_crash_dump->SetToolTip(_("Creates a dump when Cemu crashes\nOnly enable when requested by a "
							   "developer!\nThe Full option will create a very large dump file "
							   "(includes a full RAM dump of the Cemu process)"));
...
GeneralSettings2::GeneralSettings2(wxWindow* parent, bool game_launched)
	: wxDialog(parent, wxID_ANY, _("General settings"), wxDefaultPosition, wxDefaultSize,
			   wxCLOSE_BOX | wxCLIP_CHILDREN | wxCAPTION | wxRESIZE_BORDER),
	  m_game_launched(game_launched)
{
...
void GeneralSettings2::StoreConfig()
{
	...
	config.window_position =
		m_save_window_position_size->IsChecked() ? Vector2i{0, 0} : Vector2i{-1, -1};
	config.window_size =
		m_save_window_position_size->IsChecked() ? Vector2i{0, 0} : Vector2i{-1, -1};
	config.pad_position =
		m_save_padwindow_position_size->IsChecked() ? Vector2i{0, 0} : Vector2i{-1, -1};
	config.pad_size =
		m_save_padwindow_position_size->IsChecked() ? Vector2i{0, 0} : Vector2i{-1, -1};
	...
	if (tv_device != wxNOT_FOUND && tv_device != 0 && m_tv_device->HasClientObjectData())
	{
		const auto* device_description =
			(wxDeviceDescription*)m_tv_device->GetClientObject(tv_device);
	...
	{
		const auto* device_description =
			(wxDeviceDescription*)m_pad_device->GetClientObject(pad_device);
		if (device_description)
	...
	if (active_account == wxNOT_FOUND)
		...
		config.account.m_persistent_id =
			dynamic_cast<wxAccountData*>(m_active_account->GetClientObject(active_account))
				->GetAccount()
				.GetPersistentId();
...
void GeneralSettings2::OnAccountDelete(wxCommandEvent& event)
{
	...
	{
		wxMessageBox(_("Can't delete the only account!"), _("Error"),
	...
	const std::wstring format_str =
		_("Are you sure you want to delete the account {} with id {:x}?").ToStdWstring();
	const std::wstring msg = fmt::format(
		fmt::runtime(format_str), std::wstring{account.GetMiiName()}, account.GetPersistentId());

	const int answer =
		wxMessageBox(msg, _("Confirmation"), wxYES_NO | wxCENTRE | wxICON_QUESTION, this);
	if (answer == wxNO)
		return;
...
void GeneralSettings2::UpdateAccountInformation()
{
	...
	if (selection == wxNOT_FOUND)
	{
		...
		m_validate_online->SetWindowStyleFlag(m_validate_online->GetWindowStyleFlag() &
											  ~wxBORDER_NONE);
	...
	m_account_grid->GetProperty(kPropertyPersistentId)
		->SetValueFromString(fmt::format("{:x}", account.GetPersistentId()));
	m_account_grid->GetProperty(kPropertyMiiName)
		->SetValueFromString(std::wstring{account.GetMiiName()});
	m_account_grid->GetProperty(kPropertyBirthday)
		->SetValueFromString(fmt::format("{:04d}-{:02d}-{:02d}", account.GetBirthYear(),
										 account.GetBirthMonth(), account.GetBirthDay()));

	const auto gender_property =
		m_account_grid->GetProperty(kPropertyGender); // gender 2 can be also female?
	gender_property->SetChoiceSelection(
		std::min(gender_property->GetChoices().GetCount() - 1, (uint32)account.GetGender()));

	m_account_grid->GetProperty(kPropertyEmail)
		->SetValueFromString(std::string{account.GetEmail()});

	auto* country_property =
		dynamic_cast<wxEnumProperty*>(m_account_grid->GetProperty(kPropertyCountry));
	...
	const bool online_valid =
		account.IsValidOnlineAccount() && ActiveSettings::HasRequiredOnlineFiles();
	if (online_valid)
	{
...
void GeneralSettings2::UpdateOnlineAccounts()
{
	...
	{
		m_active_account->Append(fmt::format(L"{} ({:x})", std::wstring{account.GetMiiName()},
											 account.GetPersistentId()),
...
void GeneralSettings2::OnOnlineEnable(wxCommandEvent& event)
{
	...
	const auto result =
		wxMessageBox(_("Please be aware that online mode lets you connect to OFFICIAL servers and "
	...
	// tv audio device
	{
		...
		if (m_tv_device->HasClientObjectData())
		{
			...
			if (description)
			{
				...
				{
					g_tvAudio.reset();
					g_tvAudio = IAudioAPI::CreateDevice(
						(IAudioAPI::AudioAPI)config.audio_api, description->GetDescription(), 48000,
						channels, snd_core::AX_SAMPLES_PER_3MS_48KHZ * AX_FRAMES_PER_GROUP, 16);
	...
	// pad audio device
	{
		...
		if (m_pad_device->HasClientObjectData())
		{
			...
			if (description)
			{
				...
				{
					g_padAudio.reset();
					g_padAudio = IAudioAPI::CreateDevice(
						(IAudioAPI::AudioAPI)config.audio_api, description->GetDescription(), 48000,
						channels, snd_core::AX_SAMPLES_PER_3MS_48KHZ * AX_FRAMES_PER_GROUP, 16);
...
void GeneralSettings2::OnAudioChannelsSelected(wxCommandEvent& event)
{
	...
	if (m_game_launched)
		wxMessageBox(_("You have to restart the game in order to apply the new settings."),
...
{
	wxDirDialog path_dialog(this, _("Select a directory containing games."), wxEmptyString,
