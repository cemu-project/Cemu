...
void unused_translation_dummy()
{
	...
	void(_("IsPasswordCacheEnabled is set to false (The remember password option on your Wii U "
		   "must be enabled for this account before dumping it)"));
	void(_("AccountPasswordCache is empty (The remember password option on your Wii U must be "
...
bool CemuApp::OnInit()
{
	...
	{
		const auto it = std::find_if(m_languages.begin(), m_languages.end(),
									 [language](const wxLanguageInfo* info)
	...
	if (!TestWriteAccess(ActiveSettings::GetPath()))
		wxMessageBox(_("Cemu can't write to its directory.\nPlease move it to a different location "
					   "or run Cemu as administrator!"),
...
void CemuApp::OnAssertFailure(const wxChar* file, int line, const wxChar* func, const wxChar* cond,
							  const wxChar* msg)
{
...
int CemuApp::FilterEvent(wxEvent& event)
{
	if (event.GetEventType() == wxEVT_KEY_DOWN)
	{
		...
		g_window_info
			.keydown[fix_raw_keycode(key_event.GetRawKeyCode(), key_event.GetRawKeyFlags())] = true;
	}
	else if (event.GetEventType() == wxEVT_KEY_UP)
	{
		const auto& key_event = (wxKeyEvent&)event;
		g_window_info
			.keydown[fix_raw_keycode(key_event.GetRawKeyCode(), key_event.GetRawKeyFlags())] =
			false;
	}
...
void CemuApp::CreateDefaultFiles(bool first_start)
{
	...
	if (!fs::exists(mlc) && !first_start)
	{
		const std::wstring message = fmt::format(
			fmt::runtime(_(L"Your mlc01 folder seems to be missing.\n\nThis is where Cemu stores "
						   L"save files, game updates and other Wii U files.\n\nThe expected path "
						   L"is:\n{}\n\nDo you want to create the folder at the expected path?")
							 .ToStdWstring()),
			mlc);

		wxMessageDialog dialog(nullptr, message, "Error",
							   wxCENTRE | wxYES_NO | wxCANCEL | wxICON_WARNING);
	...
	try
	{
		...
		if (!fs::exists(langFile))
		{
			...
			if (file.is_open())
			{
				const char* langStrings[] = {"ja", "en", "fr", "de", "it", "es",
											 "zh", "ko", "nl", "pt", "ru", "zh"};
	...
	catch (const std::exception& ex)
	{
		...
		errorMsg << fmt::format(fmt::runtime(_("Couldn't create a required mlc01 subfolder or "
											   "file!\n\nError: {0}\nTarget path:\n{1}")
												 .ToStdString()),
								ex.what(), boost::nowide::narrow(mlc));
	...
	catch (const std::exception& ex)
	{
		std::stringstream errorMsg;
		errorMsg << fmt::format(
			fmt::runtime(_("Couldn't create a required cemu directory or file!\n\nError: {0}")
							 .ToStdString()),
			ex.what());
...
bool CemuApp::SelectMLCPath(wxWindow* parent)
{
	...
	while (true)
	{
		wxDirDialog path_dialog(parent, _("Select a mlc directory"), default_path,
