...

	{
		...
		m_verify_online->Bind(wxEVT_ENABLE, [this](wxCommandEvent&)
		{
			...
			if (m_enable_verify_button >= 2)
			{
				...
				const auto checksum_path = GetPath(ActiveSettings::GetUserDataPath(), "resources/checksums/{}", default_file);
...
void ChecksumTool::LoadOnlineData() const
{
	try
	{
		...
		const auto checksum_path = GetPath(ActiveSettings::GetUserDataPath(), "resources/checksums");
		if (exists(checksum_path))
		{
