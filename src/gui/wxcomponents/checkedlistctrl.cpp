...
bool wxCheckedListCtrl::Create(wxWindow* parent, wxWindowID id, const wxPoint& pt, const wxSize& sz,
...
bool wxCheckedListCtrl::GetItem(wxListItem& info) const
{
	...
	bool imagecheck =
		(info.m_image == wxCLC_CHECKED_IMGIDX) || (info.m_image == wxCLC_DISABLED_CHECKED_IMGIDX);
	bool imageenabled =
		(info.m_image == wxCLC_CHECKED_IMGIDX) || (info.m_image == wxCLC_UNCHECKED_IMGIDX);
...
long wxCheckedListCtrl::InsertItem(wxListItem& info)
{
	...
	wxASSERT_MSG((int)m_stateList.GetCount() == (int)GetItemCount(), wxS("Something wrong !"));
