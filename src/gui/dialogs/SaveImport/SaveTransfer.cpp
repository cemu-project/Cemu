...
SaveTransfer::SaveTransfer(wxWindow* parent, uint64 title_id, const wxString& source_account,
						   uint32 source_id)
	: wxDialog(parent, wxID_ANY, _("Save transfer"), wxDefaultPosition, wxDefaultSize,
...
void SaveTransfer::OnTransfer(wxCommandEvent& event)
{
	...
	const fs::path source_path =
		ActiveSettings::GetMlcPath("usr/save/{:08x}/{:08x}/user/{:08x}", (uint32)(m_title_id >> 32),
								   (uint32)(m_title_id & 0xFFFFFFFF), m_source_id);
	...
	if (fs::exists(saveinfo) || fs::is_regular_file(saveinfo))
	{
		...
		if (doc.load_file(saveinfo.c_str()))
		{
			...
			if (info_node)
			{
				...
				auto move_entry = info_node.find_child(
					[&new_persistend_id_string](const pugi::xml_node& node) {
						return boost::iequals(node.attribute("persistentId").as_string(),
											  new_persistend_id_string);
					});
				if (move_entry)
					move_entry.attribute("persistentId")
						.set_value(old_persistend_id_string.c_str());
				else
				{
	...
	if (!meta_file_edited)
		forceLog_printf("SaveTransfer::OnTransfer: couldn't update save entry in saveinfo.xml: %s",
