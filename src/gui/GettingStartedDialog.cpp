...
wxPanel* GettingStartedDialog::CreatePage1()
{
	...
	auto* page1_sizer = new wxBoxSizer(wxVERTICAL);
	{
		...
		sizer->Add(new wxStaticBitmap(result, wxID_ANY, wxICON(M_WND_ICON128)), 0,
				   wxALIGN_CENTER_VERTICAL | wxALL, 5);

		auto* m_staticText11 =
			new wxStaticText(result, wxID_ANY,
							 _("It looks like you're starting Cemu for the first time.\nThis quick "
							   "setup assistant will help you get the best experience"),
...

	{
		...
		m_mlc_box_sizer->Add(
			new wxStaticText(
				m_mlc_box_sizer->GetStaticBox(), wxID_ANY,
				_("The mlc path is the root folder of the emulated Wii U internal flash storage. "
				  "It contains all your saves, installed updates and DLCs.\nIt is strongly "
				  "recommend that you create a dedicated folder for it (example: C:\\wiiu\\mlc\\) "
				  "\nIf left empty, the mlc folder will be created inside the Cemu folder.")),
			0, wxALL, 5);

		m_prev_mlc_warning = new wxStaticText(
			m_mlc_box_sizer->GetStaticBox(), wxID_ANY,
			_("A custom mlc path from a previous Cemu installation has been found and filled in."));
		...
		mlc_path_sizer->Add(
			new wxStaticText(m_mlc_box_sizer->GetStaticBox(), wxID_ANY, _("Custom mlc01 path")), 0,
			wxALIGN_CENTER_VERTICAL | wxALL, 5);

		// workaround since we can't specify our own browse label? >> _("Browse")
		m_mlc_folder = new wxDirPickerCtrl(m_mlc_box_sizer->GetStaticBox(), wxID_ANY, wxEmptyString,
		...
		if (m_mlc_folder->HasTextCtrl())
		{
			...
			m_mlc_folder->GetTextCtrl()->Bind(wxEVT_CHAR, &GettingStartedDialog::OnMLCPathChar,
											  this);
		}
		mlc_path_sizer->Add(m_mlc_folder, 1, wxALL, 5);

		mlc_path_sizer->Add(
			new wxStaticText(m_mlc_box_sizer->GetStaticBox(), wxID_ANY, _("(optional)")), 0,
			wxALIGN_CENTER_VERTICAL | wxALL, 5);
...

	{
		...
		sizer->Add(
			new wxStaticText(sizer->GetStaticBox(), wxID_ANY,
							 _("The game path is scanned by Cemu to locate your games. We "
							   "recommend creating a dedicated directory in which\nyou place all "
							   "your Wii U games. (example: C:\\wiiu\\games\\)\n\nYou can also set "
							   "additional paths in the general settings of Cemu.")),
			0, wxALL, 5);

		auto* game_path_sizer = new wxBoxSizer(wxHORIZONTAL);

		game_path_sizer->Add(new wxStaticText(sizer->GetStaticBox(), wxID_ANY, _("Game path")), 0,
							 wxALIGN_CENTER_VERTICAL | wxALL, 5);

		m_game_path = new wxDirPickerCtrl(sizer->GetStaticBox(), wxID_ANY, wxEmptyString,
										  _("Select a folder"));
...

	{
		...
		sizer->Add(new wxStaticText(
					   sizer->GetStaticBox(), wxID_ANY,
					   _("Graphic packs improve games by offering the possibility to change "
						 "resolution, tweak FPS or add other visual or gameplay "
						 "modifications.\nDownload the community graphic packs to get started.\n")),
				   0, wxALL, 5);

		auto* download_gp =
			new wxButton(sizer->GetStaticBox(), wxID_ANY, _("Download community graphic packs"));
...
wxPanel* GettingStartedDialog::CreatePage2()
{
	...
	{
		auto* sizer = new wxStaticBoxSizer(new wxStaticBox(result, wxID_ANY, _("Input settings")),
										   wxVERTICAL);

		sizer->Add(
			new wxStaticText(
				sizer->GetStaticBox(), wxID_ANY,
				_("You can configure one controller for each player.\nWe advise you to always use "
				  "GamePad as emulated input for the first player, since many games expect the "
				  "GamePad to be present.\nIt is also required for touch functionality.\nThe "
				  "default global hotkeys are:\nCTRL - show pad screen\nCTRL + TAB - toggle pad "
				  "screen\nALT + ENTER - toggle fullscreen\nESC - leave fullscreen\n\nIf you're "
				  "having trouble configuring your controller, make sure to have it in idle state "
	...
	{
		auto* sizer = new wxStaticBoxSizer(
			new wxStaticBox(result, wxID_ANY, _("Additional options")), wxVERTICAL);
		...
		m_fullscreen =
			new wxCheckBox(sizer->GetStaticBox(), wxID_ANY, _("Start games with fullscreen"));
		...
		m_update =
			new wxCheckBox(sizer->GetStaticBox(), wxID_ANY, _("Automatically check for updates"));
...
void GettingStartedDialog::OnDownloadGPs(wxCommandEvent& event)
{
	...
	wxMessageDialog ask_dialog(this, _("Do you want to view the downloaded graphic packs?"),
							   _("Graphic packs"), wxCENTRE | wxYES_NO);
