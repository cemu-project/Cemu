...
bool GameUpdateWindow::ParseUpdate(const fs::path& metaPath)
{
	...
	if (fs::exists(target_location, ec))
	{
		try
		{
			...
			else
			{
				...
				if (tip.GetType() != tipOther.GetType())
				{
					...
					std::string wxMsg = wxHelper::MakeUTF8(
						_("It seems that there is already a title installed at the target location "
						  "but it has a different type.\nCurrently installed: \'{}\' Installing: "
						  "\'{}\'\n\nThis can happen for titles which were installed with very old "
						  "Cemu versions.\nDo you still want to continue with the installation? It "
						  "will replace the currently installed title."));
					wxMessageDialog dialog(this,
										   fmt::format(fmt::runtime(wxMsg),
													   typeStrCurrentlyInstalled, typeStrToInstall),
										   _("Warning"), wxCENTRE | wxYES_NO | wxICON_EXCLAMATION);
				...
				{
					wxMessageDialog dialog(this,
										   _("It seems that the selected title is already "
				...
				{
					wxMessageDialog dialog(this,
										   _("It seems that a newer version is already installed, "
											 "do you still want to install the older version?"),
										   _("Warning"), wxCENTRE | wxYES_NO);
					if (dialog.ShowModal() != wxID_YES)
						return false;
				}
			...
			while (fs::exists(m_backup_folder, ec) || ec)
			{
				...
				if (ec)
				{
					const auto error_msg = wxStringFormat2(
						_("Error when trying to move former title installation:\n{}"),
		...
		{
			forceLog_printf("GameUpdateWindow::ParseUpdate exist-error: %s at %s", ex.what(),
							target_location.generic_u8string().c_str());
		}
	}
	...
	m_source_paths = {fs::path(source).append("content"), fs::path(source).append("code"),
	...
	// checking size is buggy on Wine (on Steam Deck this would return values too small to install
	// bigger updates) - we therefore skip this step
	if (!IsRunningInWine())
	{
		const fs::space_info targetSpace = fs::space(target_location.root_path());
		if (targetSpace.free <= m_required_size)
		{
			auto string = wxStringFormat(
				_("Not enough space available.\nRequired: {0} MB\nAvailable: {1} MB"), L"%lld %lld",
				(m_required_size / 1024 / 1024), (targetSpace.free / 1024 / 1024));
...
void GameUpdateWindow::ThreadWork()
{
	...
	catch (const std::exception& ex)
	{
		...
		if (currentDirEntry != fs::directory_entry{})
			error_msg << fmt::format("\n{}\n{}", _("Current file:").ToStdString(),
									 _utf8Wrapper(currentDirEntry.path()));
