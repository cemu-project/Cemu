...
bool checkGraphicPackDownloadedVersion(const char* nameVersion, bool& hasVersionFile)
{
	...
	const auto path = GetPath(ActiveSettings::GetUserDataPath(), "graphicPacks/downloadedGraphicPacks/version.txt");
...
{
	const auto path = GetPath(ActiveSettings::GetUserDataPath(), "graphicPacks/downloadedGraphicPacks");
	std::error_code er;
...
void DownloadGraphicPacksWindow::UpdateThread()
{
	...
	auto path = GetPath(ActiveSettings::GetUserDataPath(), "graphicPacks/downloadedGraphicPacks");
	std::error_code er;
	...
	for (sint32 i = 0; i < numEntries; i++)
	{
		...
		path = GetPath(ActiveSettings::GetUserDataPath(), "graphicPacks/downloadedGraphicPacks/{}", sb.name);
