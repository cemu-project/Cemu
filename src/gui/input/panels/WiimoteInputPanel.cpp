...
constexpr WiimoteController::ButtonId g_kFirstColumnItems[] = {
	WiimoteController::kButtonId_A,	   WiimoteController::kButtonId_B,
	WiimoteController::kButtonId_1,	   WiimoteController::kButtonId_2,
	WiimoteController::kButtonId_Plus, WiimoteController::kButtonId_Minus,
	WiimoteController::kButtonId_Home};

constexpr WiimoteController::ButtonId g_kSecondColumnItems[] = {
	WiimoteController::kButtonId_Up, WiimoteController::kButtonId_Down,
	WiimoteController::kButtonId_Left, WiimoteController::kButtonId_Right};
...
constexpr WiimoteController::ButtonId g_kThirdColumnItems[] = {
	...
	WiimoteController::kButtonId_None,			WiimoteController::kButtonId_Nunchuck_Up,
	WiimoteController::kButtonId_Nunchuck_Down, WiimoteController::kButtonId_Nunchuck_Left,
	WiimoteController::kButtonId_Nunchuck_Right};

WiimoteInputPanel::WiimoteInputPanel(wxWindow* parent) : InputPanel(parent)
{
	...
	for (const auto& id : g_kFirstColumnItems)
	{
		...
		m_item_sizer->Add(
			new wxStaticText(this, wxID_ANY, to_wxString(WiimoteController::get_button_name(id))),
		...
		m_item_sizer->Add(text_ctrl, wxGBPosition(row, column + 1), wxDefaultSpan, wxALL | wxEXPAND,
						  5);
	}

	m_item_sizer->Add(
		new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVERTICAL),
		wxGBPosition(0, column + 2), wxGBSpan(11, 1), wxLEFT | wxRIGHT | wxBOTTOM | wxEXPAND, 5);
	...
	m_volume->Disable();
	m_item_sizer->Add(m_volume, wxGBPosition(row, column + 1), wxDefaultSpan,
	...
	m_volume->Bind(wxEVT_SLIDER, &WiimoteInputPanel::on_volume_change, this, wxID_ANY, wxID_ANY,
				   new wxControlObject(volume_text));
	row++;

	m_item_sizer->Add(
		new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVERTICAL),
		wxGBPosition(0, column + 3), wxGBSpan(11, 1), wxLEFT | wxRIGHT | wxBOTTOM | wxEXPAND, 5);
	...
	m_item_sizer->Add(5, 0, wxGBPosition(3, column + 3), wxDefaultSpan,
					  wxTOP | wxBOTTOM | wxEXPAND | wxALIGN_CENTER, 5);
	m_item_sizer->Add(m_draw, wxGBPosition(3, column + 4), wxGBSpan(4, 1),
					  wxTOP | wxBOTTOM | wxEXPAND | wxALIGN_CENTER, 5);
...
void WiimoteInputPanel::set_active_device_type(WPADDeviceType type)
{
	...
	m_motion_plus->SetValue(type == kWAPDevMPLS || type == kWAPDevMPLSFreeStyle ||
							type == kWAPDevMPLSClassic);
	switch (type)
	{
...
void WiimoteInputPanel::on_timer(const EmulatedControllerPtr& emulated_controller,
