...
{
  public:
	constexpr ConfigValueAtomic() : m_value(TType()), m_init_value(TType()) {}
	...
	constexpr ConfigValueAtomic(TType&& init_value) : m_value(init_value), m_init_value(init_value)
	{
	}
...
{
  public:
	constexpr ConfigValueNoneAtomic() : m_value({}), m_init_value({}) {}
...
template<typename TType>
constexpr bool is_atomic_type_v = std::is_trivially_copyable_v<TType>&&
	std::is_copy_constructible_v<TType>&& std::is_move_constructible_v<TType>&&
		std::is_copy_assignable_v<TType>&& std::is_move_assignable_v<TType>;

template<typename TType>
class ConfigValue : public std::conditional<is_atomic_type_v<TType>, ConfigValueAtomic<TType>,
											ConfigValueNoneAtomic<TType>>::type
{
  public:
	using base_type = typename std::conditional<is_atomic_type_v<TType>, ConfigValueAtomic<TType>,
												ConfigValueNoneAtomic<TType>>::type;
...
class ConfigValueBounds : public ConfigValue<TType>
{
  public:
	...
	template<typename TEnum = typename std::enable_if<
				 std::is_enum<TType>::value && EnableEnumIterators<TType>::enable, TType>>
	...
	constexpr ConfigValueBounds(const TType& init_value)
		: base_type(std::forward<TType>(init_value)), m_min_value(begin(init_value)),
		  m_max_value(rbegin(init_value))
	{
		assert(m_min_value <= init_value && init_value <= m_max_value);
	}
