...
// In this binding, ImTextureID is used to store a 'VkDescriptorSet' texture identifier. Read the
...
// and by
//   the back-end itself (imgui_impl_vulkan.cpp), but should PROBABLY NOT be used by your own
//   engine/app code.
...
//  other bindings. 2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer)
{
	...
	// Upload vertex/index data into a single contiguous GPU buffer
	{
		...
		err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0, aligned_vertex_size, 0,
						  (void**)(&vtx_dst));
		check_vk_result(err);
		err = vkMapMemory(v->Device, rb->IndexBufferMemory, 0, aligned_index_size, 0,
						  (void**)(&idx_dst));
		check_vk_result(err);
			...
			else
			{
					...
					clip_rect.w >= 0.0f)
				{
					...
					VkDescriptorSet desc_set[1] = {
						((ImGuiTexture*)pcmd->TextureId)->descriptor_set};
					vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
											g_PipelineLayout, 0, 1, desc_set, 0, NULL);

					// Draw
					// vkCmdDrawIndexed(command_buffer, pcmd->ElemCount, 1, pcmd->IdxOffset +
					// global_idx_offset, pcmd->VtxOffset + global_vtx_offset, 0);
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
	...
	err =
		vkCreateGraphicsPipelines(v->Device, v->PipelineCache, 1, &info, v->Allocator, &g_Pipeline);
	check_vk_result(err);
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
	...
	// ImDrawCmd::VtxOffset field, allowing for large meshes.
...
void ImGui_ImplVulkan_NewFrame(VkCommandBuffer command_buffer, VkFramebuffer framebuffer,
							   VkExtent2D& extend)
{
	auto& io = ImGui::GetIO();
...
void ImGui_ImplVulkanH_CreateWindow(VkInstance instance, VkPhysicalDevice physical_device,
...
ImTextureID ImGui_ImplVulkan_GenerateTexture(VkCommandBuffer commandBuffer,
											 const std::vector<uint8>& data, const Vector2i& size)
{
	try
	{
		auto* texture = new ImGuiTexture();
		...
		// Create the Image View:
		{
			...
			info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
			info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
			err = vkCreateImage(g_VulkanInitInfo.Device, &info, g_VulkanInitInfo.Allocator,
								&texture->image);
			check_vk_result(err);
			VkMemoryRequirements req;
			...
			err = vkAllocateMemory(g_VulkanInitInfo.Device, &alloc_info, g_VulkanInitInfo.Allocator,
								   &texture->memory);
			check_vk_result(err);
		...
		// Create the Image View:
		{
			...
			info.subresourceRange.levelCount = 1;
			info.subresourceRange.layerCount = 1;
			err = vkCreateImageView(g_VulkanInitInfo.Device, &info, g_VulkanInitInfo.Allocator,
									&texture->view);
			check_vk_result(err);
		}
		...
		// create sampler
		{
			...
			info.maxLod = 1000;
			info.maxAnisotropy = 1.0f;
			err = vkCreateSampler(g_VulkanInitInfo.Device, &info, g_VulkanInitInfo.Allocator,
								  &texture->sampler);
			check_vk_result(err);
		}

		texture->descriptor_set = (VkDescriptorSet)ImGui_ImplVulkan_AddTexture(
		...
		// Create the Upload Buffer:
		{
			...
			buffer_info.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
			buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
			err = vkCreateBuffer(g_VulkanInitInfo.Device, &buffer_info, g_VulkanInitInfo.Allocator,
			...
			err = vkAllocateMemory(g_VulkanInitInfo.Device, &alloc_info, g_VulkanInitInfo.Allocator,
								   &texture->uploadBufferMemory);
			check_vk_result(err);
			err = vkBindBufferMemory(g_VulkanInitInfo.Device, texture->uploadBuffer,
									 texture->uploadBufferMemory, 0);
			check_vk_result(err);
		}
		...
		// Upload to Buffer:
		{
			...
			char* map = NULL;
			err = vkMapMemory(g_VulkanInitInfo.Device, texture->uploadBufferMemory, 0, data.size(),
							  0, (void**)(&map));
			check_vk_result(err);
...
void ImGui_ImplVulkan_DeleteTexture(ImTextureID id)
{
	...
	vkFreeDescriptorSets(g_VulkanInitInfo.Device, g_VulkanInitInfo.DescriptorPool, 1,
						 &textureInfo->descriptor_set);
