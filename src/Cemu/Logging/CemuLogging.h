...
bool cemuLog_log(LogType type, std::basic_string<T> format, TArgs&&... args)
{
	...
	const auto text = fmt::vformat(
		format_view, fmt::make_format_args<fmt::buffer_context<T>>(ForwardEnum(args)...));
...
#define LOG_TYPE_LAST                                                                              \
	(64) // set to 64 so we can easily add more log types in the future without destroying settings
		 // file compatibility
...
#define gx2Log_printf(...)                                                                         \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_GX2))                                                \
		cafeLog_log(LOG_TYPE_GX2, __VA_ARGS__);
#define coreinitMemLog_printf(...)                                                                 \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_COREINIT_MEM))                                       \
		cafeLog_log(LOG_TYPE_COREINIT_MEM, __VA_ARGS__);
#define sndApiLog_printf(...)                                                                      \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_SNDAPI))                                             \
		cafeLog_log(LOG_TYPE_SNDAPI, __VA_ARGS__);
#define socketLog_printf(...)                                                                      \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_SOCKET))                                             \
		cafeLog_log(LOG_TYPE_SOCKET, __VA_ARGS__);
#define inputLog_printf(...)                                                                       \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_INPUT))                                              \
		cafeLog_log(LOG_TYPE_INPUT, __VA_ARGS__);
#define saveLog_printf(...)                                                                        \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_SAVE))                                               \
		cafeLog_log(LOG_TYPE_SAVE, __VA_ARGS__);
#define nfpLog_printf(...)                                                                         \
	if (cafeLog_isLoggingFlagEnabled(LOG_TYPE_NFP))                                                \
		cafeLog_log(LOG_TYPE_NFP, __VA_ARGS__);
