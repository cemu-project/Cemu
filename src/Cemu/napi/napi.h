...
bool NAPI_MakeAuthInfoFromCurrentAccount(
	AuthInfo& authInfo); // helper function. Returns false if online credentials/dumped files are
						 // not available
...
enum class EC_ERROR_CODE
{
	...
	IAS_DEVICE_CERT_ERROR =
		928, // either invalid signature or mismatching incorrect device cert chain
	IAS_INVALID_CHALLENGE = 954,
	ECS_NO_PERMISSION = 701, // AccountGetETickets
	NUS_SOAP_INVALID_VERSION =
		1301, // seen when accidentally passing wrong SOAP version to GetSystemCommonETicket
};
...
NAPI_NUSGetSystemCommonETicket_Result NUS_GetSystemCommonETicket(AuthInfo& authInfo,
																 uint64 titleId);
...
std::vector<uint8> IDBE_RequestRawEncrypted(
...
NAPI_VersionList_Result TAG_GetVersionList(AuthInfo& authInfo, std::string_view fqdnURL,
										   uint32 versionListVersion);
