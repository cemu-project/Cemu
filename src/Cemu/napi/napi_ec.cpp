...
{
	// this is the first url queried (GetAccountStatus). The others are dynamically set if provided
	// by the server but will fallback to hardcoded defaults otherwise
	if (!s_serviceURL_EcsURL.empty())
		return s_serviceURL_EcsURL;
	return LaunchSettings::
		GetServiceURL_ecs(); // by default this is
...
NAPI_NUSGetSystemCommonETicket_Result NUS_GetSystemCommonETicket(AuthInfo& authInfo, uint64 titleId)
{
	...
	soapHelper.SOAP_addRequestField("DeviceId",
									fmt::format("{}", authInfo.getDeviceIdWithPlatform()));
	...
	if (!_parseResponseInit(soapHelper, "GetSystemCommonETicketResponse", responseNode, result, doc,
							responseNode))
		return result;
	...
	if (result.eTicket.empty())
	{
		cemuLog_log(LogType::Force,
					"GetSystemCommonETicketResponse: Invalid eTicket data in response");
	...
	for (pugi::xml_node certNode : responseNode.children("Certs"))
	{
		...
		{
			cemuLog_log(LogType::Force,
						"GetSystemCommonETicketResponse: Invalid cert data in response");
			result.apiError = NAPI_RESULT::DATA_ERROR;
			return result;
		}
...
NAPI_IASGetChallenge_Result IAS_GetChallenge(AuthInfo& authInfo)
{
	...
	soapHelper.SOAP_addRequestField(
		"DeviceId",
		fmt::format("{}",
					authInfo.getDeviceIdWithPlatform())); // not validated but the generated
														  // Challenge is bound to this DeviceId
	...
	pugi::xml_document doc;
	pugi::xml_node responseNode;
	if (!_parseResponseInit(soapHelper, "GetChallengeResponse", responseNode, result, doc,
							responseNode))
		return result;
...
NAPI_IASGetRegistrationInfo_Result IAS_GetRegistrationInfo_QueryInfo(AuthInfo& authInfo,
																	 std::string challenge)
{
	...
	soapHelper.SOAP_addRequestField(
		"DeviceId",
		fmt::format("{}", authInfo.getDeviceIdWithPlatform())); // this must match the DeviceId used
																// to generate Challenge
	soapHelper.SOAP_addRequestField("Region", NCrypto::GetRegionAsString(authInfo.region));
	soapHelper.SOAP_addRequestField("Country", authInfo.country);
	...
	NCrypto::ECCSig signature =
		NCrypto::signHash(signerTitleIdHigh, signerTitleIdLow, hash.b, sizeof(hash.b), certChain);
	...
	pugi::xml_document doc;
	pugi::xml_node responseNode;
	if (!_parseResponseInit(soapHelper, "GetRegistrationInfoResponse", responseNode, result, doc,
							responseNode))
		return result;
...
NAPI_ECSGetAccountStatus_Result ECS_GetAccountStatus(AuthInfo& authInfo)
{
	...
	if (!authInfo.IASToken.deviceToken.empty())
		soapHelper.SOAP_addRequestField("DeviceToken",
										_getDeviceTokenWT(authInfo.IASToken.deviceToken));
	...
	pugi::xml_document doc;
	pugi::xml_node responseNode;
	if (!_parseResponseInit(soapHelper, "GetAccountStatusResponse", responseNode, result, doc,
							responseNode))
		return result;
	...
	if (accountStatusStr)
	{
		...
		{
			cemuLog_force("ECS_GetAccountStatus: Account has unknown status code {}",
						  accountStatusStr);
		}
	}
...
NAPI_ECSAccountListETicketIds_Result ECS_AccountListETicketIds(AuthInfo& authInfo)
{
	...
	pugi::xml_document doc;
	pugi::xml_node responseNode;
	if (!_parseResponseInit(soapHelper, "AccountListETicketIdsResponse", responseNode, result, doc,
							responseNode))
		return result;
	...
	for (pugi::xml_node tivNode : responseNode.children("TIV"))
	{
		...
		// ticketVersion starts at 0 and increments every time the ticket is updated (e.g. for AOC
		// content, when purchasing additional DLC packages)
...
NAPI_ECSAccountGetETickets_Result ECS_AccountGetETickets(AuthInfo& authInfo, sint64 ticketId)
{
	...
	pugi::xml_document doc;
	pugi::xml_node responseNode;
	if (!_parseResponseInit(soapHelper, "AccountGetETicketsResponse", responseNode, result, doc,
							responseNode))
		return result;
...
NAPI_CCSGetTMD_Result CCS_GetTMD(AuthInfo& authInfo, uint64 titleId, uint16 titleVersion)
{
	...
	req.initate(fmt::format("{}/{:016x}/tmd.{}?deviceId={}&accountId={}", _getCCSUncachedUrl(),
							titleId, titleVersion, authInfo.getDeviceIdWithPlatform(),
							authInfo.IASToken.accountId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::CCS);
	req.setTimeout(180);
	if (!req.submitRequest(false))
	{
		cemuLog_log(LogType::Force, fmt::format("Failed to request TMD for title {0:016X} v{1}",
...
NAPI_CCSGetTMD_Result CCS_GetTMD(AuthInfo& authInfo, uint64 titleId)
{
	...
	CurlRequestHelper req;
	req.initate(fmt::format("{}/{:016x}/tmd?deviceId={}&accountId={}", _getCCSUncachedUrl(),
							titleId, authInfo.getDeviceIdWithPlatform(),
							authInfo.IASToken.accountId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::CCS);
	req.setTimeout(180);
	if (!req.submitRequest(false))
	{
		cemuLog_log(LogType::Force,
					fmt::format("Failed to request TMD for title {0:016X}", titleId));
		return result;
	}
	result.tmdData = req.getReceivedData();
	result.isValid = true;
...
NAPI_CCSGetETicket_Result CCS_GetCETK(AuthInfo& authInfo, uint64 titleId, uint16 titleVersion)
{
	...
	CurlRequestHelper req;
	req.initate(fmt::format("{}/{:016x}/cetk", _getCCSUncachedUrl(), titleId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::CCS);
	req.setTimeout(180);
	if (!req.submitRequest(false))
	{
		cemuLog_log(LogType::Force, fmt::format("Failed to request eTicket for title {0:016X} v{1}",
												titleId, titleVersion));
		return result;
	}
...
bool CCS_GetContentFile(uint64 titleId, uint32 contentId,
						bool (*cbWriteCallback)(void* userData, const void* ptr, size_t len,
												bool isLast),
						void* userData)
{
	CurlRequestHelper req;
	req.initate(fmt::format("{}/{:016x}/{:08x}", _getCCSUrl(), titleId, contentId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::CCS);
	...
	{
		cemuLog_log(LogType::Force,
					fmt::format("Failed to request content file {:08x} for title {:016X}",
								contentId, titleId));
		return false;
	}
	return true;
...
NAPI_CCSGetContentH3_Result CCS_GetContentH3File(uint64 titleId, uint32 contentId)
{
	...
	CurlRequestHelper req;
	req.initate(fmt::format("{}/{:016x}/{:08x}.h3", _getCCSUrl(), titleId, contentId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::CCS);
	if (!req.submitRequest(false))
	{
		cemuLog_log(LogType::Force,
					fmt::format("Failed to request content hash file {:08x}.h3 for title {:016X}",
								contentId, titleId));
		return result;
	}
	result.tmdData = req.getReceivedData();
