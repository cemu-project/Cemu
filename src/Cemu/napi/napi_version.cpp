...
NAPI_VersionListVersion_Result TAG_GetVersionListVersion(AuthInfo& authInfo)
{
	...
	req.initate(
		fmt::format("https://tagaya.wup.shop.nintendo.net/tagaya/versionlist/{}/{}/latest_version",
					NCrypto::GetRegionAsString(authInfo.region), authInfo.country),
		CurlRequestHelper::SERVER_SSL_CONTEXT::TAGAYA);
	...
	if (!doc.child("version_list_info").child("version") ||
		!doc.child("version_list_info").child("fqdn"))
	{
										   ...
										   uint32 versionListVersion)
{
	...
	CurlRequestHelper req;
	req.initate(fmt::format("https://{}/tagaya/versionlist/{}/{}/list/{}.versionlist", fqdnURL,
							NCrypto::GetRegionAsString(authInfo.region), authInfo.country,
							versionListVersion),
				CurlRequestHelper::SERVER_SSL_CONTEXT::TAGAYA);
	if (!req.submitRequest(false))
	...
	for (pugi::xml_node title : doc.child("version_list").child("titles").children("title"))
	{
		...
		if (const auto res = std::from_chars(str.data(), str.data() + str.size(), titleId, 16);
			res.ec != std::errc())
			continue;
		str = title.child_value("version");
		if (const auto res = std::from_chars(str.data(), str.data() + str.size(), titleVersion, 10);
			res.ec != std::errc())
			continue;
