...
bool _parseActResponse(CurlRequestHelper& requestHelper, _NAPI_CommonResultACT& result,
					   pugi::xml_document& doc)
{
	if (!doc.load_buffer(requestHelper.getReceivedData().data(),
						 requestHelper.getReceivedData().size()))
	{
...
{
	req.addHeaderField("X-Nintendo-Device-ID",
					   fmt::format("{}", authInfo.deviceId)); // deviceId without platform field
...
{
	OAuthTokenCacheEntry(std::string_view accountId, std::array<uint8, 32>& passwordHash,
						 std::string_view token, std::string_view refreshToken, uint64 expiresIn)
		: accountId(accountId), passwordHash(passwordHash), token(token), refreshToken(refreshToken)
	{
...
ACTOauthToken ACT_GetOauthToken_WithCache(AuthInfo& authInfo, uint64 titleId, uint16 titleVersion)
{
	...
	req.initate(
		fmt::format("{}/v1/api/oauth20/access_token/generate", LaunchSettings::GetActURLPrefix()),
		CurlRequestHelper::SERVER_SSL_CONTEXT::ACT);
	...
	*/
	...
	if (!node)
	{
		cemuLog_log(LogType::Force,
					fmt::format("Response does not contain OAuth20/access_token node"));
		result.apiError = NAPI_RESULT::XML_ERROR;
		return result;
	}
	...
	expiration = std::max(expiration - 30LL,
						  0LL); // subtract a few seconds to compensate for the web request delay

	// update cache
	if (expiration > 0)
	{
		g_oauthTokenCacheMtx.lock();
		g_oauthTokenCache.emplace_back(authInfo.accountId, authInfo.passwordHash, result.token,
									   result.refreshToken, expiration);
		g_oauthTokenCacheMtx.unlock();
	}
	return result;
}
...
bool ACT_GetProfile(AuthInfo& authInfo, ACTGetProfileResult& result)
{
	...
	req.initate(fmt::format("{}/v1/api/people/@me/profile", LaunchSettings::GetActURLPrefix()),
				CurlRequestHelper::SERVER_SSL_CONTEXT::ACT);
...
{
	NexTokenCacheEntry(std::string_view accountId, std::array<uint8, 32>& passwordHash,
					   uint32 gameServerId, ACTNexToken& nexToken)
		: accountId(accountId), passwordHash(passwordHash), nexToken(nexToken),
...
ACTGetNexTokenResult ACT_GetNexToken_WithCache(AuthInfo& authInfo, uint64 titleId,
											   uint16 titleVersion, uint32 serverId)
{
	...
	// do request
	CurlRequestHelper req;
	req.initate(fmt::format("{}/v1/api/provider/nex_token/@me?game_server_id={:08X}",
							LaunchSettings::GetActURLPrefix(), serverId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::ACT);
	_ACTSetCommonHeaderParameters(req, authInfo);
	_ACTSetDeviceParameters(req, authInfo);
	_ACTSetRegionAndCountryParameters(req, authInfo);
	...
	*/
	...
	result.apiError = NAPI_RESULT::SUCCESS;
	g_nexTokenCacheMtx.lock();
	g_nexTokenCache.emplace_back(authInfo.accountId, authInfo.passwordHash, serverId,
								 result.nexToken);
	g_nexTokenCacheMtx.unlock();
	return result;
}
...
{
	IndependentTokenCacheEntry(std::string_view accountId, std::array<uint8, 32>& passwordHash,
							   std::string_view clientId, std::string_view independentToken,
							   sint64 expiresIn)
		: accountId(accountId), passwordHash(passwordHash), clientId(clientId),
...
ACTGetIndependentTokenResult ACT_GetIndependentToken_WithCache(AuthInfo& authInfo, uint64 titleId,
															   uint16 titleVersion,
															   std::string_view clientId)
{
	...
	while (itr != g_IndependentTokenCache.end())
	{
		...
		else if (itr->IsMatch(authInfo, clientId))
		{
			...
			result.expiresIn = std::max(
				itr->expires - (sint64)HighResolutionTimer::now().getTickInSeconds(), (sint64)0);
			result.apiError = NAPI_RESULT::SUCCESS;
	...
	// do request
	CurlRequestHelper req;
	req.initate(fmt::format("{}/v1/api/provider/service_token/@me?client_id={}",
							LaunchSettings::GetActURLPrefix(), clientId),
				CurlRequestHelper::SERVER_SSL_CONTEXT::ACT);
	_ACTSetCommonHeaderParameters(req, authInfo);
	_ACTSetDeviceParameters(req, authInfo);
	_ACTSetRegionAndCountryParameters(req, authInfo);
	...
	*/
	...
	g_IndependentTokenCacheMtx.lock();
	g_IndependentTokenCache.emplace_back(authInfo.accountId, authInfo.passwordHash, clientId,
										 result.token, 3600);
	g_IndependentTokenCacheMtx.unlock();
	return result;
}

ACTConvertNnidToPrincipalIdResult ACT_ACTConvertNnidToPrincipalId(AuthInfo& authInfo,
																  std::string_view nnid)
{
	...
	CurlRequestHelper req;
	req.initate(
		fmt::format("{}/v1/api/admin/mapped_ids?input_type=user_id&output_type=pid&input={}",
					LaunchSettings::GetActURLPrefix(), nnid),
		CurlRequestHelper::SERVER_SSL_CONTEXT::ACT);
	_ACTSetCommonHeaderParameters(req, authInfo);
	_ACTSetDeviceParameters(req, authInfo);
	_ACTSetRegionAndCountryParameters(req, authInfo);
...
bool NAPI_MakeAuthInfoFromCurrentAccount(AuthInfo& authInfo)
{
	...
	if (std::all_of(passwordHash.cbegin(), passwordHash.cend(), [](uint8 v) { return v == 0; }))
	{
		...
		{
			cemuLog_force(
				"Account login is impossible because the cached password hash is not set");
