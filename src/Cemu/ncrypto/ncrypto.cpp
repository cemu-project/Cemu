...
struct ETicketFileHeaderWiiU
{
	...
	*/
	...
	/* +0x1D8 */ uint32be
		deviceId; // ticket personalized to this deviceId. Zero if not personalized
...
bool ETicketParser::parse(const uint8* data, size_t size)
{
	...
	if (size < sizeof(ETicketFileHeaderWiiU))
		return false;
	ETicketFileHeaderWiiU* header =
		(ETicketFileHeaderWiiU*)readStruct(0, sizeof(ETicketFileHeaderWiiU));
	if (!header)
		return false;
	...
	if (m_ticketFormatVersion >= 1)
	{
		...
		{
			ETicketFileHeaderExtV1* extHeader = (ETicketFileHeaderExtV1*)readStruct(
				0x2A4,
				sizeof(
					ETicketFileHeaderExtV1)); //  (ETicketFileHeaderExtV1*)((uint8*)header + 0x2A4);
			if (!extHeader)
				return false;
			cemu_assert_debug(extHeader->sectionTableEntrySize == 0x14);
			for (uint32 i = 0; i < extHeader->sectionTableNumEntries; i++)
			{
				ETicketFileHeaderExtV1SectionHeader* sectHeader =
					(ETicketFileHeaderExtV1SectionHeader*)readStruct(
						0x2A4 + extHeader->sectionTableOffset,
						sizeof(ETicketFileHeaderExtV1SectionHeader));
				if (!sectHeader)
					return false;
				if (sectHeader->type ==
					ETicketFileHeaderExtV1SectionHeader::SECTION_TYPE_CONTENT_RIGHTS)
				{
					if (sectHeader->entrySize != sizeof(ETicketV1_ContentRights))
					{
						cemuLog_log(LogType::Force,
									"ETicket: Failed to parse ticket with invalid rights size");
						return false;
					}
					cemu_assert_debug(sectHeader->entryCount == 1);
					for (uint32 r = 0; r < sectHeader->entryCount; r++)
					{
						ETicketV1_ContentRights* rights = (ETicketV1_ContentRights*)readStruct(
							0x2A4 + sectHeader->sectionOffset + r * sectHeader->entrySize,
							sizeof(ETicketV1_ContentRights));
						cemu_assert_debug(rights->baseIndex == 0);
						if (rights->baseIndex > 0x1000)
						{
							cemuLog_log(LogType::Force,
										"ETicket: Invalid content rights index ({})",
										(uint32)rights->baseIndex);
							continue;
						}
...
void ETicketParser::GetTitleKey(AesKey& key)
{
	...
	*(uint64be*)iv = m_titleId;
	...
	uint8 commonKey[16] = {0xD7, 0xB0, 0x04, 0x02, 0x65, 0x9B, 0xA2, 0xAB,
...
bool ETicketParser::Depersonalize(uint8* ticketData, size_t ticketSize, uint32 deviceId,
								  const ECCPrivKey& devicePrivKey)
{
	...
	uint8 sharedKey[128]{};
	int sharedKeyLen =
		ECDH_compute_key(sharedKey, sizeof(sharedKey), ec_publicKey, ec_privKey, nullptr);
...
bool TMDParser::parse(const uint8* data, size_t size)
{
	...
	{
		cemuLog_force("TMD size {} below expected size of {}. Content count: {}", size,
					  expectedSize, (uint16)header->numContent);
		return false;
	}
...
ECDSA_SIG* ECCSig::getSignature()
{
	...
	ECDSA_SIG_set0(
		ec_sig, bn_r,
		bn_s); // ownership of bn_r and bn_s transferred to SIG as well, do not free manually
...
// generate a new public key + certificate from privateKey. Certificate is signed with the device
// key
CertECC CertECC::generateCertificate(uint32 signerTitleIdHigh, uint32 signerTitleIdLow,
									 ECCPrivKey& privKeySigner, ECCPrivKey& privKeyIn,
									 ECCPubKey& pubKeyOut)
{
...
ECCSig signHash(uint32 signerTitleIdHigh, uint32 signerTitleIdLow, uint8* hash, sint32 hashLen,
				CertECC& certChainOut)
{
	...
	certChainOut = CertECC::generateCertificate(signerTitleIdHigh, signerTitleIdLow, signerPrivKey,
												privKey, pubKey);
...
const std::unordered_map<sint32, const char*> g_countryTable = {
	{1, "JP"},	 {8, "AI"},	  {9, "AG"},   {10, "AR"},	{11, "AW"},	 {12, "BS"},  {13, "BB"},
	{14, "BZ"},	 {15, "BO"},  {16, "BR"},  {17, "VG"},	{18, "CA"},	 {19, "KY"},  {20, "CL"},
	{21, "CO"},	 {22, "CR"},  {23, "DM"},  {24, "DO"},	{25, "EC"},	 {26, "SV"},  {27, "GF"},
	{28, "GD"},	 {29, "GP"},  {30, "GT"},  {31, "GY"},	{32, "HT"},	 {33, "HN"},  {34, "JM"},
	{35, "MQ"},	 {36, "MX"},  {37, "MS"},  {38, "AN"},	{39, "NI"},	 {40, "PA"},  {41, "PY"},
	{42, "PE"},	 {43, "KN"},  {44, "LC"},  {45, "VC"},	{46, "SR"},	 {47, "TT"},  {48, "TC"},
	{49, "US"},	 {50, "UY"},  {51, "VI"},  {52, "VE"},	{64, "AL"},	 {65, "AU"},  {66, "AT"},
	{67, "BE"},	 {68, "BA"},  {69, "BW"},  {70, "BG"},	{71, "HR"},	 {72, "CY"},  {73, "CZ"},
	{74, "DK"},	 {75, "EE"},  {76, "FI"},  {77, "FR"},	{78, "DE"},	 {79, "GR"},  {80, "HU"},
	{81, "IS"},	 {82, "IE"},  {83, "IT"},  {84, "LV"},	{85, "LS"},	 {86, "LI"},  {87, "LT"},
	{88, "LU"},	 {89, "MK"},  {90, "MT"},  {91, "ME"},	{92, "MZ"},	 {93, "NA"},  {94, "NL"},
	{95, "NZ"},	 {96, "NO"},  {97, "PL"},  {98, "PT"},	{99, "RO"},	 {100, "RU"}, {101, "RS"},
	{102, "SK"}, {103, "SI"}, {104, "ZA"}, {105, "ES"}, {106, "SZ"}, {107, "SE"}, {108, "CH"},
	{109, "TR"}, {110, "GB"}, {111, "ZM"}, {112, "ZW"}, {113, "AZ"}, {114, "MR"}, {115, "ML"},
	{116, "NE"}, {117, "TD"}, {118, "SD"}, {119, "ER"}, {120, "DJ"}, {121, "SO"}, {122, "AD"},
	{123, "GI"}, {124, "GG"}, {125, "IM"}, {126, "JE"}, {127, "MC"}, {128, "TW"}, {136, "KR"},
	{144, "HK"}, {145, "MO"}, {152, "ID"}, {153, "SG"}, {154, "TH"}, {155, "PH"}, {156, "MY"},
	{160, "CN"}, {168, "AE"}, {170, "EG"}, {171, "OM"}, {172, "QA"}, {173, "KW"}, {174, "SA"},
	{175, "SY"}, {176, "BH"}, {177, "JO"}, {184, "SM"}, {185, "VA"}, {186, "BM"}, {187, "IN"},
	{192, "NG"}, {193, "AO"}, {194, "GH"}};
