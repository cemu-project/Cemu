...
)
{
	...
	else // Need to read 3 bytes
	{
		nRegion = (DWORD)(pPacket[nBytePosition + 0]) << 16 |
				  (DWORD)(pPacket[nBytePosition + 1]) << 8 | (DWORD)(pPacket[nBytePosition + 2]);
...
__inline DWORD GetXmaFrameBitPosition(__in_bcount(nXmaDataBytes)
...
)
{
	...
	for (;;)
	{
		...
		if (nFrameCountSoFar + GetXmaPacketFrameCount(pCurrentPacket) > nDesiredFrame)
		{
			...
			nFrameBitOffset =
				XMA_PACKET_HEADER_BITS + GetXmaPacketFirstFrameOffsetInBits(pCurrentPacket);
...
__inline DWORD GetLastXmaFrameBitPosition(__in_bcount(nXmaDataBytes)
											  const BYTE* pXmaData, // Pointer to XMA block[s]
										  DWORD nXmaDataBytes,		// Size of pXmaData in bytes
										  DWORD nStreamIndex		// Stream within which to seek
)
...
__inline HRESULT
GetXmaDecodePositionForSample(__in_bcount(nXmaDataBytes)
								  const BYTE* pXmaData, // Pointer to XMA block[s]
							  DWORD nXmaDataBytes,		// Size of pXmaData in bytes
							  DWORD nStreamIndex,		// Stream within which to seek
