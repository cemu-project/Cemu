...
DirectInputControllerProvider::DirectInputControllerProvider()
{
	...
	m_DirectInput8Create = (decltype(&DirectInput8Create))GetProcAddress(m_module,
	"DirectInput8Create"); m_GetdfDIJoystick = (decltype(&GetdfDIJoystick))GetProcAddress(m_module,
	"GetdfDIJoystick");
	...
	const auto r = DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION,
									  IID_IDirectInput8, (void**)&m_dinput8, nullptr);
	if (FAILED(r) || !m_dinput8)
	{
		const auto error = GetLastError();
		// FreeLibrary(m_module);
		throw std::runtime_error(
			fmt::format("can't create direct input object (error: {:#x})", error));
	}
}
	...
	m_dinput8->EnumDevices(
		...
		[](LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef) -> BOOL
		{
			...
			controllers->emplace_back(
				std::make_shared<DirectInputController>(lpddi->guidInstance, display_name));
