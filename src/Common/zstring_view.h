...
class basic_zstring_view : private std::basic_string_view<CharT, Traits>
{
  ...
  public: /// Special member functions ///
	...
	basic_zstring_view(
		std::basic_string<CharT, std::char_traits<CharT>, std::allocator<CharT>> const& s)
		: string_view_base{s}
	{
	}
  ...
  public: /// Convertions ///
	...
	// From [class.conv.fct]: A conversion function is never used to convert a (possibly
	// cv-qualified) object to the (possibly cv-qualified) same object type (or a reference to it),
	// to a (possibly cv-qualified) base class of that type (or a reference to it), or to (possibly
	// cv-qualified) void.
