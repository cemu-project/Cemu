...
XMLDocument::XMLDocument(bool processEntities, Whitespace whitespaceMode)
	: XMLNode(0), _writeBOM(false), _processEntities(processEntities), _errorID(XML_SUCCESS),
	  _whitespaceMode(whitespaceMode), _errorStr1(), _errorStr2(), _errorLineNum(0), _charBuffer(0),
	  _parseCurLineNum(0), _unlinked(), _elementPool(), _attributePool(), _textPool(),
	  _commentPool()
{
...
void XMLDocument::PrintError() const
{
	if (Error())
	{
		...
		printf("XMLDocument error id=%d '%s' str1=%s str2=%s line=%d\n", static_cast<int>(_errorID),
			   ErrorName(), buf1, buf2, _errorLineNum);
	}
}
