...
{
	// note: replacing this with two 64bit xor operations decreased performance in benchmarks,
	// probably because the state bytes need to be stored back in memory
...
//
//		//stateVal(i, 0) = Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^ Multiply(d,
// 0x09);
//		//stateVal(i, 1) = Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^ Multiply(d,
// 0x0d);
//		//stateVal(i, 2) = Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^ Multiply(d,
// 0x0b);
//		//stateVal(i, 3) = Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^ Multiply(d,
// 0x0e);
//
...
void InvMixColumns(aes128Ctx_t* aesCtx)
{
	...
	c = stateVal(0, 2);
	d = stateVal(0, 3);
	stateValU32(0) = lookupTable_multiply[a] ^ std::rotl<uint32>(lookupTable_multiply[b], 8) ^
					 std::rotl<uint32>(lookupTable_multiply[c], 16) ^
					 std::rotl<uint32>(lookupTable_multiply[d], 24);
	// i1
	a = stateVal(1, 0);
	b = stateVal(1, 1);
	c = stateVal(1, 2);
	d = stateVal(1, 3);
	stateValU32(1) = lookupTable_multiply[a] ^ std::rotl<uint32>(lookupTable_multiply[b], 8) ^
					 std::rotl<uint32>(lookupTable_multiply[c], 16) ^
					 std::rotl<uint32>(lookupTable_multiply[d], 24);
	// i2
	a = stateVal(2, 0);
	...
	c = stateVal(2, 2);
	d = stateVal(2, 3);
	stateValU32(2) = lookupTable_multiply[a] ^ std::rotl<uint32>(lookupTable_multiply[b], 8) ^
					 std::rotl<uint32>(lookupTable_multiply[c], 16) ^
					 std::rotl<uint32>(lookupTable_multiply[d], 24);
	// i3
	a = stateVal(3, 0);
	...
	c = stateVal(3, 2);
	d = stateVal(3, 3);
	stateValU32(3) = lookupTable_multiply[a] ^ std::rotl<uint32>(lookupTable_multiply[b], 8) ^
					 std::rotl<uint32>(lookupTable_multiply[c], 16) ^
					 std::rotl<uint32>(lookupTable_multiply[d], 24);
}
...
void AES128_CBC_encrypt(uint8* output, uint8* input, uint32 length, const uint8* key,
						const uint8* iv)
{
	aes128Ctx_t aesCtx;
	intptr_t i;
...
void __soft__AES128_CBC_decrypt(uint8* output, uint8* input, uint32 length, const uint8* key,
								const uint8* iv)
{
	aes128Ctx_t aesCtx;
	intptr_t i;
...
void AES128_CBC_decrypt_buffer_depr(uint8* output, uint8* input, uint32 length, const uint8* key,
									const uint8* iv)
{
	aes128Ctx_t aesCtx;
	intptr_t i;
...
void AES128_CBC_decrypt_updateIV(uint8* output, uint8* input, uint32 length, const uint8* key,
								 uint8* iv)
{
	length &= ~0xF;
	uint8 newIv[16];
...
inline __m128i AESNI128_ASSIST(__m128i temp1, __m128i temp2)
{
	__m128i temp3;
...
void AESNI128_CBC_encrypt(const unsigned char* in, unsigned char* out, unsigned char ivec[16],
...
void AESNI128_CBC_decryptWithExpandedKey(const unsigned char* in, unsigned char* out,
										 const unsigned char ivec[16], unsigned long length,
										 unsigned char* key)
{
...
void __aesni__AES128_CBC_decrypt(uint8* output, uint8* input, uint32 length, const uint8* key,
								 const uint8* iv)
{
...
void (*AES128_CBC_decrypt)(uint8* output, uint8* input, uint32 length, const uint8* key,
						   const uint8* iv) = nullptr;
...
void AES128CTR_transform(uint8* data, sint32 length, uint8* key, uint8* nonceIv)
{
	for (sint32 i = 0; i < length; i += 16)
	{
		...
		*(uint32*)(nonceIv + 0xC) = _swapEndianU32(_swapEndianU32(*(uint32*)(nonceIv + 0xC)) + 1);
		...
		{
			*(uint32*)(nonceIv + 0x8) =
				_swapEndianU32(_swapEndianU32(*(uint32*)(nonceIv + 0x8)) + 1);
			if (*(uint32*)(nonceIv + 0x8) == 0)
			{
				*(uint32*)(nonceIv + 0x4) =
					_swapEndianU32(_swapEndianU32(*(uint32*)(nonceIv + 0x4)) + 1);
				if (*(uint32*)(nonceIv + 0x4) == 0)
				{
					*(uint32*)(nonceIv + 0) =
						_swapEndianU32(_swapEndianU32(*(uint32*)(nonceIv + 0)) + 1);
				}
			}
		}
	}
