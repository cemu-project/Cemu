...
class IniParser
{
  private:
	...
	{
	  public:
		IniSection(std::string_view sectionName, size_t lineNumber)
			: m_sectionName(sectionName), m_lineNumber(lineNumber)
		{
		}
  ...
  public:
	...
	IniParser(std::span<unsigned char> iniContents, std::string_view name = {})
		: IniParser(std::span<char>((char*)iniContents.data(), iniContents.size()), name){};
