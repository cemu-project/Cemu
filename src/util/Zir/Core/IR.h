...
enum class OpCode : uint8
{
	...
	// conversion
	BITCAST, // like MOV, but allows registers of different types. No value conversion happens, raw
			 // bit copy
	...
	// misc
	IMPORT_SINGLE, // import into a single IRReg. Depr: Make this like EXPORT where there is a 1-4
				   // regs variant and one for more
	...
	// some notes: We need this for texture read instructions. Where the result is a vec4 (f32x4)
	// and this is how we can extract individual registers from that
	//             update -> We may also instead just let the texture sample instruction specify 4
	//             output registers
};
...
{
  public:
	InsRRR(OpCode opcode, IRReg rA, IRReg rB, IRReg rC)
		: __InsBaseWithForm(opcode), rA(rA), rB(rB), rC(rC){};
...
// should we support RRI format with 32bit signed integer as a way to avoid having to generate
// dozens of IR const regs for stuff like shift and other logical instructions with constant rhs?
...
{
  public:
	InsEXPORT(LocationSymbolName exportSymbol, IRReg r)
		: __InsBaseWithForm(OpCode::EXPORT), exportSymbol(exportSymbol)
	{
		regArray[0] = r;
		count = 1;
	};

	InsEXPORT(LocationSymbolName exportSymbol, IRReg r0, IRReg r1)
		: __InsBaseWithForm(OpCode::EXPORT), exportSymbol(exportSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsEXPORT(LocationSymbolName exportSymbol, IRReg r0, IRReg r1, IRReg r2)
		: __InsBaseWithForm(OpCode::EXPORT), exportSymbol(exportSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsEXPORT(LocationSymbolName exportSymbol, IRReg r0, IRReg r1, IRReg r2, IRReg r3)
		: __InsBaseWithForm(OpCode::EXPORT), exportSymbol(exportSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsEXPORT(LocationSymbolName exportSymbol, std::span<IRReg> regs)
		: __InsBaseWithForm(OpCode::EXPORT), exportSymbol(exportSymbol)
	{
...
{
  public:
	InsIMPORT(LocationSymbolName importSymbol, IRReg r)
		: __InsBaseWithForm(OpCode::IMPORT), importSymbol(importSymbol)
	{
		regArray[0] = r;
		count = 1;
	};

	InsIMPORT(LocationSymbolName importSymbol, IRReg r0, IRReg r1)
		: __InsBaseWithForm(OpCode::IMPORT), importSymbol(importSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsIMPORT(LocationSymbolName importSymbol, IRReg r0, IRReg r1, IRReg r2)
		: __InsBaseWithForm(OpCode::IMPORT), importSymbol(importSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsIMPORT(LocationSymbolName importSymbol, IRReg r0, IRReg r1, IRReg r2, IRReg r3)
		: __InsBaseWithForm(OpCode::IMPORT), importSymbol(importSymbol)
	{
		regArray[0] = r0;
		regArray[1] = r1;
	...
	InsIMPORT(LocationSymbolName importSymbol, std::span<IRReg> regs)
		: __InsBaseWithForm(OpCode::IMPORT), importSymbol(importSymbol)
	{
		zpir_debug_assert(regs.size() <= 4);
		for (size_t i = 0; i < regs.size(); i++)
...
struct IRRegDef
{
	...
	// todo - information about spilling location? (it depends on the architecture so we should keep
	// this out of the core IR)
...
struct ZpIRBasicBlock
{
	...
	ZpIRBasicBlock* m_branchNotTaken{
		nullptr};							// next block if branch not taken or no branch present
