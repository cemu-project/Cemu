...
// string buffer helper class which keeps buffer space at the front and end, allow fast prepend and
// append
class DualStringBuffer
{
  ...
  public:
	...
	{
		return std::basic_string_view<char>((char*)(m_strBuffer + m_offsetBegin),
											m_offsetEnd - m_offsetBegin);
	}
...
void GLSL::GenerateBasicBlockCode(ZpIR::ZpIRBasicBlock& basicBlock)
{
	...
	// every register which is only consumed once can be directly inlined instead of storing and
	// referencing it via a variable
	ZpIR::IR::__InsBase* instruction = basicBlock.m_instructionFirst;
	while (instruction)
	{
		ZpIR::ZpIRCmdUtil::forEachAccessedReg(
...
void GLSL::HandleInstruction(ZpIR::IR::InsRR* ins)
{
	...
	case ZpIR::IR::OpCode::SWAP_ENDIAN:
	{
		...
		// todo - should we store expressionBuf in a temporary variable? We reference it multiple
		// times and reducing complexity would be good
		if (srcType == ZpIR::DataType::U32)
		{
			...
			assert_dbg();
		forceNoInline = true; // avoid inlining endian-swapping, since it would add too much
...
void GLSL::HandleInstruction(ZpIR::IR::InsIMPORT* ins)
{
	...
	else if (loc.IsVertexAttribute())
	{
		...
		cemu_assert_debug(m_blockContext.currentBasicBlock->getRegType(ins->regArray[0]) ==
						  ZpIR::DataType::U32);
...
void GLSL::getRegisterName(char buf[16], ZpIR::IRReg irReg)
{
	...
	auto r = fmt::format_to(buf, "r{}{}",
							ZirPass::RegisterAllocatorForGLSL::GetPhysRegIndex(physReg), typeChar);
	*r = '\0';
}
